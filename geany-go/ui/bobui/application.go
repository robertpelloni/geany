...
// App is the bobui (Qt6) implementation of the ui.Application interface.
type App struct {
	mainWindow *Window
	isRunning  bool
}

// NewApp creates a new bobui application instance.
...
func (a *App) Init(args []string) error {
	fmt.Println("[bobui] Initializing Qt6 subsystem...")
	// Calls to C++ Qt Application init
	return nil
}

func (a *App) Run() int {
	fmt.Println("[bobui] Starting Qt6 main event loop...")
	a.isRunning = true
	// Calls to QApplication::exec()
	return 0
}

func (a *App) Quit() {
	fmt.Println("[bobui] Shutting down Qt6 subsystem...")
	if a.isRunning {
		a.isRunning = false
	}
}
...
func (w *Window) SetTitle(title string) {
	w.title = title
	fmt.Printf("[bobui] Window title set to: %s\n", title)
}

func (w *Window) Show() {
	fmt.Println("[bobui] Qt6 Window shown.")
}

func (w *Window) Hide() {
	fmt.Println("[bobui] Qt6 Window hidden.")
}

func (w *Window) AddDocument(doc *editor.Document) ui.EditorWidget {
	w.docs = append(w.docs, doc)
	fmt.Printf("[bobui] Document added to Window: %s\n", doc.BaseName)
...
func (e *Editor) EnableMultiLineTabs(enable bool) {
	fmt.Printf("[bobui] Qt6 MultiLineTabs parity feature: %v\n", enable)
}

func (e *Editor) ShowDocumentMap(show bool) {
	fmt.Printf("[bobui] Qt6 DocumentMap parity feature: %v\n", show)
}
