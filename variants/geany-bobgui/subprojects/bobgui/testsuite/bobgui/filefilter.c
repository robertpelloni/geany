...
test_basic (void)
{
  ...
  bobgui_file_filter_set_name (filter, "test");
  g_assert_cmpstr (bobgui_file_filter_get_name (filter), ==, "test");
...
test_pattern (void)
{
  ...
  filter = bobgui_file_filter_new ();
  bobgui_file_filter_add_pattern (filter, "D*.st[xy]");
  attrs = g_strjoinv (",", (char **)bobgui_file_filter_get_attributes (filter));

  info = g_file_info_new ();

  g_file_info_set_display_name (info, "abracadabra");
  g_assert_false (bobgui_filter_match (BOBGUI_FILTER (filter), info));

  g_file_info_set_display_name (info, "dro.stx");
  g_assert_false (bobgui_filter_match (BOBGUI_FILTER (filter), info));

  g_file_info_set_display_name (info, "Dro.sty");
  g_assert_true (bobgui_filter_match (BOBGUI_FILTER (filter), info));
...
test_buildable (void)
{
  ...
  const char *buffer =
  "<interface>\n"
  "<object class=\"BobguiFileFilter\" id=\"filter\">\n"
  ...
  bobgui_builder_add_from_string (builder, buffer, strlen (buffer), NULL);
  filter = BOBGUI_FILE_FILTER (bobgui_builder_get_object (builder, "filter"));

  v1 = bobgui_file_filter_to_gvariant (filter);
...
test_variant_no_name (void)
{
  ...
  filter = bobgui_file_filter_new ();
  variant = bobgui_file_filter_to_gvariant (filter);
  ...
  filter = bobgui_file_filter_new ();
  bobgui_file_filter_add_mime_types (filter,
  ...
  filter = bobgui_file_filter_new ();
  bobgui_file_filter_add_suffix (filter, "txt");
  bobgui_file_filter_add_suffix (filter, "html");
  variant = bobgui_file_filter_to_gvariant (filter);
  g_variant_unref (variant);
  g_object_unref (filter);
