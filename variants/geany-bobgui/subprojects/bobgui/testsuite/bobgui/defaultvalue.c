...
test_type (gconstpointer data)
{
  ...
  if (g_type_is_a (type, BOBGUI_TYPE_APPLICATION) ||
      g_type_is_a (type, GDK_TYPE_PIXBUF_LOADER) ||
      g_type_is_a (type, BOBGUI_TYPE_LAYOUT_CHILD) ||
      g_type_is_a (type, BOBGUI_TYPE_STACK_PAGE) ||
#ifdef G_OS_UNIX
      g_type_is_a (type, BOBGUI_TYPE_PRINT_JOB) ||
#endif
  ...
  if (g_type_is_a (type, BOBGUI_TYPE_APP_CHOOSER_DIALOG))
    return;

  /* These leak their GDBusConnections */
  if (g_type_is_a (type, BOBGUI_TYPE_FILE_CHOOSER_DIALOG) ||
      g_type_is_a (type, BOBGUI_TYPE_FILE_CHOOSER_WIDGET) ||
      g_str_equal (g_type_name (type), "BobguiPlacesSidebar"))
    return;
 
  if (g_type_is_a (type, BOBGUI_TYPE_SHORTCUT_TRIGGER) ||
      g_type_is_a (type, BOBGUI_TYPE_SHORTCUT_ACTION))
    return;

  klass = g_type_class_ref (type);

  if (g_type_is_a (type, BOBGUI_TYPE_SETTINGS))
    instance = G_OBJECT (g_object_ref (bobgui_settings_get_default ()));
  else if (g_type_is_a (type, GDK_TYPE_SURFACE))
    {
      instance = G_OBJECT (g_object_ref (gdk_surface_new_toplevel (display)));
    }
  else if (g_type_is_a (type, BOBGUI_TYPE_FILTER_LIST_MODEL) ||
           g_type_is_a (type, BOBGUI_TYPE_NO_SELECTION) ||
           g_type_is_a (type, BOBGUI_TYPE_SINGLE_SELECTION) ||
           g_type_is_a (type, BOBGUI_TYPE_MULTI_SELECTION))
    {
  ...
  for (i = 0; i < n_pspecs; ++i)
    {
      ...
      if ((g_type_is_a (type, GDK_TYPE_CLIPBOARD) ||
           ...
           g_type_is_a (type, BOBGUI_TYPE_DROP_TARGET)) &&
...
G_GNUC_END_IGNORE_DEPRECATIONS
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_DIALOG) &&
          (strcmp (pspec->name, "use-header-bar") == 0))
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_ASSISTANT) &&
          (strcmp (pspec->name, "use-header-bar") == 0 ||
           strcmp (pspec->name, "pages") == 0)) /* pages always gets a non-NULL value */
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_STACK) &&
          (strcmp (pspec->name, "pages") == 0)) /* pages always gets a non-NULL value */
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_NOTEBOOK) &&
          (strcmp (pspec->name, "pages") == 0)) /* pages always gets a non-NULL value */
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_COMBO_BOX) &&
          (strcmp (pspec->name, "child") == 0))
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_POPOVER) &&
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_ABOUT_DIALOG) &&
          (strcmp (pspec->name, "program-name") == 0))
        check = FALSE;

      /* These are set to the current date */
      if (g_type_is_a (type, BOBGUI_TYPE_CALENDAR) &&
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_CELL_AREA_CONTEXT) &&
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_CELL_RENDERER_TEXT) &&
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_CELL_VIEW) &&
          (strcmp (pspec->name, "background-gdk") == 0 ||
           strcmp (pspec->name, "foreground-gdk") == 0 ||
           strcmp (pspec->name, "foreground-rgba") == 0 ||
           strcmp (pspec->name, "background-rgba") == 0 ||
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_COLOR_BUTTON) &&
          (strcmp (pspec->name, "color") == 0 ||
           strcmp (pspec->name, "rgba") == 0))
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_COLUMN_VIEW) &&
...
G_GNUC_END_IGNORE_DEPRECATIONS
       ...
       */
      if (g_type_is_a (type, BOBGUI_TYPE_ENTRY) &&
          (strcmp (pspec->name, "invisible-char") == 0 ||
           strcmp (pspec->name, "buffer") == 0))
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_TEXT) &&
          (strcmp (pspec->name, "invisible-char") == 0 ||
           strcmp (pspec->name, "buffer") == 0))
        check = FALSE;
...
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_ENTRY_COMPLETION) &&
          (strcmp (pspec->name, "cell-area") == 0 ||
           strcmp (pspec->name, "cell-area-context") == 0))
        check = FALSE;
...
G_GNUC_END_IGNORE_DEPRECATIONS
      ...
      if ((g_type_is_a (type, BOBGUI_TYPE_FILTER_LIST_MODEL) ||
           g_type_is_a (type, BOBGUI_TYPE_NO_SELECTION) ||
           g_type_is_a (type, BOBGUI_TYPE_SINGLE_SELECTION) ||
           g_type_is_a (type, BOBGUI_TYPE_MULTI_SELECTION)) &&
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_TREE_LIST_MODEL) &&
          (strcmp (pspec->name, "item-type") == 0)) /* might be a treelistrow */
        check = FALSE;

      /* This is set in init() */
      if (g_type_is_a (type, BOBGUI_TYPE_FONT_CHOOSER_WIDGET) &&
          strcmp (pspec->name, "tweak-action") == 0)
        check = FALSE;

G_GNUC_BEGIN_IGNORE_DEPRECATIONS

      if (g_type_is_a (type, BOBGUI_TYPE_ICON_VIEW) &&
          (strcmp (pspec->name, "cell-area") == 0 ||
           strcmp (pspec->name, "cell-area-context") == 0))
        check = FALSE;
...
G_GNUC_END_IGNORE_DEPRECATIONS
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_MESSAGE_DIALOG) &&
          (strcmp (pspec->name, "image") == 0 ||
           strcmp (pspec->name, "message-area") == 0))
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_PANED) &&
          strcmp (pspec->name, "max-position") == 0)
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_PRINT_OPERATION) &&
          strcmp (pspec->name, "job-name") == 0)
        check = FALSE;

#ifdef G_OS_UNIX
      if (g_type_is_a (type, BOBGUI_TYPE_PRINT_UNIX_DIALOG) &&
          (strcmp (pspec->name, "page-setup") == 0 ||
           strcmp (pspec->name, "print-settings") == 0))
        check = FALSE;
#endif

      if (g_type_is_a (type, BOBGUI_TYPE_PROGRESS_BAR) &&
          strcmp (pspec->name, "adjustment") == 0)
        check = FALSE;

      /* filename value depends on $HOME */
      if (g_type_is_a (type, BOBGUI_TYPE_RECENT_MANAGER) &&
          (strcmp (pspec->name, "filename") == 0 ||
           strcmp (pspec->name, "size") == 0))
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_SCALE_BUTTON) &&
          strcmp (pspec->name, "adjustment") == 0)
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_SCROLLED_WINDOW) &&
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_SHORTCUT) &&
          (strcmp (pspec->name, "action") == 0 ||
           strcmp (pspec->name, "trigger") == 0))
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_SPIN_BUTTON) &&
          (strcmp (pspec->name, "adjustment") == 0))
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_STYLE_CONTEXT) &&
           strcmp (pspec->name, "display") == 0)
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_TEXT_BUFFER) &&
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_TEXT_TAG) &&
          (strcmp (pspec->name, "background-gdk") == 0 ||
           strcmp (pspec->name, "foreground-gdk") == 0 ||
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_TEXT_VIEW) &&
          strcmp (pspec->name, "buffer") == 0)
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_TREE_VIEW) &&
          (strcmp (pspec->name, "hadjustment") == 0 ||
           strcmp (pspec->name, "vadjustment") == 0))
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_TREE_VIEW_COLUMN) &&
          (strcmp (pspec->name, "cell-area") == 0 ||
           strcmp (pspec->name, "cell-area-context") == 0))
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_VIEWPORT) &&
          (strcmp (pspec->name, "hadjustment") == 0 ||
           strcmp (pspec->name, "vadjustment") == 0))
        check = FALSE;
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_WINDOW) &&
          strcmp (pspec->name, "resize-grip-visible") == 0)
        check = FALSE;

      /* show-desktop depends on desktop environment */
      if (g_str_equal (g_type_name (type), "BobguiPlacesSidebar") &&
          strcmp (pspec->name, "show-desktop") == 0)
        check = FALSE;

      /* BobguiRange constructs an adjustment on its own if NULL is set and
       * the property is a CONSTRUCT one, so the returned value is never NULL. */
      if (g_type_is_a (type, BOBGUI_TYPE_RANGE) &&
          strcmp (pspec->name, "adjustment") == 0)
        check = FALSE;

      /* ... and BobguiScrollbar wraps that property. */
      if (g_type_is_a (type, BOBGUI_TYPE_SCROLLBAR) &&
          strcmp (pspec->name, "adjustment") == 0)
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_DROP_DOWN) &&
          strcmp (pspec->name, "factory") == 0)
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_BOOKMARK_LIST) &&
          (strcmp (pspec->name, "filename") == 0 ||
           strcmp (pspec->name, "loading") == 0))
        check = FALSE;

      /* All the icontheme properties depend on the environment */
      if (g_type_is_a (type, BOBGUI_TYPE_ICON_THEME))
        check = FALSE;

      /* Non-NULL default */
      if (g_type_is_a (type, BOBGUI_TYPE_COLOR_DIALOG_BUTTON) &&
          strcmp (pspec->name, "rgba") == 0)
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_FONT_DIALOG_BUTTON) &&
          strcmp (pspec->name, "font-desc") == 0)
        check = FALSE;

      if (g_type_is_a (type, BOBGUI_TYPE_FONT_DIALOG) &&
          strcmp (pspec->name, "language") == 0)
        check = FALSE;
  ...
  for (i = 0; otypes[i]; i++)
    {
      ...
      if (otypes[i] == BOBGUI_TYPE_FILE_CHOOSER_NATIVE)
        continue;
