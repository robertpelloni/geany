...
{
  static BobguiIconTheme *icon_theme = NULL;
  ...
  icon_theme = bobgui_icon_theme_new ();
  bobgui_icon_theme_set_theme_name (icon_theme, "icons");
  current_dir[0] = g_test_get_dir (G_TEST_DIST);
  current_dir[1] = NULL;
  bobgui_icon_theme_set_search_path (icon_theme, current_dir);

  return icon_theme;
}

static char *
lookup_flags_to_string (BobguiIconLookupFlags flags)
{
  GValue flags_value = { 0, }, string_value = { 0, };
  char *result;

  g_value_init (&flags_value, BOBGUI_TYPE_ICON_LOOKUP_FLAGS);
                         ...
                         int                 pixbuf_size)
{
  ...
  if (fallbacks)
    {
      ...
      info = bobgui_icon_theme_lookup_icon (get_test_icontheme (FALSE), icon_name, &fallback_names[1], size, 1, direction, flags);
      g_object_unref (fallback_icons);
    }
  else
    {
      info = bobgui_icon_theme_lookup_icon (get_test_icontheme (FALSE), icon_name, NULL, size, 1, direction, flags);
    }
  ...
  file = bobgui_icon_paintable_get_file (info);
  if (file)
    {
...
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
  g_assert_cmpstr (bobgui_icon_paintable_get_icon_name (info), ==, "image-missing");
...
{                                                                                 \
  ...
  if (g_test_subprocess ())                                                       \
    {                                                                             \
      guint debug_flags = bobgui_get_debug_flags ();                                 \
      bobgui_set_debug_flags (debug_flags | BOBGUI_DEBUG_ICONTHEME);                    \
...
LOOKUP_ORDER_TEST (test_lookup_order0, "foo-bar-baz", 16, BOBGUI_TEXT_DIR_NONE, 0, TRUE,
                   "foo-bar-baz",
                   "foo-bar",
                   "foo",
...
LOOKUP_ORDER_TEST (test_lookup_order1, "foo-bar-baz", 16, BOBGUI_TEXT_DIR_RTL, 0, TRUE,
                   "foo-bar-baz-rtl",
                   "foo-bar-baz",
                   "foo-bar-rtl",
                   "foo-bar",
...
LOOKUP_ORDER_TEST (test_lookup_order2, "foo-bar-baz", 16, BOBGUI_TEXT_DIR_RTL, 0, FALSE,
                   "foo-bar-baz-rtl",
                   "foo-bar-baz")

LOOKUP_ORDER_TEST (test_lookup_order3, "foo-bar-baz-symbolic", 16, BOBGUI_TEXT_DIR_NONE, 0, TRUE,
                   "foo-bar-baz-symbolic",
                   "foo-bar-symbolic",
                   "foo-symbolic",
                   "foo-bar-baz",
...
LOOKUP_ORDER_TEST (test_lookup_order4, "bla-bla", 16, BOBGUI_TEXT_DIR_NONE, BOBGUI_ICON_LOOKUP_FORCE_SYMBOLIC, TRUE,
                   "bla-bla-symbolic",
                   "bla-symbolic",
...
LOOKUP_ORDER_TEST (test_lookup_order5, "bla-bla-symbolic", 16, BOBGUI_TEXT_DIR_NONE, BOBGUI_ICON_LOOKUP_FORCE_SYMBOLIC, TRUE,
                   "bla-bla-symbolic",
                   "bla-symbolic",
                   "bla-bla-symbolic", /* awkward */
                   "bla-symbolic", /* awkward */
...
LOOKUP_ORDER_TEST (test_lookup_order6, "bar-baz", 16, BOBGUI_TEXT_DIR_RTL, BOBGUI_ICON_LOOKUP_FORCE_SYMBOLIC, TRUE,
...
LOOKUP_ORDER_TEST (test_lookup_order7, "bar-baz-symbolic", 16, BOBGUI_TEXT_DIR_RTL, BOBGUI_ICON_LOOKUP_FORCE_SYMBOLIC, TRUE,
                   "bar-baz-symbolic-rtl",
                   "bar-baz-symbolic",
                   "bar-symbolic-rtl",
                   "bar-symbolic",
...
LOOKUP_ORDER_TEST (test_lookup_order8, "bar-baz", 16, BOBGUI_TEXT_DIR_LTR, BOBGUI_ICON_LOOKUP_FORCE_SYMBOLIC, TRUE,
...
LOOKUP_ORDER_TEST (test_lookup_order9, "bar-baz-symbolic", 16, BOBGUI_TEXT_DIR_LTR, BOBGUI_ICON_LOOKUP_FORCE_SYMBOLIC, TRUE,
                   "bar-baz-symbolic-ltr",
                   "bar-baz-symbolic",
                   "bar-symbolic-ltr",
                   "bar-symbolic",
...
test_basics (void)
{
  ...
  assert_icon_lookup ("simple", 16, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/16x16/simple.png");
}
...
test_force_symbolic (void)
{
  ...
  assert_icon_lookup ("everything",
                      SCALABLE_IMAGE_SIZE,
                      BOBGUI_TEXT_DIR_NONE,
                      BOBGUI_ICON_LOOKUP_FORCE_SYMBOLIC,
                      FALSE,
                      "/icons/scalable/everything-symbolic.svg");
...
test_force_regular (void)
{
  ...
  assert_icon_lookup ("everything",
                      SCALABLE_IMAGE_SIZE,
                      BOBGUI_TEXT_DIR_NONE,
                      BOBGUI_ICON_LOOKUP_FORCE_REGULAR,
                      FALSE,
...
test_svg_size (void)
{
  ...
  assert_icon_lookup_size ("twosize-fixed", 48, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/32x32/twosize-fixed.svg", 48);
  assert_icon_lookup_size ("twosize-fixed", 32, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/32x32/twosize-fixed.svg", 32);
  assert_icon_lookup_size ("twosize-fixed", 20, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/32x32/twosize-fixed.svg", 20);
  assert_icon_lookup_size ("twosize-fixed", 16, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/16x16/twosize-fixed.svg", 16);

  /* Check that we still properly load it even if a different size is requested */
  assert_icon_lookup_size ("twosize", 64, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/32x32s/twosize.svg", 64);
  assert_icon_lookup_size ("twosize", 48, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/32x32s/twosize.svg", 48);
  assert_icon_lookup_size ("twosize", 32, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/32x32s/twosize.svg", 32);
  assert_icon_lookup_size ("twosize", 24, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/32x32s/twosize.svg", 24);
  assert_icon_lookup_size ("twosize", 16, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/16x16s/twosize.svg", 16);
  assert_icon_lookup_size ("twosize", 12, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/16x16s/twosize.svg", 12);
  assert_icon_lookup_size ("twosize",  8, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/16x16s/twosize.svg", 8);
}

static void
test_size (void)
{
  assert_icon_lookup_size ("size-test", 12, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/15/size-test.png", 15);
  assert_icon_lookup_size ("size-test", 13, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/15/size-test.png", 15);
  assert_icon_lookup_size ("size-test", 14, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/15/size-test.png", 15);
  assert_icon_lookup_size ("size-test", 15, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/15/size-test.png", 15);
  assert_icon_lookup_size ("size-test", 16, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/16-22/size-test.png", 19);
  assert_icon_lookup_size ("size-test", 17, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/16-22/size-test.png", 19);
  assert_icon_lookup_size ("size-test", 18, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/16-22/size-test.png", 19);
  assert_icon_lookup_size ("size-test", 19, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/16-22/size-test.png", 19);
  /* the next 3 are because we never scale up */
  assert_icon_lookup_size ("size-test", 20, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/25+/size-test.svg", 20);
  assert_icon_lookup_size ("size-test", 21, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/25+/size-test.svg", 21);
  assert_icon_lookup_size ("size-test", 22, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/25+/size-test.svg", 22);

  assert_icon_lookup_size ("size-test", 23, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/25+/size-test.svg", 23);
  assert_icon_lookup_size ("size-test", 23, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/25+/size-test.svg", 23);
  assert_icon_lookup_size ("size-test", 25, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/25+/size-test.svg", 25);
  assert_icon_lookup_size ("size-test", 28, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/25+/size-test.svg", 28);
  /* the next 2 are because we never scale up */
  assert_icon_lookup_size ("size-test", 31, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/35+/size-test.svg", 31);
  assert_icon_lookup_size ("size-test", 34, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/35+/size-test.svg", 34);

  assert_icon_lookup_size ("size-test", 37, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/35+/size-test.svg", 37);
  assert_icon_lookup_size ("size-test", 40, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/35+/size-test.svg", 40);
  assert_icon_lookup_size ("size-test", 45, BOBGUI_TEXT_DIR_NONE, 0, FALSE, "/icons/35+/size-test.svg", 45);
}
...
test_list (void)
{
  ...
  icons = bobgui_icon_theme_get_icon_names (theme);
  ...
  g_assert_true (bobgui_icon_theme_has_icon (theme, "size-test"));
  g_assert_true (bobgui_icon_theme_has_icon (theme, "simple"));
  g_assert_true (bobgui_icon_theme_has_icon (theme, "twosize-fixed"));
  g_assert_true (bobgui_icon_theme_has_icon (theme, "twosize"));
  g_assert_true (bobgui_icon_theme_has_icon (theme, "only32-symbolic"));
  g_assert_true (bobgui_icon_theme_has_icon (theme, "everything"));
  g_assert_true (bobgui_icon_theme_has_icon (theme, "everything-rtl"));
  g_assert_true (bobgui_icon_theme_has_icon (theme, "everything-symbolic"));
  g_assert_true (bobgui_icon_theme_has_icon (theme, "everything-justregular"));
  g_assert_true (bobgui_icon_theme_has_icon (theme, "everything-justrtl-rtl"));
  g_assert_true (bobgui_icon_theme_has_icon (theme, "everything-symbolic-rtl"));
  g_assert_true (bobgui_icon_theme_has_icon (theme, "everything-justsymbolic-symbolic"));
