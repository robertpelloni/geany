                            ...
                            int            expected_dirtyness)
{
  ...
  if (expected_dirtyness)
    {
      g_assert_true (BOBGUI_TREE_RBNODE_FLAG_SET (node, BOBGUI_TREE_RBNODE_COLUMN_INVALID) ||
  ...
  else
    {
      g_assert_true (!BOBGUI_TREE_RBNODE_FLAG_SET (node, BOBGUI_TREE_RBNODE_COLUMN_INVALID) &&
                !BOBGUI_TREE_RBNODE_FLAG_SET (node, BOBGUI_TREE_RBNODE_INVALID));
      if (!bobgui_tree_rbtree_is_nil (node->left))
        g_assert_true (!BOBGUI_TREE_RBNODE_FLAG_SET (node->left, BOBGUI_TREE_RBNODE_DESCENDANTS_INVALID));
      if (!bobgui_tree_rbtree_is_nil (node->right))
        g_assert_true (!BOBGUI_TREE_RBNODE_FLAG_SET (node->right, BOBGUI_TREE_RBNODE_DESCENDANTS_INVALID));
      if (node->children != NULL)
        g_assert_true (!BOBGUI_TREE_RBNODE_FLAG_SET (node->children->root, BOBGUI_TREE_RBNODE_DESCENDANTS_INVALID));
    }
                                       ...
                                       BobguiTreeRBNode *node)
{
  ...
  g_assert_false (bobgui_tree_rbtree_is_nil (node));
  ...
  if (!bobgui_tree_rbtree_is_nil (node->left))
    {
      ...
      left_blacks = bobgui_tree_rbtree_test_structure_helper (tree, node->left);
    }
  else
    left_blacks = 0;
  ...
  if (!bobgui_tree_rbtree_is_nil (node->right))
    {
      ...
      right_blacks = bobgui_tree_rbtree_test_structure_helper (tree, node->right);
    }
  else
    right_blacks = 0;
  ...
  return left_blacks + (BOBGUI_TREE_RBNODE_GET_COLOR (node) == BOBGUI_TREE_RBNODE_BLACK ? 1 : 0);
}
...
bobgui_tree_rbtree_test_structure (BobguiTreeRBTree *tree)
{
  ...
  g_assert_true (bobgui_tree_rbtree_is_nil (tree->root->parent));
  bobgui_tree_rbtree_test_structure_helper (tree, tree->root);
}

static void
bobgui_tree_rbtree_test (BobguiTreeRBTree *tree)
{
...
/* bobgui_rbtree_print() - unused, for debugging only */

static void
bobgui_rbtree_print_node (BobguiTreeRBTree *tree,
                       BobguiTreeRBNode *node,
                       int            depth)
{
  int i;
  ...
  if (node->children != NULL)
    {
      ...
      bobgui_rbtree_print_node (node->children, node->children->root, depth + 1);
      g_print ("Done looking at child.\n");
    }
  if (!bobgui_tree_rbtree_is_nil (node->left))
    {
      bobgui_rbtree_print_node (tree, node->left, depth + 1);
    }
  if (!bobgui_tree_rbtree_is_nil (node->right))
    {
      bobgui_rbtree_print_node (tree, node->right, depth + 1);
    }
}

/* not static so the debugger finds it. */
void bobgui_rbtree_print (BobguiTreeRBTree *tree);

void
bobgui_rbtree_print (BobguiTreeRBTree *tree)
{
  ...
  if (bobgui_tree_rbtree_is_nil (tree->root))
    ...
    bobgui_rbtree_print_node (tree, tree->root, 0);
}

/* actual tests */

static guint
append_elements (BobguiTreeRBTree *tree,
                 guint          depth,
                 ...
                 guint          height)
{
  ...
  for (i = 0; i < elements_per_depth; i++)
    {
      node = bobgui_tree_rbtree_insert_after (tree, node, ++height, TRUE);
      if (depth)
        {
      ...
      if (check)
        bobgui_tree_rbtree_test (tree);
    }

  return height;
}

static BobguiTreeRBTree *
...
test_insert_after (void)
{
  ...
  for (i = 1; i <= 100; i++)
    {
      node = bobgui_tree_rbtree_insert_after (tree, node, i, TRUE);
      bobgui_tree_rbtree_test (tree);
...
test_insert_before (void)
{
  ...
  for (i = 1; i <= 100; i++)
    {
      node = bobgui_tree_rbtree_insert_before (tree, node, i, TRUE);
      bobgui_tree_rbtree_test (tree);
      g_assert_cmpint (tree->root->count, ==, i);
      g_assert_cmpint (tree->root->total_count, ==, i);
      g_assert_cmpint (tree->root->offset, ==, i * (i + 1) / 2);
...
test_remove_node (void)
{
  ...
  while (tree->root->count > 1)
    {
      BobguiTreeRBTree *find_tree;
      BobguiTreeRBNode *find_node;
      guint i;

      i = g_test_rand_int_range (0, tree->root->total_count);
      if (!bobgui_tree_rbtree_find_index (tree, i, &find_tree, &find_node))
        {
          /* We search an available index, so we mustn't fail. */
          g_assert_not_reached ();
        }

      bobgui_tree_rbtree_test (find_tree);

      if (find_tree->root->count == 1)
        {
          bobgui_tree_rbtree_remove (find_tree);
        }
      else
        bobgui_tree_rbtree_remove_node (find_tree, find_node);
      bobgui_tree_rbtree_test (tree);
    }
...
test_remove_root (void)
{
  ...
  node = bobgui_tree_rbtree_insert_after (tree, NULL, 1, TRUE);
  bobgui_tree_rbtree_insert_after (tree, node, 2, TRUE);
  bobgui_tree_rbtree_insert_before (tree, node, 3, TRUE);
                      ...
                      guint n)
{
  ...
  for (i = 0; i < n; i++)
    {
      node = bobgui_tree_rbtree_insert_after (tree, node, 0, TRUE);
    }

  for (i = 0; i < n; i++)
    {
      node = bobgui_tree_rbtree_find_count (tree, order[i] + 1);
      bobgui_tree_rbtree_node_set_height (tree, node, i);
    }
...
test_reorder (void)
{
  ...
  bobgui_tree_rbtree_reorder (tree, reorder, n);
  ...
  for (node = bobgui_tree_rbtree_first (tree), i = 0;
    ...
    {
      g_assert_cmpint (BOBGUI_TREE_RBNODE_GET_HEIGHT (node), ==, i);
    }
  g_assert_cmpint (i, ==, n);
