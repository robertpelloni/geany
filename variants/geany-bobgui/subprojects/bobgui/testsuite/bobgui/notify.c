...
check_property (GObject *instance, GParamSpec *pspec)
{
  ...
  if (G_TYPE_IS_ENUM (pspec->value_type))
  ...
  else if (pspec->value_type == G_TYPE_LIST_MODEL)
    {
      ...
      value = g_list_store_new (BOBGUI_TYPE_WIDGET);
  ...
  else if (pspec->value_type == BOBGUI_TYPE_ADJUSTMENT)
    {
      NotifyData data;
      gulong id;
      ...
      value = bobgui_adjustment_new (100, 0, 200, 1, 1, 10);
  ...
  else if (pspec->value_type == BOBGUI_TYPE_WIDGET)
    {
      NotifyData data;
      gulong id;
      BobguiWidget *value;
      ...
      value = bobgui_box_new (BOBGUI_ORIENTATION_HORIZONTAL, 0);
      g_object_ref_sink (value);
...
test_type (gconstpointer data)
{
  ...
  if (g_type_is_a (type, BOBGUI_TYPE_FILE_CHOOSER_DIALOG) ||
      ...
      g_type_is_a (type, BOBGUI_TYPE_FILE_CHOOSER_NATIVE))
    return;

  if (g_type_is_a (type, BOBGUI_TYPE_STACK_PAGE))
    return;

  /* these assert in constructed */
 if (g_type_is_a (type, BOBGUI_TYPE_ALTERNATIVE_TRIGGER) ||
     g_type_is_a (type, BOBGUI_TYPE_SIGNAL_ACTION) ||
     g_type_is_a (type, BOBGUI_TYPE_NAMED_ACTION))
    return;

  /* needs a surface */
  if (g_type_is_a (type, BOBGUI_TYPE_DRAG_ICON))
    return;

  /* Needs debugging */
  if (g_type_is_a (type, BOBGUI_TYPE_SHORTCUTS_WINDOW))
    return;
...
G_GNUC_END_IGNORE_DEPRECATIONS
  ...
  else if (g_type_is_a (type, BOBGUI_TYPE_CALENDAR))
    {
  ...
  else if (g_type_is_a (type, BOBGUI_TYPE_NEVER_TRIGGER))
    instance = (GObject *) g_object_ref (bobgui_never_trigger_get ());
  else if (g_type_is_a (type, BOBGUI_TYPE_NOTHING_ACTION))
    instance = (GObject *) g_object_ref (bobgui_nothing_action_get ());
  else if (g_type_is_a (type, BOBGUI_TYPE_ACTIVATE_ACTION))
    instance = (GObject *) g_object_ref (bobgui_activate_action_get ());
  else if (g_type_is_a (type, BOBGUI_TYPE_MNEMONIC_ACTION))
    instance = (GObject *) g_object_ref (bobgui_mnemonic_action_get ());
  else
  ...
  for (i = 0; i < n_pspecs; ++i)
    {
      ...
      /* These are special */
      if (g_type_is_a (pspec->owner_type, BOBGUI_TYPE_WIDGET) &&
      ...
      if (pspec->owner_type == BOBGUI_TYPE_ENTRY &&
          g_str_equal (pspec->name, "im-module"))
        continue;

      if (type == BOBGUI_TYPE_SETTINGS)
        continue;

      if (g_type_is_a (pspec->owner_type, BOBGUI_TYPE_ENTRY_COMPLETION) &&
          g_str_equal (pspec->name, "text-column"))
        continue;

      if (g_type_is_a (pspec->owner_type, BOBGUI_TYPE_COLOR_CHOOSER) &&
          g_str_equal (pspec->name, "show-editor"))
        continue;

      if (g_type_is_a (pspec->owner_type, BOBGUI_TYPE_NOTEBOOK) &&
          g_str_equal (pspec->name, "page"))
        continue;

      /* Too many special cases involving -set properties */
      if (pspec->owner_type == BOBGUI_TYPE_CELL_RENDERER_TEXT ||
          pspec->owner_type == BOBGUI_TYPE_TEXT_TAG)
        continue;

      /* Most things assume a model is set */
      if (pspec->owner_type == BOBGUI_TYPE_COMBO_BOX)
        continue;

      /* Can only be set on unmapped windows */
      if (pspec->owner_type == BOBGUI_TYPE_WINDOW &&
          g_str_equal (pspec->name, "type-hint"))
        continue;

      if (pspec->owner_type == BOBGUI_TYPE_ENTRY_COMPLETION &&
          g_str_equal (pspec->name, "text-column"))
        continue;

      if (pspec->owner_type == BOBGUI_TYPE_PRINT_OPERATION &&
          (g_str_equal (pspec->name, "current-page") ||
           g_str_equal (pspec->name, "n-pages")))
        continue;

      if (pspec->owner_type == BOBGUI_TYPE_RANGE &&
          g_str_equal (pspec->name, "fill-level"))
        continue;

      if (pspec->owner_type == BOBGUI_TYPE_SPIN_BUTTON &&
          g_str_equal (pspec->name, "value"))
        continue;

      if (pspec->owner_type == BOBGUI_TYPE_STACK &&
          (g_str_equal (pspec->name, "visible-child-name") ||
           g_str_equal (pspec->name, "visible-child")))
        continue;

      if (pspec->owner_type == BOBGUI_TYPE_STACK_PAGE && /* Can't change position without a stack */
      ...
      if (pspec->owner_type == BOBGUI_TYPE_POPOVER_MENU &&
          g_str_equal (pspec->name, "visible-submenu"))
        continue;

      if (pspec->owner_type == BOBGUI_TYPE_TEXT_VIEW &&
          g_str_equal (pspec->name, "im-module"))
        continue;

      if (pspec->owner_type == BOBGUI_TYPE_TREE_SELECTION &&
          g_str_equal (pspec->name, "mode")) /* requires a treeview */
        continue;

      if (pspec->owner_type == BOBGUI_TYPE_TREE_VIEW &&
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_SHORTCUTS_SHORTCUT) &&
          g_str_equal (pspec->name, "accelerator"))
        continue;

      if (g_type_is_a (type, BOBGUI_TYPE_SHORTCUT_LABEL) &&
          g_str_equal (pspec->name, "accelerator"))
        continue;

      if (g_type_is_a (type, BOBGUI_TYPE_FONT_CHOOSER) &&
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_SINGLE_SELECTION) &&
      ...
      if (g_type_is_a (type, BOBGUI_TYPE_NOTEBOOK_PAGE) &&
          g_str_equal (pspec->name, "position"))
        continue;

      if (pspec->owner_type == BOBGUI_TYPE_TREE_VIEW_COLUMN &&
          g_str_equal (pspec->name, "widget"))
        continue;

      /* Interface does not do explicit notify, so we can't fix it */
      if (pspec->owner_type == BOBGUI_TYPE_SCROLLABLE &&
          (g_str_equal (pspec->name, "hadjustment") ||
           g_str_equal (pspec->name, "vadjustment")))
        continue;

      /* deprecated, not getting fixed */
      if (pspec->owner_type == BOBGUI_TYPE_CELL_RENDERER_SPIN &&
