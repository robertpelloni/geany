...
test_change (void)
{
  ...
  box = g_object_ref_sink (bobgui_box_new (BOBGUI_ORIENTATION_HORIZONTAL, 0));

  model = bobgui_widget_observe_children (box);
  ...
  bobgui_box_append (BOBGUI_BOX (box), a);
  bobgui_box_append (BOBGUI_BOX (box), b);

  item = g_list_model_get_item (model, 0);
  g_assert_true (BOBGUI_WIDGET (item) == a);
  g_object_unref (item);
  item = g_list_model_get_item (model, 1);
  g_assert_true (BOBGUI_WIDGET (item) == b);
  g_object_unref (item);
...
test_exhaustive (void)
{
  ...
  model = bobgui_widget_observe_children (BOBGUI_WIDGET (box));
  compare = G_LIST_MODEL (g_list_store_new (BOBGUI_TYPE_WIDGET));
      ...
      switch (g_test_rand_int_range (0, 4))
      {
          ...
          if (g_list_model_get_n_items (compare) > 0)
            {
              ...
              BobguiWidget *child = g_list_model_get_item (compare, position);
              bobgui_box_remove (box, child);
        ...
        case 2:
          /* add a widget */
          {
            ...
            BobguiWidget *child = bobgui_box_new (BOBGUI_ORIENTATION_HORIZONTAL, 0);
