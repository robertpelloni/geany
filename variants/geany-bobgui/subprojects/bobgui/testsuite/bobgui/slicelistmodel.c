...
static BobguiSliceListModel *
new_model (GListModel *store, guint offset, guint size)
{
  BobguiSliceListModel *result;
  GString *changes;

  if (store)
    g_object_ref (store);
  result = bobgui_slice_list_model_new (store, offset, size);
...
test_set_model (void)
{
  ...
  bobgui_slice_list_model_set_model (slice, G_LIST_MODEL (store));
  assert_model (slice, "1 3");
  assert_changes (slice, "0+2*");

  bobgui_slice_list_model_set_model (slice, NULL);
  assert_model (slice, "");
  assert_changes (slice, "0-2*");
...
test_set_slice (void)
{
  ...
  bobgui_slice_list_model_set_offset (slice, 1);
  assert_model (slice, "3 5 7");
  assert_changes (slice, "0-3+3");

  bobgui_slice_list_model_set_size (slice, 2);
  assert_model (slice, "3 5");
  assert_changes (slice, "-2*");

  bobgui_slice_list_model_set_size (slice, 10);
  assert_model (slice, "3 5 7");
  assert_changes (slice, "+2*");

  g_assert_cmpuint (bobgui_slice_list_model_get_offset (slice), ==, 1);
...
test_sections (void)
{
  ...
  bobgui_slice_list_model_set_size (slice, 5);
