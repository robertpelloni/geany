...
get_with_parents (GObject *object)
{
  ...
  if (BOBGUI_IS_TREE_LIST_ROW (object))
    {
      BobguiTreeListRow *r;

      r = BOBGUI_TREE_LIST_ROW (object);
      object = bobgui_tree_list_row_get_item (r);
      number = 10 * get_with_parents (G_OBJECT (bobgui_tree_list_row_get_parent (r)));
      g_object_unref (r);
    }
  else
    number = 0;
...
{
  return G_LIST_MODEL (bobgui_tree_list_model_new (G_LIST_MODEL (new_store (1, size, 1)),
                                                FALSE,
                                                TRUE,
                                                new_child_model,
                                                NULL, NULL));
...
test_simple (void)
{
  ...
  sorter = BOBGUI_SORTER (bobgui_tree_list_row_sorter_new (NULL));
  sort = bobgui_sort_list_model_new (model, sorter);
...
test_compare_total_order (void)
{
  ...
  sorter = BOBGUI_SORTER (bobgui_tree_list_row_sorter_new (new_numeric_sorter ()));
      ...
      for (j = 0; j < n; j++)
        {
          ...
          g_assert_cmpint (bobgui_sorter_compare (sorter, item1, item2), ==, bobgui_ordering_from_cmpfunc ((int) i - j));
