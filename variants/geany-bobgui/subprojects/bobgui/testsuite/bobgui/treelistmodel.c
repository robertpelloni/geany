           ...
           gboolean expanded)
{
  ...
  tree = bobgui_tree_list_model_new (G_LIST_MODEL (new_store (size, size, size)), TRUE, expanded, create_sub_model_cb, NULL, NULL);
...
test_expand (void)
{
  BobguiTreeListModel *tree = new_model (100, FALSE);
  guint i;
  ...
  for (i = g_list_model_get_n_items (G_LIST_MODEL (tree)); i > 0; i--)
    {
      BobguiTreeListRow *row = bobgui_tree_list_model_get_row (tree, i - 1);
...
{
  BobguiTreeListModel *tree = new_model (100, TRUE);
  gpointer item;
...
/* Test for https://gitlab.gnome.org/GNOME/bobgui/-/issues/4595 */
...
test_collapse_change (void)
{
  ...
  treemodel = bobgui_tree_list_model_new (G_LIST_MODEL (model),
                                       FALSE,
                                       FALSE,
                                       demo_node_children,
                                       NULL,
  ...
  row = bobgui_tree_list_model_get_row (treemodel, 0);
  bobgui_tree_list_row_set_expanded (row, TRUE);
  bobgui_tree_list_row_set_expanded (row, FALSE);
  g_object_unref (row);
...
#define N_MODELS 4
  ...
  treelist = bobgui_tree_list_model_new (g_object_ref (G_LIST_MODEL (models[0])),
