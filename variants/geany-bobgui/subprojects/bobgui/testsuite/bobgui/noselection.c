...
new_model (GListModel *store, gboolean autoselect, gboolean can_unselect)
{
  ...
  result = BOBGUI_SELECTION_MODEL (bobgui_no_selection_new (g_object_ref (store)));
...
test_set_model (void)
{
  ...
  bobgui_no_selection_set_model (BOBGUI_NO_SELECTION (selection), m2);
  assert_changes (selection, "0-5+3*");
  assert_selection (selection, "");
  assert_selection_changes (selection, "");

  bobgui_no_selection_set_model (BOBGUI_NO_SELECTION (selection), m1);
...
test_empty (void)
{
  ...
  selection = bobgui_no_selection_new (NULL);
  ...
  g_assert_null (bobgui_no_selection_get_model (BOBGUI_NO_SELECTION (selection)));

  store = g_list_store_new (G_TYPE_OBJECT);
  bobgui_no_selection_set_model (selection, G_LIST_MODEL (store));
  g_object_unref (store);
