...
static Signal *
signal_new (SignalName signal, BobguiTreePath *path)
{
  Signal *s;

  s = g_new0 (Signal, 1);
  s->signal = signal;
  s->path = bobgui_tree_path_copy (path);
  s->new_order = NULL;
...
static Signal *
signal_new_with_order (SignalName signal, BobguiTreePath *path,
                                ...
                                int           *new_order)
{
  ...
  g_print ("signal=%s path=%s\n", signal_name_to_string (signal),
           bobgui_tree_path_to_string (path));
#endif

  if (s->signal != signal ||
      (bobgui_tree_path_get_depth (s->path) == 0 &&
       bobgui_tree_path_get_depth (path) != 0) ||
      (bobgui_tree_path_get_depth (s->path) != 0 &&
       bobgui_tree_path_compare (s->path, path) != 0))
    {
      char *path_str, *s_path_str;

      s_path_str = bobgui_tree_path_to_string (s->path);
      path_str = bobgui_tree_path_to_string (path);
...
static void
signal_monitor_row_inserted (BobguiTreeModel *model,
                             BobguiTreePath  *path,
                             BobguiTreeIter  *iter,
                             gpointer      data)
{
...
static void
signal_monitor_row_deleted (BobguiTreeModel *model,
                            BobguiTreePath  *path,
                            gpointer      data)
{
...
static void
signal_monitor_row_changed (BobguiTreeModel *model,
                            BobguiTreePath  *path,
                            BobguiTreeIter  *iter,
                            gpointer      data)
{
...
static void
signal_monitor_row_has_child_toggled (BobguiTreeModel *model,
                                      BobguiTreePath  *path,
                                      BobguiTreeIter  *iter,
                                      gpointer      data)
{
...
static void
signal_monitor_rows_reordered (BobguiTreeModel *model,
                               BobguiTreePath  *path,
                               BobguiTreeIter  *iter,
                               int          *new_order,
                               gpointer      data)
...
SignalMonitor *
signal_monitor_new (BobguiTreeModel *client)
{
  SignalMonitor *m;
