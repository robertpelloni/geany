...
static void
assert_section_is_not_null (BobguiCssProvider *provider,
...
test_section_in_load_from_data (void)
{
  ...
  bobgui_css_provider_load_from_string (provider, "random garbage goes here");
...
test_section_load_nonexisting_file (void)
{
  ...
  g_signal_connect (provider, "parsing-error",
                    G_CALLBACK (assert_section_is_not_null), NULL);
  bobgui_css_provider_load_from_path (provider, "this/path/does/absolutely/not/exist.css");
  g_object_unref (provider);
}
...
test_load_with_media_query (void)
{
  ...
  rendered_css = bobgui_css_provider_to_string (provider);
  g_object_unref (provider);
...
static void
assert_media_query_parse_warning (BobguiCssProvider *provider,
                                  BobguiCssSection  *section,
                                  const GError   *error,
                                  gpointer        unused)
{
  g_assert_error (error, BOBGUI_CSS_PARSER_WARNING, BOBGUI_CSS_PARSER_WARNING_SYNTAX);
...
test_load_with_and_media_query (void)
{
  ...
  g_object_set (provider,
                "prefers-color-scheme", BOBGUI_INTERFACE_COLOR_SCHEME_LIGHT,
                "prefers-contrast", BOBGUI_INTERFACE_CONTRAST_MORE,
                "prefers-reduced-motion", BOBGUI_REDUCED_MOTION_REDUCE,
                NULL);
...
test_update_media_features_after_style_sheet_is_loaded (void)
{
  ...
  g_object_set (provider,
                "prefers-color-scheme", BOBGUI_INTERFACE_COLOR_SCHEME_DARK,
                NULL);
