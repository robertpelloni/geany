...
new_model (GListModel *store)
{
  ...
  if (store)
    g_object_ref (store);
  result = bobgui_map_list_model_new (store, map_multiply, GUINT_TO_POINTER (2), NULL);
...
test_set_model (void)
{
  ...
  store = new_store (1, 5, 1);
  bobgui_map_list_model_set_model (map, G_LIST_MODEL (store));
  assert_model (map, "2 4 6 8 10");
  assert_changes (map, "0+5*");

  bobgui_map_list_model_set_model (map, NULL);
  assert_model (map, "");
  assert_changes (map, "0-5*");
...
test_set_map_func (void)
{
  ...
  bobgui_map_list_model_set_map_func (map, map_multiply, GUINT_TO_POINTER (3), NULL);
  assert_model (map, "3 6 9 12 15");
  assert_changes (map, "0-5+5");

  bobgui_map_list_model_set_map_func (map, NULL, NULL, NULL);
  assert_model (map, "1 2 3 4 5");
  assert_changes (map, "0-5+5");

  bobgui_map_list_model_set_map_func (map, map_multiply, GUINT_TO_POINTER (2), NULL);
  assert_model (map, "2 4 6 8 10");
  assert_changes (map, "0-5+5");
