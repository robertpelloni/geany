		...
		gboolean         has_keysym)
{
  ...
  accel_key = 0;
  ret = bobgui_accelerator_parse_with_keycode (accel,
  ...
  if (has_keysym)
    {
      ...
      ret = bobgui_accelerator_parse (accel, &accel_key_2, &mods_2);
      g_assert_true (ret);
  ...
  label = bobgui_accelerator_get_label_with_keycode (NULL,
						  accel_key,
						  *keycodes,
						  mods);
  ...
  accessible_label = bobgui_accelerator_get_accessible_label (accel_key, mods);

  g_assert_cmpstr (accessible_label, ==, exp_accessible_label);

  name = bobgui_accelerator_name_with_keycode (NULL,
					    accel_key,
					    *keycodes,
					    mods);
