...
#include "../bobgui/bobguicomposetable.h"
#include "../bobgui/bobguiimcontextsimpleseqs.h"
...
static char *
bobgui_compose_table_print (BobguiComposeTable *table)
{
  GString *str;
  ...
  bobgui_compose_table_foreach (table, print_sequence, str);
...
generate_output (const char *file)
{
  ...
  table = bobgui_compose_table_parse (file, NULL);
  output = bobgui_compose_table_print (table);
...
compose_table_match (void)
{
  ...
  buffer[1] = 0;
  ret = bobgui_compose_table_check (table, buffer, 1, &finish, &match, output);
  g_assert_true (ret);
  ...
  buffer[1] = GDK_KEY_s;
  buffer[2] = GDK_KEY_e;
  ret = bobgui_compose_table_check (table, buffer, 3, &finish, &match, output);
  g_assert_true (ret);
  g_assert_false (finish);
  g_assert_false (match);
  g_assert_true (output->len == 0);
  ...
  buffer[2] = GDK_KEY_e;
  buffer[3] = GDK_KEY_q;
  ret = bobgui_compose_table_check (table, buffer, 4, &finish, &match, output);
  g_assert_true (ret);
  g_assert_false (finish);
  g_assert_true (match);
  ...
  buffer[3] = GDK_KEY_q;
  buffer[4] = GDK_KEY_u;
  ret = bobgui_compose_table_check (table, buffer, 5, &finish, &match, output);
  g_assert_true (ret);
  ...
  buffer[1] = GDK_KEY_s;
  buffer[2] = 0;
  ret = bobgui_compose_table_check (table, buffer, 2, &finish, &match, output);
  g_assert_true (ret);
  g_assert_true (finish);
  g_assert_true (match);
  g_assert_cmpstr (output->str, ==, "∫");
...
compose_table_match_builtin (void)
{
  ...
  ret = bobgui_compose_table_check (table, buffer, 1, &finish, &match, s);
  g_assert_true (ret);
  g_assert_false (finish);
  g_assert_false (match);
  ...
  ret = bobgui_compose_table_check (table, buffer, 3, &finish, &match, s);
  g_assert_false (ret);
  g_assert_false (finish);
  g_assert_false (match);
  g_assert_true (s->len == 0);
  ...
  ret = bobgui_compose_table_check (table, buffer, 4, &finish, &match, s);
  g_assert_true (ret);
  g_assert_true (finish);
  g_assert_true (match);
  ...
  ret = bobgui_compose_table_check (table, buffer, 2, &finish, &match, s);
  g_assert_true (ret);
  g_assert_true (finish);
  g_assert_true (match);
...
match_algorithmic (void)
{
  ...
  ret = bobgui_check_algorithmically (buffer, 2, output);
  g_assert_false (ret);
  g_assert_cmpstr (output->str, ==, "");
  ...
  ret = bobgui_check_algorithmically (buffer, 3, output);
  g_assert_true (ret);
  ...
  if (argc == 3 && strcmp (argv[1], "--generate") == 0)
    {
      ...
      BobguiIMContext *ctx = bobgui_im_context_simple_new ();
      g_object_unref (ctx);
