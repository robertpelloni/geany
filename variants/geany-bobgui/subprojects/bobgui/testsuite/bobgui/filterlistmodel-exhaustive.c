...
#define assert_sections_equal(model1, model2) G_STMT_START{ \
  ...
  for (_i = 0; _i < _n; _i = _end1) \
    { \
      bobgui_section_model_get_section (model1, _i, &_start1, &_end1); \
      bobgui_section_model_get_section (model2, _i, &_start2, &_end2); \
                       ...
                       BobguiFilter  *filter)
{
  ...
  if (filter)
    g_object_ref (filter);
  model = bobgui_filter_list_model_new (source, filter);
                          ...
                          BobguiFilter     *filter)
{
  ...
  switch (id >> 2)
  {
    ...
    case 1:
      bobgui_filter_list_model_set_incremental (model, TRUE);
      break;
  ...
  if (id & 1)
    bobgui_filter_list_model_set_model (model, source);
  if (id & 2)
    bobgui_filter_list_model_set_filter (model, filter);
...
create_filter (gsize id)
{
  ...
  switch (id)
  {
    case 0:
      ...
      return BOBGUI_FILTER (bobgui_string_filter_new (NULL));

    case 1:
      /* BOBGUI_FILTER_MATCH_NONE */
      filter = BOBGUI_FILTER (bobgui_string_filter_new (NULL));
      bobgui_string_filter_set_search (BOBGUI_STRING_FILTER (filter), "does not matter, because no expression");
      return filter;
    ...
    case 4:
      ...
      if (id == 2)
        bobgui_string_filter_set_search (BOBGUI_STRING_FILTER (filter), "A");
      else if (id == 3)
        bobgui_string_filter_set_search (BOBGUI_STRING_FILTER (filter), "B");
      else
        bobgui_string_filter_set_search (BOBGUI_STRING_FILTER (filter), "does-not-match");
      return filter;
...
test_no_filter (gconstpointer model_id)
{
  ...
  bobgui_filter_list_model_set_filter (model, NULL);
  ensure_updated ();
  assert_model_equal (G_LIST_MODEL (model), source);
...
{
  BobguiFilterListModel *compare;
  BobguiFilterListModel *model1, *model2;
  GListModel *source;
  BobguiFilter *every, *filter;
  guint i, j, k;

  source = create_source_model (10, 10);
  model1 = create_filter_list_model (model_id, source, NULL);
  model2 = create_filter_list_model (model_id, G_LIST_MODEL (model1), NULL);
  every = BOBGUI_FILTER (bobgui_every_filter_new ());
  ...
  for (i = 0; i < N_FILTERS; i++)
    {
      ...
      bobgui_filter_list_model_set_filter (model1, filter);
      bobgui_multi_filter_append (BOBGUI_MULTI_FILTER (every), filter);

      for (j = 0; j < N_FILTERS; j++)
        {
          filter = create_filter (i);
          bobgui_filter_list_model_set_filter (model2, filter);
          bobgui_multi_filter_append (BOBGUI_MULTI_FILTER (every), filter);
          ...
          for (k = 0; k < 10; k++)
            {
              ...
              bobgui_filter_list_model_set_model (compare, source);
              bobgui_filter_list_model_set_model (model1, source);
              g_object_unref (source);
          ...
          bobgui_multi_filter_remove (BOBGUI_MULTI_FILTER (every), 1);
        }

      bobgui_multi_filter_remove (BOBGUI_MULTI_FILTER (every), 0);
    }
...
test_model_changes (gconstpointer model_id)
{
  ...
  BobguiFlattenListModel *flatten1, *flatten2;
  BobguiFilterListModel *model2;
  BobguiFilter *multi, *filter;
  gsize i;

  filter = create_random_filter (TRUE);
  multi = BOBGUI_FILTER (bobgui_every_filter_new ());
  if (filter)
    bobgui_multi_filter_append (BOBGUI_MULTI_FILTER (multi), filter);

  store1 = g_list_store_new (G_TYPE_OBJECT);
  store2 = g_list_store_new (G_TYPE_OBJECT);
  flatten1 = bobgui_flatten_list_model_new (G_LIST_MODEL (store1));
  flatten2 = bobgui_flatten_list_model_new (G_LIST_MODEL (store2));
  ...
  for (i = 0; i < 500; i++)
    {
      ...
      switch (g_test_rand_int_range (0, 4))
      {
        case 0:
          /* change the filter */
          filter = create_random_filter (TRUE);
          bobgui_multi_filter_remove (BOBGUI_MULTI_FILTER (multi), 0); /* no-op if no filter */
          if (filter)
            bobgui_multi_filter_append (BOBGUI_MULTI_FILTER (multi), filter);
          bobgui_filter_list_model_set_filter (model2, filter);
          break;
      ...
      if (add)
        {
          ...
          BobguiFilterListModel *model1 = create_filter_list_model (model_id, source, multi);
      ...
      if (g_test_rand_bit ())
        {
          ...
          assert_sections_equal (BOBGUI_SECTION_MODEL (flatten1), BOBGUI_SECTION_MODEL (model2));
        }
    }
