...
G_GNUC_END_IGNORE_DEPRECATIONS
  ...
  else if (g_type_is_a (test_type, BOBGUI_TYPE_FILTER_LIST_MODEL) ||
           g_type_is_a (test_type, BOBGUI_TYPE_NO_SELECTION) ||
           g_type_is_a (test_type, BOBGUI_TYPE_SINGLE_SELECTION) ||
           g_type_is_a (test_type, BOBGUI_TYPE_MULTI_SELECTION))
    {
  ...
  else if (g_type_is_a (test_type, BOBGUI_TYPE_LAYOUT_CHILD))
    {
#if 0
  ...
  /* Toplevels are owned by BOBGUI, just tell BOBGUI to destroy it */
  if (BOBGUI_IS_WINDOW (object))
    bobgui_window_destroy (BOBGUI_WINDOW (object));
  else
  ...
  for (i = 0; i < n_types; i++)
    {
	  ...
	  all_types[i] != gdk_pixbuf_simple_anim_iter_get_type() &&
          !g_type_is_a (all_types[i], BOBGUI_TYPE_SHORTCUT_TRIGGER) &&
          !g_type_is_a (all_types[i], BOBGUI_TYPE_SHORTCUT_ACTION) &&
          /* can't instantiate empty stack pages */
          all_types[i] != BOBGUI_TYPE_STACK_PAGE)
	{
