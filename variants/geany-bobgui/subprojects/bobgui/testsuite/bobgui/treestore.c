/* Extensive BobguiTreeStore tests.
 ...
 *    here as in BobguiListStore, so we need to check for corner cases on
	     ...
	     BobguiTreeIter *b)
{
  ...
  /* user_data2 and user_data3 are not used in BobguiTreeStore */

  return TRUE;
}

static gboolean
iter_position (BobguiTreeStore *store,
	       BobguiTreeIter  *iter,
	       int           n)
{
  gboolean ret = TRUE;
  ...
  if (bobgui_tree_path_get_indices (path)[0] != n)
    ret = FALSE;
		  ...
		  gconstpointer  test_data)
{
  ...
  fixture->store = bobgui_tree_store_new (1, G_TYPE_INT);

  for (i = 0; i < 5; i++)
    {
      bobgui_tree_store_insert (fixture->store, &fixture->iter[i], NULL, i);
      bobgui_tree_store_set (fixture->store, &fixture->iter[i], 0, i, -1);
    }
}
	     ...
	     int        skip)
{
  ...
  for (i = 0; i < 5; i++)
	...
	continue;
      ...
      g_assert_true (bobgui_tree_store_iter_is_valid (fixture->store, &iter));
...
tree_store_test_insert_high_values (void)
{
  ...
  BobguiTreeIter iter_copy;
  BobguiTreeStore *store;

  store = bobgui_tree_store_new (1, G_TYPE_INT);

  bobgui_tree_store_insert (store, &iter, NULL, 1234);
  g_assert_true (bobgui_tree_store_iter_is_valid (store, &iter));
  g_assert_cmpint (bobgui_tree_model_iter_n_children (BOBGUI_TREE_MODEL (store), NULL), ==, 1);
  g_assert_true (bobgui_tree_model_get_iter_first (BOBGUI_TREE_MODEL (store), &iter_copy));
  g_assert_true (iters_equal (&iter, &iter_copy));
  g_assert_true (iter_position (store, &iter, 0));

  bobgui_tree_store_insert (store, &iter2, NULL, 765);
  g_assert_true (bobgui_tree_store_iter_is_valid (store, &iter2));
  g_assert_cmpint (bobgui_tree_model_iter_n_children (BOBGUI_TREE_MODEL (store), NULL), ==, 2);
  ...
  g_assert_true (bobgui_tree_model_iter_next (BOBGUI_TREE_MODEL (store), &iter_copy));
  g_assert_true (iters_equal (&iter2, &iter_copy));
  g_assert_true (iter_position (store, &iter2, 1));

  g_assert_false (bobgui_tree_model_iter_next (BOBGUI_TREE_MODEL (store), &iter_copy));

  g_assert_true (bobgui_tree_model_iter_nth_child (BOBGUI_TREE_MODEL (store), &iter_copy, NULL, 1));
  g_assert_true (iters_equal (&iter2, &iter_copy));
  g_assert_true (iter_position (store, &iter2, 1));

  g_assert_true (bobgui_tree_model_iter_previous (BOBGUI_TREE_MODEL (store), &iter_copy));
  g_assert_true (iters_equal (&iter, &iter_copy));
  g_assert_true (iter_position (store, &iter, 0));

  g_assert_false (bobgui_tree_model_iter_previous (BOBGUI_TREE_MODEL (store), &iter_copy));
...
tree_store_test_append (void)
{
  ...
  bobgui_tree_store_append (store, &iter2, NULL);
  g_assert_true (bobgui_tree_store_iter_is_valid (store, &iter2));
  g_assert_cmpint (bobgui_tree_model_iter_n_children (BOBGUI_TREE_MODEL (store), NULL), ==, 2);
...
tree_store_test_prepend (void)
{
  ...
  bobgui_tree_store_prepend (store, &iter2, NULL);
  g_assert_true (bobgui_tree_store_iter_is_valid (store, &iter2));
  g_assert_cmpint (bobgui_tree_model_iter_n_children (BOBGUI_TREE_MODEL (store), NULL), ==, 2);
...
tree_store_test_insert_after (void)
{
  ...
  bobgui_tree_store_insert_after (store, &iter3, NULL, &iter);
  g_assert_true (bobgui_tree_store_iter_is_valid (store, &iter3));
  g_assert_cmpint (bobgui_tree_model_iter_n_children (BOBGUI_TREE_MODEL (store), NULL), ==, 3);
  g_assert_true (bobgui_tree_model_iter_nth_child (BOBGUI_TREE_MODEL (store), &iter_copy, NULL, 1));
  ...
  g_assert_true (bobgui_tree_model_iter_nth_child (BOBGUI_TREE_MODEL (store), &iter_copy, NULL, 2));
  g_assert_true (iters_equal (&iter2, &iter_copy));
...
tree_store_test_insert_after_NULL (void)
{
  ...
  bobgui_tree_store_insert_after (store, &iter2, NULL, NULL);
  g_assert_true (bobgui_tree_store_iter_is_valid (store, &iter2));
  g_assert_cmpint (bobgui_tree_model_iter_n_children (BOBGUI_TREE_MODEL (store), NULL), ==, 2);
  ...
  g_assert_true (bobgui_tree_model_iter_nth_child (BOBGUI_TREE_MODEL (store), &iter_copy, NULL, 0));
  g_assert_true (iters_equal (&iter2, &iter_copy));
...
tree_store_test_insert_before (void)
{
  ...
  bobgui_tree_store_insert_before (store, &iter3, NULL, &iter2);
  g_assert_true (bobgui_tree_store_iter_is_valid (store, &iter3));
  g_assert_cmpint (bobgui_tree_model_iter_n_children (BOBGUI_TREE_MODEL (store), NULL), ==, 3);
  g_assert_true (bobgui_tree_model_iter_nth_child (BOBGUI_TREE_MODEL (store), &iter_copy, NULL, 1));
  g_assert_true (iters_equal (&iter3, &iter_copy));
...
tree_store_test_insert_before_NULL (void)
{
  ...
  bobgui_tree_store_insert_before (store, &iter2, NULL, NULL);
  g_assert_true (bobgui_tree_store_iter_is_valid (store, &iter2));
  g_assert_cmpint (bobgui_tree_model_iter_n_children (BOBGUI_TREE_MODEL (store), NULL), ==, 2);
...
tree_store_set_gvalue_to_transform (void)
{
  ...
  store = bobgui_tree_store_new (1, G_TYPE_LONG);
  bobgui_tree_store_append (store, &iter, NULL);

  g_value_init (&value, G_TYPE_INT);
  g_value_set_int (&value, 42);
  bobgui_tree_store_set_value (store, &iter, 0, &value);
}
			      ...
			      gconstpointer  user_data)
{
  ...
  bobgui_tree_model_get_iter (BOBGUI_TREE_MODEL (fixture->store), &iter, path);
  bobgui_tree_path_free (path);

  g_assert_true (bobgui_tree_store_remove (fixture->store, &iter) == TRUE);
  g_assert_false (bobgui_tree_store_iter_is_valid (fixture->store, &fixture->iter[0]));
			       ...
			       gconstpointer  user_data)
{
  ...
  g_assert_true (bobgui_tree_store_remove (fixture->store, &iter));
  g_assert_false (bobgui_tree_store_iter_is_valid (fixture->store, &fixture->iter[2]));
			    ...
			    gconstpointer  user_data)
{
  ...
  /* Remove node at 4 */
  path = bobgui_tree_path_new_from_indices (4, -1);
  bobgui_tree_model_get_iter (BOBGUI_TREE_MODEL (fixture->store), &iter, path);
  bobgui_tree_path_free (path);

  g_assert_false (bobgui_tree_store_remove (fixture->store, &iter));
  g_assert_false (bobgui_tree_store_iter_is_valid (fixture->store, &fixture->iter[4]));
		       ...
		       gconstpointer  user_data)
{
  ...
  bobgui_tree_store_clear (fixture->store);

  g_assert_cmpint (bobgui_tree_model_iter_n_children (BOBGUI_TREE_MODEL (fixture->store), NULL), ==, 0);

  for (i = 0; i < 5; i++)
    g_assert_false (bobgui_tree_store_iter_is_valid (fixture->store, &fixture->iter[i]));
}
			 ...
			 gconstpointer  user_data)
{
  ...
  bobgui_tree_store_reorder (fixture->store, NULL, new_order);
		            ...
		            gconstpointer  user_data)
{
  ...
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter_a, "0"));
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter_b, "1"));

  bobgui_tree_store_swap (fixture->store, &iter_a, &iter_b);
  check_model (fixture, new_order, -1);
}
		                  ...
		                  gconstpointer  user_data)
{
  ...
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter_a, "2"));
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter_b, "3"));
		                   ...
		                   gconstpointer  user_data)
{
  ...
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter_a, "1"));
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter_b, "3"));
		          ...
		          gconstpointer  user_data)
{
  ...
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter_a, "3"));
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter_b, "4"));
...
tree_store_test_swap_single (void)
{
  ...
  bobgui_tree_store_swap (store, &iter, &iter);
  g_assert_true (iters_equal (&iter, &iter_copy));
				       ...
				       gconstpointer  user_data)
{
  ...
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter, "0"));
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &position, "2"));

  bobgui_tree_store_move_after (fixture->store, &iter, &position);
  check_model (fixture, new_order, -1);
}
			         ...
			         gconstpointer  user_data)
{
  ...
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter, "2"));
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &position, "3"));
			          ...
			          gconstpointer  user_data)
{
  ...
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter, "1"));
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &position, "3"));
			        ...
			        gconstpointer  user_data)
{
  ...
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter, "2"));
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &position, "4"));
			             ...
			             gconstpointer  user_data)
{
  ...
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter, "4"));
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &position, "1"));
			         ...
			         gconstpointer  user_data)
{
  ...
  bobgui_tree_store_move_after (fixture->store, &iter, NULL);
  check_model (fixture, new_order, -1);
}
...
tree_store_test_move_after_single (void)
{
  ...
  bobgui_tree_store_move_after (store, &iter, NULL);
  g_assert_true (iters_equal (&iter, &iter_copy));
  g_assert_true (bobgui_tree_model_get_iter_first (BOBGUI_TREE_MODEL (store), &iter));
  g_assert_true (iters_equal (&iter, &iter_copy));

  bobgui_tree_store_move_after (store, &iter, &iter);
  g_assert_true (iters_equal (&iter, &iter_copy));
  g_assert_true (bobgui_tree_model_get_iter_first (BOBGUI_TREE_MODEL (store), &iter));
  g_assert_true (iters_equal (&iter, &iter_copy));
		                  ...
		                  gconstpointer  user_data)
{
  ...
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter, "3"));
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &position, "2"));

  bobgui_tree_store_move_before (fixture->store, &iter, &position);
  check_model (fixture, new_order, -1);
}
				      ...
				      gconstpointer  user_data)
{
  ...
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &iter, "2"));
  g_assert_true (bobgui_tree_model_get_iter_from_string (BOBGUI_TREE_MODEL (fixture->store), &position, "0"));
			          ...
			          gconstpointer  user_data)
{
  ...
  bobgui_tree_store_move_before (fixture->store, &iter, NULL);
  check_model (fixture, new_order, -1);
}
...
tree_store_test_move_before_single (void)
{
  ...
  bobgui_tree_store_move_before (store, &iter, NULL);
  g_assert_true (iters_equal (&iter, &iter_copy));
  g_assert_true (bobgui_tree_model_get_iter_first (BOBGUI_TREE_MODEL (store), &iter));
  g_assert_true (iters_equal (&iter, &iter_copy));

  bobgui_tree_store_move_before (store, &iter, &iter);
  g_assert_true (iters_equal (&iter, &iter_copy));
  g_assert_true (bobgui_tree_model_get_iter_first (BOBGUI_TREE_MODEL (store), &iter));
  g_assert_true (iters_equal (&iter, &iter_copy));
                                       ...
                                       gconstpointer  user_data)
{
  ...
  bobgui_tree_model_get_iter_first (BOBGUI_TREE_MODEL (fixture->store), &iter);

  g_assert_true (bobgui_tree_model_iter_previous (BOBGUI_TREE_MODEL (fixture->store),
                                          &iter) == FALSE);
  g_assert_false (bobgui_tree_store_iter_is_valid (fixture->store, &iter));
                                   ...
                                   gconstpointer  user_data)
{
  ...
  g_assert_false (bobgui_tree_model_iter_next (BOBGUI_TREE_MODEL (fixture->store),
                                            &iter));
  g_assert_false (bobgui_tree_store_iter_is_valid (fixture->store, &iter));
  g_assert_cmpint (iter.stamp, ==, 0);
}
...
tree_store_test_iter_children_invalid (TreeStore     *fixture,
                                       gconstpointer  user_data)
{
  BobguiTreeIter iter, child;

  bobgui_tree_model_get_iter_first (BOBGUI_TREE_MODEL (fixture->store), &iter);
  g_assert_true (bobgui_tree_store_iter_is_valid (fixture->store, &iter));

  g_assert_false (bobgui_tree_model_iter_children (BOBGUI_TREE_MODEL (fixture->store),
                                          &child, &iter));
  g_assert_false (bobgui_tree_store_iter_is_valid (fixture->store, &child));
                                        ...
                                        gconstpointer  user_data)
{
  ...
  g_assert_false (bobgui_tree_model_iter_nth_child (BOBGUI_TREE_MODEL (fixture->store),
                                                 &child, &iter, 0));
  g_assert_false (bobgui_tree_store_iter_is_valid (fixture->store, &child));
  g_assert_cmpint (child.stamp, ==, 0);
}
                                     ...
                                     gconstpointer  user_data)
{
  ...
  bobgui_tree_model_get_iter_first (BOBGUI_TREE_MODEL (fixture->store), &child);
  g_assert_true (bobgui_tree_store_iter_is_valid (fixture->store, &child));

  g_assert_false (bobgui_tree_model_iter_parent (BOBGUI_TREE_MODEL (fixture->store),
                                              &iter, &child));
  g_assert_false (bobgui_tree_store_iter_is_valid (fixture->store, &iter));
  g_assert_cmpint (iter.stamp, ==, 0);
}
...
specific_bug_77977 (void)
{
  ...
  tree_store = bobgui_tree_store_new (1, G_TYPE_STRING);

  bobgui_tree_store_append (tree_store, &iter1, NULL);
  bobgui_tree_store_set (tree_store, &iter1, 0, "Window1", -1);

  bobgui_tree_store_append (tree_store, &iter2, &iter1);
  bobgui_tree_store_set (tree_store, &iter2, 0, "Table1", -1);

  bobgui_tree_store_append (tree_store, &iter3, &iter2);
  bobgui_tree_store_set (tree_store, &iter3, 0, "Button1", -1);

  path = bobgui_tree_path_new_from_indices (0, 0, 0, -1);
  row_ref = bobgui_tree_row_reference_new (BOBGUI_TREE_MODEL (tree_store), path);
  bobgui_tree_path_free (path);

  bobgui_tree_store_remove (tree_store, &iter1);
  ...
  if (g_test_subprocess ())
    {
      ...
      tree_store = bobgui_tree_store_new (1, G_TYPE_STRING);
      bobgui_tree_store_reorder (tree_store, NULL, new_order);
      g_object_unref (tree_store);
