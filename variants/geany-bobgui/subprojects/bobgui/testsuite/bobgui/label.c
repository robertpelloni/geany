...
test_label_markup (void)
{
  ...
  bobgui_window_set_mnemonics_visible (BOBGUI_WINDOW (window), TRUE);

  bobgui_label_set_use_underline (BOBGUI_LABEL (label), TRUE);
  bobgui_label_set_use_markup (BOBGUI_LABEL (label), TRUE);
  bobgui_label_set_label (BOBGUI_LABEL (label), "<a href=\"test\"><span font_style=\"italic\">abc</span> _def</a>");

  g_assert_cmpuint (bobgui_label_get_mnemonic_keyval (BOBGUI_LABEL (label)), ==, 'd');

  text = pango_layout_get_text (bobgui_label_get_layout (BOBGUI_LABEL (label)));
  g_assert_cmpstr (text, ==, "abc def");

  attrs = pango_layout_get_attributes (bobgui_label_get_layout (BOBGUI_LABEL (label)));
  str = g_string_new ("");
  ...
  bobgui_window_set_mnemonics_visible (BOBGUI_WINDOW (window), FALSE);
  ...
  bobgui_window_set_mnemonics_visible (BOBGUI_WINDOW (window), TRUE);
  bobgui_label_set_use_underline (BOBGUI_LABEL (label), FALSE);
...
test_label_underline (void)
{
  ...
  bobgui_label_set_use_underline (BOBGUI_LABEL (label), TRUE);
  bobgui_label_set_label (BOBGUI_LABEL (label), "tes_t & no markup <<");

  g_assert_cmpint (bobgui_label_get_mnemonic_keyval (BOBGUI_LABEL (label)), ==, GDK_KEY_t);
...
test_label_parse_more (void)
{
  ...
  for (int i = 0; i < G_N_ELEMENTS (tests); i++)
    {
      bobgui_label_set_use_underline (BOBGUI_LABEL (label), tests[i].use_underline);
      bobgui_label_set_use_markup (BOBGUI_LABEL (label), tests[i].use_markup);
      bobgui_label_set_label (BOBGUI_LABEL (label), tests[i].input);

      g_assert_cmpstr (bobgui_label_get_label (BOBGUI_LABEL (label)), ==, tests[i].input);
      g_assert_cmpstr (bobgui_label_get_text (BOBGUI_LABEL (label)), ==, tests[i].text);
      g_assert_cmpuint (bobgui_label_get_mnemonic_keyval (BOBGUI_LABEL (label)), ==, tests[i].accel);
    }
