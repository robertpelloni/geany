...
struct {
  ...
  { BOBGUI_DIR_TAB_FORWARD, "tab" },
  { BOBGUI_DIR_TAB_BACKWARD, "tab-backward" },
  { BOBGUI_DIR_UP, "up" },
  { BOBGUI_DIR_DOWN, "down" },
  { BOBGUI_DIR_LEFT, "left" },
  { BOBGUI_DIR_RIGHT, "right" }
};

static void
check_focus_states (BobguiWidget *focus_widget)
{
  ...
  state = bobgui_widget_get_state_flags (focus_widget);
  g_assert_true ((state & (BOBGUI_STATE_FLAG_FOCUSED|BOBGUI_STATE_FLAG_FOCUS_WITHIN)) ==
                  (BOBGUI_STATE_FLAG_FOCUSED|BOBGUI_STATE_FLAG_FOCUS_WITHIN));

  parent = bobgui_widget_get_parent (focus_widget);
  while (parent)
    {
      state = bobgui_widget_get_state_flags (parent);
      g_assert_true ((state & BOBGUI_STATE_FLAG_FOCUS_WITHIN) == BOBGUI_STATE_FLAG_FOCUS_WITHIN);
      g_assert_true ((state & BOBGUI_STATE_FLAG_FOCUSED) == 0);
...
static char *
generate_focus_chain (BobguiWidget        *window,
                      BobguiDirectionType  dir)
{
  char *first = NULL;
  char *last = NULL;
  ...
  /* start without focus */
  bobgui_window_set_focus (BOBGUI_WINDOW (window), NULL);

  while (TRUE)
    {
      g_signal_emit_by_name (window, "move-focus", dir);

      focus = bobgui_window_get_focus (BOBGUI_WINDOW (window));
      ...
      if (focus)
        {
           ...
           */
          if (BOBGUI_IS_TEXT (focus))
            name = g_strdup_printf ("%s %s",
                                    bobgui_widget_get_name (bobgui_widget_get_parent (focus)),
                                    bobgui_widget_get_name (focus));
          else
            name = g_strdup (bobgui_widget_get_name (focus));
              ...
              const char *ext)
{
  ...
  builder = bobgui_builder_new_from_file (ui_path);
  ...
  timeout_handle_id = g_timeout_add (2000,
                                     quit_iteration_loop,
                                     &keep_running);
  while (keep_running && !bobgui_window_is_active (BOBGUI_WINDOW (window)))
    g_main_context_iteration (NULL, TRUE);

  if (keep_running)
    g_source_remove (timeout_handle_id);

  if (!bobgui_window_is_active (BOBGUI_WINDOW (window)))
    {
