 ...
 * hierarchy works as expected. Since BobguiWidget does
...
test_inheritance (void)
{
  ...
  bobgui_widget_insert_action_group (window, "win", G_ACTION_GROUP (win_actions));
  bobgui_widget_insert_action_group (box, "box", G_ACTION_GROUP (box_actions));

  g_assert_cmpint (win_activated, ==, 0);
  g_assert_cmpint (box_activated, ==, 0);

  found = bobgui_widget_activate_action (button, "win.action", NULL);

  g_assert_true (found);
  g_assert_cmpint (win_activated, ==, 1);
  g_assert_cmpint (box_activated, ==, 0);

  found = bobgui_widget_activate_action (box, "win.action", NULL);

  g_assert_true (found);
  g_assert_cmpint (win_activated, ==, 2);
  g_assert_cmpint (box_activated, ==, 0);

  found = bobgui_widget_activate_action (button, "box.action", NULL);

  g_assert_true (found);
  g_assert_cmpint (win_activated, ==, 2);
  g_assert_cmpint (box_activated, ==, 1);

  found = bobgui_widget_activate_action (window, "box.action", NULL);
...
test_inheritance2 (void)
{
  ...
  bobgui_widget_insert_action_group (window, "win", G_ACTION_GROUP (win_actions));
  bobgui_widget_insert_action_group (box1, "box1", G_ACTION_GROUP (box1_actions));
  bobgui_widget_insert_action_group (box2, "box2", G_ACTION_GROUP (box2_actions));
  ...
  found = bobgui_widget_activate_action (button, "box1.action", NULL);
  ...
  found = bobgui_widget_activate_action (button, "box2.action", NULL);
  ...
  g_object_ref (button);
  bobgui_box_remove (BOBGUI_BOX (box1), button);
...
test_inheritance3 (void)
{
  ...
  bobgui_actionable_set_action_name (BOBGUI_ACTIONABLE (button), "box1.action");

  g_assert_true (bobgui_widget_get_sensitive (button));
  ...
  g_assert_false (bobgui_widget_get_sensitive (button));

  g_object_ref (button);
  bobgui_box_remove (BOBGUI_BOX (box2), button);
  bobgui_box_append (BOBGUI_BOX (box1), button);
  g_object_unref (button);
  ...
  g_object_ref (box2);
  bobgui_box_remove (BOBGUI_BOX (box), box2);
  bobgui_box_append (BOBGUI_BOX (box1), box2);
  g_object_unref (box2);

  g_assert_true (bobgui_widget_get_sensitive (button));

  bobgui_widget_insert_action_group (box1, "box1", NULL);
...
test_inheritance4 (void)
{
  ...
  bobgui_actionable_set_action_name (BOBGUI_ACTIONABLE (button), "box.action");
...
/* Spot-check that BobguiText has the class actions
...
test_text (void)
{
  ...
  box = bobgui_box_new (BOBGUI_ORIENTATION_HORIZONTAL, 0);
  text = bobgui_text_new ();

  bobgui_box_append (BOBGUI_BOX (box), text);
  ...
  bobgui_widget_insert_action_group (box, "clipboard", G_ACTION_GROUP (clipboard_actions));

  bobgui_widget_activate_action (text, "cut.clipboard", NULL);
  bobgui_widget_activate_action (text, "copy.clipboard", NULL);
  bobgui_widget_activate_action (text, "paste.clipboard", NULL);
  ...
  bobgui_widget_activate_action (text, "misc.toggle-visibility", NULL);
 ...
 * This is a change from the way things work in BOBGUI3.
 */
static void
test_overlap (void)
{
  ...
  bobgui_widget_insert_action_group (window, "actions", G_ACTION_GROUP (win_actions));
  bobgui_widget_insert_action_group (box, "actions", G_ACTION_GROUP (box_actions));

  win_activated = 0;
  box_activated = 0;

  bobgui_widget_activate_action (box, "actions.win", NULL);

  g_assert_cmpint (win_activated, ==, 1);
  g_assert_cmpint (box_activated, ==, 0);

  bobgui_widget_activate_action (box, "actions.box", NULL);
...
test_overlap2 (void)
{
  ...
  bobgui_widget_set_parent (child, text);

  g_assert_cmpint (toggled, ==, 0);
  g_assert_cmpint (act1, ==, 0);
  g_assert_cmpint (act2, ==, 0);

  bobgui_widget_activate_action (child, "misc.toggle-visibility", NULL);
  ...
  g_action_map_add_action_entries (G_ACTION_MAP (group1),
                                   ...
                                   NULL);
  bobgui_widget_insert_action_group (text, "misc", G_ACTION_GROUP (group1));
  bobgui_widget_activate_action (child, "misc.toggle-visibility", NULL);
  ...
  g_action_map_add_action_entries (G_ACTION_MAP (group2),
                                   ...
                                   NULL);
  bobgui_widget_insert_action_group (child, "misc", G_ACTION_GROUP (group2));
...
/* Test that bobgui_widget_class_query_action
 * yields the expected results
 */
static void
test_introspection (void)
{
  BobguiWidgetClass *class = g_type_class_ref (BOBGUI_TYPE_TEXT);
  guint i, j;
  guint found;
  GType owner;
  const char *name;
  ...
  struct {
    ...
    { BOBGUI_TYPE_TEXT, "misc.toggle-visibility", NULL, "visibility" },
    { BOBGUI_TYPE_TEXT, "misc.toggle-direction", NULL, NULL },
    { BOBGUI_TYPE_TEXT, "misc.insert-emoji", NULL, NULL },
    { BOBGUI_TYPE_TEXT, "selection.select-all", NULL, NULL },
    { BOBGUI_TYPE_TEXT, "selection.delete", NULL, NULL },
    { BOBGUI_TYPE_TEXT, "clipboard.paste", NULL, NULL },
    { BOBGUI_TYPE_TEXT, "clipboard.copy", NULL, NULL },
    { BOBGUI_TYPE_TEXT, "clipboard.cut", NULL, NULL },
    { BOBGUI_TYPE_TEXT, "menu.popup", NULL, NULL },
    { BOBGUI_TYPE_TEXT, "text.redo", NULL, NULL },
    { BOBGUI_TYPE_TEXT, "text.undo", NULL, NULL },
    { BOBGUI_TYPE_TEXT, "text.clear", NULL, NULL },
  };

  j = 0;
  found = 0;
  while (bobgui_widget_class_query_action (class,
                                        j,
                                        &owner,
                                        &name,
                                        &params,
                                        &property))
    {
      ...
      if (i == G_N_ELEMENTS (expected))
        g_error ("Unexpected BobguiText action: %s", name);
      j++;
    }
...
test_enabled (void)
{
  ...
  bobgui_widget_action_set_enabled (text, "misc.toggle-visibility", FALSE);
...
#define MY_TYPE_BOBGUI_ACTIONABLE (my_bobgui_actionable_get_type ())
G_DECLARE_FINAL_TYPE          (MyBobguiActionable, my_bobgui_actionable, MY, BOBGUI_ACTIONABLE, BobguiButton)

struct _MyBobguiActionable
{
  BobguiButton parent_instance;
};

G_DEFINE_FINAL_TYPE (MyBobguiActionable, my_bobgui_actionable, BOBGUI_TYPE_BUTTON);

static void
test_cb (BobguiWidget  *sender,
         const char *name,
         GVariant   *param)
{
}
...
static void
my_bobgui_actionable_init (MyBobguiActionable *actionable)
{
  bobgui_actionable_set_action_name (BOBGUI_ACTIONABLE (actionable), "test.test");
}

static void
my_bobgui_actionable_class_init (MyBobguiActionableClass *klass)
{
  BobguiWidgetClass *widget_class = BOBGUI_WIDGET_CLASS (klass);

  bobgui_widget_class_install_action (widget_class, "test.test", NULL, test_cb);
}
...
{
  BobguiWidget *window, *actionable;

  window = bobgui_window_new ();

  actionable = g_object_new (MY_TYPE_BOBGUI_ACTIONABLE, NULL);
  bobgui_window_set_child (BOBGUI_WINDOW (window), actionable);
  g_assert_true (bobgui_widget_get_sensitive (actionable));

  g_object_ref (actionable);
  bobgui_window_set_child (BOBGUI_WINDOW (window), NULL);
  g_assert_false (bobgui_widget_get_sensitive (actionable));
