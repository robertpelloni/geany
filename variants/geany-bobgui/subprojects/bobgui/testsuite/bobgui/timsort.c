...
#include "bobgui/timsort/bobguitimsortprivate.h"
                ...
                gpointer         data)
{
  ...
  bobgui_tim_sort (a, n, element_size, compare_func, data);
...
test_steps (void)
{
  ...
  bobgui_tim_sort_init (&sort, a, n, sizeof (int), compare_int, NULL);
  bobgui_tim_sort_set_max_merge_size (&sort, g_test_rand_int_range (512, 2048));

  while (bobgui_tim_sort_step (&sort, &change))
    {
      if (change.len)
        {
