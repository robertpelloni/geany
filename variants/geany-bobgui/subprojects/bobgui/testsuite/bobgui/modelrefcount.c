/* BobguiTreeModel ref counting tests
...
test_list_reference_reordered (void)
{
  ...
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter1, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter2, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter3, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter4, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter5, NULL);
...
test_tree_no_reference (void)
{
  ...
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &child, &iter);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &child, &iter);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &child, &iter);
...
test_tree_reference_reordered (void)
{
  ...
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &parent, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter1, &parent);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter2, &parent);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter3, &parent);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter4, &parent);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter5, &parent);
...
{
  BobguiTreeIter parent1, parent2, child;
  BobguiTreePath *path1, *path2;
  BobguiTreeModel *model;
  BobguiTreeModelRefCount *ref_model;
  BobguiWidget *tree_view;
  ...
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &child, &parent1);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &child, &parent1);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &parent2, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &parent2, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &child, &parent2);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &child, &parent2);

  path1 = bobgui_tree_model_get_path (model, &parent1);
  path2 = bobgui_tree_model_get_path (model, &parent2);

  assert_level_unreferenced (ref_model, &parent1);
  assert_level_unreferenced (ref_model, &parent2);

  bobgui_tree_view_expand_row (BOBGUI_TREE_VIEW (tree_view), path1, FALSE);

  assert_level_referenced (ref_model, 1, &parent1);
  assert_level_unreferenced (ref_model, &parent2);

  bobgui_tree_view_collapse_row (BOBGUI_TREE_VIEW (tree_view), path1);

  assert_level_unreferenced (ref_model, &parent1);
  assert_level_unreferenced (ref_model, &parent2);

  bobgui_tree_view_expand_row (BOBGUI_TREE_VIEW (tree_view), path2, FALSE);

  assert_level_unreferenced (ref_model, &parent1);
  assert_level_referenced (ref_model, 1, &parent2);

  bobgui_tree_view_collapse_row (BOBGUI_TREE_VIEW (tree_view), path2);
...
{
  BobguiTreeIter iter0, iter1, iter2;
  BobguiTreePath *path;
  BobguiTreeModel *model;
  BobguiTreeModelRefCount *ref_model;
  ...
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter0, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter1, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter2, NULL);
  ...
  row_ref = bobgui_tree_row_reference_new (model, path);
...
test_row_reference_list_remove (void)
{
  ...
  bobgui_tree_store_remove (BOBGUI_TREE_STORE (model), &iter1);
  ...
  bobgui_tree_store_remove (BOBGUI_TREE_STORE (model), &iter0);

  assert_root_level_referenced (ref_model, 1);

  bobgui_tree_store_remove (BOBGUI_TREE_STORE (model), &iter2);

  g_assert_false (bobgui_tree_model_get_iter_first (model, &iter0));
...
{
  BobguiTreeIter iter0, iter1, iter2;
  BobguiTreeIter child0, child1, child2;
  BobguiTreeIter grandchild0, grandchild1, grandchild2;
  BobguiTreePath *path;
  BobguiTreeModel *model;
  BobguiTreeModelRefCount *ref_model;
  BobguiTreeRowReference *row_ref, *row_ref1;
  BobguiWidget *tree_view;
  ...
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter0, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &child0, &iter0);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &grandchild0, &child0);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter1, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &child1, &iter1);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &grandchild1, &child1);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &iter2, NULL);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &child2, &iter2);
  bobgui_tree_store_append (BOBGUI_TREE_STORE (model), &grandchild2, &child2);

  assert_entire_model_unreferenced (ref_model);

  /* create and remove a row ref and check reference counts */
  path = bobgui_tree_path_new_from_indices (1, 0, 0, -1);
  row_ref = bobgui_tree_row_reference_new (model, path);
  bobgui_tree_path_free (path);
  ...
  /* again, with path 1:1 */
  path = bobgui_tree_path_new_from_indices (1, 0, -1);
  row_ref = bobgui_tree_row_reference_new (model, path);
  bobgui_tree_path_free (path);
  ...
  path = bobgui_tree_path_new_from_indices (1, 0, -1);
  row_ref1 = bobgui_tree_row_reference_new (model, path);
  bobgui_tree_path_free (path);
  ...
  bobgui_tree_row_reference_free (row_ref1);
...
test_row_reference_tree_remove (void)
{
  ...
  BobguiTreeModel *model;
  BobguiTreeModelRefCount *ref_model;
  BobguiTreeRowReference *row_ref, *row_ref1, *row_ref2;
  ...
  path = bobgui_tree_path_new_from_indices (2, 0, -1);
  row_ref1 = bobgui_tree_row_reference_new (model, path);
  bobgui_tree_path_free (path);

  path = bobgui_tree_path_new_from_indices (2, -1);
  row_ref2 = bobgui_tree_row_reference_new (model, path);
  bobgui_tree_path_free (path);
  ...
  bobgui_tree_store_remove (BOBGUI_TREE_STORE (model), &grandchild1);
  ...
  bobgui_tree_row_reference_free (row_ref);
  bobgui_tree_row_reference_free (row_ref1);
  bobgui_tree_row_reference_free (row_ref2);
...
test_row_reference_tree_remove_ancestor (void)
{
  ...
  bobgui_tree_store_remove (BOBGUI_TREE_STORE (model), &child1);
...
{
  /* lists (though based on BobguiTreeStore) */
