...
new_model (GListModel *store)
{
  ...
  result = BOBGUI_SELECTION_MODEL (bobgui_multi_selection_new (g_object_ref (store)));
...
static BobguiSelectionFilterModel *
new_filter_model (BobguiSelectionModel *model)
{
  BobguiSelectionFilterModel *result;
  GString *changes;

  result = bobgui_selection_filter_model_new (model);
...
test_create (void)
{
  ...
  g_assert_true (bobgui_multi_selection_get_model (BOBGUI_MULTI_SELECTION (selection)) == G_LIST_MODEL (store));
...
test_create_empty (void)
{
  ...
  selection = bobgui_multi_selection_new (NULL);
...
test_changes (void)
{
  ...
  ret = bobgui_selection_model_select_range (selection, 1, 2, FALSE);
  g_assert_true (ret);
...
test_selection (void)
{
  ...
  ret = bobgui_selection_model_unselect_range (selection, 3, 2);
  g_assert_true (ret);
  assert_selection (selection, "2");
  assert_selection_changes (selection, "3:2");
...
test_select_range (void)
{
  ...
  ret = bobgui_selection_model_select_range (selection, 2, 2, FALSE);
  g_assert_true (ret);
  ...
  ret = bobgui_selection_model_select_range (selection, 0, 1, TRUE);
  g_assert_true (ret);
...
test_set_selection (void)
{
  ...
  bobgui_bitset_add_range (selected, 2, 3);
  bobgui_bitset_add_range (selected, 6, 3);
  mask = bobgui_bitset_new_empty ();
  bobgui_bitset_add_range (mask, 0, 100); /* too big on purpose */
  ret = bobgui_selection_model_set_selection (selection, selected, mask);
  g_assert_true (ret);
  ...
  selected = bobgui_bitset_new_empty ();
  mask = bobgui_bitset_new_empty ();
  bobgui_bitset_add (mask, 3);
  bobgui_bitset_add (mask, 7);
  ret = bobgui_selection_model_set_selection (selection, selected, mask);
  g_assert_true (ret);
  bobgui_bitset_unref (selected);
  bobgui_bitset_unref (mask);
...
{
  BobguiSelectionModel *selection;
  BobguiSelectionFilterModel *filter;
  GListStore *store;
  gboolean ret;
  ...
  ret = bobgui_selection_model_select_item (selection, 0, FALSE);
  g_assert_true (ret);
  ...
  ret = bobgui_selection_model_select_range (selection, 1, 3, FALSE);
  g_assert_true (ret);
  ...
  g_assert_true (bobgui_selection_filter_model_get_model (filter) == selection);
...
test_set_model (void)
{
  ...
  bobgui_multi_selection_set_model (BOBGUI_MULTI_SELECTION (selection), m2);
  assert_changes (selection, "0-5+3*");
  assert_selection (selection, "2 3");
  assert_selection_changes (selection, "");

  bobgui_multi_selection_set_model (BOBGUI_MULTI_SELECTION (selection), NULL);
  ...
  bobgui_multi_selection_set_model (BOBGUI_MULTI_SELECTION (selection), m1);
...
test_empty (void)
{
  ...
  bobgui_multi_selection_set_model (selection, G_LIST_MODEL (store));
  g_object_unref (store);
...
test_empty_filter (void)
{
  ...
  BobguiMultiSelection *selection;
  BobguiSelectionFilterModel *selection_filter;

  stringlist = bobgui_string_list_new (NULL);
  bobgui_string_list_append (stringlist, "first item");

  selection = bobgui_multi_selection_new (G_LIST_MODEL (stringlist));
  selection_filter = bobgui_selection_filter_model_new (BOBGUI_SELECTION_MODEL (selection));
