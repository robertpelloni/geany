...
{
  BobguiStringList *self = BOBGUI_STRING_LIST (model);
  ...
  for (i = 0; i < g_list_model_get_n_items (model); i++)
    {
      ...
      g_string_append_printf (string, "%s", bobgui_string_list_get_string (self, i));
    }
...
static BobguiStringList *
new_model (const char **strings)
{
  BobguiStringList *result;
  GString *changes;

  result = bobgui_string_list_new (strings);
...
{
  BobguiStringObject *so;

  so = bobgui_string_object_new ("Hello");
  g_assert_cmpstr (bobgui_string_object_get_string (so), ==, "Hello");
  g_object_unref (so);
}
...
test_create_builder (void)
{
  const char *ui =
"<interface>"
"  <object class=\"BobguiStringList\" id=\"list\">"
"    <items>"
...
"</interface>";
  ...
  builder = bobgui_builder_new_from_string (ui, -1);
  list = BOBGUI_STRING_LIST (bobgui_builder_get_object (builder, "list"));
  assert_model (list, "a b c");
...
test_get_string (void)
{
  ...
  g_assert_cmpstr (bobgui_string_list_get_string (list, 0), ==, "a");
  g_assert_cmpstr (bobgui_string_list_get_string (list, 1), ==, "b");
  g_assert_cmpstr (bobgui_string_list_get_string (list, 2), ==, "c");
  g_assert_null (bobgui_string_list_get_string (list, 3));
...
test_splice (void)
{
  ...
  bobgui_string_list_splice (list, 2, 2, (const char *[]){ "x", "y", "z", NULL });
...
test_add_remove (void)
{
  ...
  bobgui_string_list_remove (list, 2);

  assert_model (list, "a b d e");
  assert_changes (list, "-2");

  bobgui_string_list_append (list, "x");
...
test_take (void)
{
  ...
  bobgui_string_list_take (list, g_strdup_printf ("%d dollars", (int)1e6));
...
test_find (void)
{
  ...
  pos = bobgui_string_list_find (list, "a");
  g_assert_true (pos == 0);

  pos = bobgui_string_list_find (list, "ab");
  g_assert_true (pos == G_MAXUINT);

  pos = bobgui_string_list_find (list, "e");
  g_assert_true (pos == 4);
