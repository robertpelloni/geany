...
#define assert_model_sections(model) G_STMT_START{ \
  ...
  for (_i = 0; _i < G_MAXUINT; _i = _end) \
    { \
      bobgui_section_model_get_section (BOBGUI_SECTION_MODEL (model), _i, &_start, &_end); \
\
  ...
  for (i = 0; i < n; i++)
    {
      ...
      if (BOBGUI_IS_TREE_LIST_ROW (model_item))
        item = bobgui_tree_list_row_get_item (model_item);
      else
        item = model_item;
      ...
      if (G_IS_LIST_MODEL (item))
        ...
        g_string_append (string, bobgui_string_object_get_string (item));
                     ...
                     BobguiSorter  *sorter)
{
  ...
  model = bobgui_sort_list_model_new (source, sorter);
                        ...
                        BobguiSorter     *sorter)
{
  ...
  if (track_changes)
    ...
    model = bobgui_sort_list_model_new (((id & 1) || !source) ? NULL : g_object_ref (source), ((id & 2) || !sorter) ? NULL : g_object_ref (sorter));
  ...
  switch (id >> 2)
  {
    ...
    case 1:
      bobgui_sort_list_model_set_incremental (model, TRUE);
      break;
  ...
  if (id & 1)
    bobgui_sort_list_model_set_model (model, source);
  if (id & 2)
...
create_source_model (guint min_size, guint max_size)
{
  ...
  for (i = 0; i < size; i++)
    bobgui_string_list_append (list, strings[g_test_rand_int_range (0, G_N_ELEMENTS (strings))]);
...
create_sorter (gsize id)
{
  ...
  {
    case 0:
      return BOBGUI_SORTER (bobgui_string_sorter_new (NULL));
...
{
  BobguiSortListModel *compare;
  BobguiSortListModel *model1, *model2;
  GListModel *source;
  BobguiSorter *every, *sorter;
  guint i, j, k;
  ...
  every = BOBGUI_SORTER (bobgui_multi_sorter_new ());
  ...
  for (i = 0; i < N_SORTERS; i++)
    {
      ...
      bobgui_sort_list_model_set_sorter (model1, sorter);
      bobgui_multi_sorter_append (BOBGUI_MULTI_SORTER (every), sorter);

      for (j = 0; j < N_SORTERS; j++)
        {
          sorter = create_sorter (i);
          bobgui_sort_list_model_set_sorter (model2, sorter);
          bobgui_multi_sorter_append (BOBGUI_MULTI_SORTER (every), sorter);
          ...
          for (k = 0; k < 10; k++)
            {
              ...
              bobgui_sort_list_model_set_model (compare, source);
              bobgui_sort_list_model_set_model (model2, source);
              g_object_unref (source);
          ...
          bobgui_multi_sorter_remove (BOBGUI_MULTI_SORTER (every), 1);
        }

      bobgui_multi_sorter_remove (BOBGUI_MULTI_SORTER (every), 0);
    }
...
test_stability (gconstpointer model_id)
{
  ...
  BobguiSortListModel *sort1, *sort2;
  BobguiSorter *sorter;
  gsize i;
  ...
  for (i = 0; i < 500; i++)
    {
      ...
      switch (g_test_rand_int_range (0, 4))
      {
        case 0:
          /* change the sorter */
          sorter = create_random_sorter (TRUE);
          bobgui_sort_list_model_set_sorter (sort1, sorter);
          bobgui_sort_list_model_set_sorter (sort2, sorter);
          g_clear_object (&sorter);
          break;
...
static gboolean
string_is_lowercase (BobguiStringObject *o)
{
  return g_ascii_islower (*bobgui_string_object_get_string (o));
}
  ...
  for (i = 0; i < 500; i++)
    {
      ...
      switch (g_test_rand_int_range (0, 4))
      {
        case 0:
          ...
          sorter = create_random_sorter (TRUE);
          bobgui_sort_list_model_set_section_sorter (sort1, sorter);
          bobgui_sort_list_model_set_sorter (sort1, NULL);
          bobgui_sort_list_model_set_sorter (sort2, sorter);
          g_clear_object (&sorter);
          break;
        ...
        case 1:
          ...
          bobgui_sort_list_model_set_sorter (sort2, sorter);
          g_clear_object (&sorter);
          sorter = BOBGUI_SORTER (bobgui_numeric_sorter_new (bobgui_cclosure_expression_new (G_TYPE_BOOLEAN, NULL, 0, NULL, G_CALLBACK (string_is_lowercase), NULL, NULL)));
          bobgui_sort_list_model_set_section_sorter (sort1, sorter);
          g_clear_object (&sorter);
          break;
  ...
  for (i = 0; i < 500; i++)
    {
      ...
      switch (g_test_rand_int_range (0, 4))
      {
        case 0:
          /* set the same sorter, once as section sorter, once as sorter */
          sorter = create_random_sorter (TRUE);
          bobgui_sort_list_model_set_sorter (sort, sorter);
          g_clear_object (&sorter);
          break;
        ...
        case 1:
          /* set the same sorter, once as section sorter, once as sorter */
          sorter = create_random_sorter (TRUE);
          bobgui_sort_list_model_set_section_sorter (sort, sorter);
          g_clear_object (&sorter);
          break;
      ...
      if (g_test_rand_bit ())
        {
          ...
          sorter = bobgui_sort_list_model_get_section_sorter (sort);
          start = end = 0;
          prev_item = item = NULL;

          for (pos = 0; pos < n; pos++)
            {
              bobgui_section_model_get_section (BOBGUI_SECTION_MODEL (sort), pos, &sec_start, &sec_end);
              prev_item = item;
                  ...
                  if (prev_item)
                    {
                      ...
                      g_assert_cmpint (bobgui_sorter_compare (sorter, prev_item, item), !=, BOBGUI_ORDERING_EQUAL);
                    }
                }
              else
                {
          ...
          if (n < G_MAXINT32)
            {
              bobgui_section_model_get_section (BOBGUI_SECTION_MODEL (sort), g_test_rand_int_range (n, G_MAXINT32), &sec_start, &sec_end);
