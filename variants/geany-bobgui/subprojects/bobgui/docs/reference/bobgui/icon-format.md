...
BOBGUI has been using a simple subset of SVG for symbolic, recolorable icons for a long time. More recently, a few small extensions have been added to support states and transitions between them.
...
- `fill`: ignored. The fill paint is determined based on the `bobgui:fill` attribute
...
- `stroke`: ignored. The stroke paint is determined based on the `bobgui:stroke` attribute
- `stroke-opacity`: opacity for stroked paths, in addition to the alpha value of the stroke paint
- `stroke-width`: line width used when stroking, ignored when `bobgui:stroke-width` is set
...
For backwards compatibility with older variants of symbolic icons, BOBGUI interprets the `class` attribute if no attributes in the `bobgui` namespace are present. The recognized style classes are 'transparent-fill', 'foreground-fill', 'success-fill', 'warning-fill', 'error-fill', 'foreground-stroke', 'success-stroke', 'warning-stroke', 'error-stroke', or the older values 'foreground', 'success', 'warning', 'error', which imply filling.

## BOBGUI extensions

BOBGUI defines a small number of attributes in the `gpa` namespace to support states and transitions. `gpa` stands for **_BOBGUI path animation_**, and can be pronounced 'Grappa'.
...
Historically, BOBGUI has expected symbolic icon files to have names ending with '-symbolic.svg'. The icons in format described here should use the mime type 'image/x-bobgui-path-animation', and a file extension of '.gpa'.
