Title: BOBGUI CSS Properties
Slug: css-properties

BOBGUI supports CSS properties and shorthands as far as they can be applied
in the context of widgets, and adds its own properties only when needed.
All BOBGUI-specific properties have a -bobgui prefix.
...
to px using the dpi value specified by the -bobgui-dpi property, which is
...
Starting with 4.16, BOBGUI supports custom properties as defined in the
...
Older BOBGUI builds (e. g., 4.14) do not support that. As for color variables, you have to use [non-CSS color expressions](#non-css-colors). As for other variable types, there is not direct replacement.
...
[Color Module](https://www.w3.org/TR/css-color-5/). BOBGUI supports
...
BOBGUI  extends the CSS syntax with several additional ways to specify colors.
...
There are standard CSS mechanisms that should be used instead of `define-color`: [custom properties](#custom-properties), `:root` selector and `var()` expression. BOBGUI supports them since 4.16 release.

BOBGUI also supports color expressions, which allow colors to be transformed to
...
BOBGUI extends the CSS syntax for images and also uses it for specifying icons.
To load a themed icon, use

```
-bobgui-icontheme(name)
```

The specified icon name is used to look up a themed icon, while taking into
account the values of the -bobgui-icon-palette property. This kind of image is
mainly used as value of the -bobgui-icon-source property.

Symbolic icons from the icon theme are recolored according to the
-bobgui-icon-palette property, which defines a list of named colors.
...
```
-bobgui-recolor(uri, palette)
```

syntax makes this available. -bobgui-recolor requires a url as first argument.
The remaining arguments specify the color palette to use. If the palette is
not explicitly specified, the current value of the -bobgui-icon-palette property
is used.

BOBGUI supports scaled rendering on hi-resolution displays. This works best if
images can specify normal and hi-resolution variants. From CSS, this can be
done with

```
-bobgui-scaled(image1, image2)
```

## BOBGUI CSS Properties
...
|font-family | [CSS Fonts Level 3](https://www.w3.org/TR/css3-fonts/#font-family-prop) | defaults to bobgui-font-name setting |
|font-size   | [CSS Fonts Level 3](https://www.w3.org/TR/css3-fonts/#font-size-prop) | defaults to bobgui-font-name setting |
...
|-bobgui-dpi|[Number](https://www.w3.org/TR/css3-values/#number-value) | defaults to screen resolution |
...
|-bobgui-secondary-caret-color|[Color](https://www.w3.org/TR/css-color-3/#valuea-def-color) | used for the secondary caret in bidirectional text |
...
|-bobgui-icon-source| [Image](https://www.w3.org/TR/css-backgrounds-3/#typedef-image), `builtin` or `none` | used for builtin icons in buttons and expanders |
|-bobgui-icon-size| [Length](https://www.w3.org/TR/css3-values/#length-value) | size used for builtin icons in buttons and expanders |
|-bobgui-icon-style| `requested`, `regular` or `symbolic` | preferred style for application-loaded icons |
|-bobgui-icon-transform| [Transform list](https://www.w3.org/TR/css-transforms-1/#typedef-transform-list) or `none` | applied to builtin and application-loaded icons |
|-bobgui-icon-palette| Color palette, as explained above | used to recolor symbolic icons |
|-bobgui-icon-shadow| [Shadow](https://www.w3.org/TR/css-backgrounds-3/#typedef-shadow) or `none` | applied to builtin and application-loaded icons |
|-bobgui-icon-filter| [Filter value list](https://www.w3.org/TR/filter-effects-1/#typedef-filter-value-list) or `none` | applied to builtin and application-loaded icons |
|-bobgui-icon-weight | Like [font weight](https://www.w3.org/TR/css3-fonts/#font-weight-prop) | applied to builtin and application-loaded, stroked symbolic icons |
...
|border-spacing| [CSS Table Level 3](https://www.w3.org/TR/css-tables-3/#border-spacing-property) | respected by BobguiBoxLayout, BobguiGridLayout, BobguiCenterLayout |

## Media Queries

Since BOBGUI 4.20 [CSS Media Queries](https://www.w3.org/TR/mediaqueries-5/) are supported.
Media queries can include `not`, `and`, and `or` expressions.

BOBGUI supports the following media features:
