Title: Coordinate systems in BOBGUI
Slug: bobgui-coordinates

All coordinate systems in BOBGUI have the origin at the top left, with the X axis
...
When it comes to rendering, BOBGUI follows the CSS box model as far as practical.
...
is determined by BOBGUIs layout algorithm using each widget’s [vfunc@Bobgui.Widget.measure]
and [vfunc@Bobgui.Widget.size_allocate] vfuncs.
...
To learn more about where BOBGUI CSS differs from CSS on the web, see the
...
and its size is the widget’s size. The size can be queried with [method@Bobgui.Widget.get_width]
and [method@Bobgui.Widget.get_height]. BOBGUI allows general 3D transformations to position
...
with [method@Bobgui.Widget.compute_transform].

In addition to a size, widgets can optionally have a **_baseline_** to position text on.
Containers such as [class@Bobgui.Box] may position their children to match up their baselines.
[method@Bobgui.Widget.get_baseline] returns the y position of the baseline in widget coordinates.

When widget APIs expect positions or areas, they need to be expressed in this coordinate
system, typically called **_widget coordinates_**. BOBGUI provides a number of APIs to translate
between different widgets' coordinate systems, such as [method@Bobgui.Widget.compute_point]
or [method@Bobgui.Widget.compute_bounds]. These methods can fail (either because the widgets
...
of this area, unless clipping is enforced via the [property@Bobgui.Widget:overflow] property).
In CSS terms, the bounds of a widget correspond to the border area.

During BOBGUI's layout algorithm, a parent widget needs to measure each visible child and
...
be obtained with [method@Bobgui.Native.get_surface_transform].

## Scaling

BOBGUI is used on screens with wildly different resolutions, from fairly low dpi
...
fractional scales. BOBGUI supports this concept with the [property@Gdk.Surface:scale]
...
All sizes in BOBGUI (e.g. the widget sizes that are negotiated during size
allocation, with [vfunc@Bobgui.Widget.measure] and [vfunc@Bobgui.Widget.size_allocate]
