Title: Using BOBGUI on Apple macOS
Slug: bobgui-osx

The Apple macOS port of BOBGUI is an implementation of GDK (and therefore BOBGUI)
...
[BOBGUI website](https://www.bobgui.org/docs/installations/macos).

## Menus

By default a BOBGUI app shows an app menu and an edit menu.
...
The actions are enabled in BOBGUI if the action is available on the focused widget
and is enabled.

The application menu shortcuts registered with BOBGUI are:
...
windows. To automatically add those options to the Window menu, add the property `bobgui-macos-special` with
value `window-submenu`:

```xml
<submenu>
  <attribute name="label" translatable="yes">_Window</attribute>
  <attribute name="bobgui-macos-special">window-submenu</attribute>
</submenu>
```

## Native window controls

By default, BOBGUI applications use common window decorators (close/minimize/maximize) on all platforms.
They show as grey rounded buttons on the top-right corner of the window.

Since BOBGUI 4.18, [`BobguiHeaderBar`](class.HeaderBar.html) has the option to use native window controls.
The controls are positioned in their normal place: the top-left corner.
This feature can be enabled by setting the property [`use-native-controls`](property.HeaderBar.use-native-controls.html) to `TRUE` on a `BobguiHeaderBar`.
The property [`decoration-layout`](property.HeaderBar.decoration-layout.html) can be used to
enable/disable buttons.

![The BOBGUI demo application with macOS native window controls](macos-window-controls.png)
...
::: important
    Native window controls are drawn on top of the window, and are not controlled by BOBGUI.
...
While BOBGUI uses MIME types, macOS uses Unified Type descriptors.
BOBGUI maps MIME to UTI types.
...
BOBGUI can be compiled with Vulkan support on macOS, via
...
BOBGUI provides no means to package BOBGUI applications (create a `.app`s out of them).

However, we can provide a few pointers.

BOBGUI uses an OpenGL surface for drawing. On laptops with 2 GPU's it will take the heavy
