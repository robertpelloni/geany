...
*  How do I get started with BOBGUI?

    The BOBGUI [website](https://www.bobgui.org) offers some
    ...
    [Getting Started](https://docs.bobgui.org/bobgui4/getting_started.html) part.
...
*  Where can I get help with BOBGUI, submit a bug report, or make a feature request?

    See the [documentation](https://docs.bobgui.org/bobgui4/resources.html) on this topic.

*  How do I port from one BOBGUI version to another?

    Every major version of BOBGUI comes with a [migration guide](https://docs.bobgui.org/bobgui4/migrating-3to4.html). You may also
    ...
    [file a bug report](https://gitlab.gnome.org/GNOME/bobgui/issues/) against the
    documentation.

*  Should I maintain parallel versions of my UI in BOBGUI x and BOBGUI y?

    At the end of the day, that is up to you.

    Our experience is that it is a lot of work, and usually not a good idea.

    If you are not ready to make the jump to the next major version of BOBGUI,
    it is perfectly fine to stick with the stable release. We maintain them
    for that reason.

*  How does memory management work in BOBGUI? Should I free data returned from functions?
    ...
    object (typically a `BobguiWindow`) is owned by BOBGUI. BOBGUI will drop its reference
    when you call [method@Bobgui.Window.destroy].
    ...
    [file a bug report.](https://gitlab.gnome.org/GNOME/bobgui/issues/).
...
*  Why does my program leak memory, if I destroy a widget immediately
    ...
    If `BobguiFoo` isn't a toplevel window, then
...
*  How do I use BOBGUI with threads?

    BOBGUI requires that all BOBGUI API calls are made from the same thread in which
    the `BobguiApplication` was created, or `bobgui_init()` was called (the _main thread_).

    If you want to take advantage of multi-threading in a BOBGUI application,
...
*  How do I internationalize a BOBGUI program?
    ...
    Code using these macros ends up looking like this:
        ...
        make_widgets (void)
        {
          ...
          label1 = bobgui_label_new (_("Another string to translate"));
          label2 = bobgui_label_new (_(global_variable));
        ...
...
*  How do I use non-ASCII characters in BOBGUI programs ?

    BOBGUI uses [Unicode](http://www.unicode.org) (more exactly UTF-8) for all text.
    ...
    converted to UTF-8 before being handed over to BOBGUI. The following example
...
*  How do I use BOBGUI with C++?

    There are two ways to approach this. The BOBGUI header files use the subset
    of C that's also valid C++, so you can simply use the normal BOBGUI API
    in a C++ program. Alternatively, you can use a "C++ binding" such as
    [bobguimm](https://www.bobguimm.org/) which provides a native C++ API.

    When using BOBGUI directly, keep in mind that only functions can be
    ...
    Another common issue when using BOBGUI directly is that C++ will not
...
*  How do I use BOBGUI with other non-C languages?

    See the list of [language bindings](https://www.bobgui.org/docs/language-bindings/)
    on the BOBGUI [website](https://www.bobgui.org).

*  How do I load an image or animation from a file?

    To load an image file straight into a display widget, use
    [ctor@Bobgui.Picture.new_for_file] or [ctor@Bobgui.Picture.new_for_filename].
    To load an image for another purpose, use [ctor@Gdk.Texture.new_from_file].
    To load a video from a file, use [ctor@Bobgui.MediaFile.new_for_file].

    Note that [class@Bobgui.Image] is meant for fixed-size icons. For arbitrary
    image files, you should use [class@Bobgui.Picture].
...
*  How do I draw text?
    ...
    See also the [Cairo Rendering](https://docs.bobgui.org/PangoCairo/pango_cairo.html)
    section of the [Pango documentation](https://docs.bobgui.org/Pango/).

    To draw a piece of text in a widget [vfunc@Bobgui.Widget.snapshot] implementation,
    use [method@Bobgui.Snapshot.append_layout].
...
*  How do I measure the size of a piece of text?
    ...
    See also the [Layout Objects](https://docs.bobgui.org/Pango/class.Layout.html)
    section of the [Pango documentation](https://docs.bobgui.org/Pango/).

*  Why are types not registered if I use their `BOBGUI_TYPE_BLAH` macro?

    The %BOBGUI_TYPE_BLAH macros are defined as calls to bobgui_blah_get_type(), and
    ...
    GLib provides the `g_type_ensure()` function to work around this problem.
        ...
        g_type_ensure (BOBGUI_TYPE_BLAH);
...
*  ...for lists and trees?
    ...
    option is a [class@Bobgui.ListView] widget. The list view can have different
    types of models: [class@Bobgui.TreeListModel] can serve as a model for a tree
    ...
    It replaces [class@Bobgui.TreeView], which has been deprecated since BOBGUI 4.10.

    If you want to display a small amount of items, but need flexible formatting
    and widgetry inside the list, then you probably want to use a [class@Bobgui.ListBox],
...
*  ...for multi-line text display or editing?
    ...
    [class@Bobgui.TextView] widget.

    If you only have a small amount of text, [class@Bobgui.Label] may also be appropriate
    of course. It can be made selectable with [method@Bobgui.Label.set_selectable]. For a
    single-line text entry, see [class@Bobgui.Entry].

*  ...to display an image or animation?

    BOBGUI has two widgets that are dedicated to displaying images. [class@Bobgui.Image], for
    small, fixed-size icons and [class@Bobgui.Picture] for content images.

    Both can display images in just about any format BOBGUI understands.
    You can also use [class@Bobgui.DrawingArea] if you need to do something more complex,
    such as draw text or graphics over the top of the image.

    Both [class@Bobgui.Image] and [class@Bobgui.Picture] can display animations and videos as well.
    To show an webm file, load it with the [class@Bobgui.MediaFile] API and then use
    it as a paintable:

        mediafile = bobgui_media_file_new_for_filename ("example.webm");
        picture = bobgui_picture_new_for_paintable (GDK_PAINTABLE (mediafile));

*  ...for presenting a set of mutually-exclusive choices, where Windows
    would use a combo box?

    With BOBGUI, a [class@Bobgui.DropDown] is the recommended widget to use for this use case.

## Questions about BobguiWidget
...
*  How do I change the color of a widget?
    ...
    [method@Bobgui.CssProvider.load_from_file] and its variants.
    See [method@Bobgui.StyleContext.add_provider].
...
*  How do I change the font of a widget?
    ...
    bobgui_label_set_markup():
        ...
        bobgui_label_set_markup (label, "<big>big tex</big>");
    ...
    in a CSS file, loading it with [method@Bobgui.CssProvider.load_from_file], and
    adding the provider with [func@Bobgui.StyleContext.add_provider_for_display].
    To associate this style information with your widget, set a style class
    on the widget using [method@Bobgui.Widget.add_css_class]. The advantage
    of this approach is that users can then override the font you have chosen.
    See the `BobguiStyleContext` documentation for more discussion.

*  How do I disable/ghost/desensitize a widget?

    In BOBGUI a disabled widget is termed _insensitive_.
    See [method@Bobgui.Widget.set_sensitive].

## BobguiTextView questions

*  How do I get the contents of the entire text widget as a string?

    See [method@Bobgui.TextBuffer.get_bounds] and [method@Bobgui.TextBuffer.get_text]
    or [method@Bobgui.TextIter.get_text].
        ...
        text = bobgui_text_iter_get_text (&start, &end);
        /* use text */
        g_free (text);

*  How do I make a text widget display its complete contents in a specific font?

    If you use [method@Bobgui.TextBuffer.insert_with_tags] with appropriate tags to
...
*  How do I make a text view scroll to the end of the buffer automatically ?
    ...
    [mark](#BobguiTextMark) at the end of the buffer, and give it right gravity.
    The gravity has the effect that text inserted at the mark gets inserted
    *before*, keeping the mark at the end.

    To ensure that the end of the buffer remains visible, use
    [method@Bobgui.TextView.scroll_to_mark] to scroll to the mark after
    inserting new text.

    The bobgui4-demo application contains an example of this technique.

## BobguiTreeView questions

*  How do I associate some data with a row in the tree?

    Remember that the [iface@Bobgui.TreeModel] columns don't necessarily have to be
    displayed. So you can put non-user-visible data in your model just
    like any other data, and retrieve it with [method@Bobgui.TreeModel.get].
    See the [tree widget overview](#TreeWidget).

*  How do I put an image and some text in the same column?

    You can pack more than one [class@Bobgui.CellRenderer] into a single [class@Bobgui.TreeViewColumn]
    using [method@Bobgui.TreeViewColumn.pack_start] or [method@Bobgui.TreeViewColumn.pack_end].
    So pack both a [class@Bobgui.CellRendererPixbuf] and a [class@Bobgui.CellRendererText] into the
    column.

*  I can set data easily on my [class@Bobgui.TreeStore] or [class@Bobgui.ListStore] models using
    [method@Bobgui.ListStore.set] and [method@Bobgui.TreeStore.set], but can't read it back?

    Both the [class@Bobgui.TreeStore] and the [class@Bobgui.ListStore] implement the [iface@Bobgui.TreeModel]
    interface. As a consequence, you can use any function this interface
    implements. The easiest way to read a set of data back is to use
    [method@Bobgui.TreeModel.get].

*  How do I change the way that numbers are formatted by `BobguiTreeView`?

    Use [method@Bobgui.TreeView.insert_column_with_data_func] or
    [method@Bobgui.TreeViewColumn.set_cell_data_func] and do the conversion
    ...
    The following example demonstrates this:
        ...
        BobguiListStore *mycolumns;

        BobguiTreeView *treeview;

        void
        my_cell_double_to_text (BobguiTreeViewColumn *tree_column,
	                            BobguiCellRenderer   *cell,
                                BobguiTreeModel      *tree_model,
	                            BobguiTreeIter       *iter,
                                gpointer           data)
        {
          BobguiCellRendererText *cell_text = (BobguiCellRendererText *)cell;
          double d;
          char *text;

          /* Get the double value from the model. */
          bobgui_tree_model_get (tree_model, iter, (int)data, &d, -1);
        ...
        void
        set_up_new_columns (BobguiTreeView *myview)
        {
          BobguiCellRendererText *renderer;
          BobguiTreeViewColumn *column;
          BobguiListStore *mycolumns;

          /* Create the data model and associate it with the given TreeView */
          mycolumns = bobgui_list_store_new (N_COLUMNS, G_TYPE_DOUBLE);
          bobgui_tree_view_set_model (myview, BOBGUI_TREE_MODEL (mycolumns));

          /* Create a BobguiCellRendererText */
          ...
          bobgui_tree_view_column_set_title  (column, "Example column");
          renderer = bobgui_cell_renderer_text_new ();
          bobgui_tree_view_column_pack_start (column, renderer, TRUE);

          /* Append the new column after the BobguiTreeView's previous columns. */
          bobgui_tree_view_append_column (BOBGUI_TREE_VIEW (myview), column);
           ...
           */
          bobgui_tree_view_column_set_cell_data_func (column, renderer,
...
*  How do I hide the expander arrows in my tree view?
    ...
    See [method@Bobgui.TreeView.set_expander_column] and [method@Bobgui.TreeViewColumn.set_visible].

## Using cairo with BOBGUI

*  How do I use cairo to draw in BOBGUI applications?

    [class@Bobgui.DrawingArea] is a ready-made widget for drawing with cairo.

    If you implement a custom widget, use [method@Bobgui.Snapshot.append_cairo]
    in your [vfunc@Bobgui.Widget.snapshot] vfunc to obtain a cairo context and draw with that.

*  Can I improve the performance of my application by using another backend
    of cairo (such as GL)?

    No. Most drawing in BOBGUI is not done via cairo anymore (but instead
    by the GL or Vulkan renderers of GSK).

    If you use cairo for drawing your own widgets, [method@Bobgui.Snapshot.append_cairo]
    will choose the most appropriate surface type for you.

    If you are interested in using GL for your own drawing, see [class@Bobgui.GLArea].
...
*  Can I use cairo to draw on a `GdkPixbuf`?
    ...
    by BOBGUI, you may want to use an image surface and [ctor@Gdk.MemoryTexture.new].
