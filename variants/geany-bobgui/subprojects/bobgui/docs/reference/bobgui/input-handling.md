Title: Overview of BOBGUI input and event handling
Slug: input-overview

This chapter describes in detail how BOBGUI handles input. If you are interested
in what happens to translate a key press or mouse motion of the users into a
change of a BOBGUI widget, you should read this chapter. This knowledge will also
be useful if you decide to implement your own widgets.

## Devices and events

The most basic input devices that every computer user has interacted with are
keyboards and mice; beyond these, BOBGUI supports touchpads, touchscreens and
more exotic input devices such as graphics tablets. Inside BOBGUI, every such
input device is represented by a `GdkDevice` object.

To simplify dealing with the variability between these input devices, BOBGUI
...
BOBGUI widgets generally deal with the logical devices, and thus can be used
with any pointing device or keyboard.

When a user interacts with an input device (e.g. moves a mouse or presses
a key on the keyboard), BOBGUI receives events from the windowing system.
...
When BOBGUI creates a `GdkSurface`, it connects to the ::event
...
The function which initially receives input events on the BOBGUI
...
controllers attached with `BOBGUI_PHASE_BUBBLE` are run.
...
Every `BobguiWindow` maintains a single focus location (in the :focus-widget
...
“keyboard navigation”).
BOBGUI reserves the <kbd>Tab</kbd> key to move the focus to the next location,
...
entries or text views, BOBGUI widgets can use key events for activating
...
BOBGUI has traditionally supported different kinds of shortcuts:
...
- Mnemonics are usually triggered using <kbd>Alt</kbd>
  ...
  case, inside menus (i.e. inside `BobguiPopoverMenu`), mnemonics can be
...
BOBGUI handles accelerators and mnemonics in a global scope, during the
capture phase, and key bindings locally, during the target phase.

Under the hood, all shortcuts are represented as instances of `BobguiShortcut`,
and they are managed by `BobguiShortcutController`.

Note that BOBGUI does not do anything to map the primary shortcut modifier
...
The <kbd>Command</kbd> is named `Meta` (`GDK_META_MASK`) in BOBGUI.
...
input method context, use bobgui_im_multicontext_new(), to provide it with
input, use bobgui_event_controller_key_set_im_context().
...
Shortcuts are handled by `BobguiShortcutController`, which is
