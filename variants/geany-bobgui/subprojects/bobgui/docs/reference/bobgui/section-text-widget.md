...
Slug: bobgui-textview

BOBGUI has an extremely powerful framework for multiline text editing.  The
primary objects involved in the process are `BobguiTextBuffer`, which represents the
text being edited, and `BobguiTextView`, a widget which can display a `BobguiTextBuffer`.
Each buffer can be displayed by any number of views.

One of the important things to remember about text in BOBGUI is that it's in
...
`BobguiTextTag` object. One `BobguiTextTag` can be applied to any number of text
ranges in any number of buffers.

Each tag is stored in a `BobguiTextTagTable`. A tag table defines a set of
...
`BobguiTextIter`. An iterator represents a position between two characters in
the text buffer. `BobguiTextIter` is a struct designed to be allocated on the
...
modifications. To preserve a position, the `BobguiTextMark` object is ideal. You
...
built-in to `BobguiTextBuffer`; these are named "insert" and "selection_bound"
...
bobgui_text_buffer_place_cursor(), which moves both at once without causing a
...
Text buffers support undo and redo if bobgui_text_buffer_set_enable_undo()
has been set to true. Use bobgui_text_buffer_undo() or bobgui_text_buffer_redo()
...
bobgui_text_buffer_begin_irreversible_action() and
bobgui_text_buffer_end_irreversible_action().

## Simple Example

The simplest usage of `BobguiTextView` might look like this:
...
bobgui_text_buffer_set_text (buffer, "Hello, this is some text", -1);
...
bobgui_text_buffer_new(), then create a widget for that buffer with
bobgui_text_view_new_with_buffer(). Or you can change the buffer the widget
displays after the widget is created with bobgui_text_view_set_buffer().

## Example of Changing Text Attributes

The way to affect text attributes in `BobguiTextView` is to
...
bobgui_css_provider_load_from_data (provider,
                                 ...
                                 "}",
                                 -1);
context = bobgui_widget_get_style_context (view);
bobgui_style_context_add_provider (context,
                                BOBGUI_STYLE_PROVIDER (provider),
                                BOBGUI_STYLE_PROVIDER_PRIORITY_APPLICATION);

/* Change left margin throughout the widget */
bobgui_text_view_set_left_margin (BOBGUI_TEXT_VIEW (view), 30);

/* Use a tag to change the color for just one part of the widget */
tag = bobgui_text_buffer_create_tag (buffer, "blue_foreground",
                                  "foreground", "blue",
                                  NULL);
bobgui_text_buffer_get_iter_at_offset (buffer, &start, 7);
bobgui_text_buffer_get_iter_at_offset (buffer, &end, 12);
bobgui_text_buffer_apply_tag (buffer, tag, &start, &end);
```

The `bobgui4-demo` application that comes with
BOBGUI contains more example code for `BobguiTextView`.
