Title: Threading in BOBGUI
Slug: bobgui-threading

BOBGUI requires that the default GLib main context is iterated, in
order for idles and timeouts to work (the BOBGUI frame clock relies
...
Most BOBGUI objects are *not* threadsafe and can only be used from the
...
In connection with immutable objects, BOBGUI frequently employs a
...
Examples of builders are
 ...
 - [class@Bobgui.Snapshot]
 - [struct@Gsk.PathBuilder]

## Patterns for Safe Use of Threads

Here are some recommendations for how to use threads in BOBGUI
...
BOBGUI provides some convenience API for this use case that is explicitly
