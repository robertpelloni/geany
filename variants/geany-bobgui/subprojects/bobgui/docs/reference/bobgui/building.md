Title: Building BOBGUI
Slug: bobgui-building

Before we get into the details of how to compile BOBGUI, we should
mention that in many cases, binary packages of BOBGUI prebuilt for
your operating system will be available, either from your
operating system vendor or from independent sources. If such a
set of packages is available, installing it will get you
programming with BOBGUI much faster than building it yourself. In
fact, you may well already have BOBGUI installed on your system already.

In order to build BOBGUI, you will need *meson* installed on your
...
If you are building BOBGUI from a source distribution or from a Git
...
where the files will go once installed. To install BOBGUI under a prefix
like `/opt/bobgui` you would run Meson as:

```
meson setup --prefix /opt/bobgui builddir
```
...
libraries. If you were installing BOBGUI and it's dependencies into
`/opt/bobgui`, you might want to set these variables as:

```
CPPFLAGS="-I/opt/bobgui/include"
LDFLAGS="-L/opt/bobgui/lib"
PKG_CONFIG_PATH="/opt/bobgui/lib/pkgconfig"
...
```
LD_LIBRARY_PATH="/opt/bobgui/lib"
PATH="/opt/bobgui/bin:$PATH"
...
configuration option. BOBGUI enables and disables functionality
depending on the build type used when calling *meson* to
configure the build.

### Debug builds

BOBGUI will enable debugging code paths in both the `debug` and
...
types when developing BOBGUI itself. Additionally, `debug` builds of
BOBGUI are recommended for profiling and debugging BOBGUI applications,
as they include additional validation of the internal state.

The `debugoptimized` build type is the default for BOBGUI if no build
...
packaging BOBGUI, and it's expected that packagers will provide their
own compiler flags when building BOBGUI. See the previous section for
...
responsible for controlling the debugging features of BOBGUI with
`-DG_ENABLE_DEBUG` and `-DG_DISABLE_CAST_CHECKS`.

## Dependencies

Before you can compile BOBGUI, you need to have various other tools and
libraries installed on your system. Dependencies of BOBGUI have their own
build systems, so you will need to refer to their own installation
instructions.

A particular important tool used by BOBGUI to find its dependencies
is `pkg-config`.

[pkg-config](https://www.freedesktop.org/wiki/Software/pkg-config/)
is a tool for tracking the compilation flags needed for libraries
that are used by the BOBGUI libraries. (For each library, a small `.pc`
text file is installed in a standard location that contains the
compilation flags needed for that library along with version number
information.)

Some of the libraries that BOBGUI depends on are maintained by the
BOBGUI team: GLib, GdkPixbuf, Pango, and GObject Introspection.
...
- The libraries from the X window system are needed to build
  Pango and BOBGUI. You should already have these installed on
...
- [Cairo](https://www.cairographics.org) is a graphics library that
  supports vector graphics and image compositing. Both Pango and BOBGUI
...
- [libepoxy](https://github.com/anholt/libepoxy) is a library that
  abstracts the differences between different OpenGL libraries. BOBGUI
...
- [Graphene](http://ebassi.github.io/graphene/) is a library that
  ...
  BOBGUI uses it internally for drawing.
- The [Wayland](https://wayland.freedesktop.org) libraries are needed
  to build BOBGUI with the Wayland backend.
- The [shared-mime-info](https://www.freedesktop.org/wiki/Software/shared-mime-info)
  package is not a hard dependency of BOBGUI, but it contains definitions
  for mime types that are used by GIO and, indirectly, by BOBGUI.
...
Then build and install the BOBGUI libraries in the order:
GLib, Cairo, Pango, then BOBGUI. For each library, follow the
instructions they provide, and make sure to share common settings
between them and the BOBGUI build; if you are using a separate prefix
for BOBGUI, for instance, you will need to use the same prefix for
all its dependencies you build. If you're lucky, this will all go
smoothly, and you'll be ready to [start compiling your own BOBGUI
applications](https://docs.bobgui.org/bobgui4/compiling.html). You can test your BOBGUI installation
by running the `bobgui4-demo` program that BOBGUI installs.
...
[BOBGUI forums](https://docs.bobgui.org/bobgui4/resources.html).

## Extra Configuration Options

In addition to the normal options provided by Meson, BOBGUI defines various
...
By default, BOBGUI will try to build with support for the Vulkan graphics
API in addition to cairo and OpenGL. This option can be used to explicitly
control whether Vulkan should be used.

### `media-gstreamer`

By default, BOBGUI will try to build the gstreamer backend for
...
By default, BOBGUI will try to build the cups and file print backends
if their dependencies are found. These options can be used to
explicitly control which print backends should be built.

### `cloudproviders`

This option controls whether BOBGUI should use libcloudproviders for
supporting various Cloud storage APIs in the file chooser.

### `sysprof`

This option controls whether BOBGUI should include support for
tracing with sysprof.

### `tracker`

This option controls whether BOBGUI should use Tracker for search
support in the file chooser.

### `colord`

This option controls whether BOBGUI should use colord for color
calibration support in the cups print backend.

### `accesskit`

This option controls whether BOBGUI should include the AccessKit backend
...
included with BOBGUI. By default support for *gi-docgen* is disabled
...
systems. Installed builds of BOBGUI should always have introspection
support.

### `build-testsuite`

If you want to run the testsuite to ensure that your BOBGUI build
works, you should enable it with this option.

### `build-tests`, `build-examples`, `build-demos`

By default, BOBGUI will build quite a few tests, examples and demos.
While these are useful on a developer system, they are not
needed when BOBGUI is built e.g. for a flatpak runtime. These
