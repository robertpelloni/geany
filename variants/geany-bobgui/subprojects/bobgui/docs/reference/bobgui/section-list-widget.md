...
Slug: bobgui-list-widget

BOBGUI provides powerful widgets to display and edit lists of data. This document
...
Examples of these widgets would be [`class@Bobgui.ListView`] or [`class@Bobgui.GridView`].
 ...
 * List model implementations for many specific types of data already exist, for
   example [`class@Bobgui.DirectoryList`] or [`class@Bobgui.StringList`].

 * There are generic list model implementations like [`class@Gio.ListStore`] that allow building
   lists of arbitrary objects.

 * Wrapping list models like [`class@Bobgui.FilterListModel`] or [`class@Bobgui.SortListModel`]
...
a [`class@Bobgui.ListItemFactory`] implementation that takes care of mapping the
...
is currently in use. List items are always [`class@Bobgui.ListItem`] instances.
They are only ever created by BOBGUI and provide information about what item they
...
an implementation of the [`iface@Bobgui.SelectionModel`] interface on top of the
...
either by implementing `BobguiSelectionModel` directly or by wrapping a model with
one of the BOBGUI models provided for this purposes, such as [`class@Bobgui.NoSelection`]
or [`class@Bobgui.SingleSelection`].
...
item is exposed in the listitem via the [`property@Bobgui.ListItem:selected`] property.
...
to emit a signal such as [`signal@Bobgui.ListView::activate`]. This provides an
...
[actions](https://docs.bobgui.org/bobgui4/actions.html). This allows developers to add widgets to their
lists that cause selections to change or to trigger activation via the
[`iface@Bobgui.Actionable`] interface. For a list of all supported actions
...
the [`iface@Bobgui.Scrollable`] interface and expecting to be placed directly into
a [`class@Bobgui.ScrolledWindow`].
...
the number of listitems they create such as with [`method@Bobgui.GridView.set_max_columns`],
...
BOBGUI offers a wide variety of wrapping models which change or supplement an
...
[`class@Gio.ListStore`], [`class@Bobgui.StringList`], and [`class@Bobgui.DirectoryList`].
...
`BobguiStringList` is not a general store - it can only handle strings. It is
backed by an dynamically allocated array and has performance characteristics
that are expected for that data structure. `BobguiStringList` is a good fit for any
place where you would otherwise use `char*[]` and works best if the dataset
is not very dynamic.

`BobguiDirectoryList` is a list model that wraps [`method@Gio.File.enumerate_children_async`].
...
While `BobguiTreeView` provided built-in support for trees, the list widgets, and
...
However, BOBGUI provides functionality to make lists look and behave like trees
for use cases that require trees. This is achieved by using the
[`class@Bobgui.TreeListModel`] model to flatten a tree into a list. The
[`class@Bobgui.TreeExpander`] widget can then be used inside a listitem to allow
...
One of the advantages of the new list widgets over `BobguiTreeView` and cell
renderers is that they are styleable using BOBGUI CSS. This provides a lot of
flexibility. The themes that ship with BOBGUI provide a few predefined list
...
the `BobguiSectionModel` interface. `BobguiListView` can
display headers for sections, by installing a separate **_header factory_**.

Many BOBGUI list models support section inherently, or they pass through the
section of a model they are wrapping.

## Comparison to BobguiTreeView

Developers familiar with `BobguiTreeView` may wonder how this way of doing lists
...
The other big difference is the massive change to the data model. `BobguiTreeModel`
...
`GListModel::items-changed` signal while `BobguiTreeModel` only allows a single
...
while `BobguiTreeRowReference` was a very slow mechanism to achieve the same.
...
| Old                  | New                                                     |
| -------------------- | ------------------------------------------------------- |
| `BobguiTreeModel`       | [`iface@Gio.ListModel`]                                 |
| `BobguiTreePath`        | `guint` position, [`class@Bobgui.TreeListRow`]             |
| `BobguiTreeIter`        | `guint` position                                        |
| `BobguiTreeRowReference`| [`class@GObject.Object`] item                           |
| `BobguiListStore`       | [`class@Gio.ListStore`]                                 |
| `BobguiTreeStore`       | [`class@Bobgui.TreeListModel`], [`class@Bobgui.TreeExpander`] |
| `BobguiTreeSelection`   | [`iface@Bobgui.SelectionModel`]                            |
| `BobguiTreeViewColumn`  | [`class@Bobgui.ColumnView`]                                |
| `BobguiTreeView`        | [`class@Bobgui.ListView`], [`class@Bobgui.ColumnView`]        |
| `BobguiCellView`        | [`class@Bobgui.ListItem`]                                  |
| `BobguiComboBox`        | [`class@Bobgui.DropDown`]                                  |
| `BobguiIconView`        | [`class@Bobgui.GridView`]                                  |
| `BobguiTreeSortable`    | [`class@Bobgui.ColumnView`]                                |
| `BobguiTreeModelSort`   | [`class@Bobgui.SortListModel`]                             |
| `BobguiTreeModelFilter` | [`class@Bobgui.FilterListModel`]                           |
| `BobguiCellLayout`      | [`class@Bobgui.ListItemFactory`]                           |
| `BobguiCellArea`        | [`class@Bobgui.Widget`]                                    |
| `BobguiCellRenderer`    | [`class@Bobgui.Widget`]                                    |
