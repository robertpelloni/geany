...
controllers in BOBGUI.

## Drag sources

To make data available via DND, you create a [class@Bobgui.DragSource]. Drag sources
...
is about to begin, by connecting to the [signal@Bobgui.DragSource::prepare] signal.

The BobguiDragSource emits the [signal@Bobgui.DragSource::drag-begin] signal when the DND
operation starts, and the [signal@Bobgui.DragSource::drag-end] signal when it is done.
...
To receive data via DND, you create a [class@Bobgui.DropTarget], and tell it what kind of
data to accept. You need to connect to the [signal@Bobgui.DropTarget::drop] signal to receive
the data when a DND operation occurs.

While a DND operation is ongoing, BOBGUI provides updates when the pointer moves over
the widget to which the drop target is associated. The [signal@Bobgui.DropTarget::enter],
[signal@Bobgui.DropTarget::leave] and [signal@Bobgui.DropTarget::motion] signals get emitted
for this purpose.

BobguiDropTarget provides a simple API, and only provides the data when it has been completely
transferred. If you need to handle the data transfer yourself (for example to provide progress
information during the transfer), you can use the more complicated [class@Bobgui.DropTargetAsync].
...
[class@Bobgui.DropControllerMotion] is an event controller that can help with implementing such
behaviors. It is very similar to [class@Bobgui.EventControllerMotion], but provides events during
a DND operation.
