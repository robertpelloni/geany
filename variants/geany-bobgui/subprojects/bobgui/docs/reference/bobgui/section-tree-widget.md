...
Slug: bobgui-treeview

This document describes the `BobguiTreeView` widget and auxiliary
classes, like tree models and cell renderers. All of these have
been deprecated and will be removed in BOBGUI 5. Their replacements
are described in the [List Widget Overview](section-list-widget.html).

## Introduction

To create a tree or list in BOBGUI, use the `BobguiTreeModel` interface in
conjunction with the `BobguiTreeView` widget. This widget is designed around
a _Model/View/Controller_ design and consists of four major parts:

- The tree view widget (BobguiTreeView)
- The view column (BobguiTreeViewColumn)
- The cell renderers (BobguiCellRenderer etc.)
- The model interface (BobguiTreeModel)
...
BOBGUI provides two simple models that can be used: the `BobguiListStore`
and the `BobguiTreeStore`. `BobguiListStore` is used to model list widgets,
while the `BobguiTreeStore` models trees. It is possible to develop a new
...
``` {.c}
BobguiListStore *store = bobgui_list_store_new (2, G_TYPE_STRING, G_TYPE_BOOLEAN);
```
...
BobguiTreeStore *store = bobgui_tree_store_new (N_COLUMNS,       /* Total number of columns */
                                          G_TYPE_STRING,   /* Book title              */
                                          G_TYPE_STRING,   /* Author                  */
                                          G_TYPE_BOOLEAN); /* Is checked out?         */
```

Adding data to the model is done using bobgui_tree_store_set() or
bobgui_list_store_set(), depending upon which sort of model was
created. To do this, a `BobguiTreeIter` must be acquired. The iterator
points to the location where data will be added.

Once an iterator has been acquired, bobgui_tree_store_set() is used to
...
bobgui_tree_store_append (store, &iter, NULL);  /* Acquire an iterator */
...
The third argument to bobgui_tree_store_append() is the parent iterator.
It is used to add a row to a BobguiTreeStore as a child of an existing row.
...
``` {.c}
BobguiTreeIter iter1;  /* Parent iter */
BobguiTreeIter iter2;  /* Child iter  */

bobgui_tree_store_append (store, &iter1, NULL);  /* Acquire a top-level iterator */
bobgui_tree_store_set (store, &iter1,
                    TITLE_COLUMN, "The Art of Computer Programming",
                    AUTHOR_COLUMN, "Donald E. Knuth",
                    CHECKED_COLUMN, FALSE,
                    -1);

bobgui_tree_store_append (store, &iter2, &iter1);  /* Acquire a child iterator */
bobgui_tree_store_set (store, &iter2,
                    TITLE_COLUMN, "Volume 1: Fundamental Algorithms",
                    -1);

bobgui_tree_store_append (store, &iter2, &iter1);
bobgui_tree_store_set (store, &iter2,
...
or the tree store. Setting up a `BobguiTreeView` is not a difficult
matter. It needs a `BobguiTreeModel` to know where to retrieve its data
from.

``` {.c}
BobguiWidget *tree;

tree = bobgui_tree_view_new_with_model (BOBGUI_TREE_MODEL (store));
```

## Columns and cell renderers

Once the `BobguiTreeView` widget has a model, it will need to know how
...
way. There are a number of cell renderers that come with BOBGUI,
including the `BobguiCellRendererText`, `BobguiCellRendererPixbuf` and
the `BobguiCellRendererToggle`. It is relatively easy to write a
custom renderer.

A `BobguiTreeViewColumn` is the object that `BobguiTreeView` uses to organize
...
column = bobgui_tree_view_column_new_with_attributes ("Author",
                                                   renderer,
                                                   "text", AUTHOR_COLUMN,
                                                   NULL);
bobgui_tree_view_append_column (BOBGUI_TREE_VIEW (tree), column);
```
...
`BobguiTreeSelection::changed` signal.

``` {.c}
/* Prototype for selection handler callback */
static void tree_selection_changed_cb (BobguiTreeSelection *selection, gpointer data);

/* Setup the selection handler */
BobguiTreeSelection *select;

select = bobgui_tree_view_get_selection (BOBGUI_TREE_VIEW (tree));
bobgui_tree_selection_set_mode (select, BOBGUI_SELECTION_SINGLE);
...
``` {.c}
static void
tree_selection_changed_cb (BobguiTreeSelection *selection, gpointer data)
{
  BobguiTreeIter iter;
  BobguiTreeModel *model;
  char *author;

  if (bobgui_tree_selection_get_selected (selection, &model, &iter))
    {
      bobgui_tree_model_get (model, &iter, AUTHOR_COLUMN, &author, -1);
...
Here is a simple example of using a `BobguiTreeView` widget in context
...
More information can be found on this in the `BobguiTreeModel` section.
...
setup_tree (void)
{
    ...
    * could use any other BobguiTreeModel */
   ...
   column = bobgui_tree_view_column_new_with_attributes ("Author", renderer,
                                                      "text", AUTHOR_COLUMN,
                                                      NULL);
   ...
   column = bobgui_tree_view_column_new_with_attributes ("Checked out",
                                                      renderer,
                                                      "active", CHECKED_COLUMN,
                                                      NULL);
   bobgui_tree_view_append_column (BOBGUI_TREE_VIEW (tree), column);

   /* Now we can manipulate the view just like any other BOBGUI widget */
   ...
}
```
