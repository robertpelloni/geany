...
#include "bobguiistringprivate.h"
#include "bobguitexthistoryprivate.h"

/*
 * The BobguiTextHistory works in a way that allows text widgets to deliver
 * information about changes to the underlying text at given offsets within
 * their text. The BobguiTextHistory object uses a series of callback functions
 * (see BobguiTextHistoryFuncs) to apply changes as undo/redo is performed.
 *
 * The BobguiTextHistory object is careful to avoid tracking changes while
 * applying specific undo/redo actions.
 *
 * Changes are tracked within a series of actions, contained in groups.  The
 * group may be coalesced when bobgui_text_history_end_user_action() is
 * called.
 *
 * Calling bobgui_text_history_begin_irreversible_action() and
 * bobgui_text_history_end_irreversible_action() can be used to denote a
 * section of operations that cannot be undone. This will cause all previous
 * changes tracked by the BobguiTextHistory to be discarded.
 */
...
struct _BobguiTextHistory
{
  GObject             parent_instance;

  BobguiTextHistoryFuncs funcs;
  gpointer            funcs_data;
...
G_DEFINE_TYPE (BobguiTextHistory, bobgui_text_history, G_TYPE_OBJECT)
...
static inline void
bobgui_text_history_printf_space (GString *str,
                               guint    depth)
{
...
static void
bobgui_text_history_printf_action (Action  *action,
                                GString *str,
                                guint    depth)
{
  bobgui_text_history_printf_space (str, depth);
  g_string_append_printf (str, "%s {\n", action_kind_name (action->kind));

  bobgui_text_history_printf_space (str, depth+1);
    ...
    case ACTION_KIND_DELETE_SELECTION:
      {
        ...
        bobgui_text_history_printf_space (str, depth+2);
      ...
      for (const GList *iter = action->u.group.actions.head; iter; iter = iter->next)
        {
          ...
          bobgui_text_history_printf_action (child, str, depth+2);
          bobgui_text_history_printf_space (str, depth+1);
          g_string_append (str, "}\n");
        }
...
bobgui_text_history_printf (BobguiTextHistory *history)
{
  GString *str = g_string_new (NULL);

  g_string_append (str, "undo {\n");
  for (const GList *iter = history->undo_queue.head; iter; iter = iter->next)
    bobgui_text_history_printf_action (iter->data, str, 1);
  g_string_append (str, "}\n");
  ...
  if (action->kind == ACTION_KIND_GROUP)
    {
       ...
       * items when bobgui_text_history_end_user_action() is called.
       *
...
static void
bobgui_text_history_do_change_state (BobguiTextHistory *self,
                                  gboolean        is_modified,
                                  gboolean        can_undo,
                                  gboolean        can_redo)
{
  g_assert (BOBGUI_IS_TEXT_HISTORY (self));

  self->funcs.change_state (self->funcs_data, is_modified, can_undo, can_redo);
}

static void
bobgui_text_history_do_insert (BobguiTextHistory *self,
                            guint           begin,
                            guint           end,
                            const char     *text,
                            guint           len)
...
static void
bobgui_text_history_do_delete (BobguiTextHistory *self,
                            guint           begin,
                            guint           end,
...
static void
bobgui_text_history_do_select (BobguiTextHistory *self,
...
static void
bobgui_text_history_truncate_one (BobguiTextHistory *self)
{
...
static void
bobgui_text_history_truncate (BobguiTextHistory *self)
{
  g_assert (BOBGUI_IS_TEXT_HISTORY (self));
  ...
  while (self->undo_queue.length + self->redo_queue.length > self->max_undo_levels)
    bobgui_text_history_truncate_one (self);
}

static void
bobgui_text_history_finalize (GObject *object)
{
  BobguiTextHistory *self = (BobguiTextHistory *)object;

  clear_action_queue (&self->undo_queue);
  clear_action_queue (&self->redo_queue);

  G_OBJECT_CLASS (bobgui_text_history_parent_class)->finalize (object);
}

static void
bobgui_text_history_class_init (BobguiTextHistoryClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = bobgui_text_history_finalize;
}

static void
bobgui_text_history_init (BobguiTextHistory *self)
{
  self->enabled = TRUE;
...
static void
bobgui_text_history_update_state (BobguiTextHistory *self)
{
  g_assert (BOBGUI_IS_TEXT_HISTORY (self));
  ...
  bobgui_text_history_do_change_state (self, self->is_modified, self->can_undo, self->can_redo);
}

static void
bobgui_text_history_push (BobguiTextHistory *self,
                       Action         *action)
{
  Action *peek;
  gboolean in_user_action;
  ...
  bobgui_text_history_truncate (self);
  bobgui_text_history_update_state (self);
}

BobguiTextHistory *
bobgui_text_history_new (const BobguiTextHistoryFuncs *funcs,
                      gpointer                   funcs_data)
{
  BobguiTextHistory *self;

  g_return_val_if_fail (funcs != NULL, NULL);

  self = g_object_new (BOBGUI_TYPE_TEXT_HISTORY, NULL);
  self->funcs = *funcs;
...
gboolean
bobgui_text_history_get_can_undo (BobguiTextHistory *self)
{
  g_return_val_if_fail (BOBGUI_IS_TEXT_HISTORY (self), FALSE);

  return self->can_undo;
}

gboolean
bobgui_text_history_get_can_redo (BobguiTextHistory *self)
{
  g_return_val_if_fail (BOBGUI_IS_TEXT_HISTORY (self), FALSE);
...
static void
bobgui_text_history_apply (BobguiTextHistory *self,
                        Action         *action,
                        Action         *peek)
{
  g_assert (BOBGUI_IS_TEXT_HISTORY (self));
  ...
  switch (action->kind)
    {
    case ACTION_KIND_INSERT:
      bobgui_text_history_do_insert (self,
                                  action->u.insert.begin,
                                  action->u.insert.end,
      ...
      if (peek != NULL && peek->kind == ACTION_KIND_DELETE_SELECTION)
        bobgui_text_history_do_select (self,
                                    peek->u.delete.begin,
                                    peek->u.delete.end);
      else
        bobgui_text_history_do_select (self,
    ...
    case ACTION_KIND_DELETE_PROGRAMMATIC:
    case ACTION_KIND_DELETE_SELECTION:
      bobgui_text_history_do_delete (self,
                                  action->u.delete.begin,
                                  action->u.delete.end,
    ...
    case ACTION_KIND_GROUP: {
      ...
      for (const GList *iter = actions; iter; iter = iter->next)
        bobgui_text_history_apply (self, iter->data, NULL);
...
static void
bobgui_text_history_reverse (BobguiTextHistory *self,
                          Action         *action)
{
  g_assert (BOBGUI_IS_TEXT_HISTORY (self));
  g_assert (action != NULL);
  ...
  switch (action->kind)
    ...
    case ACTION_KIND_GROUP: {
      ...
      for (const GList *iter = actions; iter; iter = iter->prev)
        bobgui_text_history_reverse (self, iter->data);
...
void
bobgui_text_history_undo (BobguiTextHistory *self)
{
  g_return_if_fail (BOBGUI_IS_TEXT_HISTORY (self));
...
#if 0
  {
    char *str = bobgui_text_history_printf (self);
    g_print ("%s\n", str);
    g_free (str);
  }
#endif

  if (bobgui_text_history_get_can_undo (self))
    {
      Action *action;
      ...
      bobgui_text_history_reverse (self, action);
      bobgui_text_history_update_state (self);
...
void
bobgui_text_history_redo (BobguiTextHistory *self)
{
  g_return_if_fail (BOBGUI_IS_TEXT_HISTORY (self));
  ...
  if (bobgui_text_history_get_can_redo (self))
    {
      Action *action;
      Action *peek;
      ...
      bobgui_text_history_apply (self, action, peek);
      bobgui_text_history_update_state (self);
...
void
bobgui_text_history_begin_user_action (BobguiTextHistory *self)
{
  Action *group;
  ...
  if (group == NULL || group->kind != ACTION_KIND_GROUP)
    {
      ...
      bobgui_text_history_push (self, group);
    }
...
void
bobgui_text_history_end_user_action (BobguiTextHistory *self)
{
  Action *peek;
  ...
  if (peek->u.group.actions.length == 1)
    {
      ...
      bobgui_text_history_push (self, replaced);
   ...
   */
  bobgui_text_history_push (self, action_new (ACTION_KIND_BARRIER));
...
void
bobgui_text_history_begin_irreversible_action (BobguiTextHistory *self)
{
  g_return_if_fail (BOBGUI_IS_TEXT_HISTORY (self));
...
void
bobgui_text_history_end_irreversible_action (BobguiTextHistory *self)
{
  g_return_if_fail (BOBGUI_IS_TEXT_HISTORY (self));
...
static void
bobgui_text_history_clear_modified (BobguiTextHistory *self)
{
  const GList *iter;
...
void
bobgui_text_history_modified_changed (BobguiTextHistory *self,
                                   gboolean        modified)
{
  Action *peek;
  ...
  bobgui_text_history_clear_modified (self);
...
void
bobgui_text_history_selection_changed (BobguiTextHistory *self,
...
void
bobgui_text_history_text_inserted (BobguiTextHistory *self,
                                guint           position,
                                const char     *text,
                                int             len)
{
  ...
  bobgui_text_history_push (self, action);
}

void
bobgui_text_history_text_deleted (BobguiTextHistory *self,
                               guint           begin,
                               guint           end,
                               const char     *text,
                               int             len)
...
gboolean
bobgui_text_history_get_enabled (BobguiTextHistory *self)
{
  g_return_val_if_fail (BOBGUI_IS_TEXT_HISTORY (self), FALSE);
...
void
bobgui_text_history_set_enabled (BobguiTextHistory *self,
                              gboolean        enabled)
{
  g_return_if_fail (BOBGUI_IS_TEXT_HISTORY (self));
...
guint
bobgui_text_history_get_max_undo_levels (BobguiTextHistory *self)
{
  g_return_val_if_fail (BOBGUI_IS_TEXT_HISTORY (self), 0);

  return self->max_undo_levels;
}

void
bobgui_text_history_set_max_undo_levels (BobguiTextHistory *self,
                                      guint           max_undo_levels)
{
  g_return_if_fail (BOBGUI_IS_TEXT_HISTORY (self));
