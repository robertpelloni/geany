 ...
 *
 * You likely want to use `BobguiLabel` instead as this widget is intended only
 * for a small subset of use cases. The main scenario envisaged is inside lists
 * such as `BobguiColumnView`.
 *
 * While a `BobguiLabel` sizes itself depending on the text that is displayed,
 * `BobguiInscription` is given a size and inscribes the given text into that
 ...
 * ## CSS nodes
 *
 * `BobguiInscription` has a single CSS node with the name label.
 *
 * Since: 4.8
 */
...
/* Unlike BobguiLabel, we default to not centering text */
#define DEFAULT_XALIGN 0.f
/* But just like BobguiLabel, we center vertically */
#define DEFAULT_YALIGN 0.5f

struct _BobguiInscription
{
...
static void     bobgui_inscription_accessible_text_init (BobguiAccessibleTextInterface *iface);

G_DEFINE_TYPE_WITH_CODE (BobguiInscription, bobgui_inscription, BOBGUI_TYPE_WIDGET,
                         G_IMPLEMENT_INTERFACE (BOBGUI_TYPE_ACCESSIBLE_TEXT,
                                                bobgui_inscription_accessible_text_init))

static GParamSpec *properties[N_PROPS] = { NULL, };

static void
bobgui_inscription_dispose (GObject *object)
{
  BobguiInscription *self = BOBGUI_INSCRIPTION (object);

  g_clear_pointer (&self->text, g_free);

  G_OBJECT_CLASS (bobgui_inscription_parent_class)->dispose (object);
}

static void
bobgui_inscription_finalize (GObject *object)
{
  BobguiInscription *self = BOBGUI_INSCRIPTION (object);

  g_clear_object (&self->layout);

  G_OBJECT_CLASS (bobgui_inscription_parent_class)->finalize (object);
}

static void
bobgui_inscription_get_property (GObject    *object,
                              guint       property_id,
                              GValue     *value,
                              GParamSpec *pspec)
{
...
static void
bobgui_inscription_set_property (GObject      *object,
                              guint         property_id,
                              const GValue *value,
                              GParamSpec   *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_ATTRIBUTES:
      bobgui_inscription_set_attributes (self, g_value_get_boxed (value));
      break;

    case PROP_MARKUP:
      bobgui_inscription_set_markup (self, g_value_get_string (value));
      break;

    case PROP_MIN_CHARS:
      bobgui_inscription_set_min_chars (self, g_value_get_uint (value));
      break;

    case PROP_MIN_LINES:
      bobgui_inscription_set_min_lines (self, g_value_get_uint (value));
      break;

    case PROP_NAT_CHARS:
      bobgui_inscription_set_nat_chars (self, g_value_get_uint (value));
      break;

    case PROP_NAT_LINES:
      bobgui_inscription_set_nat_lines (self, g_value_get_uint (value));
      break;

    case PROP_TEXT:
      bobgui_inscription_set_text (self, g_value_get_string (value));
      break;

    case PROP_TEXT_OVERFLOW:
      bobgui_inscription_set_text_overflow (self, g_value_get_enum (value));
      break;

    case PROP_WRAP_MODE:
      bobgui_inscription_set_wrap_mode (self, g_value_get_enum (value));
      break;

    case PROP_XALIGN:
      bobgui_inscription_set_xalign (self, g_value_get_float (value));
      break;

    case PROP_YALIGN:
      bobgui_inscription_set_yalign (self, g_value_get_float (value));
      break;
...
static void
update_pango_alignment (BobguiInscription *self)
{
  PangoAlignment align;
  gboolean ltr;

  ltr = _bobgui_widget_get_direction (BOBGUI_WIDGET (self)) != BOBGUI_TEXT_DIR_RTL;
...
static void
bobgui_inscription_update_layout_attributes (BobguiInscription *self,
                                          PangoAttrList  *css_attrs)
{
  PangoAttrList *new_attrs;

  if (css_attrs == NULL)
    css_attrs = bobgui_css_style_get_pango_attributes (bobgui_css_node_get_style (bobgui_widget_get_css_node (BOBGUI_WIDGET (self))));

  new_attrs = css_attrs;

  new_attrs = _bobgui_pango_attr_list_merge (new_attrs, self->attrs);
...
static void
bobgui_inscription_css_changed (BobguiWidget         *widget,
                             BobguiCssStyleChange *change)
{
  BobguiInscription *self = BOBGUI_INSCRIPTION (widget);

  BOBGUI_WIDGET_CLASS (bobgui_inscription_parent_class)->css_changed (widget, change);

  if (bobgui_css_style_change_affects (change, BOBGUI_CSS_AFFECTS_TEXT_ATTRS))
    {
      bobgui_inscription_update_layout_attributes (self,
          bobgui_css_style_get_pango_attributes (bobgui_css_style_change_get_new_style (change)));
...
static void
bobgui_inscription_direction_changed (BobguiWidget        *widget,
                                   BobguiTextDirection  previous_direction)
{
  BobguiInscription *self = BOBGUI_INSCRIPTION (widget);

  BOBGUI_WIDGET_CLASS (bobgui_inscription_parent_class)->direction_changed (widget, previous_direction);

  update_pango_alignment (self);
}

static PangoFontMetrics *
bobgui_inscription_get_font_metrics (BobguiInscription *self)
{
...
static int
get_char_pixels (BobguiInscription *self)
{
  int char_width, digit_width;
  PangoFontMetrics *metrics;

  metrics = bobgui_inscription_get_font_metrics (self);
...
static void
bobgui_inscription_measure_width (BobguiInscription *self,
                               int            *minimum,
                               int            *natural)
{
...
static int
get_line_pixels (BobguiInscription *self,
                 int            *baseline)
{
  PangoFontMetrics *metrics;
  int ascent, descent;
...
static void
bobgui_inscription_measure_height (BobguiInscription *self,
                                int            *minimum,
                                int            *natural,
...
static void
bobgui_inscription_measure (BobguiWidget      *widget,
                         ...
                         int            *natural_baseline)
{
  ...
  if (orientation == BOBGUI_ORIENTATION_HORIZONTAL)
    bobgui_inscription_measure_width (self, minimum, natural);
  else
    bobgui_inscription_measure_height (self, minimum, natural, minimum_baseline, natural_baseline);
...
void
bobgui_inscription_get_layout_location (BobguiInscription *self,
                                     float          *x_out,
                                     float          *y_out)
{
...
static gboolean
bobgui_inscription_get_layout_index (BobguiInscription *self,
                                  int             x,
                                  int             y,
                                  int            *index)
{
  ...
  *index = 0;
  ...
  bobgui_inscription_get_layout_location (self, &lx, &ly);
...
static void
bobgui_inscription_allocate (BobguiWidget *widget,
                          int        width,
                          int        height,
                          int        baseline)
{
  ...
  switch (self->overflow)
    {
    case BOBGUI_INSCRIPTION_OVERFLOW_CLIP:
    ...
    case BOBGUI_INSCRIPTION_OVERFLOW_ELLIPSIZE_START:
    case BOBGUI_INSCRIPTION_OVERFLOW_ELLIPSIZE_MIDDLE:
    case BOBGUI_INSCRIPTION_OVERFLOW_ELLIPSIZE_END:
...
static void
bobgui_inscription_snapshot (BobguiWidget   *widget,
                          BobguiSnapshot *snapshot)
{
  ...
  bobgui_snapshot_push_clip (snapshot, &GRAPHENE_RECT_INIT(0, 0, bobgui_widget_get_width (widget), bobgui_widget_get_height (widget)));
  bobgui_inscription_get_layout_location (self, &lx, &ly);
...
static void
bobgui_inscription_class_init (BobguiInscriptionClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  BobguiWidgetClass *widget_class = BOBGUI_WIDGET_CLASS (klass);

  gobject_class->dispose = bobgui_inscription_dispose;
  gobject_class->finalize = bobgui_inscription_finalize;
  gobject_class->get_property = bobgui_inscription_get_property;
  gobject_class->set_property = bobgui_inscription_set_property;

  widget_class->css_changed = bobgui_inscription_css_changed;
  widget_class->direction_changed = bobgui_inscription_direction_changed;
  widget_class->measure = bobgui_inscription_measure;
  widget_class->size_allocate = bobgui_inscription_allocate;
  widget_class->snapshot = bobgui_inscription_snapshot;

  /**
   * BobguiInscription:attributes:
   *
  ...
  /**
   * BobguiInscription:markup:
   *
   * Utility property that sets both the [property@Bobgui.Inscription:text] and
   * [property@Bobgui.Inscription:attributes] properties, mainly intended for use in
   * BobguiBuilder ui files to ease translation support and bindings.
   *
   ...
   *
   * If this property is set to a value smaller than [property@Bobgui.Inscription:min-chars],
   ...
   * The widget might turn out larger as well as smaller.
   *
   * If this property is set to a value smaller than [property@Bobgui.Inscription:min-lines],
   * that value will be used. In particular, for the default value of 0, this will always
   * be the case.
   *
   * Since: 4.8
  ...
  /**
   * BobguiInscription:text-overflow:
   *
  ...
  properties[PROP_TEXT_OVERFLOW] =
    g_param_spec_enum ("text-overflow", NULL, NULL,
                       BOBGUI_TYPE_INSCRIPTION_OVERFLOW,
                       BOBGUI_INSCRIPTION_OVERFLOW_CLIP,
                       G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_STATIC_STRINGS);

  /**
   * BobguiInscription:wrap-mode:
   *
   * Controls how the line wrapping is done.
   *
   * Note that unlike `BobguiLabel`, the default here is %PANGO_WRAP_WORD_CHAR.
   *
   * Since: 4.8
   */
  ...
  bobgui_widget_class_set_css_name (widget_class, "label");
  bobgui_widget_class_set_accessible_role (widget_class, BOBGUI_ACCESSIBLE_ROLE_LABEL);
}

static void
bobgui_inscription_init (BobguiInscription *self)
{
  ...
  self->layout = bobgui_widget_create_pango_layout (BOBGUI_WIDGET (self), NULL);
...
/* for a11y */
PangoLayout *
bobgui_inscription_get_layout (BobguiInscription *self)
{
  return self->layout;
}

/**
 * bobgui_inscription_new:
 * @text: (nullable): The text to display.
 *
 * Creates a new `BobguiInscription` with the given text.
 *
 * Returns: a new `BobguiInscription`
 *
 * Since: 4.8
 */
BobguiWidget *
bobgui_inscription_new (const char *text)
{
  return g_object_new (BOBGUI_TYPE_INSCRIPTION,
                       "text", text,
                       NULL);
}

/**
 * bobgui_inscription_set_text:
 * @self: a `BobguiInscription`
 * @text: (nullable): The text to display
 *
 ...
 */
void
bobgui_inscription_set_text (BobguiInscription *self,
                          const char  *text)
{
  g_return_if_fail (BOBGUI_IS_INSCRIPTION (self));
  ...
  /* This here not being a bobgui_widget_queue_resize() is why this widget exists */
 ...
 */
const char *
bobgui_inscription_get_text (BobguiInscription *self)
{
  g_return_val_if_fail (BOBGUI_IS_INSCRIPTION (self), NULL);

  return self->text;
}

/**
 * bobgui_inscription_set_min_chars:
 * @self: a `BobguiInscription`
 * @min_chars: the minimum number of characters that should fit, approximately
 *
 * Sets the `min-chars` of the inscription.
 *
 * See the [property@Bobgui.Inscription:min-chars] property.
 *
 * Since: 4.8
 */
void
bobgui_inscription_set_min_chars (BobguiInscription *self,
                               guint           min_chars)
{
  g_return_if_fail (BOBGUI_IS_INSCRIPTION (self));
...
/**
 * bobgui_inscription_get_min_chars:
 * @self: a `BobguiInscription`
 *
 ...
 */
guint
bobgui_inscription_get_min_chars (BobguiInscription *self)
{
  g_return_val_if_fail (BOBGUI_IS_INSCRIPTION (self), DEFAULT_MIN_CHARS);

  return self->min_chars;
}

/**
 * bobgui_inscription_set_nat_chars:
 * @self: a `BobguiInscription`
 * @nat_chars: the number of characters that should ideally fit, approximately
 *
 * Sets the `nat-chars` of the inscription.
 *
 * See the [property@Bobgui.Inscription:nat-chars] property.
 *
 * Since: 4.8
 */
void
bobgui_inscription_set_nat_chars (BobguiInscription *self,
                               guint           nat_chars)
{
  g_return_if_fail (BOBGUI_IS_INSCRIPTION (self));
...
/**
 * bobgui_inscription_get_nat_chars:
 * @self: a `BobguiInscription`
 *
 ...
 */
guint
bobgui_inscription_get_nat_chars (BobguiInscription *self)
{
  g_return_val_if_fail (BOBGUI_IS_INSCRIPTION (self), DEFAULT_NAT_CHARS);

  return self->nat_chars;
}

/**
 * bobgui_inscription_set_min_lines:
 * @self: a `BobguiInscription`
 * @min_lines: the minimum number of lines that should fit, approximately
 *
 * Sets the `min-lines` of the inscription.
 *
 * See the [property@Bobgui.Inscription:min-lines] property.
 *
 * Since: 4.8
 */
void
bobgui_inscription_set_min_lines (BobguiInscription *self,
                               guint           min_lines)
{
  g_return_if_fail (BOBGUI_IS_INSCRIPTION (self));
...
/**
 * bobgui_inscription_get_min_lines:
 * @self: a `BobguiInscription`
 *
 ...
 */
guint
bobgui_inscription_get_min_lines (BobguiInscription *self)
{
  g_return_val_if_fail (BOBGUI_IS_INSCRIPTION (self), DEFAULT_MIN_LINES);

  return self->min_lines;
}

/**
 * bobgui_inscription_set_nat_lines:
 * @self: a `BobguiInscription`
 * @nat_lines: the number of lines that should ideally fit
 *
 * Sets the `nat-lines` of the inscription.
 *
 * See the [property@Bobgui.Inscription:nat-lines] property.
 *
 * Since: 4.8
 */
void
bobgui_inscription_set_nat_lines (BobguiInscription *self,
                               guint           nat_lines)
{
  g_return_if_fail (BOBGUI_IS_INSCRIPTION (self));
...
/**
 * bobgui_inscription_get_nat_lines:
 * @self: a `BobguiInscription`
 *
 ...
 */
guint
bobgui_inscription_get_nat_lines (BobguiInscription *self)
{
  g_return_val_if_fail (BOBGUI_IS_INSCRIPTION (self), DEFAULT_NAT_LINES);

  return self->nat_lines;
}

/**
 * bobgui_inscription_set_xalign:
 * @self: a `BobguiInscription`
 * @xalign: the new xalign value, between 0 and 1
 *
 * Sets the `xalign` of the inscription.
 *
 * See the [property@Bobgui.Inscription:xalign] property.
 *
 * Since: 4.8
 */
void
bobgui_inscription_set_xalign (BobguiInscription *self,
                            float           xalign)
{
  g_return_if_fail (BOBGUI_IS_INSCRIPTION (self));
 ...
 */
float
bobgui_inscription_get_xalign (BobguiInscription *self)
{
  g_return_val_if_fail (BOBGUI_IS_INSCRIPTION (self), DEFAULT_XALIGN);

  return self->xalign;
}

/**
 * bobgui_inscription_set_yalign:
 * @self: a `BobguiInscription`
 * @yalign: the new yalign value, between 0 and 1
 *
 * Sets the `yalign` of the inscription.
 *
 * See the [property@Bobgui.Inscription:yalign] property.
 *
 * Since: 4.8
 */
void
bobgui_inscription_set_yalign (BobguiInscription *self,
                            float           yalign)
{
  g_return_if_fail (BOBGUI_IS_INSCRIPTION (self));
 ...
 */
float
bobgui_inscription_get_yalign (BobguiInscription *self)
{
  g_return_val_if_fail (BOBGUI_IS_INSCRIPTION (self), DEFAULT_YALIGN);

  return self->yalign;
}

/**
 * bobgui_inscription_set_attributes:
 * @self: a `BobguiInscription`
 ...
 */
void
bobgui_inscription_set_attributes (BobguiInscription *self,
                                PangoAttrList  *attrs)
{
  g_return_if_fail (BOBGUI_IS_INSCRIPTION (self));
  ...
  bobgui_inscription_update_layout_attributes (self, NULL);
...
/**
 * bobgui_inscription_get_attributes:
 * @self: a `BobguiInscription`
 *
 ...
 */
PangoAttrList *
bobgui_inscription_get_attributes (BobguiInscription *self)
{
  g_return_val_if_fail (BOBGUI_IS_INSCRIPTION (self), NULL);
...
/**
 * bobgui_inscription_set_text_overflow:
 * @self: a `BobguiInscription`
 ...
 */
void
bobgui_inscription_set_text_overflow (BobguiInscription         *self,
                                   BobguiInscriptionOverflow  overflow)
{
  g_return_if_fail (BOBGUI_IS_INSCRIPTION (self));
...
/**
 * bobgui_inscription_get_text_overflow:
 * @self: a `BobguiInscription`
 *
...
bobgui_inscription_get_text_overflow (BobguiInscription *self)
{
  g_return_val_if_fail (BOBGUI_IS_INSCRIPTION (self), BOBGUI_INSCRIPTION_OVERFLOW_CLIP);

  return self->overflow;
}

/**
 * bobgui_inscription_set_wrap_mode:
 * @self: a `BobguiInscription`
 ...
 */
void
bobgui_inscription_set_wrap_mode (BobguiInscription *self,
...
/**
 * bobgui_inscription_get_wrap_mode:
 * @self: a `BobguiInscription`
 *
 * Returns line wrap mode used by the inscription.
 *
 * See [method@Bobgui.Inscription.set_wrap_mode].
 *
 ...
 */
PangoWrapMode
bobgui_inscription_get_wrap_mode (BobguiInscription *self)
{
  g_return_val_if_fail (BOBGUI_IS_INSCRIPTION (self), PANGO_WRAP_WORD_CHAR);

  return pango_layout_get_wrap (self->layout);
}

/**
 * bobgui_inscription_set_markup:
 * @self: a `BobguiInscription`
 * @markup: (nullable): The markup to display
 *
 * Utility function to set the text and attributes to be displayed.
 *
 * See the [property@Bobgui.Inscription:markup] property.
 *
 * Since: 4.8
 */
void
bobgui_inscription_set_markup (BobguiInscription *self,
                            const char     *markup)
{
  PangoAttrList *attrs;
  char *text;
  ...
  bobgui_inscription_set_text (self, text);
  bobgui_inscription_set_attributes (self, attrs);
...
static GBytes *
bobgui_inscription_accessible_text_get_contents (BobguiAccessibleText *self,
                                              unsigned int       start,
                                              unsigned int       end)
{
  const char *text;
  ...
  text = bobgui_inscription_get_text (BOBGUI_INSCRIPTION (self));
...
static GBytes *
bobgui_inscription_accessible_text_get_contents_at (BobguiAccessibleText            *self,
                                                 unsigned int                  offset,
                                                 BobguiAccessibleTextGranularity  granularity,
                                                 unsigned int                 *start,
                                                 unsigned int                 *end)
{
  PangoLayout *layout = bobgui_inscription_get_layout (BOBGUI_INSCRIPTION (self));
...
static unsigned int
bobgui_inscription_accessible_text_get_caret_position (BobguiAccessibleText *self)
{
  return 0;
}

static gboolean
bobgui_inscription_accessible_text_get_selection (BobguiAccessibleText       *self,
                                               gsize                   *n_ranges,
...
static gboolean
bobgui_inscription_accessible_text_get_attributes (BobguiAccessibleText        *self,
                                                unsigned int              offset,
                                                gsize                    *n_ranges,
                                                BobguiAccessibleTextRange  **ranges,
...
static void
bobgui_inscription_accessible_text_get_default_attributes (BobguiAccessibleText   *self,
                                                        char              ***attribute_names,
                                                        char              ***attribute_values)
{
  PangoLayout *layout = bobgui_inscription_get_layout (BOBGUI_INSCRIPTION (self));
...
static gboolean
bobgui_inscription_accessible_text_get_extents (BobguiAccessibleText *self,
                                             unsigned int       start,
                                             unsigned int       end,
                                             graphene_rect_t   *extents)
{
  BobguiInscription *inscription = BOBGUI_INSCRIPTION (self);
  PangoLayout *layout;
  const char *text;
  float lx, ly;
  ...
  bobgui_inscription_get_layout_location (inscription, &lx, &ly);
...
static gboolean
bobgui_inscription_accessible_text_get_offset (BobguiAccessibleText      *self,
                                            ...
                                            unsigned int           *offset)
{
  ...
  if (!bobgui_inscription_get_layout_index (inscription, point->x, point->y, &index))
    return FALSE;
...
static void
bobgui_inscription_accessible_text_init (BobguiAccessibleTextInterface *iface)
{
  iface->get_contents = bobgui_inscription_accessible_text_get_contents;
  iface->get_contents_at = bobgui_inscription_accessible_text_get_contents_at;
  iface->get_caret_position = bobgui_inscription_accessible_text_get_caret_position;
  iface->get_selection = bobgui_inscription_accessible_text_get_selection;
  iface->get_attributes = bobgui_inscription_accessible_text_get_attributes;
  iface->get_default_attributes = bobgui_inscription_accessible_text_get_default_attributes;
  iface->get_extents = bobgui_inscription_accessible_text_get_extents;
  iface->get_offset = bobgui_inscription_accessible_text_get_offset;
}
