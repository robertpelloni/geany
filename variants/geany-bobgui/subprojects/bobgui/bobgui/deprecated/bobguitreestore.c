 ...
 * BobguiTreeStore:
 *
 * A tree-like data structure that can be used with the [class@Bobgui.TreeView].
 *
 * The `BobguiTreeStore` object is a list model for use with a `BobguiTreeView`
 * widget. It implements the [iface@Bobgui.TreeModel] interface, and consequently,
 * can use all of the methods available there. It also implements the
 * [iface@Bobgui.TreeSortable] interface so it can be sorted by the view.
 * Finally, it also implements the tree [drag][iface@Bobgui.TreeDragSource]
 * and [drop][iface@Bobgui.TreeDragDest] interfaces.
 *
 * `BobguiTreeStore` is deprecated since BOBGUI 4.10, and should not be used in newly
 * written code. You should use [class@Bobgui.TreeListModel] for a tree-like model
 * object.
 *
 * ## BobguiTreeStore as BobguiBuildable
 *
 * The BobguiTreeStore implementation of the `BobguiBuildable` interface allows
 ...
 * ```
 *
 * Deprecated: 4.10: Use [class@Bobgui.TreeListModel] instead
 */

struct _BobguiTreeStorePrivate
{
  int stamp;
...
#define BOBGUI_TREE_STORE_IS_SORTED(tree) (((BobguiTreeStore*)(tree))->priv->sort_column_id != BOBGUI_TREE_SORTABLE_UNSORTED_SORT_COLUMN_ID)
#define VALID_ITER(iter, tree_store) ((iter)!= NULL && (iter)->user_data != NULL && ((BobguiTreeStore*)(tree_store))->priv->stamp == (iter)->stamp)

static void         bobgui_tree_store_tree_model_init (BobguiTreeModelIface *iface);
static void         bobgui_tree_store_drag_source_init(BobguiTreeDragSourceIface *iface);
static void         bobgui_tree_store_drag_dest_init  (BobguiTreeDragDestIface   *iface);
static void         bobgui_tree_store_sortable_init   (BobguiTreeSortableIface   *iface);
static void         bobgui_tree_store_buildable_init  (BobguiBuildableIface      *iface);
static void         bobgui_tree_store_finalize        (GObject           *object);
static BobguiTreeModelFlags bobgui_tree_store_get_flags  (BobguiTreeModel      *tree_model);
static int          bobgui_tree_store_get_n_columns   (BobguiTreeModel      *tree_model);
static GType        bobgui_tree_store_get_column_type (BobguiTreeModel      *tree_model,
						    int                index);
static gboolean     bobgui_tree_store_get_iter        (BobguiTreeModel      *tree_model,
						    BobguiTreeIter       *iter,
						    BobguiTreePath       *path);
static BobguiTreePath *bobgui_tree_store_get_path        (BobguiTreeModel      *tree_model,
						    BobguiTreeIter       *iter);
static void         bobgui_tree_store_get_value       (BobguiTreeModel      *tree_model,
						    BobguiTreeIter       *iter,
						    int                column,
						    GValue            *value);
static gboolean     bobgui_tree_store_iter_next       (BobguiTreeModel      *tree_model,
						    BobguiTreeIter       *iter);
static gboolean     bobgui_tree_store_iter_previous   (BobguiTreeModel      *tree_model,
						    BobguiTreeIter       *iter);
static gboolean     bobgui_tree_store_iter_children   (BobguiTreeModel      *tree_model,
						    BobguiTreeIter       *iter,
						    BobguiTreeIter       *parent);
static gboolean     bobgui_tree_store_iter_has_child  (BobguiTreeModel      *tree_model,
						    BobguiTreeIter       *iter);
static int          bobgui_tree_store_iter_n_children (BobguiTreeModel      *tree_model,
						    BobguiTreeIter       *iter);
static gboolean     bobgui_tree_store_iter_nth_child  (BobguiTreeModel      *tree_model,
						    BobguiTreeIter       *iter,
						    BobguiTreeIter       *parent,
						    int                n);
static gboolean     bobgui_tree_store_iter_parent     (BobguiTreeModel      *tree_model,
						    BobguiTreeIter       *iter,
						    BobguiTreeIter       *child);


static void bobgui_tree_store_set_n_columns   (BobguiTreeStore *tree_store,
					    int           n_columns);
static void bobgui_tree_store_set_column_type (BobguiTreeStore *tree_store,
					    int           column,
					    GType         type);

static void bobgui_tree_store_increment_stamp (BobguiTreeStore  *tree_store);


/* DND interfaces */
static gboolean real_bobgui_tree_store_row_draggable   (BobguiTreeDragSource *drag_source,
						   BobguiTreePath       *path);
static gboolean bobgui_tree_store_drag_data_delete   (BobguiTreeDragSource *drag_source,
						   BobguiTreePath       *path);
static GdkContentProvider *
                bobgui_tree_store_drag_data_get      (BobguiTreeDragSource *drag_source,
						   BobguiTreePath       *path);
static gboolean bobgui_tree_store_drag_data_received (BobguiTreeDragDest   *drag_dest,
						   BobguiTreePath       *dest,
						   const GValue      *value);
static gboolean bobgui_tree_store_row_drop_possible  (BobguiTreeDragDest   *drag_dest,
						   BobguiTreePath       *dest_path,
						   const GValue      *value);

/* Sortable Interfaces */

static void     bobgui_tree_store_sort                    (BobguiTreeStore           *tree_store);
static void     bobgui_tree_store_sort_iter_changed       (BobguiTreeStore           *tree_store,
							BobguiTreeIter            *iter,
							int                     column,
							gboolean                emit_signal);
static gboolean bobgui_tree_store_get_sort_column_id      (BobguiTreeSortable        *sortable,
							int                    *sort_column_id,
							BobguiSortType            *order);
static void     bobgui_tree_store_set_sort_column_id      (BobguiTreeSortable        *sortable,
							int                     sort_column_id,
							BobguiSortType             order);
static void     bobgui_tree_store_set_sort_func           (BobguiTreeSortable        *sortable,
							int                     sort_column_id,
							BobguiTreeIterCompareFunc  func,
							gpointer                data,
							GDestroyNotify          destroy);
static void     bobgui_tree_store_set_default_sort_func   (BobguiTreeSortable        *sortable,
							BobguiTreeIterCompareFunc  func,
							gpointer                data,
							GDestroyNotify          destroy);
static gboolean bobgui_tree_store_has_default_sort_func   (BobguiTreeSortable        *sortable);


/* buildable */

static gboolean bobgui_tree_store_buildable_custom_tag_start (BobguiBuildable       *buildable,
                                                           BobguiBuilder         *builder,
                                                           GObject            *child,
                                                           const char         *tagname,
                                                           BobguiBuildableParser *parser,
                                                           gpointer           *data);
static void     bobgui_tree_store_buildable_custom_tag_end   (BobguiBuildable       *buildable,
                                                           BobguiBuilder         *builder,
                                                           GObject            *child,
                                                           const char         *tagname,
                                                           gpointer            parser_data);

static void     bobgui_tree_store_move                    (BobguiTreeStore           *tree_store,
                                                        BobguiTreeIter            *iter,
...
static inline void
validate_tree (BobguiTreeStore *tree_store)
{
...
G_DEFINE_TYPE_WITH_CODE (BobguiTreeStore, bobgui_tree_store, G_TYPE_OBJECT,
                         G_ADD_PRIVATE (BobguiTreeStore)
			 G_IMPLEMENT_INTERFACE (BOBGUI_TYPE_TREE_MODEL,
						bobgui_tree_store_tree_model_init)
			 G_IMPLEMENT_INTERFACE (BOBGUI_TYPE_TREE_DRAG_SOURCE,
						bobgui_tree_store_drag_source_init)
			 G_IMPLEMENT_INTERFACE (BOBGUI_TYPE_TREE_DRAG_DEST,
						bobgui_tree_store_drag_dest_init)
			 G_IMPLEMENT_INTERFACE (BOBGUI_TYPE_TREE_SORTABLE,
						bobgui_tree_store_sortable_init)
			 G_IMPLEMENT_INTERFACE (BOBGUI_TYPE_BUILDABLE,
						bobgui_tree_store_buildable_init))

static void
bobgui_tree_store_class_init (BobguiTreeStoreClass *class)
{
  GObjectClass *object_class;

  object_class = (GObjectClass *) class;

  object_class->finalize = bobgui_tree_store_finalize;
}

static void
bobgui_tree_store_tree_model_init (BobguiTreeModelIface *iface)
{
  iface->get_flags = bobgui_tree_store_get_flags;
  iface->get_n_columns = bobgui_tree_store_get_n_columns;
  iface->get_column_type = bobgui_tree_store_get_column_type;
  iface->get_iter = bobgui_tree_store_get_iter;
  iface->get_path = bobgui_tree_store_get_path;
  iface->get_value = bobgui_tree_store_get_value;
  iface->iter_next = bobgui_tree_store_iter_next;
  iface->iter_previous = bobgui_tree_store_iter_previous;
  iface->iter_children = bobgui_tree_store_iter_children;
  iface->iter_has_child = bobgui_tree_store_iter_has_child;
  iface->iter_n_children = bobgui_tree_store_iter_n_children;
  iface->iter_nth_child = bobgui_tree_store_iter_nth_child;
  iface->iter_parent = bobgui_tree_store_iter_parent;
}

static void
bobgui_tree_store_drag_source_init (BobguiTreeDragSourceIface *iface)
{
  iface->row_draggable = real_bobgui_tree_store_row_draggable;
  iface->drag_data_delete = bobgui_tree_store_drag_data_delete;
  iface->drag_data_get = bobgui_tree_store_drag_data_get;
}

static void
bobgui_tree_store_drag_dest_init (BobguiTreeDragDestIface *iface)
{
  iface->drag_data_received = bobgui_tree_store_drag_data_received;
  iface->row_drop_possible = bobgui_tree_store_row_drop_possible;
}

static void
bobgui_tree_store_sortable_init (BobguiTreeSortableIface *iface)
{
  iface->get_sort_column_id = bobgui_tree_store_get_sort_column_id;
  iface->set_sort_column_id = bobgui_tree_store_set_sort_column_id;
  iface->set_sort_func = bobgui_tree_store_set_sort_func;
  iface->set_default_sort_func = bobgui_tree_store_set_default_sort_func;
  iface->has_default_sort_func = bobgui_tree_store_has_default_sort_func;
}

void
bobgui_tree_store_buildable_init (BobguiBuildableIface *iface)
{
  iface->custom_tag_start = bobgui_tree_store_buildable_custom_tag_start;
  iface->custom_tag_end = bobgui_tree_store_buildable_custom_tag_end;
}

static void
bobgui_tree_store_init (BobguiTreeStore *tree_store)
{
  BobguiTreeStorePrivate *priv;

  priv = bobgui_tree_store_get_instance_private (tree_store);
 ...
 *
 * ```c
 * bobgui_tree_store_new (3, G_TYPE_INT, G_TYPE_STRING, GDK_TYPE_TEXTURE);
 * ```
 *
 * will create a new `BobguiTreeStore` with three columns of type
 * `int`, `gchararray`, and `GdkTexture` respectively.
 *
 * Returns: a new `BobguiTreeStore`
 *
 * Deprecated: 4.10: Use [class@Bobgui.TreeListModel] instead
 **/
BobguiTreeStore *
bobgui_tree_store_new (int n_columns,
			       ...)
{
  BobguiTreeStore *retval;
  va_list args;
  int i;

  g_return_val_if_fail (n_columns > 0, NULL);

  retval = g_object_new (BOBGUI_TYPE_TREE_STORE, NULL);
  bobgui_tree_store_set_n_columns (retval, n_columns);
  ...
  for (i = 0; i < n_columns; i++)
      ...
      if (! _bobgui_tree_data_list_check_type (type))
	{
      ...
      bobgui_tree_store_set_column_type (retval, i, type);
    }
  va_end (args);
...
/**
 * bobgui_tree_store_newv: (rename-to bobgui_tree_store_new)
 * @n_columns: number of columns in the tree store
 ...
 *
 * Returns: (transfer full): a new `BobguiTreeStore`
 *
 * Deprecated: 4.10: Use [class@Bobgui.TreeListModel] instead
 **/
BobguiTreeStore *
bobgui_tree_store_newv (int    n_columns,
		     GType *types)
{
  BobguiTreeStore *retval;
  int i;
  ...
  bobgui_tree_store_set_n_columns (retval, n_columns);
   ...
   for (i = 0; i < n_columns; i++)
      ...
      if (! _bobgui_tree_data_list_check_type (types[i]))
	{
      ...
      bobgui_tree_store_set_column_type (retval, i, types[i]);
    }
...
/**
 * bobgui_tree_store_set_column_types:
 * @tree_store: A `BobguiTreeStore`
 ...
 * `BobguiTreeStore`, and should only be used when constructing a new
 * `BobguiTreeStore`.
 *
 * This functions cannot be called after a row has been added,
 * or a method on the `BobguiTreeModel` interface is called on the
 * tree store.
 *
 * Deprecated: 4.10: Use [class@Bobgui.TreeListModel] instead
 **/
void
bobgui_tree_store_set_column_types (BobguiTreeStore *tree_store,
				 int           n_columns,
				 GType        *types)
{
  int i;

  g_return_if_fail (BOBGUI_IS_TREE_STORE (tree_store));
  g_return_if_fail (tree_store->priv->columns_dirty == 0);

  bobgui_tree_store_set_n_columns (tree_store, n_columns);
   for (i = 0; i < n_columns; i++)
    {
      if (! _bobgui_tree_data_list_check_type (types[i]))
	{
      ...
      bobgui_tree_store_set_column_type (tree_store, i, types[i]);
    }
}

static void
bobgui_tree_store_set_n_columns (BobguiTreeStore *tree_store,
			      int           n_columns)
{
  BobguiTreeStorePrivate *priv = tree_store->priv;
  int i;
...
/**
 * bobgui_tree_store_set_column_type:
 * @tree_store: a `BobguiTreeStore`
 ...
 **/
static void
bobgui_tree_store_set_column_type (BobguiTreeStore *tree_store,
				int           column,
				GType         type)
{
  BobguiTreeStorePrivate *priv = tree_store->priv;
...
{
  if (node->data)
    _bobgui_tree_data_list_free (node->data, (GType*)data);
  node->data = NULL;
...
static void
bobgui_tree_store_finalize (GObject *object)
{
  BobguiTreeStore *tree_store = BOBGUI_TREE_STORE (object);
  BobguiTreeStorePrivate *priv = tree_store->priv;
  ...
  /* must chain up */
  G_OBJECT_CLASS (bobgui_tree_store_parent_class)->finalize (object);
}

/* fulfill the BobguiTreeModel requirements */
/* NOTE: BobguiTreeStore::root is a GNode, that acts as the parent node.  However,
 * it is not visible to the tree or to the user., and the path “0” refers to the
 * first child of BobguiTreeStore::root.
 */


static BobguiTreeModelFlags
bobgui_tree_store_get_flags (BobguiTreeModel *tree_model)
{
  return BOBGUI_TREE_MODEL_ITERS_PERSIST;
}

static int
bobgui_tree_store_get_n_columns (BobguiTreeModel *tree_model)
{
  BobguiTreeStore *tree_store = (BobguiTreeStore *) tree_model;
  BobguiTreeStorePrivate *priv = tree_store->priv;
...
static GType
bobgui_tree_store_get_column_type (BobguiTreeModel *tree_model,
				int           index)
{
  BobguiTreeStore *tree_store = (BobguiTreeStore *) tree_model;
  BobguiTreeStorePrivate *priv = tree_store->priv;
...
static gboolean
bobgui_tree_store_get_iter (BobguiTreeModel *tree_model,
			 BobguiTreeIter  *iter,
			 BobguiTreePath  *path)
{
  BobguiTreeStore *tree_store = (BobguiTreeStore *) tree_model;
  ...
  if (!bobgui_tree_store_iter_nth_child (tree_model, iter, &parent, indices[0]))
    {
      iter->stamp = 0;
      return FALSE;
    }
  ...
  for (i = 1; i < depth; i++)
    {
      parent = *iter;
      if (!bobgui_tree_store_iter_nth_child (tree_model, iter, &parent, indices[i]))
        {
          iter->stamp = 0;
          return FALSE;
        }
...
bobgui_tree_store_get_path (BobguiTreeModel *tree_model,
			 BobguiTreeIter  *iter)
{
  BobguiTreeStore *tree_store = (BobguiTreeStore *) tree_model;
  BobguiTreeStorePrivate *priv = tree_store->priv;
  ...
  if (G_NODE (iter->user_data)->parent == G_NODE (priv->root))
  ...
  else
    {
      ...
      retval = bobgui_tree_store_get_path (tree_model, &tmp_iter);
...
static void
bobgui_tree_store_get_value (BobguiTreeModel *tree_model,
			  BobguiTreeIter  *iter,
			  int           column,
			  GValue       *value)
{
...
static gboolean
bobgui_tree_store_iter_next (BobguiTreeModel  *tree_model,
			  BobguiTreeIter   *iter)
{
  g_return_val_if_fail (iter->user_data != NULL, FALSE);
  g_return_val_if_fail (iter->stamp == BOBGUI_TREE_STORE (tree_model)->priv->stamp, FALSE);
...
static gboolean
bobgui_tree_store_iter_previous (BobguiTreeModel *tree_model,
                              BobguiTreeIter  *iter)
{
  g_return_val_if_fail (iter->user_data != NULL, FALSE);
  g_return_val_if_fail (iter->stamp == BOBGUI_TREE_STORE (tree_model)->priv->stamp, FALSE);
...
static gboolean
bobgui_tree_store_iter_children (BobguiTreeModel *tree_model,
			      BobguiTreeIter  *iter,
			      BobguiTreeIter  *parent)
{
  BobguiTreeStore *tree_store = (BobguiTreeStore *) tree_model;
...
static gboolean
bobgui_tree_store_iter_has_child (BobguiTreeModel *tree_model,
			       BobguiTreeIter  *iter)
{
  g_return_val_if_fail (iter->user_data != NULL, FALSE);
...
static int
bobgui_tree_store_iter_n_children (BobguiTreeModel *tree_model,
				BobguiTreeIter  *iter)
{
  GNode *node;
  int i = 0;
  ...
  if (iter == NULL)
    node = G_NODE (BOBGUI_TREE_STORE (tree_model)->priv->root)->children;
  else
...
static gboolean
bobgui_tree_store_iter_nth_child (BobguiTreeModel *tree_model,
			       BobguiTreeIter  *iter,
			       BobguiTreeIter  *parent,
			       int           n)
{
...
static gboolean
bobgui_tree_store_iter_parent (BobguiTreeModel *tree_model,
			    BobguiTreeIter  *iter,
			    BobguiTreeIter  *child)
{
  BobguiTreeStore *tree_store = (BobguiTreeStore *) tree_model;
...
static gboolean
bobgui_tree_store_real_set_value (BobguiTreeStore *tree_store,
			       BobguiTreeIter  *iter,
			       int           column,
			       GValue       *value,
			       gboolean      sort)
{
  ...
  while (list != NULL)
      ...
      if (column == 0)
	{
	  ...
	  if (converted)
	    g_value_unset (&real_value);
          if (sort && BOBGUI_TREE_STORE_IS_SORTED (tree_store))
            bobgui_tree_store_sort_iter_changed (tree_store, iter, old_column, TRUE);
	  return retval;
	}
  ...
  if (G_NODE (iter->user_data)->data == NULL)
    {
      G_NODE (iter->user_data)->data = list = _bobgui_tree_data_list_alloc ();
      list->next = NULL;
    }
  else
    {
 ...
 **/
void
bobgui_tree_store_set_value (BobguiTreeStore *tree_store,
			  BobguiTreeIter  *iter,
			  int           column,
			  GValue       *value)
{
  ...
  if (bobgui_tree_store_real_set_value (tree_store, iter, column, value, TRUE))
    {
      BobguiTreePath *path;

      path = bobgui_tree_store_get_path (BOBGUI_TREE_MODEL (tree_store), iter);
      bobgui_tree_model_row_changed (BOBGUI_TREE_MODEL (tree_store), path, iter);
      bobgui_tree_path_free (path);
    }
}

static BobguiTreeIterCompareFunc
bobgui_tree_store_get_compare_func (BobguiTreeStore *tree_store)
{
  BobguiTreeStorePrivate *priv = tree_store->priv;
  BobguiTreeIterCompareFunc func = NULL;

  if (BOBGUI_TREE_STORE_IS_SORTED (tree_store))
    {
...
static void
bobgui_tree_store_set_vector_internal (BobguiTreeStore *tree_store,
				    BobguiTreeIter  *iter,
				    gboolean     *emit_signal,
				    ...
				    int           n_values)
{
  ...
  func = bobgui_tree_store_get_compare_func (tree_store);
  if (func != _bobgui_tree_data_list_compare_func)
    *maybe_need_sort = TRUE;

  for (i = 0; i < n_values; i++)
    {
      *emit_signal = bobgui_tree_store_real_set_value (tree_store, iter,
						    columns[i], &values[i],
						    FALSE) || *emit_signal;
...
static void
bobgui_tree_store_set_valist_internal (BobguiTreeStore *tree_store,
                                    BobguiTreeIter  *iter,
                                    gboolean     *emit_signal,
                                    gboolean     *maybe_need_sort,
                                    va_list       var_args)
{
  ...
  while (column != -1)
      ...
      if (error)
	{
      ...
      *emit_signal = bobgui_tree_store_real_set_value (tree_store,
						    iter,
						    column,
						    &value,
						    FALSE) || *emit_signal;
...
/**
 * bobgui_tree_store_set_valuesv: (rename-to bobgui_tree_store_set)
 * @tree_store: A `BobguiTreeStore`
 * @iter: A valid `BobguiTreeIter` for the row being modified
 ...
 *
 * A variant of bobgui_tree_store_set_valist() which takes
 ...
 **/
void
bobgui_tree_store_set_valuesv (BobguiTreeStore *tree_store,
			    BobguiTreeIter  *iter,
			    int          *columns,
			    GValue       *values,
			    int           n_values)
{
  ...
  bobgui_tree_store_set_vector_internal (tree_store, iter,
				      &emit_signal,
				      &maybe_need_sort,
				      columns, values, n_values);

  if (maybe_need_sort && BOBGUI_TREE_STORE_IS_SORTED (tree_store))
    bobgui_tree_store_sort_iter_changed (tree_store, iter, priv->sort_column_id, TRUE);
...
/**
 * bobgui_tree_store_set_valist:
 * @tree_store: A `BobguiTreeStore`
 * @iter: A valid `BobguiTreeIter` for the row being modified
 * @var_args: va_list of column/value pairs
 *
 * A version of bobgui_tree_store_set() using `va_list`.
 *
 * Deprecated: 4.10: Use [class@Bobgui.TreeListModel] instead
 **/
void
bobgui_tree_store_set_valist (BobguiTreeStore *tree_store,
                           BobguiTreeIter  *iter,
                           va_list       var_args)
{
  BobguiTreeStorePrivate *priv = tree_store->priv;
  ...
  bobgui_tree_store_set_valist_internal (tree_store, iter,
				      &emit_signal,
				      &maybe_need_sort,
				      var_args);
 ...
 *
 * ```c
 * bobgui_tree_store_set (store, iter, 0, "Foo", -1);
 * ```
 *
 ...
 **/
void
bobgui_tree_store_set (BobguiTreeStore *tree_store,
		    BobguiTreeIter  *iter,
		    ...)
{
  va_list var_args;
  ...
  bobgui_tree_store_set_valist (tree_store, iter, var_args);
  va_end (var_args);
}

/**
 * bobgui_tree_store_remove:
 * @tree_store: A `BobguiTreeStore`
 ...
 **/
gboolean
bobgui_tree_store_remove (BobguiTreeStore *tree_store,
		       BobguiTreeIter  *iter)
{
  BobguiTreeStorePrivate *priv = tree_store->priv;
  BobguiTreePath *path;
  BobguiTreeIter new_iter = {0,};
  GNode *parent;
  GNode *next_node;

  g_return_val_if_fail (BOBGUI_IS_TREE_STORE (tree_store), FALSE);
  ...
  bobgui_tree_model_row_deleted (BOBGUI_TREE_MODEL (tree_store), path);
  ...
  if (parent != G_NODE (priv->root))
      ...
      if (parent->children == NULL)
	{
	  ...
	  bobgui_tree_model_row_has_child_toggled (BOBGUI_TREE_MODEL (tree_store), path, &new_iter);
	}
    }
  bobgui_tree_path_free (path);
...
/**
 * bobgui_tree_store_insert:
 * @tree_store: A `BobguiTreeStore`
 * @iter: (out): An unset `BobguiTreeIter` to set to the new row
 * @parent: (nullable): A valid `BobguiTreeIter`
 ...
 * need to call bobgui_tree_store_set() or bobgui_tree_store_set_value().
 *
 * Deprecated: 4.10: Use [class@Bobgui.TreeListModel] instead
 **/
void
bobgui_tree_store_insert (BobguiTreeStore *tree_store,
		       BobguiTreeIter  *iter,
		       BobguiTreeIter  *parent,
		       int           position)
{
  ...
  path = bobgui_tree_store_get_path (BOBGUI_TREE_MODEL (tree_store), iter);
  bobgui_tree_model_row_inserted (BOBGUI_TREE_MODEL (tree_store), path, iter);
  ...
  if (parent_node != priv->root)
    {
      if (new_node->prev == NULL && new_node->next == NULL)
        {
          bobgui_tree_path_up (path);
          bobgui_tree_model_row_has_child_toggled (BOBGUI_TREE_MODEL (tree_store), path, parent);
        }
    }
...
/**
 * bobgui_tree_store_insert_before:
 * @tree_store: A `BobguiTreeStore`
 * @iter: (out): An unset `BobguiTreeIter` to set to the new row
 * @parent: (nullable): A valid `BobguiTreeIter`
 ...
 * bobgui_tree_store_set() or bobgui_tree_store_set_value().
 *
 * Deprecated: 4.10: Use [class@Bobgui.TreeListModel] instead
 **/
void
bobgui_tree_store_insert_before (BobguiTreeStore *tree_store,
			      BobguiTreeIter  *iter,
			      BobguiTreeIter  *parent,
      ...
      if (new_node->prev == NULL && new_node->next == NULL)
        {
          ...
          bobgui_tree_path_up (path);
          bobgui_tree_model_row_has_child_toggled (BOBGUI_TREE_MODEL (tree_store), path, &parent_iter);
        }
    }
...
/**
 * bobgui_tree_store_insert_after:
 * @tree_store: A `BobguiTreeStore`
 * @iter: (out): An unset `BobguiTreeIter` to set to the new row
 * @parent: (nullable): A valid `BobguiTreeIter`
 * @sibling: (nullable): A valid `BobguiTreeIter`
 ...
 **/
void
bobgui_tree_store_insert_after (BobguiTreeStore *tree_store,
			     BobguiTreeIter  *iter,
			     BobguiTreeIter  *parent,
			     BobguiTreeIter  *sibling)
{
...
/**
 * bobgui_tree_store_insert_with_values:
 * @tree_store: A `BobguiTreeStore`
 * @iter: (out) (optional): An unset `BobguiTreeIter` to set the new row
 * @parent: (nullable): A valid `BobguiTreeIter`
 ...
 * Calling
 *
 *     bobgui_tree_store_insert_with_values (tree_store, iter, position, ...)
 *
 * has the same effect as calling
 *
 * ```c
 * bobgui_tree_store_insert (tree_store, iter, position);
 * bobgui_tree_store_set (tree_store, iter, ...);
 * ```
 *
 ...
 * performance of the program, bobgui_tree_store_insert_with_values() should
 * generally be preferred when inserting rows in a sorted tree store.
 *
 * Deprecated: 4.10: Use [class@Bobgui.TreeListModel] instead
 */
void
bobgui_tree_store_insert_with_values (BobguiTreeStore *tree_store,
				   BobguiTreeIter  *iter,
				   BobguiTreeIter  *parent,
				   int           position,
				   ...)
{
  ...
  if (maybe_need_sort && BOBGUI_TREE_STORE_IS_SORTED (tree_store))
    bobgui_tree_store_sort_iter_changed (tree_store, iter, priv->sort_column_id, FALSE);
...
/**
 * bobgui_tree_store_insert_with_valuesv: (rename-to bobgui_tree_store_insert_with_values)
 * @tree_store: A `BobguiTreeStore`
 * @iter: (out) (optional): An unset `BobguiTreeIter` to set the new row
 * @parent: (nullable): A valid `BobguiTreeIter`
 * @position: position to insert the new row, or -1 for last
 ...
 * @n_values: the length of the @columns and @values arrays
 *
 * A variant of bobgui_tree_store_insert_with_values() which takes
 ...
 */
void
bobgui_tree_store_insert_with_valuesv (BobguiTreeStore *tree_store,
				    BobguiTreeIter  *iter,
				    BobguiTreeIter  *parent,
				    int           position,
				    int          *columns,
...
/**
 * bobgui_tree_store_prepend:
 * @tree_store: A `BobguiTreeStore`
 * @iter: (out): An unset `BobguiTreeIter` to set to the prepended row
 * @parent: (nullable): A valid `BobguiTreeIter`
 *
 ...
 * call bobgui_tree_store_set() or bobgui_tree_store_set_value().
 *
 * Deprecated: 4.10: Use [class@Bobgui.TreeListModel] instead
 **/
void
bobgui_tree_store_prepend (BobguiTreeStore *tree_store,
			BobguiTreeIter  *iter,
			BobguiTreeIter  *parent)
{
  BobguiTreeStorePrivate *priv = tree_store->priv;
  ...
  if (parent_node->children == NULL)
    ...
    {
      bobgui_tree_store_insert_after (tree_store, iter, parent, NULL);
    }
 ...
 **/
void
bobgui_tree_store_append (BobguiTreeStore *tree_store,
		       BobguiTreeIter  *iter,
		       BobguiTreeIter  *parent)
{
  BobguiTreeStorePrivate *priv = tree_store->priv;
  ...
  if (parent_node->children == NULL)
    ...
    {
      bobgui_tree_store_insert_before (tree_store, iter, parent, NULL);
    }
...
/**
 * bobgui_tree_store_is_ancestor:
 * @tree_store: A `BobguiTreeStore`
 * @iter: A valid `BobguiTreeIter`
 * @descendant: A valid `BobguiTreeIter`
 *
 ...
 **/
gboolean
bobgui_tree_store_is_ancestor (BobguiTreeStore *tree_store,
			    BobguiTreeIter  *iter,
 ...
 **/
int
bobgui_tree_store_iter_depth (BobguiTreeStore *tree_store,
			   BobguiTreeIter  *iter)
{
  g_return_val_if_fail (BOBGUI_IS_TREE_STORE (tree_store), 0);
...
static gboolean
bobgui_tree_store_clear_traverse (GNode        *node,
  ...
  if (node->children)
    {
	  ...
	  current = child;
	  child = current->next;
	  if (bobgui_tree_store_clear_traverse (current, store))
	    return TRUE;
	}
...
static void
bobgui_tree_store_increment_stamp (BobguiTreeStore *tree_store)
{
  BobguiTreeStorePrivate *priv = tree_store->priv;
  do
    {
 ...
 **/
void
bobgui_tree_store_clear (BobguiTreeStore *tree_store)
{
  g_return_if_fail (BOBGUI_IS_TREE_STORE (tree_store));

  bobgui_tree_store_clear_traverse (tree_store->priv->root, tree_store);
  bobgui_tree_store_increment_stamp (tree_store);
}

static gboolean
bobgui_tree_store_iter_is_valid_helper (BobguiTreeIter *iter,
				     GNode       *first)
{
  GNode *node;
  ...
  do
	...
	return TRUE;
      ...
      if (node->children)
	if (bobgui_tree_store_iter_is_valid_helper (iter, node->children))
	  return TRUE;
...
/**
 * bobgui_tree_store_iter_is_valid:
 * @tree_store: a tree store
 * @iter: the iterator to check
 *
 * Checks if the given iter is a valid iter for this `BobguiTreeStore`.
 *
 ...
 **/
gboolean
bobgui_tree_store_iter_is_valid (BobguiTreeStore *tree_store,
                              BobguiTreeIter  *iter)
{
  g_return_val_if_fail (BOBGUI_IS_TREE_STORE (tree_store), FALSE);
  g_return_val_if_fail (iter != NULL, FALSE);
  ...
  return bobgui_tree_store_iter_is_valid_helper (iter, tree_store->priv->root);
}
...
static gboolean
bobgui_tree_store_drag_data_delete (BobguiTreeDragSource *drag_source,
                                 BobguiTreePath       *path)
{
  BobguiTreeIter iter;

  if (bobgui_tree_store_get_iter (BOBGUI_TREE_MODEL (drag_source),
                               &iter,
                               path))
    {
      bobgui_tree_store_remove (BOBGUI_TREE_STORE (drag_source),
                             &iter);
      return TRUE;
    }
  else
...
static void
copy_node_data (BobguiTreeStore *tree_store,
                BobguiTreeIter  *src_iter,
                BobguiTreeIter  *dest_iter)
{
  BobguiTreeDataList *dl = G_NODE (src_iter->user_data)->data;
  ...
  while (dl)
    {
      copy_iter = _bobgui_tree_data_list_node_copy (dl, tree_store->priv->column_headers[col]);
  ...
  path = bobgui_tree_store_get_path (BOBGUI_TREE_MODEL (tree_store), dest_iter);
  bobgui_tree_model_row_changed (BOBGUI_TREE_MODEL (tree_store), path, dest_iter);
  bobgui_tree_path_free (path);
}

static void
recursive_node_copy (BobguiTreeStore *tree_store,
                     BobguiTreeIter  *src_iter,
                     BobguiTreeIter  *dest_iter)
{
  BobguiTreeIter child;
  ...
  model = BOBGUI_TREE_MODEL (tree_store);

  copy_node_data (tree_store, src_iter, dest_iter);

  if (bobgui_tree_store_iter_children (model,
                                    &child,
                                    src_iter))
    {
      ...
      do
        {
          ...
          bobgui_tree_store_append (tree_store,
                                 &copy,
                                 dest_iter);

          recursive_node_copy (tree_store, &child, &copy);
        }
      while (bobgui_tree_store_iter_next (model, &child));
    }
}

static gboolean
bobgui_tree_store_drag_data_received (BobguiTreeDragDest *drag_dest,
                                   BobguiTreePath     *dest,
                                   const GValue    *value)
{
  BobguiTreeModel *tree_model;
  BobguiTreeStore *tree_store;
  BobguiTreeModel *src_model = NULL;
  BobguiTreePath *src_path = NULL;
  gboolean retval = FALSE;

  tree_model = BOBGUI_TREE_MODEL (drag_dest);
  tree_store = BOBGUI_TREE_STORE (drag_dest);
  ...
  if (bobgui_tree_get_row_drag_data (value,
      ...
      src_model == tree_model)
    {
      ...
      if (!bobgui_tree_store_get_iter (src_model,
                                    &src_iter,
                                    src_path))
        {
          goto out;
      ...
      if (!bobgui_tree_path_prev (prev))
        {
          BobguiTreeIter dest_parent;
          BobguiTreePath *parent;
          BobguiTreeIter *dest_parent_p;
          ...
          dest_parent_p = NULL;
          parent = bobgui_tree_path_copy (dest);
          if (bobgui_tree_path_up (parent) &&
	      bobgui_tree_path_get_depth (parent) > 0)
            {
              bobgui_tree_store_get_iter (tree_model,
                                       &dest_parent,
                                       parent);
          ...
          bobgui_tree_store_prepend (tree_store,
                                  &dest_iter,
                                  dest_parent_p);
        ...
        {
          if (bobgui_tree_store_get_iter (tree_model, &dest_iter, prev))
            {
              BobguiTreeIter tmp_iter = dest_iter;

              bobgui_tree_store_insert_after (tree_store, &dest_iter, NULL,
                                           &tmp_iter);
...
static gboolean
bobgui_tree_store_row_drop_possible (BobguiTreeDragDest *drag_dest,
                                  BobguiTreePath     *dest_path,
				  const GValue    *value)
{
  BobguiTreeModel *src_model = NULL;
  BobguiTreePath *src_path = NULL;
  BobguiTreePath *tmp = NULL;
  gboolean retval = FALSE;

  /* don't accept drops if the tree has been sorted */
  if (BOBGUI_TREE_STORE_IS_SORTED (drag_dest))
    return FALSE;
  ...
  if (bobgui_tree_path_is_ancestor (src_path,
                                 dest_path))
    goto out;
  ...
  /* Can't drop if dest_path's parent doesn't exist */
  {
    ...
    if (bobgui_tree_path_get_depth (dest_path) > 1)
      {
	tmp = bobgui_tree_path_copy (dest_path);
	bobgui_tree_path_up (tmp);

	if (!bobgui_tree_store_get_iter (BOBGUI_TREE_MODEL (drag_dest),
				      &iter, tmp))
	  goto out;
      }
  }
...
/* Reordering */
static int
bobgui_tree_store_reorder_func (gconstpointer a,
			     gconstpointer b,
			     gpointer      user_data)
{
  SortTuple *a_reorder;
...
/**
 * bobgui_tree_store_reorder: (skip)
 * @tree_store: A `BobguiTreeStore`
 ...
 */
void
bobgui_tree_store_reorder (BobguiTreeStore *tree_store,
			BobguiTreeIter  *parent,
			int          *new_order)
{
  int i, length = 0;
  ...
  g_return_if_fail (BOBGUI_IS_TREE_STORE (tree_store));
  g_return_if_fail (!BOBGUI_TREE_STORE_IS_SORTED (tree_store));
  ...
  g_sort_array (sort_array, length, sizeof (SortTuple), bobgui_tree_store_reorder_func, NULL);
  ...
  /* emit signal */
  if (parent)
    path = bobgui_tree_store_get_path (BOBGUI_TREE_MODEL (tree_store), parent);
  else
    path = bobgui_tree_path_new ();
  bobgui_tree_model_rows_reordered (BOBGUI_TREE_MODEL (tree_store), path,
				 parent, new_order);
  bobgui_tree_path_free (path);
  g_free (sort_array);
}
 ...
 **/
void
bobgui_tree_store_swap (BobguiTreeStore *tree_store,
		     BobguiTreeIter  *a,
		     BobguiTreeIter  *b)
{
  ...
  path_a = bobgui_tree_store_get_path (BOBGUI_TREE_MODEL (tree_store), a);
  path_b = bobgui_tree_store_get_path (BOBGUI_TREE_MODEL (tree_store), b);

  g_return_if_fail (path_a && path_b);

  bobgui_tree_path_up (path_a);
  bobgui_tree_path_up (path_b);

  if (bobgui_tree_path_get_depth (path_a) == 0
      || bobgui_tree_path_get_depth (path_b) == 0)
    {
      if (bobgui_tree_path_get_depth (path_a) != bobgui_tree_path_get_depth (path_b))
        {
          bobgui_tree_path_free (path_a);
          bobgui_tree_path_free (path_b);
    ...
    {
      if (bobgui_tree_path_compare (path_a, path_b))
        {
          bobgui_tree_path_free (path_a);
          bobgui_tree_path_free (path_b);
      ...
      bobgui_tree_store_get_iter (BOBGUI_TREE_MODEL (tree_store), &parent,
                               path_a);
      parent_node = G_NODE (parent.user_data);
    }
  bobgui_tree_path_free (path_b);
  ...
  bobgui_tree_model_rows_reordered (BOBGUI_TREE_MODEL (tree_store), path_a,
 ...
 */
static void
bobgui_tree_store_move (BobguiTreeStore *tree_store,
                     BobguiTreeIter  *iter,
		     BobguiTreeIter  *position,
		     gboolean      before)
{
  ...
  BobguiTreePath *path = NULL, *tmppath, *pos_path = NULL;
  BobguiTreeIter parent_iter = { 0, };
  BobguiTreeIter dst_a = { 0, };
  BobguiTreeIter dst_b = { 0, };
  int depth = 0;
  ...
  if (position)
    {
      path = bobgui_tree_store_get_path (BOBGUI_TREE_MODEL (tree_store), iter);
      pos_path = bobgui_tree_store_get_path (BOBGUI_TREE_MODEL (tree_store),
	                                  position);
       ...
       *   moving the iter before path or "path - 1" doesn't make sense
       */
      if (!bobgui_tree_path_compare (path, pos_path))
	goto free_paths_and_out;

      if (before)
        bobgui_tree_path_next (path);
      else
        bobgui_tree_path_prev (path);
	...
	goto free_paths_and_out;
      ...
      if (bobgui_tree_path_get_depth (path) != bobgui_tree_path_get_depth (pos_path))
        {
          g_warning ("Given children are not in the same level\n");
      ...
      tmppath = bobgui_tree_path_copy (pos_path);
      bobgui_tree_path_up (path);
      bobgui_tree_path_up (tmppath);

      if (bobgui_tree_path_get_depth (path) > 0 &&
	  bobgui_tree_path_compare (path, tmppath))
        {
          g_warning ("Given children are not in the same level\n");
  ...
  if (depth)
    {
      bobgui_tree_store_get_iter (BOBGUI_TREE_MODEL (tree_store),
			       &parent_iter, path);
  ...
  if (before && position)
    {
      ...
      if (bobgui_tree_path_get_indices (pos_path)[bobgui_tree_path_get_depth (pos_path) - 1] > 0)
        {
          bobgui_tree_path_prev (pos_path);
          if (bobgui_tree_store_get_iter (BOBGUI_TREE_MODEL (tree_store),
				       &dst_a, pos_path))
            a = G_NODE (dst_a.user_data);
          else
            a = NULL;
          bobgui_tree_path_next (pos_path);
	}
  ...
  else /* !before */
    {
      ...
      if (position)
        {
          bobgui_tree_path_next (pos_path);
          if (bobgui_tree_store_get_iter (BOBGUI_TREE_MODEL (tree_store), &dst_b, pos_path))
	  ...
	  if (depth)
	    bobgui_tree_store_iter_children (BOBGUI_TREE_MODEL (tree_store), &dst_b,
	                                  &parent_iter);
	  else
	    bobgui_tree_store_iter_children (BOBGUI_TREE_MODEL (tree_store), &dst_b,
		                          NULL);
  ...
  /* emit signal */
  if (position)
    new_pos = bobgui_tree_path_get_indices (pos_path)[bobgui_tree_path_get_depth (pos_path)-1];
  else if (before)
    {
      if (depth)
        new_pos = bobgui_tree_store_iter_n_children (BOBGUI_TREE_MODEL (tree_store),
	                                          &parent_iter) - 1;
      else
	new_pos = bobgui_tree_store_iter_n_children (BOBGUI_TREE_MODEL (tree_store),
	                                          NULL) - 1;
  ...
  if (depth)
    {
      tmppath = bobgui_tree_store_get_path (BOBGUI_TREE_MODEL (tree_store),
					 &parent_iter);
      bobgui_tree_model_rows_reordered (BOBGUI_TREE_MODEL (tree_store),
  ...
  bobgui_tree_path_free (path);
  if (position)
    bobgui_tree_path_free (pos_path);
  g_free (order);
 ...
 **/
void
bobgui_tree_store_move_before (BobguiTreeStore *tree_store,
                            BobguiTreeIter  *iter,
			    BobguiTreeIter  *position)
{
  bobgui_tree_store_move (tree_store, iter, position, TRUE);
}

/**
 * bobgui_tree_store_move_after:
 * @tree_store: A `BobguiTreeStore`
 * @iter: A `BobguiTreeIter`.
 * @position: (nullable): A `BobguiTreeIter`.
 *
 ...
 **/
void
bobgui_tree_store_move_after (BobguiTreeStore *tree_store,
                           BobguiTreeIter  *iter,
			   BobguiTreeIter  *position)
{
  bobgui_tree_store_move (tree_store, iter, position, FALSE);
}

/* Sorting */
static int
bobgui_tree_store_compare_func (gconstpointer a,
			     gconstpointer b,
			     gpointer      user_data)
{
  BobguiTreeStore *tree_store = user_data;
  BobguiTreeStorePrivate *priv = tree_store->priv;
  GNode *node_a;
  GNode *node_b;
  BobguiTreeIterCompareFunc func;
  ...
  retval = (* func) (BOBGUI_TREE_MODEL (user_data), &iter_a, &iter_b, data);
...
static void
bobgui_tree_store_sort_helper (BobguiTreeStore *tree_store,
			    GNode        *parent,
			    gboolean      recurse)
{
  BobguiTreeIter iter;
  ...
  if (node == NULL || node->next == NULL)
    {
      if (recurse && node && node->children)
        bobgui_tree_store_sort_helper (tree_store, node, TRUE);
  ...
  /* Sort the array */
  g_array_sort_with_data (sort_array, bobgui_tree_store_compare_func, tree_store);
  ...
  if (recurse)
      ...
      for (tmp_node = parent->children; tmp_node; tmp_node = tmp_node->next)
	{
	  if (tmp_node->children)
	    bobgui_tree_store_sort_helper (tree_store, tmp_node, TRUE);
	}
    }
}

static void
bobgui_tree_store_sort (BobguiTreeStore *tree_store)
{
  BobguiTreeStorePrivate *priv = tree_store->priv;
  ...
  bobgui_tree_store_sort_helper (tree_store, G_NODE (priv->root), TRUE);
}

static void
bobgui_tree_store_sort_iter_changed (BobguiTreeStore *tree_store,
				  BobguiTreeIter  *iter,
				  int           column,
				  gboolean      emit_signal)
{
  ...
  if (prev != NULL)
    {
      ...
      cmp_a = (* func) (BOBGUI_TREE_MODEL (tree_store), &tmp_iter, iter, data);
    }

  if (next != NULL)
    {
      tmp_iter.user_data = next;
      cmp_b = (* func) (BOBGUI_TREE_MODEL (tree_store), iter, &tmp_iter, data);
    }
  ...
  if (priv->order == BOBGUI_SORT_DESCENDING)
    ...
    cmp_a = (* func) (BOBGUI_TREE_MODEL (tree_store), iter, &tmp_iter, data);
  ...
  tmp_path = bobgui_tree_store_get_path (BOBGUI_TREE_MODEL (tree_store), &tmp_iter);
...
static gboolean
bobgui_tree_store_get_sort_column_id (BobguiTreeSortable  *sortable,
				   int              *sort_column_id,
				   BobguiSortType      *order)
{
  BobguiTreeStore *tree_store = (BobguiTreeStore *) sortable;
  BobguiTreeStorePrivate *priv = tree_store->priv;
...
static void
bobgui_tree_store_set_sort_column_id (BobguiTreeSortable  *sortable,
				   int               sort_column_id,
				   BobguiSortType       order)
{
  BobguiTreeStore *tree_store = (BobguiTreeStore *) sortable;
  ...
  bobgui_tree_store_sort (tree_store);
}

static void
bobgui_tree_store_set_sort_func (BobguiTreeSortable        *sortable,
			      int                     sort_column_id,
			      BobguiTreeIterCompareFunc  func,
			      gpointer                data,
			      GDestroyNotify          destroy)
...
static void
bobgui_tree_store_set_default_sort_func (BobguiTreeSortable        *sortable,
				      BobguiTreeIterCompareFunc  func,
				      gpointer                data,
				      GDestroyNotify          destroy)
{
...
static gboolean
bobgui_tree_store_has_default_sort_func (BobguiTreeSortable *sortable)
{
  BobguiTreeStore *tree_store = (BobguiTreeStore *) sortable;
...
append_current_row (SubParserData *data)
{
  ...
  bobgui_tree_store_insert_with_valuesv (BOBGUI_TREE_STORE (data->object),
                                      &iter,
                                      parent,
                                      -1,
                                      data->colids,
  ...
  data->parents = g_list_prepend (data->parents, bobgui_tree_iter_copy (&iter));
...
static void
tree_store_start_element (BobguiBuildableParseContext  *context,
                          ...
                          GError                   **error)
{
  ...
  if (strcmp (element_name, "col") == 0)
    ...
    {
      if (!_bobgui_builder_check_parents (data->builder, context, error, "data", "row", NULL))
        return;
  ...
  else
    {
      _bobgui_builder_error_unhandled_tag (data->builder, context,
                                        "BobguiTreeStore", element_name,
                                        error);
    }
}

static void
tree_store_end_element (BobguiBuildableParseContext  *context,
                        const char                *element_name,
                        gpointer                   user_data,
                        GError                   **error)
{
  ...
  if (strcmp (element_name, "row") == 0)
  ...
  else if (strcmp (element_name, "columns") == 0)
    {
      ...
      bobgui_tree_store_set_column_types (BOBGUI_TREE_STORE (data->object), i, column_types);
...
static void
tree_store_text (BobguiBuildableParseContext  *context,
                 const char                *text,
                 gsize                      text_len,
                 gpointer                   user_data,
                 GError                   **error)
...
static const BobguiBuildableParser tree_store_parser =
{
...
static gboolean
bobgui_tree_store_buildable_custom_tag_start (BobguiBuildable       *buildable,
                                           BobguiBuilder         *builder,
                                           GObject            *child,
                                           const char         *tagname,
                                           BobguiBuildableParser *parser,
                                           gpointer           *parser_data)
{
  ...
  if (strcmp (tagname, "columns") == 0)
    ...
    {
      int n_columns = bobgui_tree_store_get_n_columns (BOBGUI_TREE_MODEL (buildable));
      if (n_columns == 0)
      ...
      data->column_types = BOBGUI_TREE_STORE (buildable)->priv->column_headers;
...
static void
bobgui_tree_store_buildable_custom_tag_end (BobguiBuildable *buildable,
                                         BobguiBuilder   *builder,
                                         GObject      *child,
                                         const char   *tagname,
                                         gpointer      parser_data)
