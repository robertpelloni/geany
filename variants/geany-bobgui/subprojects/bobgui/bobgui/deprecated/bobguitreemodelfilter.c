...
#include "config.h"
#include "bobguitreemodelfilter.h"
 ...
 *
 * A `BobguiTreeModel` which hides parts of an underlying tree model
 *
 * A `BobguiTreeModelFilter` is a tree model which wraps another tree model,
 ...
 *   in a `BobguiTreePath` indicating the root node for the filter at construction
 *   time.
 *
 * The basic API is similar to `BobguiTreeModelSort`. For an example on its usage,
 * see the section on `BobguiTreeModelSort`.
 *
 * When using `BobguiTreeModelFilter`, it is important to realize that
 * `BobguiTreeModelFilter` maintains an internal cache of all nodes which are
 * visible in its clients. The cache is likely to be a subtree of the tree
 * exposed by the child model. `BobguiTreeModelFilter` will not cache the entire
 * child model when unnecessary to not compromise the caching mechanism
 * that is exposed by the reference counting scheme. If the child model
 * implements reference counting, unnecessary signals may not be emitted
 * because of reference counting rule 3, see the `BobguiTreeModel`
 * documentation. (Note that e.g. `BobguiTreeStore` does not implement
 ...
 * nodes (see reference counting rule number 3 in the `BobguiTreeModel` section).
 *
 * Determining the visibility state of a given node based on the state
 * of its child nodes is a frequently occurring use case. Therefore,
 * `BobguiTreeModelFilter` explicitly supports this. For example, when a node
 * does not have any children, you might not want the node to be visible.
 * As soon as the first row is added to the node’s child level (or the
 * last row removed), the node’s visibility should be updated.
 *
 * This introduces a dependency from the node on its child nodes. In order
 * to accommodate this, `BobguiTreeModelFilter` must make sure the necessary
 * signals are received from the child model. This is achieved by building,
 * for all nodes which are exposed as visible nodes to `BobguiTreeModelFilter`'s
 * clients, the child level (if any) and take a reference on the first node
 * in this level. Furthermore, for every row-inserted, row-changed or
 * row-deleted signal (also these which were not handled because the node
 * was not cached), `BobguiTreeModelFilter` will check if the visibility state
 * of any parent node has changed.
 *
 * Beware, however, that this explicit support is limited to these two
 * cases. For example, if you want a node to be visible only if two nodes
 * in a child’s child level (2 levels deeper) are visible, you are on your
 * own. In this case, either rely on `BobguiTreeStore` to emit all signals
 * because it does not implement reference counting, or for models that
 * do implement reference counting, obtain references on these child levels
 * yourself.
 *
 * Deprecated: 4.10: Use [class@Bobgui.FilterListModel] instead.
 */

/* Notes on this implementation of BobguiTreeModelFilter
 * ==================================================
 *
 * Warnings
 * --------
 ...
 * path or value refers to the BobguiTreeModelFilter model, or to the child
 ...
 * BobguiTreeModelFilter and the child model is done through the various
 * bobgui_tree_model_filter_convert_* functions.
 *
 * Even though the BobguiTreeModelSort and BobguiTreeModelFilter have very
 * similar data structures, many assumptions made in the BobguiTreeModelSort
 * code do *not* apply in the BobguiTreeModelFilter case. Reference counting
 * in particular is more complicated in BobguiTreeModelFilter, because
 ...
 * ---------------
 *
 * The iterator format of iterators handed out by BobguiTreeModelFilter is
 * as follows:
 *
 ...
 * -----------------------
 *
 * Using FilterLevel and FilterElt, BobguiTreeModelFilter maintains a “cache”
 * of the mapping from BobguiTreeModelFilter nodes to nodes in the child model.
 ...
 * BobguiTreeModelFilter, such as get iter, get path and get value.
 *
 ...
 * BobguiTreeModelFilter. The visibility state is stored in the “visible_siter”
 ...
 *       which the nodes are exposed to clients of the BobguiTreeModelFilter.
 ...
 *
 * Throughout the code, two kinds of paths relative to the BobguiTreeModelFilter
 ...
 * ------------------
 *
 * BobguiTreeModelFilter forwards all reference and unreference operations
 * to the corresponding node in the child model. In addition,
 * BobguiTreeModelFilter will also add references of its own. The full reference
 ...
 * fields.
 *
 * In a few cases, BobguiTreeModelFilter takes additional references on
 * nodes. The first case is that a reference is taken on the parent
 * (if any) of each level. This happens in bobgui_tree_model_filter_build_level()
 * and the reference is released again in bobgui_tree_model_filter_free_level().
 ...
 * rule 1 in the BobguiTreeModel documentation.
 *
 ...
 * BobguiTreeModelFilter above). We build the child level of each node that
 ...
 * rule 3 in the BobguiTreeModel documentation. If due to changes in the level,
 ...
 * of the parent. This is an important difference from BobguiTreeModelSort!
 *
 * Signals are only required for levels with an external ref count > 0.
 * This due to reference counting rule 3, see the BobguiTreeModel
 * documentation. In the BobguiTreeModelFilter we try hard to stick to this
 ...
 * we simply forward the signal emitted by e.g. BobguiTreeStore but also
 * emit our own copy).
 */
...
struct _BobguiTreeModelFilterPrivate
{
  BobguiTreeModel *child_model;
  gpointer root;
  BobguiTreePath *virtual_root;
  ...
  BobguiTreeModelFilterVisibleFunc visible_func;
  gpointer visible_data;
  GDestroyNotify visible_destroy;

  GType *modify_types;
  BobguiTreeModelFilterModifyFunc modify_func;
 ...
 */
#if 1
#  define BOBGUI_TREE_MODEL_FILTER_CACHE_CHILD_ITERS(filter) \
        (((BobguiTreeModelFilter *)filter)->priv->child_flags & BOBGUI_TREE_MODEL_ITERS_PERSIST)
#else
#  define BOBGUI_TREE_MODEL_FILTER_CACHE_CHILD_ITERS(filter) (FALSE)
#endif
...
static void         bobgui_tree_model_filter_tree_model_init                 (BobguiTreeModelIface       *iface);
static void         bobgui_tree_model_filter_drag_source_init                (BobguiTreeDragSourceIface  *iface);
static void         bobgui_tree_model_filter_finalize                        (GObject                 *object);
static void         bobgui_tree_model_filter_set_property                    (GObject                 *object,
                                                                           guint                    prop_id,
                                                                           const GValue            *value,
                                                                           GParamSpec              *pspec);
static void         bobgui_tree_model_filter_get_property                    (GObject                 *object,
                                                                           guint                    prop_id,
                                                                           GValue                 *value,
                                                                           GParamSpec             *pspec);

/* signal handlers */
static void         bobgui_tree_model_filter_row_changed                     (BobguiTreeModel           *c_model,
                                                                           BobguiTreePath            *c_path,
                                                                           BobguiTreeIter            *c_iter,
                                                                           gpointer                data);
static void         bobgui_tree_model_filter_row_inserted                    (BobguiTreeModel           *c_model,
                                                                           BobguiTreePath            *c_path,
                                                                           BobguiTreeIter            *c_iter,
                                                                           gpointer                data);
static void         bobgui_tree_model_filter_row_has_child_toggled           (BobguiTreeModel           *c_model,
                                                                           BobguiTreePath            *c_path,
                                                                           BobguiTreeIter            *c_iter,
                                                                           gpointer                data);
static void         bobgui_tree_model_filter_row_deleted                     (BobguiTreeModel           *c_model,
                                                                           BobguiTreePath            *c_path,
                                                                           gpointer                data);
static void         bobgui_tree_model_filter_rows_reordered                  (BobguiTreeModel           *c_model,
                                                                           BobguiTreePath            *c_path,
                                                                           BobguiTreeIter            *c_iter,
                                                                           int                    *new_order,
                                                                           gpointer                data);

/* BobguiTreeModel interface */
static BobguiTreeModelFlags bobgui_tree_model_filter_get_flags                       (BobguiTreeModel           *model);
static int          bobgui_tree_model_filter_get_n_columns                   (BobguiTreeModel           *model);
static GType        bobgui_tree_model_filter_get_column_type                 (BobguiTreeModel           *model,
                                                                           int                     index);
static gboolean     bobgui_tree_model_filter_get_iter_full                   (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter,
                                                                           BobguiTreePath            *path);
static gboolean     bobgui_tree_model_filter_get_iter                        (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter,
                                                                           BobguiTreePath            *path);
static BobguiTreePath *bobgui_tree_model_filter_get_path                        (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter);
static void         bobgui_tree_model_filter_get_value                       (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter,
                                                                           int                     column,
                                                                           GValue                 *value);
static gboolean     bobgui_tree_model_filter_iter_next                       (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter);
static gboolean     bobgui_tree_model_filter_iter_previous                   (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter);
static gboolean     bobgui_tree_model_filter_iter_children                   (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter,
                                                                           BobguiTreeIter            *parent);
static gboolean     bobgui_tree_model_filter_iter_has_child                  (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter);
static int          bobgui_tree_model_filter_iter_n_children                 (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter);
static gboolean     bobgui_tree_model_filter_iter_nth_child                  (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter,
                                                                           BobguiTreeIter            *parent,
                                                                           int                     n);
static gboolean     bobgui_tree_model_filter_iter_parent                     (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter,
                                                                           BobguiTreeIter            *child);
static void         bobgui_tree_model_filter_ref_node                        (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter);
static void         bobgui_tree_model_filter_unref_node                      (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter);

/* TreeDragSource interface */
static gboolean    bobgui_tree_model_filter_row_draggable                    (BobguiTreeDragSource      *drag_source,
                                                                           BobguiTreePath            *path);
static GdkContentProvider *
                   bobgui_tree_model_filter_drag_data_get                    (BobguiTreeDragSource      *drag_source,
                                                                           BobguiTreePath            *path);
static gboolean    bobgui_tree_model_filter_drag_data_delete                 (BobguiTreeDragSource      *drag_source,
                                                                           BobguiTreePath            *path);

/* private functions */
static void        bobgui_tree_model_filter_build_level                      (BobguiTreeModelFilter     *filter,
                                                                           FilterLevel            *parent_level,
                                                                           FilterElt              *parent_elt,
                                                                           gboolean                emit_inserted);

static void        bobgui_tree_model_filter_free_level                       (BobguiTreeModelFilter     *filter,
                                                                           FilterLevel            *filter_level,
                                                                           gboolean                unref_self,
                                                                           gboolean                unref_parent,
                                                                           gboolean                unref_external);

static BobguiTreePath *bobgui_tree_model_filter_elt_get_path                    (FilterLevel            *level,
                                                                           FilterElt              *elt,
                                                                           BobguiTreePath            *root);

static BobguiTreePath *bobgui_tree_model_filter_add_root                        (BobguiTreePath            *src,
                                                                           BobguiTreePath            *root);
static BobguiTreePath *bobgui_tree_model_filter_remove_root                     (BobguiTreePath            *src,
                                                                           BobguiTreePath            *root);

static void         bobgui_tree_model_filter_increment_stamp                 (BobguiTreeModelFilter     *filter);

static void         bobgui_tree_model_filter_real_modify                     (BobguiTreeModelFilter     *self,
                                                                           BobguiTreeModel           *child_model,
                                                                           BobguiTreeIter            *iter,
                                                                           GValue                 *value,
                                                                           int                     column);
static gboolean     bobgui_tree_model_filter_real_visible                    (BobguiTreeModelFilter     *filter,
                                                                           BobguiTreeModel           *child_model,
                                                                           BobguiTreeIter            *child_iter);
static gboolean     bobgui_tree_model_filter_visible                         (BobguiTreeModelFilter     *filter,
                                                                           BobguiTreeIter            *child_iter);
static void         bobgui_tree_model_filter_clear_cache_helper              (BobguiTreeModelFilter     *filter,
                                                                           FilterLevel            *level);

static void         bobgui_tree_model_filter_real_ref_node                   (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter,
                                                                           gboolean                external);
static void         bobgui_tree_model_filter_real_unref_node                 (BobguiTreeModel           *model,
                                                                           BobguiTreeIter            *iter,
                                                                           gboolean                external,
                                                                           gboolean                propagate_unref);

static void         bobgui_tree_model_filter_set_model                       (BobguiTreeModelFilter     *filter,
                                                                           BobguiTreeModel           *child_model);
static void         bobgui_tree_model_filter_ref_path                        (BobguiTreeModelFilter     *filter,
                                                                           BobguiTreePath            *path);
static void         bobgui_tree_model_filter_unref_path                      (BobguiTreeModelFilter     *filter,
                                                                           BobguiTreePath            *path,
                                                                           int                     depth);
static void         bobgui_tree_model_filter_set_root                        (BobguiTreeModelFilter     *filter,
                                                                           BobguiTreePath            *root);

static BobguiTreePath *bobgui_real_tree_model_filter_convert_child_path_to_path (BobguiTreeModelFilter     *filter,
                                                                           BobguiTreePath            *child_path,
                                                                           gboolean                build_levels,
                                                                           gboolean                fetch_children);

static gboolean    bobgui_tree_model_filter_elt_is_visible_in_target         (FilterLevel            *level,
                                                                           FilterElt              *elt);

static FilterElt   *bobgui_tree_model_filter_insert_elt_in_level             (BobguiTreeModelFilter     *filter,
                                                                           BobguiTreeIter            *c_iter,
                                                                           FilterLevel            *level,
                                                                           int                     offset,
                                                                           int                    *index);
static FilterElt   *bobgui_tree_model_filter_fetch_child                     (BobguiTreeModelFilter     *filter,
                                                                           FilterLevel            *level,
                                                                           int                     offset,
                                                                           int                    *index);
static void         bobgui_tree_model_filter_remove_elt_from_level           (BobguiTreeModelFilter     *filter,
                                                                           FilterLevel            *level,
                                                                           FilterElt              *elt);
static void         bobgui_tree_model_filter_update_children                 (BobguiTreeModelFilter     *filter,
                                                                           FilterLevel            *level,
                                                                           FilterElt              *elt);
static void         bobgui_tree_model_filter_emit_row_inserted_for_path      (BobguiTreeModelFilter     *filter,
                                                                           BobguiTreeModel           *c_model,
                                                                           BobguiTreePath            *c_path,
                                                                           BobguiTreeIter            *c_iter);


G_DEFINE_TYPE_WITH_CODE (BobguiTreeModelFilter, bobgui_tree_model_filter, G_TYPE_OBJECT,
                         G_ADD_PRIVATE (BobguiTreeModelFilter)
			 G_IMPLEMENT_INTERFACE (BOBGUI_TYPE_TREE_MODEL,
						bobgui_tree_model_filter_tree_model_init)
			 G_IMPLEMENT_INTERFACE (BOBGUI_TYPE_TREE_DRAG_SOURCE,
						bobgui_tree_model_filter_drag_source_init))

static void
bobgui_tree_model_filter_init (BobguiTreeModelFilter *filter)
{
  filter->priv = bobgui_tree_model_filter_get_instance_private (filter);
...
static void
bobgui_tree_model_filter_class_init (BobguiTreeModelFilterClass *filter_class)
{
  GObjectClass *object_class;

  object_class = (GObjectClass *) filter_class;

  object_class->set_property = bobgui_tree_model_filter_set_property;
  object_class->get_property = bobgui_tree_model_filter_get_property;

  object_class->finalize = bobgui_tree_model_filter_finalize;

  filter_class->visible = bobgui_tree_model_filter_real_visible;
  filter_class->modify  = bobgui_tree_model_filter_real_modify;

  /**
   * BobguiTreeModelFilter:child-model:
   *
  ...
  /**
   * BobguiTreeModelFilter:virtual-root:
   *
...
static void
bobgui_tree_model_filter_tree_model_init (BobguiTreeModelIface *iface)
{
  iface->get_flags = bobgui_tree_model_filter_get_flags;
  iface->get_n_columns = bobgui_tree_model_filter_get_n_columns;
  iface->get_column_type = bobgui_tree_model_filter_get_column_type;
  iface->get_iter = bobgui_tree_model_filter_get_iter;
  iface->get_path = bobgui_tree_model_filter_get_path;
  iface->get_value = bobgui_tree_model_filter_get_value;
  iface->iter_next = bobgui_tree_model_filter_iter_next;
  iface->iter_previous = bobgui_tree_model_filter_iter_previous;
  iface->iter_children = bobgui_tree_model_filter_iter_children;
  iface->iter_has_child = bobgui_tree_model_filter_iter_has_child;
  iface->iter_n_children = bobgui_tree_model_filter_iter_n_children;
  iface->iter_nth_child = bobgui_tree_model_filter_iter_nth_child;
  iface->iter_parent = bobgui_tree_model_filter_iter_parent;
  iface->ref_node = bobgui_tree_model_filter_ref_node;
  iface->unref_node = bobgui_tree_model_filter_unref_node;
}

static void
bobgui_tree_model_filter_drag_source_init (BobguiTreeDragSourceIface *iface)
{
  iface->row_draggable = bobgui_tree_model_filter_row_draggable;
  iface->drag_data_delete = bobgui_tree_model_filter_drag_data_delete;
  iface->drag_data_get = bobgui_tree_model_filter_drag_data_get;
}


static void
bobgui_tree_model_filter_finalize (GObject *object)
{
  BobguiTreeModelFilter *filter = (BobguiTreeModelFilter *) object;

  if (filter->priv->virtual_root && !filter->priv->virtual_root_deleted)
    {
      bobgui_tree_model_filter_unref_path (filter, filter->priv->virtual_root,
                                        -1);
      filter->priv->virtual_root_deleted = TRUE;
    }

  bobgui_tree_model_filter_set_model (filter, NULL);

  if (filter->priv->virtual_root)
    bobgui_tree_path_free (filter->priv->virtual_root);

  if (filter->priv->root)
    bobgui_tree_model_filter_free_level (filter, filter->priv->root, TRUE, TRUE, FALSE);
  ...
  /* must chain up */
  G_OBJECT_CLASS (bobgui_tree_model_filter_parent_class)->finalize (object);
}

static void
bobgui_tree_model_filter_set_property (GObject      *object,
                                    guint         prop_id,
                                    const GValue *value,
                                    GParamSpec   *pspec)
{
  BobguiTreeModelFilter *filter = BOBGUI_TREE_MODEL_FILTER (object);

  switch (prop_id)
    {
      case PROP_CHILD_MODEL:
        bobgui_tree_model_filter_set_model (filter, g_value_get_object (value));
        break;
      case PROP_VIRTUAL_ROOT:
        bobgui_tree_model_filter_set_root (filter, g_value_get_boxed (value));
        break;
      default:
...
static void
bobgui_tree_model_filter_get_property (GObject    *object,
                                    guint       prop_id,
                                    GValue     *value,
                                    GParamSpec *pspec)
{
...
static void
bobgui_tree_model_filter_build_level (BobguiTreeModelFilter *filter,
                                   FilterLevel        *parent_level,
                                   FilterElt          *parent_elt,
                                   gboolean            emit_inserted)
{
  BobguiTreeIter iter;
  BobguiTreeIter first_node;
  BobguiTreeIter root;
  FilterLevel *new_level;
  FilterLevel *tmp_level;
  FilterElt *tmp_elt;
  BobguiTreeIter f_iter;
  int length = 0;
  int i;
  gboolean empty = TRUE;
  ...
  if (!parent_level)
    {
      if (filter->priv->virtual_root)
        {
          if (bobgui_tree_model_get_iter (filter->priv->child_model, &root, filter->priv->virtual_root) == FALSE)
            return;
          length = bobgui_tree_model_iter_n_children (filter->priv->child_model, &root);

          if (bobgui_tree_model_iter_children (filter->priv->child_model, &iter, &root) == FALSE)
            return;
        }
      else
        {
          if (!bobgui_tree_model_get_iter_first (filter->priv->child_model, &iter))
            return;
          length = bobgui_tree_model_iter_n_children (filter->priv->child_model, NULL);
        }
    }
  else
    {
      ...
      bobgui_tree_model_filter_convert_iter_to_child_iter (filter,
                                                        &child_parent_iter,
                                                        &parent_iter);
      if (bobgui_tree_model_iter_children (filter->priv->child_model, &iter, &child_parent_iter) == FALSE)
        return;
      ...
      bobgui_tree_model_filter_convert_iter_to_child_iter (filter,
                                                        &child_parent_iter,
                                                        &parent_iter);
      length = bobgui_tree_model_iter_n_children (filter->priv->child_model, &child_parent_iter);

      /* Take a reference on the parent */
      bobgui_tree_model_filter_real_ref_node (BOBGUI_TREE_MODEL (filter),
                                           &parent_iter, FALSE);
    }
  ...
  do
    {
      if (bobgui_tree_model_filter_visible (filter, &iter))
        {
          ...
          if (BOBGUI_TREE_MODEL_FILTER_CACHE_CHILD_ITERS (filter))
          ...
          if (emit_inserted)
            {
              ...
              f_path = bobgui_tree_model_get_path (BOBGUI_TREE_MODEL (filter),
                                                &f_iter);
              bobgui_tree_model_row_inserted (BOBGUI_TREE_MODEL (filter),
                                           f_path, &f_iter);
              bobgui_tree_path_free (f_path);

              if (bobgui_tree_model_iter_children (filter->priv->child_model,
                                                &children, &iter))
                bobgui_tree_model_filter_update_children (filter,
                                                       new_level,
  ...
  while (bobgui_tree_model_iter_next (filter->priv->child_model, &iter));
  ...
  if (empty &&
    ...
    {
      bobgui_tree_model_filter_free_level (filter, new_level, FALSE, TRUE, FALSE);
      return;
    }
  ...
  bobgui_tree_model_filter_real_ref_node (BOBGUI_TREE_MODEL (filter), &f_iter, FALSE);
}

static void
bobgui_tree_model_filter_free_level (BobguiTreeModelFilter *filter,
                                  FilterLevel        *filter_level,
                                  gboolean            unref_self,
                                  gboolean            unref_parent,
                                  gboolean            unref_external)
      ...
      if (elt->children)
        {
           ...
           * level).
           */
          bobgui_tree_model_filter_free_level (filter,
      ...
      if (unref_external)
        {
          ...
          while (elt->ext_ref_count > 0)
            bobgui_tree_model_filter_real_unref_node (BOBGUI_TREE_MODEL (filter),
                                                   &f_iter,
                                                   TRUE, unref_self);
        }
    }
 ...
 * bobgui_tree_model_filter_remove_elt_from_level(), which is the reason
 * this function is lacking a “gboolean unref” argument.
 */
static void
bobgui_tree_model_filter_prune_level (BobguiTreeModelFilter *filter,
                                   FilterLevel        *level)
{
  GSequenceIter *siter;
  GSequenceIter *end_siter;
...
static void
bobgui_tree_model_filter_level_transfer_first_ref (BobguiTreeModelFilter *filter,
                                                FilterLevel        *level,
...
static void
bobgui_tree_model_filter_level_transfer_first_ref_with_index (BobguiTreeModelFilter *filter,
                                                           FilterLevel        *level,
                                                           int                 from_index,
                                                           int                 to_index)
{
  bobgui_tree_model_filter_level_transfer_first_ref (filter, level,
...
bobgui_tree_model_filter_elt_get_path (FilterLevel *level,
                                    FilterElt   *elt,
                                    BobguiTreePath *root)
{
  ...
  if (root)
    {
      real_path = bobgui_tree_model_filter_add_root (path, root);
...
static BobguiTreePath *
bobgui_tree_model_filter_add_root (BobguiTreePath *src,
                                BobguiTreePath *root)
{
  BobguiTreePath *retval;
  int i;

  retval = bobgui_tree_path_copy (root);

  for (i = 0; i < bobgui_tree_path_get_depth (src); i++)
    bobgui_tree_path_append_index (retval, bobgui_tree_path_get_indices (src)[i]);

  return retval;
}

static BobguiTreePath *
bobgui_tree_model_filter_remove_root (BobguiTreePath *src,
                                   BobguiTreePath *root)
{
  BobguiTreePath *retval;
  int i;
  ...
  if (bobgui_tree_path_get_depth (src) <= bobgui_tree_path_get_depth (root))
    return NULL;

  depth = bobgui_tree_path_get_depth (src);
  indices = bobgui_tree_path_get_indices (src);

  for (i = 0; i < bobgui_tree_path_get_depth (root); i++)
    if (indices[i] != bobgui_tree_path_get_indices (root)[i])
      return NULL;

  retval = bobgui_tree_path_new ();

  for (; i < depth; i++)
    bobgui_tree_path_append_index (retval, indices[i]);

  return retval;
}

static void
bobgui_tree_model_filter_increment_stamp (BobguiTreeModelFilter *filter)
{
  do
    {
      filter->priv->stamp++;
  ...
  bobgui_tree_model_filter_clear_cache (filter);
}

static gboolean
bobgui_tree_model_filter_real_visible (BobguiTreeModelFilter *filter,
                                    BobguiTreeModel       *child_model,
                                    BobguiTreeIter        *child_iter)
{
  if (filter->priv->visible_func)
  ...
  else if (filter->priv->visible_column >= 0)
   {
     ...
     bobgui_tree_model_get_value (child_model, child_iter,
...
static gboolean
bobgui_tree_model_filter_visible (BobguiTreeModelFilter *self,
                               BobguiTreeIter        *child_iter)
{
  return BOBGUI_TREE_MODEL_FILTER_GET_CLASS (self)->visible (self,
      self->priv->child_model, child_iter);
}

static void
bobgui_tree_model_filter_clear_cache_helper_iter (gpointer data,
                                               gpointer user_data)
{
  BobguiTreeModelFilter *filter = user_data;
  FilterElt *elt = data;
  ...
  if (elt->zero_ref_count > 0)
    bobgui_tree_model_filter_clear_cache_helper (filter, elt->children);
}

static void
bobgui_tree_model_filter_clear_cache_helper (BobguiTreeModelFilter *filter,
                                          FilterLevel        *level)
{
  g_assert (level);

  g_sequence_foreach (level->seq, bobgui_tree_model_filter_clear_cache_helper_iter, filter);
  ...
  if (level->ext_ref_count == 0 && level != filter->priv->root &&
    ...
    {
      bobgui_tree_model_filter_free_level (filter, level, TRUE, TRUE, FALSE);
      return;
    }
}

static gboolean
bobgui_tree_model_filter_elt_is_visible_in_target (FilterLevel *level,
                                                FilterElt   *elt)
{
  if (!elt->visible_siter)
    return FALSE;
 ...
 */
static void
bobgui_tree_model_filter_check_ancestors (BobguiTreeModelFilter *filter,
                                       BobguiTreePath        *path)
{
  int i = 0;
  ...
  FilterElt *elt;
  FilterLevel *level;
  BobguiTreeIter c_iter, tmp_iter, *root_iter;
  ...
  root_iter = NULL;
  if (filter->priv->virtual_root &&
      bobgui_tree_model_get_iter (filter->priv->child_model, &tmp_iter,
                               filter->priv->virtual_root))
    root_iter = &tmp_iter;
  bobgui_tree_model_iter_nth_child (filter->priv->child_model, &c_iter,
                                 root_iter,
                                 indices[i]);

  while (i < bobgui_tree_path_get_depth (path) - 1)
    {
      gboolean requested_state;

      elt = lookup_elt_with_offset (level->seq,
                                    bobgui_tree_path_get_indices (path)[i], NULL);

      requested_state = bobgui_tree_model_filter_visible (filter, &c_iter);
      ...
      if (!elt)
        {
           ...
           */
          elt = bobgui_tree_model_filter_insert_elt_in_level (filter,
                                                           &c_iter,
                                                           level,
                                                           indices[i],
                                                           &index);
          ...
          c_path = bobgui_tree_model_get_path (filter->priv->child_model,
                                            &c_iter);

          bobgui_tree_model_filter_emit_row_inserted_for_path (filter,
                                                            filter->priv->child_model,
                                                            c_path,
                                                            &c_iter);

          bobgui_tree_path_free (c_path);
          ...
          if (!requested_state)
            {
               ...
               * chain.
               */
              bobgui_tree_model_filter_remove_elt_from_level (filter,
                                                           level, elt);
              return;
            }
              ...
              if (level->parent_level)
                {
                  ...
                  f_path = bobgui_tree_model_get_path (BOBGUI_TREE_MODEL (filter),
                                                    &f_iter);
                  bobgui_tree_model_row_has_child_toggled (BOBGUI_TREE_MODEL (filter),
                                                        f_path, &f_iter);
                  bobgui_tree_path_free (f_path);
                }
              else
...
static FilterElt *
bobgui_tree_model_filter_insert_elt_in_level (BobguiTreeModelFilter *filter,
                                           BobguiTreeIter        *c_iter,
                                           FilterLevel        *level,
                                           int                 offset,
                                           int                *index)
{
  ...
  *index = g_sequence_iter_get_position (siter);
   ...
   */
  if (*index == 0)
    bobgui_tree_model_filter_level_transfer_first_ref_with_index (filter, level,
                                                               1, 0);
...
static FilterElt *
bobgui_tree_model_filter_fetch_child (BobguiTreeModelFilter *filter,
                                   FilterLevel        *level,
                                   int                 offset,
                                   int                *index)
{
  int len;
  BobguiTreePath *c_path = NULL;
  BobguiTreeIter c_iter;
  BobguiTreePath *c_parent_path = NULL;
  BobguiTreeIter c_parent_iter;

  /* check if child exists and is visible */
  if (level->parent_elt)
    {
      c_parent_path =
        bobgui_tree_model_filter_elt_get_path (level->parent_level,
                                            level->parent_elt,
                                            filter->priv->virtual_root);
      if (!c_parent_path)
        return NULL;
    ...
    {
      if (filter->priv->virtual_root)
        c_parent_path = bobgui_tree_path_copy (filter->priv->virtual_root);
      else
        c_parent_path = NULL;
    }
  ...
  if (c_parent_path)
    {
      bobgui_tree_model_get_iter (filter->priv->child_model,
                               &c_parent_iter,
                               c_parent_path);
      len = bobgui_tree_model_iter_n_children (filter->priv->child_model,
                                            &c_parent_iter);

      c_path = bobgui_tree_path_copy (c_parent_path);
      bobgui_tree_path_free (c_parent_path);
    }
  else
    {
      len = bobgui_tree_model_iter_n_children (filter->priv->child_model, NULL);
      c_path = bobgui_tree_path_new ();
    }

  bobgui_tree_path_append_index (c_path, offset);
  bobgui_tree_model_get_iter (filter->priv->child_model, &c_iter, c_path);
  bobgui_tree_path_free (c_path);

  if (offset >= len || !bobgui_tree_model_filter_visible (filter, &c_iter))
    return NULL;

  return bobgui_tree_model_filter_insert_elt_in_level (filter, &c_iter,
                                                    level, offset,
                                                    index);
}
 ...
 */
static void
bobgui_tree_model_filter_remove_elt_from_level (BobguiTreeModelFilter *filter,
                                             FilterLevel        *level,
                                             FilterElt          *elt)
{
  FilterElt *parent;
  ...
  if (!parent || orig_level_ext_ref_count > 0)
    path = bobgui_tree_model_get_path (BOBGUI_TREE_MODEL (filter), &iter);
  else
  ...
  if (length > 1)
    {
       ...
       */
      if (elt->children)
        bobgui_tree_model_filter_free_level (filter, elt->children, TRUE, TRUE, TRUE);
      ...
      bobgui_tree_model_filter_increment_stamp (filter);

      /* Only if the node is in the root level (parent == NULL) or
       * the level is visible, a row-deleted signal is necessary.
       */
      if (!parent || orig_level_ext_ref_count > 0)
        bobgui_tree_model_row_deleted (BOBGUI_TREE_MODEL (filter), path);
    }
  else
    {
      ...
      if (! (level == filter->priv->root ||
             ...
             level->parent_level->ext_ref_count > 0))
        {
          ...
          bobgui_tree_model_filter_free_level (filter, level, TRUE, TRUE, TRUE);
        }
      else
        {
...
#endif
              if (elt->children)
                bobgui_tree_model_filter_free_level (filter, elt->children,
                                                  TRUE, TRUE, TRUE);
            }
          else
            {
               ...
               */
              if (elt->children)
                bobgui_tree_model_filter_prune_level (filter, elt->children);
            }
        }
  ...
  if (emit_child_toggled && parent->ext_ref_count > 0)
    {
      ...
      ppath = bobgui_tree_model_get_path (BOBGUI_TREE_MODEL (filter), &piter);
 ...
 */
static void
bobgui_tree_model_filter_update_children (BobguiTreeModelFilter *filter,
				       FilterLevel        *level,
				       FilterElt          *elt)
{
  BobguiTreeIter c_iter;
  ...
  bobgui_tree_model_filter_convert_iter_to_child_iter (filter, &c_iter, &iter);

  if ((!level->parent_level || level->parent_level->ext_ref_count > 0) &&
      bobgui_tree_model_iter_has_child (filter->priv->child_model, &c_iter))
    {
      if (!elt->children)
        bobgui_tree_model_filter_build_level (filter, level, elt, FALSE);
 ...
 */
static gboolean
find_elt_with_offset (BobguiTreeModelFilter *filter,
                      BobguiTreePath        *path,
                      FilterLevel       **level_,
                      FilterElt         **elt_)
{
  ...
  while (i < bobgui_tree_path_get_depth (path))
    {
      if (!level)
        return FALSE;

      elt = lookup_elt_with_offset (level->seq,
                                    bobgui_tree_path_get_indices (path)[i],
                                    NULL);
...
/* TreeModel signals */
static void
bobgui_tree_model_filter_emit_row_inserted_for_path (BobguiTreeModelFilter *filter,
                                                  BobguiTreeModel       *c_model,
                                                  BobguiTreePath        *c_path,
                                                  BobguiTreeIter        *c_iter)
{
  ...
  if (!filter->priv->root)
    {
       ...
       */
      bobgui_tree_model_filter_build_level (filter, NULL, NULL, TRUE);
   ...
   */
  path = bobgui_real_tree_model_filter_convert_child_path_to_path (filter,
                                                                c_path,
                                                                FALSE,
                                                                TRUE);
  ...
  bobgui_tree_model_filter_get_iter_full (BOBGUI_TREE_MODEL (filter), &iter, path);
  ...
  if (bobgui_tree_model_filter_elt_is_visible_in_target (level, elt))
    {
      ...
      if (!signals_emitted &&
        ...
        bobgui_tree_model_row_inserted (BOBGUI_TREE_MODEL (filter), path, &iter);
      ...
      if (level->parent_level && level->parent_elt->ext_ref_count > 0 &&
          g_sequence_get_length (level->visible_seq) == 1)
        {
          ...
          bobgui_tree_model_get_iter (BOBGUI_TREE_MODEL (filter), &iter, path);
      ...
      if (!signals_emitted
          && bobgui_tree_model_iter_children (c_model, &children, c_iter))
        bobgui_tree_model_filter_update_children (filter, level, elt);
    }
...
static void
bobgui_tree_model_filter_row_changed (BobguiTreeModel *c_model,
                                   BobguiTreePath  *c_path,
                                   BobguiTreeIter  *c_iter,
                                   gpointer      data)
{
  BobguiTreeModelFilter *filter = BOBGUI_TREE_MODEL_FILTER (data);
  BobguiTreeIter iter;
  BobguiTreeIter children;
  BobguiTreeIter real_c_iter;
  BobguiTreePath *path = NULL;
  BobguiTreePath *real_path = NULL;
  ...
  if (!c_path)
    {
      c_path = bobgui_tree_model_get_path (c_model, c_iter);
      free_c_path = TRUE;
    }

  if (filter->priv->virtual_root)
    real_path = bobgui_tree_model_filter_remove_root (c_path,
                                                   filter->priv->virtual_root);
  else
    real_path = bobgui_tree_path_copy (c_path);

  if (c_iter)
    real_c_iter = *c_iter;
  else
    bobgui_tree_model_get_iter (c_model, &real_c_iter, c_path);

  /* is this node above the virtual root? */
  if (filter->priv->virtual_root &&
      (bobgui_tree_path_get_depth (filter->priv->virtual_root)
          >= bobgui_tree_path_get_depth (c_path)))
    goto done;

  /* what's the requested state? */
  requested_state = bobgui_tree_model_filter_visible (filter, &real_c_iter);
  ...
  if (path)
    {
      bobgui_tree_model_filter_get_iter_full (BOBGUI_TREE_MODEL (filter),
                                           &iter, path);
      current_state = FILTER_ELT (iter.user_data2)->visible_siter != NULL;
    }
  else
    {
      iter = (BobguiTreeIter) { 0, }; /* MSVC... */
      current_state = FALSE;
    }
  ...
  if (current_state == TRUE && requested_state == FALSE)
    {
      ...
      if (real_path)
        bobgui_tree_model_filter_check_ancestors (filter, real_path);
      ...
      if (bobgui_tree_model_filter_elt_is_visible_in_target (level, elt))
        {
          ...
          if (level->ext_ref_count > 0)
            bobgui_tree_model_row_changed (BOBGUI_TREE_MODEL (filter), path, &iter);

          /* and update the children */
          if (bobgui_tree_model_iter_children (c_model, &children, &real_c_iter))
            bobgui_tree_model_filter_update_children (filter, level, elt);
        }
  ...
  bobgui_tree_model_filter_emit_row_inserted_for_path (filter, c_model,
                                                    c_path, c_iter);
...
static void
bobgui_tree_model_filter_row_inserted (BobguiTreeModel *c_model,
                                    BobguiTreePath  *c_path,
                                    BobguiTreeIter  *c_iter,
                                    gpointer      data)
{
  ...
  if (filter->priv->virtual_root)
    {
      if (bobgui_tree_path_get_depth (filter->priv->virtual_root) >=
          bobgui_tree_path_get_depth (c_path))
        {
          int depth;
          ...
          depth = bobgui_tree_path_get_depth (c_path) - 1;
          v_indices = bobgui_tree_path_get_indices (filter->priv->virtual_root);
          c_indices = bobgui_tree_path_get_indices (c_path);
  ...
  if (bobgui_tree_path_get_depth (real_path) - 1 >= 1)
    {
      gboolean found = FALSE;
      BobguiTreePath *parent = bobgui_tree_path_copy (real_path);
      bobgui_tree_path_up (parent);

      found = find_elt_with_offset (filter, parent, &parent_level, &elt);

      bobgui_tree_path_free (parent);
      ...
      if (elt && elt->visible_siter)
        {
          ...
          tmppath = bobgui_tree_model_get_path (BOBGUI_TREE_MODEL (filter),
                                             &tmpiter);
  ...
  offset = bobgui_tree_path_get_indices (real_path)[bobgui_tree_path_get_depth (real_path) - 1];
  ...
  if (bobgui_tree_model_filter_visible (filter, &real_c_iter))
    {
      FilterElt *felt;

      felt = bobgui_tree_model_filter_insert_elt_in_level (filter,
                                                        &real_c_iter,
                                                        level, offset,
                                                        &i);
...
static void
bobgui_tree_model_filter_row_has_child_toggled (BobguiTreeModel *c_model,
                                             BobguiTreePath  *c_path,
                                             BobguiTreeIter  *c_iter,
                                             gpointer      data)
{
  ...
  if (filter->priv->virtual_root && !filter->priv->root
      && !bobgui_tree_path_compare (c_path, filter->priv->virtual_root))
    {
      bobgui_tree_model_filter_build_level (filter, NULL, NULL, TRUE);
      return;
    }
  ...
  bobgui_tree_model_filter_get_iter_full (BOBGUI_TREE_MODEL (data), &iter, path);
  ...
  if (!elt->visible_siter && !requested_state)
  ...
  else if (elt->visible_siter && !requested_state)
    {
       ...
       * when required.
       */
      bobgui_tree_model_filter_remove_elt_from_level (filter, level, elt);
  ...
  if (elt->ref_count > 1 && !elt->children &&
      bobgui_tree_model_iter_has_child (c_model, c_iter))
    bobgui_tree_model_filter_build_level (filter, level, elt, FALSE);

  /* get a path taking only visible nodes into account */
  path = bobgui_tree_model_get_path (BOBGUI_TREE_MODEL (data), &iter);
  bobgui_tree_model_row_has_child_toggled (BOBGUI_TREE_MODEL (data), path, &iter);
  bobgui_tree_path_free (path);
}

static void
bobgui_tree_model_filter_virtual_root_deleted (BobguiTreeModelFilter *filter,
                                            BobguiTreePath        *c_path)
{
  int i, nodes;
  BobguiTreePath *path;
   ...
   */
  bobgui_tree_model_filter_unref_path (filter, filter->priv->virtual_root,
                                    bobgui_tree_path_get_depth (c_path) - 1);
  filter->priv->virtual_root_deleted = TRUE;
   ...
   * no longer there.
   */
  bobgui_tree_model_filter_free_level (filter, filter->priv->root, FALSE, TRUE, FALSE);
...
static void
bobgui_tree_model_filter_adjust_virtual_root (BobguiTreeModelFilter *filter,
                                           BobguiTreePath        *c_path)
{
  int i;
  int level;
  ...
  level = bobgui_tree_path_get_depth (c_path) - 1;
  v_indices = bobgui_tree_path_get_indices (filter->priv->virtual_root);
  c_indices = bobgui_tree_path_get_indices (c_path);
...
static void
bobgui_tree_model_filter_row_deleted_invisible_node (BobguiTreeModelFilter *filter,
                                                  BobguiTreePath        *c_path)
{
  int offset;
  BobguiTreePath *real_path;
...
static void
bobgui_tree_model_filter_row_deleted (BobguiTreeModel *c_model,
                                   BobguiTreePath  *c_path,
                                   gpointer      data)
{
  BobguiTreeModelFilter *filter = BOBGUI_TREE_MODEL_FILTER (data);
  ...
  if (filter->priv->virtual_root &&
      (bobgui_tree_path_is_ancestor (c_path, filter->priv->virtual_root) ||
       !bobgui_tree_path_compare (c_path, filter->priv->virtual_root)))
    {
      bobgui_tree_model_filter_virtual_root_deleted (filter, c_path);
      return;
    }
  ...
  if (filter->priv->virtual_root &&
      bobgui_tree_path_get_depth (filter->priv->virtual_root) >=
      bobgui_tree_path_get_depth (c_path))
    bobgui_tree_model_filter_adjust_virtual_root (filter, c_path);
  ...
  if (!path)
    {
      bobgui_tree_model_filter_row_deleted_invisible_node (filter, c_path);
      return;
    }
   ...
   */
  while (elt->ext_ref_count > 0)
    bobgui_tree_model_filter_real_unref_node (BOBGUI_TREE_MODEL (data), &iter,
                                           TRUE, FALSE);
  ...
  if (g_sequence_get_length (level->seq) == 1)
    {
      /* kill level */
      bobgui_tree_model_filter_free_level (filter, level, FALSE, TRUE, FALSE);
    }
  else
    {
      GSequenceIter *tmp;
  ...
  if (emit_child_toggled && parent_level)
    {
      ...
      path2 = bobgui_tree_model_get_path (BOBGUI_TREE_MODEL (filter), &iter2);
      bobgui_tree_model_row_has_child_toggled (BOBGUI_TREE_MODEL (filter),
                                            path2, &iter2);
      bobgui_tree_path_free (path2);
      filter->priv->in_row_deleted = FALSE;
    }
  ...
  if (filter->priv->virtual_root)
    ...
    bobgui_tree_model_filter_check_ancestors (filter, c_path);

  bobgui_tree_path_free (path);
}

static void
bobgui_tree_model_filter_rows_reordered (BobguiTreeModel *c_model,
                                      BobguiTreePath  *c_path,
                                      BobguiTreeIter  *c_iter,
                                      int          *new_order,
                                      gpointer      data)
{
  ...
  if (c_path == NULL || bobgui_tree_path_get_depth (c_path) == 0)
    {
      length = bobgui_tree_model_iter_n_children (c_model, NULL);
      ...
      if (filter->priv->virtual_root)
        {
          ...
          for (i = 0; i < length; i++)
            if (new_order[i] == bobgui_tree_path_get_indices (filter->priv->virtual_root)[0])
              new_pos = i;

          if (new_pos < 0)
            return;

          bobgui_tree_path_get_indices (filter->priv->virtual_root)[0] = new_pos;
          return;
        }
	  ...
	  bobgui_tree_path_is_ancestor (c_path, filter->priv->virtual_root))
        {
          ...
          depth = bobgui_tree_path_get_depth (c_path);
          ...
          len = bobgui_tree_model_iter_n_children (c_model, &real_c_iter);

          for (i = 0; i < len; i++)
            if (new_order[i] == bobgui_tree_path_get_indices (filter->priv->virtual_root)[depth])
              new_pos = i;

          if (new_pos < 0)
            return;

          bobgui_tree_path_get_indices (filter->priv->virtual_root)[depth] = new_pos;
          return;
        }
      ...
      if (!path)
        {
          /* root level mode */
          if (!c_iter)
            bobgui_tree_model_get_iter (c_model, c_iter, c_path);
          length = bobgui_tree_model_iter_n_children (c_model, c_iter);
          path = bobgui_tree_path_new ();
          level = FILTER_LEVEL (filter->priv->root);
        }
      else
        {
          bobgui_tree_model_filter_get_iter_full (BOBGUI_TREE_MODEL (data),
                                               &iter, path);
          ...
          bobgui_tree_model_filter_convert_iter_to_child_iter (BOBGUI_TREE_MODEL_FILTER (filter), &child_iter, &iter);
          length = bobgui_tree_model_iter_n_children (c_model, &child_iter);
        }
    }
   ...
   * BobguiTreeModelSort does. This because we do some special tricky
   * reordering.
   */
  ...
  if (g_sequence_iter_get_position (old_first_siter) != 0)
    bobgui_tree_model_filter_level_transfer_first_ref (filter,
                                                    level,
                                                    old_first_siter,
  ...
  if (g_sequence_get_length (level->visible_seq) > 0)
    {
      if (!bobgui_tree_path_get_indices (path))
        bobgui_tree_model_rows_reordered (BOBGUI_TREE_MODEL (data), path, NULL,
                                       tmp_array);
      else
        {
          /* get a path taking only visible nodes into account */
          ...
          bobgui_tree_model_rows_reordered (BOBGUI_TREE_MODEL (data), path, &iter,
                                         tmp_array);
        }
    }
...
bobgui_tree_model_filter_get_flags (BobguiTreeModel *model)
{
  BobguiTreeModelFlags flags;

  g_return_val_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (model), 0);
  g_return_val_if_fail (BOBGUI_TREE_MODEL_FILTER (model)->priv->child_model != NULL, 0);

  flags = bobgui_tree_model_get_flags (BOBGUI_TREE_MODEL_FILTER (model)->priv->child_model);
...
static int
bobgui_tree_model_filter_get_n_columns (BobguiTreeModel *model)
{
  BobguiTreeModelFilter *filter = (BobguiTreeModelFilter *)model;
  ...
  return bobgui_tree_model_get_n_columns (filter->priv->child_model);
}

static GType
bobgui_tree_model_filter_get_column_type (BobguiTreeModel *model,
                                       int           index)
{
  BobguiTreeModelFilter *filter = (BobguiTreeModelFilter *)model;

  g_return_val_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (model), G_TYPE_INVALID);
  ...
  return bobgui_tree_model_get_column_type (filter->priv->child_model, index);
}
 ...
 */
static gboolean
bobgui_tree_model_filter_get_iter_full (BobguiTreeModel *model,
                                     BobguiTreeIter  *iter,
                                     BobguiTreePath  *path)
{
  BobguiTreeModelFilter *filter = (BobguiTreeModelFilter *)model;
...
static gboolean
bobgui_tree_model_filter_get_iter (BobguiTreeModel *model,
                                BobguiTreeIter  *iter,
                                BobguiTreePath  *path)
{
  BobguiTreeModelFilter *filter = (BobguiTreeModelFilter *)model;
...
static BobguiTreePath *
bobgui_tree_model_filter_get_path (BobguiTreeModel *model,
                                BobguiTreeIter  *iter)
{
  BobguiTreePath *retval;
  FilterLevel *level;
  ...
  g_return_val_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (model), NULL);
  g_return_val_if_fail (BOBGUI_TREE_MODEL_FILTER (model)->priv->child_model != NULL, NULL);
  g_return_val_if_fail (BOBGUI_TREE_MODEL_FILTER (model)->priv->stamp == iter->stamp, NULL);
...
static void
bobgui_tree_model_filter_real_modify (BobguiTreeModelFilter *self,
                                   BobguiTreeModel       *child_model,
                                   BobguiTreeIter        *iter,
                                   GValue             *value,
                                   int                 column)
  ...
  if (self->priv->modify_func)
    {
      ...
      self->priv->modify_func (BOBGUI_TREE_MODEL (self),
                               iter, value, column,
  ...
  else
    {
      ...
      bobgui_tree_model_filter_convert_iter_to_child_iter (BOBGUI_TREE_MODEL_FILTER (self),
                                                        &child_iter, iter);
      bobgui_tree_model_get_value (child_model, &child_iter, column, value);
    }
}

static void
bobgui_tree_model_filter_get_value (BobguiTreeModel *model,
                                 BobguiTreeIter  *iter,
                                 int           column,
                                 GValue       *value)
{
  BobguiTreeModelFilter *filter = BOBGUI_TREE_MODEL_FILTER (model);

  g_return_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (model));
  g_return_if_fail (BOBGUI_TREE_MODEL_FILTER (model)->priv->child_model != NULL);
  g_return_if_fail (BOBGUI_TREE_MODEL_FILTER (model)->priv->stamp == iter->stamp);

  BOBGUI_TREE_MODEL_FILTER_GET_CLASS (model)->modify (filter,
      filter->priv->child_model, iter, value, column);
}

static gboolean
bobgui_tree_model_filter_iter_next (BobguiTreeModel *model,
                                 BobguiTreeIter  *iter)
{
  FilterElt *elt;
  GSequenceIter *siter;
  ...
  g_return_val_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (model), FALSE);
  g_return_val_if_fail (BOBGUI_TREE_MODEL_FILTER (model)->priv->child_model != NULL, FALSE);
  g_return_val_if_fail (BOBGUI_TREE_MODEL_FILTER (model)->priv->stamp == iter->stamp, FALSE);
...
static gboolean
bobgui_tree_model_filter_iter_previous (BobguiTreeModel *model,
                                     BobguiTreeIter  *iter)
{
  FilterElt *elt;
  GSequenceIter *siter;
...
static gboolean
bobgui_tree_model_filter_iter_children (BobguiTreeModel *model,
                                     BobguiTreeIter  *iter,
                                     BobguiTreeIter  *parent)
{
  BobguiTreeModelFilter *filter = (BobguiTreeModelFilter *)model;
  ...
  if (!parent)
  ...
  else
    {
      if (FILTER_ELT (parent->user_data2)->children == NULL)
        bobgui_tree_model_filter_build_level (filter,
...
static gboolean
bobgui_tree_model_filter_iter_has_child (BobguiTreeModel *model,
                                      BobguiTreeIter  *iter)
{
  BobguiTreeIter child_iter;
  BobguiTreeModelFilter *filter = (BobguiTreeModelFilter *)model;
  ...
  filter = BOBGUI_TREE_MODEL_FILTER (model);
   ...
   */
  if (!elt->children
      && bobgui_tree_model_iter_has_child (filter->priv->child_model, &child_iter))
    bobgui_tree_model_filter_build_level (filter, FILTER_LEVEL (iter->user_data),
                                       elt, FALSE);
...
static int
bobgui_tree_model_filter_iter_n_children (BobguiTreeModel *model,
                                       BobguiTreeIter  *iter)
{
  BobguiTreeIter child_iter;
  BobguiTreeModelFilter *filter = (BobguiTreeModelFilter *)model;
...
static gboolean
bobgui_tree_model_filter_iter_nth_child (BobguiTreeModel *model,
                                      BobguiTreeIter  *iter,
                                      BobguiTreeIter  *parent,
                                      int           n)
{
  ...
  g_return_val_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (model), FALSE);
  if (parent)
    g_return_val_if_fail (BOBGUI_TREE_MODEL_FILTER (model)->priv->stamp == parent->stamp, FALSE);

  /* use this instead of has_child to force us to build the level, if needed */
  if (bobgui_tree_model_filter_iter_children (model, &children, parent) == FALSE)
    {
      iter->stamp = 0;
      return FALSE;
    }
  ...
  iter->stamp = BOBGUI_TREE_MODEL_FILTER (model)->priv->stamp;
  iter->user_data = level;
  iter->user_data2 = GET_ELT (siter);
...
static gboolean
bobgui_tree_model_filter_iter_parent (BobguiTreeModel *model,
                                   BobguiTreeIter  *iter,
                                   BobguiTreeIter  *child)
{
  FilterLevel *level;
  ...
  g_return_val_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (model), FALSE);
  g_return_val_if_fail (BOBGUI_TREE_MODEL_FILTER (model)->priv->child_model != NULL, FALSE);
  g_return_val_if_fail (BOBGUI_TREE_MODEL_FILTER (model)->priv->stamp == child->stamp, FALSE);
...
static void
bobgui_tree_model_filter_ref_node (BobguiTreeModel *model,
                                BobguiTreeIter  *iter)
{
  bobgui_tree_model_filter_real_ref_node (model, iter, TRUE);
}

static void
bobgui_tree_model_filter_real_ref_node (BobguiTreeModel *model,
                                     BobguiTreeIter  *iter,
                                     gboolean      external)
{
  BobguiTreeModelFilter *filter = (BobguiTreeModelFilter *)model;
  ...
  bobgui_tree_model_ref_node (filter->priv->child_model, &child_iter);
...
static void
bobgui_tree_model_filter_unref_node (BobguiTreeModel *model,
                                  BobguiTreeIter  *iter)
{
  bobgui_tree_model_filter_real_unref_node (model, iter, TRUE, TRUE);
}

static void
bobgui_tree_model_filter_real_unref_node (BobguiTreeModel *model,
                                       BobguiTreeIter  *iter,
                                       gboolean      external,
                                       gboolean      propagate_unref)
{
  ...
  if (propagate_unref)
    {
      BobguiTreeIter child_iter;
      bobgui_tree_model_filter_convert_iter_to_child_iter (BOBGUI_TREE_MODEL_FILTER (model), &child_iter, iter);
      bobgui_tree_model_unref_node (filter->priv->child_model, &child_iter);
    }
...
static gboolean
bobgui_tree_model_filter_row_draggable (BobguiTreeDragSource *drag_source,
                                     BobguiTreePath       *path)
{
  BobguiTreeModelFilter *tree_model_filter = (BobguiTreeModelFilter *)drag_source;
  BobguiTreePath *child_path;
  gboolean draggable;

  g_return_val_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (drag_source), FALSE);
  g_return_val_if_fail (path != NULL, FALSE);

  child_path = bobgui_tree_model_filter_convert_path_to_child_path (tree_model_filter, path);
  draggable = bobgui_tree_drag_source_row_draggable (BOBGUI_TREE_DRAG_SOURCE (tree_model_filter->priv->child_model), child_path);
                                     ...
                                     BobguiTreePath       *path)
{
  ...
  g_return_val_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (drag_source), NULL);
  g_return_val_if_fail (path != NULL, NULL);

  child_path = bobgui_tree_model_filter_convert_path_to_child_path (tree_model_filter, path);
  gotten = bobgui_tree_drag_source_drag_data_get (BOBGUI_TREE_DRAG_SOURCE (tree_model_filter->priv->child_model), child_path);
  bobgui_tree_path_free (child_path);
...
static gboolean
bobgui_tree_model_filter_drag_data_delete (BobguiTreeDragSource *drag_source,
                                        BobguiTreePath       *path)
{
  BobguiTreeModelFilter *tree_model_filter = (BobguiTreeModelFilter *)drag_source;
  BobguiTreePath *child_path;
  ...
  child_path = bobgui_tree_model_filter_convert_path_to_child_path (tree_model_filter, path);
  deleted = bobgui_tree_drag_source_drag_data_delete (BOBGUI_TREE_DRAG_SOURCE (tree_model_filter->priv->child_model), child_path);
  bobgui_tree_path_free (child_path);
...
/* bits and pieces */
static void
bobgui_tree_model_filter_set_model (BobguiTreeModelFilter *filter,
                                 BobguiTreeModel       *child_model)
{
  g_return_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (filter));
  ...
  if (filter->priv->child_model)
    {
      ...
      /* reset our state */
      if (filter->priv->root)
        bobgui_tree_model_filter_free_level (filter, filter->priv->root,
                                          TRUE, TRUE, FALSE);
  ...
  if (child_model)
    {
      ...
      filter->priv->changed_id =
        g_signal_connect (child_model, "row-changed",
                          G_CALLBACK (bobgui_tree_model_filter_row_changed),
                          filter);
      filter->priv->inserted_id =
        g_signal_connect (child_model, "row-inserted",
                          G_CALLBACK (bobgui_tree_model_filter_row_inserted),
                          filter);
      filter->priv->has_child_toggled_id =
        g_signal_connect (child_model, "row-has-child-toggled",
                          G_CALLBACK (bobgui_tree_model_filter_row_has_child_toggled),
                          filter);
      filter->priv->deleted_id =
        g_signal_connect (child_model, "row-deleted",
                          G_CALLBACK (bobgui_tree_model_filter_row_deleted),
                          filter);
      filter->priv->reordered_id =
        g_signal_connect (child_model, "rows-reordered",
                          G_CALLBACK (bobgui_tree_model_filter_rows_reordered),
                          filter);

      filter->priv->child_flags = bobgui_tree_model_get_flags (child_model);
      filter->priv->stamp = g_random_int ();
    }
}

static void
bobgui_tree_model_filter_ref_path (BobguiTreeModelFilter *filter,
                                BobguiTreePath        *path)
{
  int len;
  BobguiTreePath *p;

  len = bobgui_tree_path_get_depth (path);
  p = bobgui_tree_path_copy (path);
  while (len--)
    {
      BobguiTreeIter iter;

      bobgui_tree_model_get_iter (BOBGUI_TREE_MODEL (filter->priv->child_model), &iter, p);
      bobgui_tree_model_ref_node (BOBGUI_TREE_MODEL (filter->priv->child_model), &iter);
      bobgui_tree_path_up (p);
    }
...
static void
bobgui_tree_model_filter_unref_path (BobguiTreeModelFilter *filter,
                                  BobguiTreePath        *path,
                                  int                 depth)
{
  int len;
  ...
  while (len--)
    {
      ...
      bobgui_tree_model_get_iter (BOBGUI_TREE_MODEL (filter->priv->child_model), &iter, p);
      bobgui_tree_model_unref_node (BOBGUI_TREE_MODEL (filter->priv->child_model), &iter);
      bobgui_tree_path_up (p);
    }
...
static void
bobgui_tree_model_filter_set_root (BobguiTreeModelFilter *filter,
                                BobguiTreePath        *root)
{
  g_return_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (filter));
  ...
  if (root)
    {
      filter->priv->virtual_root = bobgui_tree_path_copy (root);
      bobgui_tree_model_filter_ref_path (filter, filter->priv->virtual_root);
      filter->priv->virtual_root_deleted = FALSE;
    }
  else
...
/**
 * bobgui_tree_model_filter_new:
 * @child_model: A `BobguiTreeModel`.
 * @root: (nullable): A `BobguiTreePath`
 *
 * Creates a new `BobguiTreeModel`, with @child_model as the child_model
 * and @root as the virtual root.
 *
 * Returns: (transfer full): A new `BobguiTreeModel`.
 *
 * Deprecated: 4.10
 */
BobguiTreeModel *
bobgui_tree_model_filter_new (BobguiTreeModel *child_model,
                           BobguiTreePath  *root)
{
  g_return_val_if_fail (BOBGUI_IS_TREE_MODEL (child_model), NULL);

  return g_object_new (BOBGUI_TYPE_TREE_MODEL_FILTER,
...
/**
 * bobgui_tree_model_filter_get_model:
 * @filter: A `BobguiTreeModelFilter`
 *
 * Returns a pointer to the child model of @filter.
 *
 * Returns: (transfer none): A pointer to a `BobguiTreeModel`
 *
 * Deprecated: 4.10
 */
BobguiTreeModel *
bobgui_tree_model_filter_get_model (BobguiTreeModelFilter *filter)
{
  g_return_val_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (filter), NULL);

  return filter->priv->child_model;
}

/**
 * bobgui_tree_model_filter_set_visible_func:
 * @filter: A `BobguiTreeModelFilter`
 * @func: A `BobguiTreeModelFilterVisibleFunc`, the visible function
 ...
 * bobgui_tree_model_filter_refilter() to keep the visibility information
 ...
 * ```
 *
 * Note that bobgui_tree_model_filter_set_visible_func() or
 * bobgui_tree_model_filter_set_visible_column() can only be called
 * once for a given filter model.
 *
 * Deprecated: 4.10
 */
void
bobgui_tree_model_filter_set_visible_func (BobguiTreeModelFilter            *filter,
...
/**
 * bobgui_tree_model_filter_set_modify_func:
 * @filter: A `BobguiTreeModelFilter`
 * @n_columns: The number of columns in the filter model.
 * @types: (array length=n_columns): The `GType`s of the columns.
 * @func: A `BobguiTreeModelFilterModifyFunc`
 ...
 * modify function.
 *
 * Note that bobgui_tree_model_filter_set_modify_func()
 * can only be called once for a given filter model.
 *
 * Deprecated: 4.10
 */
void
bobgui_tree_model_filter_set_modify_func (BobguiTreeModelFilter           *filter,
                                       int                           n_columns,
                                       GType                        *types,
...
/**
 * bobgui_tree_model_filter_set_visible_column:
 * @filter: A `BobguiTreeModelFilter`
 ...
 */
void
bobgui_tree_model_filter_set_visible_column (BobguiTreeModelFilter *filter,
                                          int column)
{
  g_return_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (filter));
...
/**
 * bobgui_tree_model_filter_convert_child_iter_to_iter:
 * @filter: A `BobguiTreeModelFilter`
 * @filter_iter: (out): An uninitialized `BobguiTreeIter`
 ...
 */
gboolean
bobgui_tree_model_filter_convert_child_iter_to_iter (BobguiTreeModelFilter *filter,
                                                  ...
                                                  BobguiTreeIter        *child_iter)
{
  ...
  g_return_val_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (filter), FALSE);
  g_return_val_if_fail (filter->priv->child_model != NULL, FALSE);
  ...
  child_path = bobgui_tree_model_get_path (filter->priv->child_model, child_iter);
  g_return_val_if_fail (child_path != NULL, FALSE);

  path = bobgui_tree_model_filter_convert_child_path_to_path (filter,
                                                           child_path);
  bobgui_tree_path_free (child_path);
  ...
  ret = bobgui_tree_model_get_iter (BOBGUI_TREE_MODEL (filter), filter_iter, path);
  bobgui_tree_path_free (path);
...
/**
 * bobgui_tree_model_filter_convert_iter_to_child_iter:
 * @filter: A `BobguiTreeModelFilter`
 * @child_iter: (out): An uninitialized `BobguiTreeIter`
 * @filter_iter: A valid `BobguiTreeIter` pointing to a row on @filter.
 *
 ...
 */
void
bobgui_tree_model_filter_convert_iter_to_child_iter (BobguiTreeModelFilter *filter,
                                                  BobguiTreeIter        *child_iter,
                                                  BobguiTreeIter        *filter_iter)
{
  g_return_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (filter));
  ...
  if (BOBGUI_TREE_MODEL_FILTER_CACHE_CHILD_ITERS (filter))
  ...
  else
    {
      ...
      path = bobgui_tree_model_filter_elt_get_path (filter_iter->user_data,
                                                 filter_iter->user_data2,
                                                 filter->priv->virtual_root);
      valid = bobgui_tree_model_get_iter (filter->priv->child_model, child_iter,
                                       path);
      bobgui_tree_path_free (path);
...
static BobguiTreePath *
bobgui_real_tree_model_filter_convert_child_path_to_path (BobguiTreeModelFilter *filter,
                                                       BobguiTreePath        *child_path,
                                                       gboolean            build_levels,
                                                       gboolean            fetch_children)
{
  ...
  if (!filter->priv->virtual_root)
    real_path = bobgui_tree_path_copy (child_path);
  else
    real_path = bobgui_tree_model_filter_remove_root (child_path,
                                                   filter->priv->virtual_root);
  ...
  retval = bobgui_tree_path_new ();
  child_indices = bobgui_tree_path_get_indices (real_path);

  if (filter->priv->root == NULL && build_levels)
    bobgui_tree_model_filter_build_level (filter, NULL, NULL, FALSE);
  level = FILTER_LEVEL (filter->priv->root);

  for (i = 0; i < bobgui_tree_path_get_depth (real_path); i++)
    {
      GSequenceIter *siter;
      ...
      if (tmp)
        {
          ...
          if (!tmp->children && build_levels)
            bobgui_tree_model_filter_build_level (filter, level, tmp, FALSE);
          level = tmp->children;
          found_child = TRUE;
        }
      ...
      if (!found_child && fetch_children)
        {
          ...
          tmp = bobgui_tree_model_filter_fetch_child (filter, level,
                                                   child_indices[i],
                                                   &j);
          ...
          bobgui_tree_path_append_index (retval, j);
          if (!tmp->children && build_levels)
            bobgui_tree_model_filter_build_level (filter, level, tmp, FALSE);
          level = tmp->children;
          found_child = TRUE;
...
/**
 * bobgui_tree_model_filter_convert_child_path_to_path:
 * @filter: A `BobguiTreeModelFilter`
 ...
 */
BobguiTreePath *
bobgui_tree_model_filter_convert_child_path_to_path (BobguiTreeModelFilter *filter,
                                                  BobguiTreePath        *child_path)
{
  BobguiTreeIter iter;
  BobguiTreePath *path;
...
/**
 * bobgui_tree_model_filter_convert_path_to_child_path:
 * @filter: A `BobguiTreeModelFilter`
 * @filter_path: A `BobguiTreePath` to convert.
 *
 ...
 */
BobguiTreePath *
bobgui_tree_model_filter_convert_path_to_child_path (BobguiTreeModelFilter *filter,
                                                  BobguiTreePath        *filter_path)
{
  ...
  /* convert path */
  retval = bobgui_tree_path_new ();
  filter_indices = bobgui_tree_path_get_indices (filter_path);
  if (!filter->priv->root)
    bobgui_tree_model_filter_build_level (filter, NULL, NULL, FALSE);
  level = FILTER_LEVEL (filter->priv->root);

  for (i = 0; i < bobgui_tree_path_get_depth (filter_path); i++)
    {
      FilterElt *elt;
      GSequenceIter *siter;
  ...
  if (filter->priv->virtual_root)
    {
      BobguiTreePath *real_retval;

      real_retval = bobgui_tree_model_filter_add_root (retval,
                                                    filter->priv->virtual_root);
      bobgui_tree_path_free (retval);
...
static gboolean
bobgui_tree_model_filter_refilter_helper (BobguiTreeModel *model,
                                       BobguiTreePath  *path,
                                       BobguiTreeIter  *iter,
                                       gpointer      data)
{
  /* evil, don't try this at home, but certainly speeds things up */
  bobgui_tree_model_filter_row_changed (model, path, iter, data);

  return FALSE;
}

/**
 * bobgui_tree_model_filter_refilter:
 * @filter: A `BobguiTreeModelFilter`
 *
 ...
 */
void
bobgui_tree_model_filter_refilter (BobguiTreeModelFilter *filter)
{
  g_return_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (filter));

  /* S L O W */
  bobgui_tree_model_foreach (filter->priv->child_model,
                          bobgui_tree_model_filter_refilter_helper,
                          filter);
}

/**
 * bobgui_tree_model_filter_clear_cache:
 * @filter: A `BobguiTreeModelFilter`
 *
 * This function should almost never be called. It clears the @filter
 * of any cached iterators that haven’t been reffed with
 * bobgui_tree_model_ref_node(). This might be useful if the child model
 ...
 */
void
bobgui_tree_model_filter_clear_cache (BobguiTreeModelFilter *filter)
{
  g_return_if_fail (BOBGUI_IS_TREE_MODEL_FILTER (filter));

  if (filter->priv->zero_ref_count > 0)
    bobgui_tree_model_filter_clear_cache_helper (filter,
