 ...
 * 
 * This file contains code shared between BobguiTreeStore and BobguiListStore.  Please
 * do not use it.
 */
 ...
 */
BobguiTreeDataList *
_bobgui_tree_data_list_alloc (void)
{
  BobguiTreeDataList *list;

  list = g_slice_new0 (BobguiTreeDataList);

  return list;
}

void
_bobgui_tree_data_list_free (BobguiTreeDataList *list,
			  GType           *column_headers)
{
  BobguiTreeDataList *tmp, *next;
  int i = 0;
  ...
  while (tmp)
	...
	g_variant_unref ((gpointer) tmp->data.v_pointer);
      ...
      g_slice_free (BobguiTreeDataList, tmp);
      i++;
      tmp = next;
    }
}
...
gboolean
_bobgui_tree_data_list_check_type (GType type)
{
  int i = 0;
...
void
_bobgui_tree_data_list_node_to_value (BobguiTreeDataList *list,
				   GType            type,
				   GValue          *value)
{
...
void
_bobgui_tree_data_list_value_to_node (BobguiTreeDataList *list,
				   GValue          *value)
{
...
BobguiTreeDataList *
_bobgui_tree_data_list_node_copy (BobguiTreeDataList *list,
                               GType            type)
{
  BobguiTreeDataList *new_list;

  g_return_val_if_fail (list != NULL, NULL);
  
  new_list = _bobgui_tree_data_list_alloc ();
  new_list->next = NULL;
...
int
_bobgui_tree_data_list_compare_func (BobguiTreeModel *model,
				  BobguiTreeIter  *a,
				  BobguiTreeIter  *b,
				  gpointer      user_data)
{
  int column = GPOINTER_TO_INT (user_data);
  GType type = bobgui_tree_model_get_column_type (model, column);
  GValue a_value = G_VALUE_INIT;
  GValue b_value = G_VALUE_INIT;
  int retval;
  const char *stra, *strb;

  bobgui_tree_model_get_value (model, a, column, &a_value);
  bobgui_tree_model_get_value (model, b, column, &b_value);
...
GList *
_bobgui_tree_data_list_header_new (int    n_columns,
				GType *types)
{
  GList *retval = NULL;
  ...
  for (i = 0; i < n_columns; i ++)
    {
      ...
      header = g_slice_new (BobguiTreeDataSortHeader);

      retval = g_list_prepend (retval, header);
      header->sort_column_id = i;
      header->func = _bobgui_tree_data_list_compare_func;
      header->destroy = NULL;
...
void
_bobgui_tree_data_list_header_free (GList *list)
{
  GList *tmp;

  for (tmp = list; tmp; tmp = tmp->next)
    {
      BobguiTreeDataSortHeader *header = (BobguiTreeDataSortHeader *) tmp->data;
      ...
      if (header->destroy)
	{
      ...
      g_slice_free (BobguiTreeDataSortHeader, header);
    }
  g_list_free (list);
}

BobguiTreeDataSortHeader *
_bobgui_tree_data_list_get_header (GList   *header_list,
				int      sort_column_id)
{
  ...
  for (; header_list; header_list = header_list->next)
    {
      header = (BobguiTreeDataSortHeader*) header_list->data;
...
GList *
_bobgui_tree_data_list_set_header (GList                  *header_list,
				int                     sort_column_id,
				...
				GDestroyNotify          destroy)
{
  ...
  for (; list; list = list->next)
    {
      header = (BobguiTreeDataSortHeader*) list->data;
      if (header->sort_column_id == sort_column_id)
	break;
      header = NULL;
  ...
  if (header == NULL)
    {
      header = g_slice_new0 (BobguiTreeDataSortHeader);
