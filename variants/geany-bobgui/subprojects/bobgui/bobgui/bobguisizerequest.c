...
static void
push_recursion_check (BobguiWidget       *widget,
                      BobguiOrientation   orientation)
{
  gboolean in_measure = FALSE;

  if (recursion_check_quark == 0)
    recursion_check_quark = g_quark_from_static_string ("bobgui-size-request-in-progress");

  in_measure = GPOINTER_TO_INT (g_object_get_qdata (G_OBJECT (widget), recursion_check_quark));

  if (in_measure)
    {
      g_warning ("%s %p: widget tried to bobgui_widget_measure inside "
                 " BobguiWidget::measure implementation. "
                 "Should just invoke BOBGUI_WIDGET_GET_CLASS(widget)->measure "
                 "directly rather than using bobgui_widget_measure",
...
static void
pop_recursion_check (BobguiWidget       *widget,
                     BobguiOrientation   orientation)
{
...
fetch_request_mode (BobguiWidget *widget)
{
  BobguiLayoutManager *layout_manager = bobgui_widget_get_layout_manager (widget);

  if (layout_manager != NULL)
    return bobgui_layout_manager_get_request_mode (layout_manager);
  else
    return BOBGUI_WIDGET_GET_CLASS (widget)->get_request_mode (widget);
}
...
static int
get_number_ceil (BobguiCssValue *value)
{
  return ceil (bobgui_css_number_value_get (value, 100));
}
...
static void
bobgui_widget_query_size_for_orientation (BobguiWidget        *widget,
                                       BobguiOrientation    orientation,
                                       int               for_size,
                                       int              *minimum,
                                       int              *natural,
                                       ...
                                       int              *natural_baseline)
{
  ...
  if (cache->request_mode == BOBGUI_SIZE_REQUEST_CONSTANT_SIZE)
    for_size = -1;

  found_in_cache = _bobgui_size_request_cache_lookup (cache,
                                                   orientation,
                                                   for_size,
                                                   &min_size,
                                                   &nat_size,
  ...
  if (!found_in_cache)
    {
      ...
      widget_class = BOBGUI_WIDGET_GET_CLASS (widget);
          ...
          if (for_size < 0)
            {
              ...
              bobgui_layout_manager_measure (layout_manager, widget,
                                          orientation, -1,
      ...
      else
        {
          if (for_size < 0)
          ...
          else
            {
              ...
              /* Pull the minimum for_size from the cache as it's needed to adjust
               * the proposed 'for_size' */
              bobgui_widget_measure (widget, OPPOSITE_ORIENTATION (orientation), -1,
          ...
          if (orientation == BOBGUI_ORIENTATION_HORIZONTAL)
            {
              g_warning ("%s %p (%s) reported min width %d and natural width %d in measure() with for_size=%d; natural size must be >= min size",
                         G_OBJECT_TYPE_NAME (widget), widget,
                         g_quark_to_string (bobgui_css_node_get_name (bobgui_widget_get_css_node (widget))),
      ...
      bobgui_widget_adjust_size_request (widget, orientation,
      ...
      if (adjusted_min < min_size ||
          adjusted_natural < nat_size)
        {
          g_warning ("%s %p adjusted size %s min %d natural %d must not decrease below min %d natural %d",
                     G_OBJECT_TYPE_NAME (widget), widget,
                     orientation == BOBGUI_ORIENTATION_VERTICAL ? "vertical" : "horizontal",
                     adjusted_min, adjusted_natural,
                     min_size, nat_size);
	  ...
	  else if (min_baseline == -1 || nat_baseline == -1)
	    {
	      ...
	      nat_baseline = -1;
	  ...
	  else
            {
              ...
              bobgui_widget_adjust_baseline_request (widget, &min_baseline, &nat_baseline);
            }
        }

      _bobgui_size_request_cache_commit (cache,
                                      orientation,
                                      for_size,
                                      min_size,
                                      nat_size,
  ...
  if (BOBGUI_DISPLAY_DEBUG_CHECK (_bobgui_widget_get_display (widget), SIZE_REQUEST))
    {
      GString *s;
...
/**
 * bobgui_widget_measure:
 * @widget: A `BobguiWidget` instance
 * @orientation: the orientation to measure
 * @for_size: Size for the opposite of @orientation, i.e.
 *   if @orientation is %BOBGUI_ORIENTATION_HORIZONTAL, this is
 *   the height the widget should be measured with. The %BOBGUI_ORIENTATION_VERTICAL
 ...
 *
 * As an example, if @orientation is %BOBGUI_ORIENTATION_HORIZONTAL and @for_size
 * is 300, this functions will compute the minimum and natural width of @widget
 * if it is allocated at a height of 300 pixels.
 *
 * See [BobguiWidget’s geometry management section](class.Widget.html#height-for-width-geometry-management) for
 * a more details on implementing `BobguiWidgetClass.measure()`.
 */
void
bobgui_widget_measure (BobguiWidget        *widget,
                    BobguiOrientation    orientation,
                    int               for_size,
                    int              *minimum,
                    int              *natural,
                    ...
                    int              *natural_baseline)
{
  ...
  if (for_size >= 0)
    {
      ...
      bobgui_widget_measure (widget, OPPOSITE_ORIENTATION (orientation), -1, &min_opposite_size, NULL, NULL, NULL);
   ...
   * not cached.
   */
  if (!_bobgui_widget_get_visible (widget) && !BOBGUI_IS_ROOT (widget))
    {
      if (minimum)
        *minimum = 0;
      if (natural)
  ...
  if (G_LIKELY (!_bobgui_widget_get_sizegroups (widget)))
    {
      bobgui_widget_query_size_for_orientation (widget, orientation, for_size, minimum, natural,
  ...
  else
    {
      ...
      _bobgui_size_group_get_widget_peers (widget, orientation,
                                        &peers, &peers_for_both);

      g_hash_table_iter_init (&iter, peers);
      while (g_hash_table_iter_next (&iter, &key, NULL))
        {
          BobguiWidget *peer_widget = key;
          ...
          bobgui_widget_query_size_for_orientation (peer_widget, orientation, peer_for_size,
...
/**
 * bobgui_widget_get_request_mode:
 * @widget: a `BobguiWidget` instance
 *
 ...
 *
 * Returns: The `BobguiSizeRequestMode` preferred by @widget.
 */
BobguiSizeRequestMode
bobgui_widget_get_request_mode (BobguiWidget *widget)
{
  SizeRequestCache *cache;
  ...
  /* Constant-size widgets in a size group with BOBGUI_SIZE_GROUP_BOTH mode
   ...
   */
  if (cache->request_mode == BOBGUI_SIZE_REQUEST_CONSTANT_SIZE &&
      G_UNLIKELY (_bobgui_widget_get_sizegroups (widget)))
    {
      GHashTable *peers, *peers_for_both;
      GHashTableIter iter;
      gpointer key;
      ...
      _bobgui_size_group_get_widget_peers (widget, BOBGUI_ORIENTATION_VERTICAL,
                                        &peers, &peers_for_both);
      g_hash_table_destroy (peers);
      ...
      while (g_hash_table_iter_next (&iter, &key, NULL))
        {
          ...
          bobgui_widget_clear_resize_queued (peer_widget);
          peer_cache = _bobgui_widget_peek_request_cache (peer_widget);
      ...
      if (wfh > hfw)
        return BOBGUI_SIZE_REQUEST_WIDTH_FOR_HEIGHT;
...
/**
 * bobgui_widget_get_preferred_size:
 * @widget: a `BobguiWidget` instance
 ...
 * free-form containers such as `BobguiFixed`.
 *
 ...
 *
 * Use [method@Bobgui.Widget.measure] if you want to support baseline alignment.
 */
void
bobgui_widget_get_preferred_size (BobguiWidget      *widget,
                               ...
                               BobguiRequisition *natural_size)
{
  ...
  if (bobgui_widget_get_request_mode (widget) == BOBGUI_SIZE_REQUEST_HEIGHT_FOR_WIDTH)
    {
      ...
      if (minimum_size)
        {
          ...
          bobgui_widget_measure (widget,
                              BOBGUI_ORIENTATION_VERTICAL, min_width,
      ...
      if (natural_size)
        {
          ...
          bobgui_widget_measure (widget,
                              BOBGUI_ORIENTATION_VERTICAL, nat_width,
                              NULL, &natural_size->height, NULL, NULL);
        }
    }
  else /* BOBGUI_SIZE_REQUEST_WIDTH_FOR_HEIGHT or CONSTANT_SIZE */
    {
      ...
      if (minimum_size)
        {
          ...
          bobgui_widget_measure (widget, BOBGUI_ORIENTATION_HORIZONTAL, min_height,
      ...
      if (natural_size)
        {
          ...
          bobgui_widget_measure (widget, BOBGUI_ORIENTATION_HORIZONTAL, nat_height,
...
compare_gap (gconstpointer p1,
	     ...
	     gpointer      data)
{
  BobguiRequestedSize *sizes = data;
  const guint *c1 = p1;
  const guint *c2 = p2;
...
/**
 * bobgui_distribute_natural_allocation:
 ...
 * `BobguiRequestedSize` struct. If all sizes reach their natural size then
 ...
 */
int
bobgui_distribute_natural_allocation (int               extra_space,
