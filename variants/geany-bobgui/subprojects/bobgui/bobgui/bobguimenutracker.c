 ...
 * BobguiMenuTracker:
 *
 * BobguiMenuTracker is a simple object to ease implementations of GMenuModel.
 * Given a BobguiActionObservable (usually a GActionMuxer) along with a
 ...
 * item to remove.
 *
 * Using BobguiMenuTracker is fairly simple. The only guarantee you must make
 * to BobguiMenuTracker is that you must obey all insert signals and track the
 * position of items that BobguiMenuTracker gives you. That is, BobguiMenuTracker
 ...
 *
 * BobguiMenuTracker will give you a BobguiMenuTrackerItem in your callback. You
 ...
 *
 * Certain properties on the BobguiMenuTrackerItem are mutable, and you must
 * listen for changes in the item. For more details, see the documentation
 * for BobguiMenuTrackerItem along with GMenuModel's reference.
 *
 ...
 * "File", “Edit” menu bars. Ignoring separator items is wrong, as BobguiMenuTracker
 * expects the position to change, so we must tell BobguiMenuTracker to ignore
 * separators itself.
 */

typedef struct _BobguiMenuTrackerSection BobguiMenuTrackerSection;

struct _BobguiMenuTracker
{
  ...
  BobguiMenuTrackerSection    *toplevel;
};

struct _BobguiMenuTrackerSection
{
  gpointer    model;   /* may be a BobguiMenuTrackerItem or a GMenuModel */
  GSList     *items;
...
static BobguiMenuTrackerSection *  bobgui_menu_tracker_section_new    (BobguiMenuTracker        *tracker,
                                                                 GMenuModel            *model,
                                                                 gboolean               with_separators,
                                                                 gboolean               separator_label,
                                                                 int                    offset,
                                                                 const char            *action_namespace);
static void                    bobgui_menu_tracker_section_free    (BobguiMenuTrackerSection *section);

static BobguiMenuTrackerSection *
bobgui_menu_tracker_section_find_model (BobguiMenuTrackerSection *section,
                                     gpointer               model,
                                     int                   *offset)
{
  GSList *item;
  ...
  for (item = section->items; item; item = item->next)
    {
      BobguiMenuTrackerSection *subsection = item->data;

      if (subsection)
        {
          BobguiMenuTrackerSection *found_section;

          found_section = bobgui_menu_tracker_section_find_model (subsection, model, offset);
 ...
 */
static int
bobgui_menu_tracker_section_sync_separators (BobguiMenuTrackerSection *section,
                                          ...
                                          int                    parent_index)
{
      ...
      if (subsection)
        {
          ...
          n_items += bobgui_menu_tracker_section_sync_separators (subsection, tracker, offset + n_items,
                                                               separator,
  ...
  if (should_have_separator > section->has_separator)
    {
      /* Add a separator */
      BobguiMenuTrackerItem *separator;

      separator = _bobgui_menu_tracker_item_new (tracker->observable, parent_model, parent_index, FALSE, NULL, TRUE);
...
static void
bobgui_menu_tracker_item_visibility_changed (BobguiMenuTrackerItem *item,
                                          GParamSpec         *pspec,
                                          gpointer            user_data)
{
  BobguiMenuTracker *tracker = user_data;
  BobguiMenuTrackerSection *section;
  gboolean is_now_visible;
  gboolean was_visible;
  int offset = 0;

  is_now_visible = bobgui_menu_tracker_item_get_is_visible (item);

  /* remember: the item is our model */
  section = bobgui_menu_tracker_section_find_model (tracker->toplevel, item, &offset);
  g_assert (section);
  ...
  bobgui_menu_tracker_section_sync_separators (tracker->toplevel, tracker, 0, FALSE, NULL, 0);
}

static int
bobgui_menu_tracker_section_measure (BobguiMenuTrackerSection *section)
{
  GSList *item;
  int n_items;
  ...
  for (item = section->items; item; item = item->next)
    n_items += bobgui_menu_tracker_section_measure (item->data);

  return n_items;
}

static void
bobgui_menu_tracker_remove_items (BobguiMenuTracker  *tracker,
                               GSList         **change_point,
                               int              offset,
                               int              n_items)
{
  ...
  for (i = 0; i < n_items; i++)
    {
      BobguiMenuTrackerSection *subsection;
      int n;

      subsection = (*change_point)->data;
      *change_point = g_slist_delete_link (*change_point, *change_point);

      n = bobgui_menu_tracker_section_measure (subsection);
      bobgui_menu_tracker_section_free (subsection);
...
static void
bobgui_menu_tracker_add_items (BobguiMenuTracker         *tracker,
                            BobguiMenuTrackerSection  *section,
                            GSList                **change_point,
                            int                     offset,
                            GMenuModel             *model,
  ...
  while (n_items--)
    {
      ...
      if (submenu != NULL && tracker->merge_sections)
          ...
          if (section->action_namespace)
            {
              ...
              subsection = bobgui_menu_tracker_section_new (tracker, submenu, FALSE, has_label, offset, namespace);
              g_free (namespace);
            }
          else
            subsection = bobgui_menu_tracker_section_new (tracker, submenu, FALSE, has_label, offset, action_namespace);
      ...
      else
        {
          ...
          item = _bobgui_menu_tracker_item_new (tracker->observable, model, position + n_items,
                                             tracker->mac_os_mode,
           ...
           */
          if (bobgui_menu_tracker_item_may_disappear (item))
            {
              BobguiMenuTrackerSection *fake_section;

              fake_section = g_new0 (BobguiMenuTrackerSection, 1);
              ...
              fake_section->handler = g_signal_connect (item, "notify::is-visible",
                                                        G_CALLBACK (bobgui_menu_tracker_item_visibility_changed),
                                                        tracker);
              *change_point = g_slist_prepend (*change_point, fake_section);

              if (bobgui_menu_tracker_item_get_is_visible (item))
                {
...
static void
bobgui_menu_tracker_model_changed (GMenuModel *model,
                                int         position,
                                int         removed,
                                int         added,
                                gpointer    user_data)
{
   ...
   */
  section = bobgui_menu_tracker_section_find_model (tracker->toplevel, model, &offset);
  g_assert (section);
  ...
  for (i = 0; i < position; i++)
    {
      offset += bobgui_menu_tracker_section_measure ((*change_point)->data);
   ...
   *
   * This also has a performance advantage: BobguiMenuShell stores the
   ...
   */
  bobgui_menu_tracker_remove_items (tracker, change_point, offset, removed);
  bobgui_menu_tracker_add_items (tracker, section, change_point, offset, model, position, added);
...
static void
bobgui_menu_tracker_section_free (BobguiMenuTrackerSection *section)
{
  if (section == NULL)
    return;

  g_signal_handler_disconnect (section->model, section->handler);
  g_slist_free_full (section->items, (GDestroyNotify) bobgui_menu_tracker_section_free);
...
static BobguiMenuTrackerSection *
bobgui_menu_tracker_section_new (BobguiMenuTracker *tracker,
                              GMenuModel     *model,
                              gboolean        with_separators,
                              gboolean        separator_label,
                              int             offset,
                              const char     *action_namespace)
{
  ...
  section = g_new0 (BobguiMenuTrackerSection, 1);
  ...
  bobgui_menu_tracker_add_items (tracker, section, &section->items, offset, model, 0, g_menu_model_get_n_items (model));
  section->handler = g_signal_connect (model, "items-changed", G_CALLBACK (bobgui_menu_tracker_model_changed), tracker);

  return section;
}

/*< private >
 * bobgui_menu_tracker_new:
 ...
 *
 * Creates a BobguiMenuTracker for @model, holding a ref on @model for as
 ...
 * (unless it is empty), before bobgui_menu_tracker_new() returns.  For
 ...
 * should be ignored by the consumer because BobguiMenuTracker has already
 * handled it.
 *
 * When using BobguiMenuTracker there is no need to hold onto @model or
 * monitor it for changes.  The model will be unreffed when
 * bobgui_menu_tracker_free() is called.
 */
BobguiMenuTracker *
bobgui_menu_tracker_new (BobguiActionObservable      *observable,
                      GMenuModel               *model,
                      gboolean                  with_separators,
                      ...
                      gpointer                  user_data)
{
  ...
  tracker = g_new (BobguiMenuTracker, 1);
  ...
  tracker->toplevel = bobgui_menu_tracker_section_new (tracker, model, with_separators, FALSE, 0, action_namespace);
  bobgui_menu_tracker_section_sync_separators (tracker->toplevel, tracker, 0, FALSE, NULL, 0);
...
BobguiMenuTracker *
bobgui_menu_tracker_new_for_item_link (BobguiMenuTrackerItem       *item,
                                    ...
                                    gpointer                  user_data)
{
  ...
  submenu = _bobgui_menu_tracker_item_get_link (item, link_name);
  namespace = _bobgui_menu_tracker_item_get_link_namespace (item);

  tracker = bobgui_menu_tracker_new (_bobgui_menu_tracker_item_get_observable (item), submenu,
...
/*< private >
 * bobgui_menu_tracker_free:
 * @tracker: a BobguiMenuTracker
 *
 * Frees the tracker, ...
 */
void
bobgui_menu_tracker_free (BobguiMenuTracker *tracker)
{
  bobgui_menu_tracker_section_free (tracker->toplevel);
