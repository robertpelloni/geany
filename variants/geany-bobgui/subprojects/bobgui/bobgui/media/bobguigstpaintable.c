...
struct _BobguiGstPaintableClass
{
...
static void
bobgui_gst_paintable_paintable_snapshot (GdkPaintable *paintable,
                                      GdkSnapshot  *snapshot,
                                      double        width,
                                      double        height)
{
  BobguiGstPaintable *self = BOBGUI_GST_PAINTABLE (paintable);
  float sx, sy;
  ...
  if (self->orientation != GST_VIDEO_ORIENTATION_IDENTITY)
    {
      ...
      switch (self->orientation)
        {
        case GST_VIDEO_ORIENTATION_90R:
          bobgui_snapshot_rotate (snapshot, 90.0);
          break;
        case GST_VIDEO_ORIENTATION_180:
          bobgui_snapshot_scale (snapshot, -1.0, -1.0);
          break;
        case GST_VIDEO_ORIENTATION_90L:
          bobgui_snapshot_rotate (snapshot, 270.0);
          break;
      ...
      if (is_orientation_rotated (self->orientation))
        bobgui_snapshot_translate (snapshot, &GRAPHENE_POINT_INIT (-height / 2, -width / 2));
      else
        bobgui_snapshot_translate (snapshot, &GRAPHENE_POINT_INIT (-width / 2, -height / 2));
    }
...
static GdkPaintable *
bobgui_gst_paintable_paintable_get_current_image (GdkPaintable *paintable)
{
  BobguiGstPaintable *self = BOBGUI_GST_PAINTABLE (paintable);
...
static int
bobgui_gst_paintable_paintable_get_intrinsic_width (GdkPaintable *paintable)
{
  BobguiGstPaintable *self = BOBGUI_GST_PAINTABLE (paintable);
...
static int
bobgui_gst_paintable_paintable_get_intrinsic_height (GdkPaintable *paintable)
{
  BobguiGstPaintable *self = BOBGUI_GST_PAINTABLE (paintable);
...
static double
bobgui_gst_paintable_paintable_get_intrinsic_aspect_ratio (GdkPaintable *paintable)
{
  BobguiGstPaintable *self = BOBGUI_GST_PAINTABLE (paintable);
...
static void
bobgui_gst_paintable_paintable_init (GdkPaintableInterface *iface)
{
  iface->snapshot = bobgui_gst_paintable_paintable_snapshot;
  iface->get_current_image = bobgui_gst_paintable_paintable_get_current_image;
  iface->get_intrinsic_width = bobgui_gst_paintable_paintable_get_intrinsic_width;
  iface->get_intrinsic_height = bobgui_gst_paintable_paintable_get_intrinsic_height;
  iface->get_intrinsic_aspect_ratio = bobgui_gst_paintable_paintable_get_intrinsic_aspect_ratio;
}

static GstElement *
bobgui_gst_paintable_video_renderer_create_video_sink (GstPlayVideoRenderer *renderer,
                                                    GstPlay              *play)
{
  BobguiGstPaintable *self = BOBGUI_GST_PAINTABLE (renderer);
  GstElement *sink;
  gboolean uses_gl;
  GdkDisplay *display;
  ...
  sink = g_object_new (BOBGUI_TYPE_GST_SINK,
                       "paintable", self,
                       "gl-context", context,
                       "display", display,
                       NULL);

  g_object_get (BOBGUI_GST_SINK (sink), "uses-gl", &uses_gl, NULL);
...
static void
bobgui_gst_paintable_video_renderer_init (GstPlayVideoRendererInterface *iface)
{
  iface->create_video_sink = bobgui_gst_paintable_video_renderer_create_video_sink;
}

G_DEFINE_TYPE_WITH_CODE (BobguiGstPaintable, bobgui_gst_paintable, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (GDK_TYPE_PAINTABLE,
                                                bobgui_gst_paintable_paintable_init)
                         G_IMPLEMENT_INTERFACE (GST_TYPE_PLAY_VIDEO_RENDERER,
                                                bobgui_gst_paintable_video_renderer_init));

static void
bobgui_gst_paintable_dispose (GObject *object)
{
  BobguiGstPaintable *self = BOBGUI_GST_PAINTABLE (object);
  
  g_clear_object (&self->image);

  G_OBJECT_CLASS (bobgui_gst_paintable_parent_class)->dispose (object);
}

static void
bobgui_gst_paintable_class_init (BobguiGstPaintableClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->dispose = bobgui_gst_paintable_dispose;
}

static void
bobgui_gst_paintable_init (BobguiGstPaintable *self)
{
}

GdkPaintable *
bobgui_gst_paintable_new (void)
{
  return g_object_new (BOBGUI_TYPE_GST_PAINTABLE, NULL);
}

void
bobgui_gst_paintable_realize (BobguiGstPaintable *self,
                           GdkSurface      *surface)
{
  if (self->surface)
    return;
...
void
bobgui_gst_paintable_unrealize (BobguiGstPaintable *self,
                             GdkSurface      *surface)
{
...
static void
bobgui_gst_paintable_set_paintable (BobguiGstPaintable       *self,
...
static gboolean
bobgui_gst_paintable_set_texture_invoke (gpointer data)
{
  SetTextureInvocation *invoke = data;

  bobgui_gst_paintable_set_paintable (invoke->paintable,
...
void
bobgui_gst_paintable_queue_set_texture (BobguiGstPaintable       *self,
                                     GdkTexture            *texture,
                                     double                 pixel_aspect_ratio,
                                     const graphene_rect_t *viewport,
                                     int                    orientation)
{
  ...
  g_main_context_invoke_full (NULL,
                              G_PRIORITY_DEFAULT,
                              bobgui_gst_paintable_set_texture_invoke,
                              invoke,
