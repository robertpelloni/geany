...
#include "bobguigstsinkprivate.h"
...
GST_DEBUG_CATEGORY (bobgui_debug_gst_sink);
#define GST_CAT_DEFAULT bobgui_debug_gst_sink
...
static GstStaticPadTemplate bobgui_gst_sink_template =
...
G_DEFINE_TYPE_WITH_CODE (BobguiGstSink, bobgui_gst_sink,
    GST_TYPE_VIDEO_SINK,
    GST_DEBUG_CATEGORY_INIT (bobgui_debug_gst_sink,
        "bobguigstsink", 0, "BobguiGstMediaFile Video Sink"));

static GParamSpec *properties[N_PROPS] = { NULL, };


static void
bobgui_gst_sink_get_times (GstBaseSink  *bsink,
                        GstBuffer    *buf,
                        GstClockTime *start,
                        GstClockTime *end)
{
  BobguiGstSink *bobgui_sink;

  bobgui_sink = BOBGUI_GST_SINK (bsink);
      ...
      *start = GST_BUFFER_TIMESTAMP (buf);
      if (GST_BUFFER_DURATION_IS_VALID (buf))
        *end = *start + GST_BUFFER_DURATION (buf);
        ...
        {
          if (GST_VIDEO_INFO_FPS_N (&bobgui_sink->v_info) > 0)
            {
              *end = *start +
                  gst_util_uint64_scale_int (GST_SECOND,
                                             GST_VIDEO_INFO_FPS_D (&bobgui_sink->v_info),
                                             GST_VIDEO_INFO_FPS_N (&bobgui_sink->v_info));
            }
        }
    }
}
...
bobgui_gst_color_state_from_colorimetry (BobguiGstSink                *self,
...
static GstCaps *
bobgui_gst_sink_get_caps (GstBaseSink *bsink,
                       GstCaps     *filter)
{
  BobguiGstSink *self = BOBGUI_GST_SINK (bsink);
...
static gboolean
bobgui_gst_sink_set_caps (GstBaseSink *bsink,
                       GstCaps     *caps)
{
  BobguiGstSink *self = BOBGUI_GST_SINK (bsink);
  ...
  self->color_state = bobgui_gst_color_state_from_colorimetry (self, &self->v_info.colorimetry);
...
static gboolean
bobgui_gst_sink_query (GstBaseSink *bsink,
                    GstQuery    *query)
{
  BobguiGstSink *self = BOBGUI_GST_SINK (bsink);
  ...
  return GST_BASE_SINK_CLASS (bobgui_gst_sink_parent_class)->query (bsink, query);
}

static gboolean
bobgui_gst_sink_propose_allocation (GstBaseSink *bsink,
                                 GstQuery    *query)
{
  BobguiGstSink *self = BOBGUI_GST_SINK (bsink);
...
static gboolean
bobgui_gst_sink_event (GstBaseSink * bsink, GstEvent * event)
{
  BobguiGstSink *self = BOBGUI_GST_SINK (bsink);
  gboolean ret;
  ...
  ret = GST_BASE_SINK_CLASS (bobgui_gst_sink_parent_class)->event (bsink, event);

  return ret;
}

static GdkMemoryFormat
bobgui_gst_memory_format_from_video_info (GstVideoInfo *info)
{
...
bobgui_gst_sink_maybe_replace_frame (BobguiGstSink     *self,
                                  GstVideoFrame  *frame)
{
  GstBuffer *tmp_buffer = NULL;
  GstVideoFrame *tmp_frame;

  /* Buffer is contiguous, BOBGUI can use it */
...
static GdkTexture *
bobgui_gst_sink_texture_from_buffer (BobguiGstSink      *self,
                                  GstBuffer       *buffer,
                                  ...
                                  graphene_rect_t *viewport)
{
  ...
  if (gst_is_dmabuf_memory (mem) &&
           ...
           gst_video_frame_map (frame, &self->v_info, buffer, GST_MAP_READ | GST_MAP_GL))
    {
      ...
      gdk_gl_texture_builder_set_format (builder, bobgui_gst_memory_format_from_video_info (&frame->info));
  ...
  else if (gst_video_frame_map (frame, &self->v_info, buffer, GST_MAP_READ))
    {
      ...
      frame = bobgui_gst_sink_maybe_replace_frame (self, frame);
      if (!frame)
        return NULL;
      ...
      gdk_memory_texture_builder_set_format (builder, bobgui_gst_memory_format_from_video_info (&frame->info));
...
bobgui_gst_sink_show_frame (GstVideoSink *vsink,
                         GstBuffer    *buf)
{
  BobguiGstSink *self;
  GdkTexture *texture;
  ...
  self = BOBGUI_GST_SINK (vsink);

  GST_OBJECT_LOCK (self);

  texture = bobgui_gst_sink_texture_from_buffer (self, buf, &pixel_aspect_ratio, &viewport);
  if (texture)
    {
      bobgui_gst_paintable_queue_set_texture (self->paintable,
                                           texture,
                                           pixel_aspect_ratio,
                                           &viewport,
                                           self->orientation);
...
static gboolean
bobgui_gst_sink_initialize_gl (BobguiGstSink *self)
{
  GdkDisplay *display;
  GError *error = NULL;
...
static void
bobgui_gst_sink_set_property (GObject      *object,
                           guint         prop_id,
                           const GValue *value,
                           GParamSpec   *pspec)

{
  BobguiGstSink *self = BOBGUI_GST_SINK (object);
  ...
  switch (prop_id)
    ...
    case PROP_PAINTABLE:
      ...
      if (self->paintable == NULL)
        self->paintable = BOBGUI_GST_PAINTABLE (bobgui_gst_paintable_new ());
      break;

    case PROP_GL_CONTEXT:
      self->gdk_context = g_value_dup_object (value);
      if (self->gdk_context != NULL && !bobgui_gst_sink_initialize_gl (self))
...
static void
bobgui_gst_sink_get_property (GObject    *object,
                           guint       prop_id,
                           GValue     *value,
                           GParamSpec *pspec)
{
...
static void
bobgui_gst_sink_dispose (GObject *object)
{
  BobguiGstSink *self = BOBGUI_GST_SINK (object);
  ...
  G_OBJECT_CLASS (bobgui_gst_sink_parent_class)->dispose (object);
}

static void
bobgui_gst_sink_class_init (BobguiGstSinkClass * klass)
{
  ...
  gobject_class->set_property = bobgui_gst_sink_set_property;
  gobject_class->get_property = bobgui_gst_sink_get_property;
  gobject_class->dispose = bobgui_gst_sink_dispose;

  gstbasesink_class->set_caps = bobgui_gst_sink_set_caps;
  gstbasesink_class->get_times = bobgui_gst_sink_get_times;
  gstbasesink_class->query = bobgui_gst_sink_query;
  gstbasesink_class->propose_allocation = bobgui_gst_sink_propose_allocation;
  gstbasesink_class->get_caps = bobgui_gst_sink_get_caps;
  gstbasesink_class->event = bobgui_gst_sink_event;

  gstvideosink_class->show_frame = bobgui_gst_sink_show_frame;

  /**
   * BobguiGstSink:paintable:
   *
   * The paintable that provides the picture for this sink.
   */
  properties[PROP_PAINTABLE] =
    g_param_spec_object ("paintable", NULL, NULL,
                         BOBGUI_TYPE_GST_PAINTABLE,
                         G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

  /**
   * BobguiGstSink:gl-context:
   *
  ...
  gst_element_class_set_metadata (gstelement_class,
      "BobguiMediaStream Video Sink",
      "Sink/Video", "The video sink used by BobguiMediaStream",
      "Matthew Waters <matthew@centricular.com>, "
      "Benjamin Otte <otte@gnome.org>");

  gst_element_class_add_static_pad_template (gstelement_class,
      &bobgui_gst_sink_template);
}

static void
bobgui_gst_sink_init (BobguiGstSink * bobgui_sink)
{
}
