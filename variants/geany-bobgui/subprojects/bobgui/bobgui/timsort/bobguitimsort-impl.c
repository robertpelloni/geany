...
#define CONCAT(x, y) bobgui_tim_sort_ ## x ## _ ## y
#define MAKE_STR(x, y) CONCAT (x, y)
#define bobgui_tim_sort(x) MAKE_STR (x, NAME)
 ...
 */
static void bobgui_tim_sort(reverse_range) (BobguiTimSort *self,
                                         gpointer    a,
                                         gsize       hi)
{
  DEFINE_TEMP (t);
 ...
 */
static gsize
bobgui_tim_sort(prepare_run) (BobguiTimSort    *self,
                           BobguiTimSortRun *out_change)
{
  ...
  if (self->size <= run_hi)
    {
      bobgui_tim_sort_set_change (out_change, NULL, 0);
      return self->size;
    }
  ...
  if (bobgui_tim_sort_compare (self, cur, self->base) < 0)          /* Descending */
    {
      while (run_hi < self->size && bobgui_tim_sort_compare (self, next, cur) < 0)
        {
          run_hi++;
          cur = next;
          next = INCPTR (next);
        }
      bobgui_tim_sort(reverse_range) (self, self->base, run_hi);
      bobgui_tim_sort_set_change (out_change, self->base, run_hi);
    }
  else                          /* Ascending */
    {
      while (run_hi < self->size && bobgui_tim_sort_compare (self, next, cur) >= 0)
 ...
 */
static void bobgui_tim_sort(binary_sort) (BobguiTimSort    *self,
                                       gpointer       a,
                                       gsize          hi,
                                       gsize          start,
                                       BobguiTimSortRun *inout_change)
{
  DEFINE_TEMP (pivot);
  char *startp;
      ...
      while (0 < right)
        {
          ...
          if (bobgui_tim_sort_compare (self, startp, midp) < 0)
            {
              right = mid;
            }
          else
  ...
  if (change_max > (char *) a)
    {
      ...
      bobgui_tim_sort_set_change (inout_change, change_min, (change_max - change_min) / WIDTH);
    }
}

static gboolean
bobgui_tim_sort(merge_append) (BobguiTimSort    *self,
                            BobguiTimSortRun *out_change)
{
  /* Identify next run */
  gsize run_len;

  run_len = bobgui_tim_sort(prepare_run) (self, out_change);
  if (run_len == 0)
    return FALSE;
  ...
  if (run_len < self->min_run)
    {
      ...
      bobgui_tim_sort(binary_sort) (self, self->base, force, run_len, out_change);
      run_len = force;
    }
  /* Push run onto pending-run stack, and maybe merge */
  bobgui_tim_sort_push_run (self, self->base, run_len);
 ...
 */
static gsize
bobgui_tim_sort(gallop_left) (BobguiTimSort *self,
                           gpointer    key,
                           gpointer    base,
                           gsize       len,
                           gsize       hint)
{
  ...
  if (bobgui_tim_sort_compare (self, key, hintp) > 0)
    {
      /* Gallop right until a[hint+last_ofs] < key <= a[hint+ofs] */
      gsize max_ofs = len - hint;
      while (ofs < max_ofs
             && bobgui_tim_sort_compare (self, key, ELEM (hintp, ofs)) > 0)
        {
          last_ofs = ofs;
  ...
  /* Gallop left until a[hint-ofs] < key <= a[hint-last_ofs] */
    {
      ...
      gsize tmp;
      while (ofs < max_ofs
             && bobgui_tim_sort_compare (self, key, ELEM_REV (hintp, ofs)) <= 0)
        {
          last_ofs = ofs;
  ...
  while (last_ofs < ofs)
    {
      ...
      if (bobgui_tim_sort_compare (self, key, ELEM (base, m)) > 0)
 ...
 */
static gsize
bobgui_tim_sort(gallop_right) (BobguiTimSort *self,
                            gpointer    key,
                            gpointer    base,
                            gsize       len,
                            gsize       hint)
 ...
 */
static void
bobgui_tim_sort(merge_lo) (BobguiTimSort *self,
                        gpointer    base1,
                        gsize       len1,
                        gpointer    base2,
                        gsize       len2)
{
  ...
  gpointer tmp = bobgui_tim_sort_ensure_capacity (self, len1);
  char *cursor1;
  char *cursor2;
  char *dest;
  gsize min_gallop;
  ...
  while (TRUE)
    {
      ...
      do
        {
          ...
          if (bobgui_tim_sort_compare (self, cursor2, cursor1) < 0)
            {
              ASSIGN (dest, cursor2);
              dest = INCPTR (dest);
              cursor2 = INCPTR (cursor2);
      ...
      do
        {
          g_assert (len1 > 1 && len2 > 0);
          count1 = bobgui_tim_sort(gallop_right) (self, cursor2, cursor1, len1, 0);
          if (count1 != 0)
            {
          ...
          count2 = bobgui_tim_sort(gallop_left) (self, cursor1, cursor2, len2, 0);
          if (count2 != 0)
            {
 ...
 */
static void
bobgui_tim_sort(merge_hi) (BobguiTimSort *self,
                        gpointer    base1,
                        gsize       len1,
                        gpointer    base2,
                        gsize       len2)
{
  ...
  gpointer tmp = bobgui_tim_sort_ensure_capacity (self, len2);
  ...
  while (TRUE)
    {
      ...
      do
        {
          g_assert (len1 > 0 && len2 > 1);
          count1 = len1 - bobgui_tim_sort(gallop_right) (self, cursor2, base1, len1, len1 - 1);
          if (count1 != 0)
            {
          ...
          count2 = len2 - bobgui_tim_sort(gallop_left) (self, cursor1, tmp, len2, len2 - 1);
          if (count2 != 0)
            {
 ...
 */
static void
bobgui_tim_sort(merge_at) (BobguiTimSort    *self,
                        gsize          i,
                        BobguiTimSortRun *out_change)
{
   ...
   */
  k = bobgui_tim_sort(gallop_right) (self, base2, base1, len1, 0);
  base1 = ELEM (base1, k);
  len1 -= k;
  if (len1 == 0)
    {
   ...
   */
  len2 = bobgui_tim_sort(gallop_left) (self,
                                    ELEM (base1, len1 - 1),
                                    base2, len2, len2 - 1);
  if (len2 == 0)
    {
      ...
      if (len1 > self->max_merge_size)
        {
          ...
          bobgui_tim_sort(merge_lo) (self, base1, self->max_merge_size, base2, len2);
          bobgui_tim_sort_set_change (out_change, base1, self->max_merge_size + len2);
        ...
        {
          bobgui_tim_sort(merge_lo) (self, base1, len1, base2, len2);
          bobgui_tim_sort_set_change (out_change, base1, len1 + len2);
        }
    }
  else
    {
      if (len2 > self->max_merge_size)
        {
          bobgui_tim_sort(merge_hi) (self, base1, len1, base2, self->max_merge_size);
          bobgui_tim_sort_set_change (out_change, base1, len1 + self->max_merge_size);
          self->run[i].len += self->max_merge_size;
        ...
        {
          bobgui_tim_sort(merge_hi) (self, base1, len1, base2, len2);
          bobgui_tim_sort_set_change (out_change, base1, len1 + len2);
        }
    }
 ...
 */
static gboolean
bobgui_tim_sort(merge_collapse) (BobguiTimSort *self,
                              BobguiTimSortRun *out_change)
{
  BobguiTimSortRun *run = self->run;
  gsize n;
  ...
  bobgui_tim_sort(merge_at) (self, n, out_change);
  return TRUE;
}
 ...
 */
static gboolean
bobgui_tim_sort(merge_force_collapse) (BobguiTimSort    *self,
                                    BobguiTimSortRun *out_change)
{
  gsize n;
...
static gboolean
bobgui_tim_sort(step) (BobguiTimSort    *self,
                    BobguiTimSortRun *out_change)
{
  g_assert (self);

  if (bobgui_tim_sort(merge_collapse) (self, out_change))
    return TRUE;

  if (bobgui_tim_sort(merge_append) (self, out_change))
    return TRUE;

  if (bobgui_tim_sort(merge_force_collapse) (self, out_change))
    return TRUE;
