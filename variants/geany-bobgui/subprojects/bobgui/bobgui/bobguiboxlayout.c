/* bobguiboxlayout.c: Box layout manager
 *
 ...
 * [property@Bobgui.Orientable:orientation] property. Within the other dimension
 * all children all allocated the same size. The `BobguiBoxLayout` will respect
 * the [property@Bobgui.Widget:halign] and [property@Bobgui.Widget:valign]
 * properties of each child widget.
 *
 * If you want all children to be assigned the same size, you can use
 * the [property@Bobgui.BoxLayout:homogeneous] property.
 *
 * If you want to specify the amount of space placed between each child,
 * you can use the [property@Bobgui.BoxLayout:spacing] property.
 */

struct _BobguiBoxLayout
{
...
G_DEFINE_TYPE_WITH_CODE (BobguiBoxLayout, bobgui_box_layout, BOBGUI_TYPE_LAYOUT_MANAGER,
...
static void
bobgui_box_layout_set_orientation (BobguiBoxLayout   *self,
                                BobguiOrientation  orientation)
{
  BobguiLayoutManager *layout_manager = BOBGUI_LAYOUT_MANAGER (self);
  BobguiWidget *widget;
  ...
  widget = bobgui_layout_manager_get_widget (layout_manager);
  if (widget != NULL && BOBGUI_IS_ORIENTABLE (widget))
    bobgui_widget_update_orientation (widget, self->orientation);

  bobgui_layout_manager_layout_changed (layout_manager);

  g_object_notify (G_OBJECT (self), "orientation");
}

static void
bobgui_box_layout_set_property (GObject      *gobject,
                             guint         prop_id,
                             const GValue *value,
                             GParamSpec   *pspec)
{
  BobguiBoxLayout *self = BOBGUI_BOX_LAYOUT (gobject);

  switch (prop_id)
    {
    case PROP_HOMOGENEOUS:
      bobgui_box_layout_set_homogeneous (self, g_value_get_boolean (value));
      break;

    case PROP_SPACING:
      bobgui_box_layout_set_spacing (self, g_value_get_int (value));
      break;

    case PROP_BASELINE_CHILD:
      bobgui_box_layout_set_baseline_child (self, g_value_get_int (value));
      break;

    case PROP_BASELINE_POSITION:
      bobgui_box_layout_set_baseline_position (self, g_value_get_enum (value));
      break;

    case PROP_ORIENTATION:
      bobgui_box_layout_set_orientation (self, g_value_get_enum (value));
      break;
...
static void
bobgui_box_layout_get_property (GObject    *gobject,
                             guint       prop_id,
                             GValue     *value,
                             GParamSpec *pspec)
{
  BobguiBoxLayout *box_layout = BOBGUI_BOX_LAYOUT (gobject);
...
static void
count_expand_children (BobguiWidget *widget,
                       BobguiOrientation orientation,
...
static int
get_spacing (BobguiBoxLayout *self,
             BobguiCssNode   *node)
{
...
static void
bobgui_box_layout_compute_size (BobguiBoxLayout *self,
                             BobguiWidget    *widget,
                             int           for_size,
                             int          *minimum,
                             int          *natural,
                             ...
                             int          *natural_baseline)
{
  ...
  int spacing = get_spacing (self, bobgui_widget_get_css_node (widget));
  int child_above_min = 0, child_above_nat = 0;
  int above_min = 0, above_nat = 0;
  gboolean have_baseline = FALSE;
  int pos;

  for (child = bobgui_widget_get_first_child (widget), pos = 0;
       child != NULL;
       child = bobgui_widget_get_next_sibling (child), pos++)
    {
      ...
      bobgui_widget_measure (child, self->orientation,
                          for_size,
                          &child_min, &child_nat,
...
static void
bobgui_box_layout_compute_opposite_size (BobguiBoxLayout *self,
                                      BobguiWidget    *widget,
                                      int          *minimum,
                                      int          *natural,
                                      int          *min_baseline,
                                      int          *nat_baseline)
{
      ...
      if (self->orientation == BOBGUI_ORIENTATION_HORIZONTAL)
        {
          if (child_min_baseline > -1)
            {
              have_baseline = TRUE;
              if (bobgui_widget_get_valign (child) == BOBGUI_ALIGN_BASELINE_FILL ||
  ...
  if (self->orientation == BOBGUI_ORIENTATION_HORIZONTAL && align_baseline)
    {
...
static inline BobguiOrientation
preferred_measure_orientation (BobguiWidget *child)
{
  switch (bobgui_widget_get_request_mode (child))
    {
    case BOBGUI_SIZE_REQUEST_HEIGHT_FOR_WIDTH:
      return BOBGUI_ORIENTATION_VERTICAL;
    case BOBGUI_SIZE_REQUEST_WIDTH_FOR_HEIGHT:
      return BOBGUI_ORIENTATION_HORIZONTAL;
 ...
 */
static void
distribute_remaining_size (BobguiRequestedSize *sizes,
                           gsize             n_sizes,
                           BobguiOrientation    orientation,
                           int               available,
                           int              *min_opposite)
{
  ...
  if (n_sizes == 1)
    {
      ...
      if (preferred_measure_orientation (sizes[0].data) == orientation)
        {
          bobgui_widget_measure (sizes[0].data,
                              orientation,
                              *min_opposite,
                              &child_min, NULL,
                              NULL, NULL);
  ...
  for (i = 0; i < n_sizes; i++)
    {
      bobgui_widget_measure (sizes[i].data,
                          orientation,
                          *min_opposite,
                          &child_min, NULL,
                          NULL, NULL);
  ...
  if (n_sizes == 2 &&
      ...
      preferred_measure_orientation (sizes[1].data) == OPPOSITE_ORIENTATION (orientation))
    {
      ...
      bobgui_widget_measure (sizes[0].data,
                          ...
                          &child0_min, NULL,
                          NULL, NULL);
      bobgui_widget_measure (sizes[1].data,
                          orientation,
                          -1,
                          &child1_min, NULL,
                          NULL, NULL);
              ...
              if (check_min < child_min)
                {
                  g_critical ("%s %p reports a minimum %s of %u, but minimum %s for %s of %u is %u. Expect overlapping widgets.",
                              ...
                              orientation == BOBGUI_ORIENTATION_HORIZONTAL ? "width" : "height",
                              check_min,
                              orientation == BOBGUI_ORIENTATION_HORIZONTAL ? "width" : "height",
                              orientation == BOBGUI_ORIENTATION_HORIZONTAL ? "height" : "width",
...
static void
bobgui_box_layout_compute_opposite_size_for_size (BobguiBoxLayout *self,
                                               BobguiWidget    *widget,
                                               int           for_size,
                                               int          *minimum,
                                               int          *natural,
                                               ...
                                               int          *nat_baseline)
{
  ...
  sizes = g_newa (BobguiRequestedSize, nvis_children);
  ...
  if (self->homogeneous)
    {
      ...
      for (child = _bobgui_widget_get_first_child (widget);
           ...
           child = _bobgui_widget_get_next_sibling (child))
        {
          ...
          if (self->orientation == BOBGUI_ORIENTATION_HORIZONTAL && child_minimum_baseline > -1)
            {
              have_baseline = TRUE;
              if (bobgui_widget_get_valign (child) == BOBGUI_ALIGN_BASELINE_FILL ||
                  bobgui_widget_get_valign (child) == BOBGUI_ALIGN_BASELINE_CENTER)
  ...
  else
    {
      ...
      for (i = 0, child = _bobgui_widget_get_first_child (widget);
           child != NULL;
           child = _bobgui_widget_get_next_sibling (child))
        {
          if (!bobgui_widget_should_layout (child))
          ...
          if (bobgui_widget_get_request_mode (child) == BOBGUI_SIZE_REQUEST_CONSTANT_SIZE)
            {
              sizes[i].data = child;
      ...
      if (extra_space == 0)
        {
          ...
          for (i = 0; i < n_inconstant; i++)
            {
              BobguiRequestedSize size = sizes[nvis_children - n_inconstant + i];
              bobgui_widget_measure (size.data,
                                  OPPOSITE_ORIENTATION (self->orientation),
                                  size.minimum_size,
                                  &child_minimum, NULL,
                                  NULL, NULL);
      ...
      else
        {
          ...
          extra_space = bobgui_distribute_natural_allocation (extra_space,
                                                           nvis_children,
                                                           sizes);
        }
      ...
      for (child = _bobgui_widget_get_first_child (widget);
           ...
           child = _bobgui_widget_get_next_sibling (child))
        {
          ...
          if (bobgui_widget_compute_expand (child, self->orientation))
            {
              child_size += size_given_to_child;
  ...
  if (have_baseline && self->orientation == BOBGUI_ORIENTATION_HORIZONTAL)
    {
      if (align_baseline)
        {
...
static void
bobgui_box_layout_measure (BobguiLayoutManager *layout_manager,
                        BobguiWidget        *widget,
                        BobguiOrientation    orientation,
                        int               for_size,
                        int              *minimum,
                        ...
                        int              *nat_baseline)
{
  BobguiBoxLayout *self = BOBGUI_BOX_LAYOUT (layout_manager);

  if (self->orientation != orientation)
    {
      if (for_size < 0)
        {
          bobgui_box_layout_compute_opposite_size (self, widget,
                                                minimum, natural,
                                                min_baseline, nat_baseline);
        }
      else
        {
          bobgui_box_layout_compute_opposite_size_for_size (self, widget, for_size,
                                                         minimum, natural,
                                                         min_baseline, nat_baseline);
        }
    }
    ...
    {
      bobgui_box_layout_compute_size (self, widget, for_size,
                                   minimum, natural,
                                   min_baseline, nat_baseline);
    }
}
...
static void
bobgui_box_layout_allocate (BobguiLayoutManager *layout_manager,
                         BobguiWidget        *widget,
                         int               width,
                         int               height,
                         int               baseline)
{
  ...
  for (i = 0, child = _bobgui_widget_get_first_child (widget);
       ...
       child = _bobgui_widget_get_next_sibling (child))
    {
      ...
      if (bobgui_widget_get_request_mode (child) == BOBGUI_SIZE_REQUEST_CONSTANT_SIZE)
        {
          bobgui_widget_measure (child, self->orientation, -1,
      ...
      if (bobgui_widget_get_request_mode (child) != BOBGUI_SIZE_REQUEST_CONSTANT_SIZE)
        {
          if (!self->homogeneous && n_inconstant == 1 &&
              ...
              bobgui_widget_compute_expand (child, self->orientation))
            {
            ...
            {
              bobgui_widget_measure (child, self->orientation,
                                 self->orientation == BOBGUI_ORIENTATION_HORIZONTAL ? height : width,
                                 &sizes[i].minimum_size, &sizes[i].natural_size,
                                 NULL, NULL);
  ...
  if (self->homogeneous)
  ...
  else
    {
      ...
      extra_space = bobgui_distribute_natural_allocation (extra_space, nvis_children, sizes);
       ...
       child = _bobgui_widget_get_next_sibling (child))
    {
      ...
      if (self->orientation == BOBGUI_ORIENTATION_HORIZONTAL &&
           ...
           bobgui_widget_get_valign (child) == BOBGUI_ALIGN_BASELINE_CENTER))
        {
          ...
          bobgui_widget_measure (child, BOBGUI_ORIENTATION_VERTICAL,
  ...
  for (i = 0, child = _bobgui_widget_get_first_child (widget);
       ...
       child = _bobgui_widget_get_next_sibling (child))
    {
      ...
      else /* (self->orientation == BOBGUI_ORIENTATION_VERTICAL) */
        {
...
static void
bobgui_box_layout_class_init (BobguiBoxLayoutClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  BobguiLayoutManagerClass *layout_manager_class = BOBGUI_LAYOUT_MANAGER_CLASS (klass);

  gobject_class->set_property = bobgui_box_layout_set_property;
  gobject_class->get_property = bobgui_box_layout_get_property;

  layout_manager_class->measure = bobgui_box_layout_measure;
  layout_manager_class->allocate = bobgui_box_layout_allocate;

  /**
   * BobguiBoxLayout:homogeneous:
   *
  ...
  /**
   * BobguiBoxLayout:baseline-child:
   *
  ...
  /**
   * BobguiBoxLayout:baseline-position:
   *
...
static void
bobgui_box_layout_init (BobguiBoxLayout *self)
{
  self->homogeneous = FALSE;
  self->spacing = 0;
  self->orientation = BOBGUI_ORIENTATION_HORIZONTAL;
  self->baseline_position = BOBGUI_BASELINE_POSITION_CENTER;
  self->baseline_child = -1;
}

/**
 * bobgui_box_layout_new:
 * @orientation: the orientation for the new layout
 *
 * Creates a new `BobguiBoxLayout`.
 *
 * Returns: a new box layout
 */
BobguiLayoutManager *
bobgui_box_layout_new (BobguiOrientation orientation)
{
  return g_object_new (BOBGUI_TYPE_BOX_LAYOUT,
                       "orientation", orientation,
                       NULL);
}

/**
 * bobgui_box_layout_set_homogeneous:
 * @box_layout: a `BobguiBoxLayout`
 ...
 */
void
bobgui_box_layout_set_homogeneous (BobguiBoxLayout *box_layout,
                                gboolean      homogeneous)
{
  g_return_if_fail (BOBGUI_IS_BOX_LAYOUT (box_layout));
  ...
  bobgui_layout_manager_layout_changed (BOBGUI_LAYOUT_MANAGER (box_layout));
  g_object_notify_by_pspec (G_OBJECT (box_layout), box_layout_props[PROP_HOMOGENEOUS]);
}

/**
 * bobgui_box_layout_get_homogeneous:
 * @box_layout: a `BobguiBoxLayout`
 *
 ...
 */
gboolean
bobgui_box_layout_get_homogeneous (BobguiBoxLayout *box_layout)
{
  g_return_val_if_fail (BOBGUI_IS_BOX_LAYOUT (box_layout), FALSE);

  return box_layout->homogeneous;
}

/**
 * bobgui_box_layout_set_spacing:
 * @box_layout: a `BobguiBoxLayout`
 ...
 */
void
bobgui_box_layout_set_spacing (BobguiBoxLayout *box_layout,
                            guint         spacing)
{
  g_return_if_fail (BOBGUI_IS_BOX_LAYOUT (box_layout));
 ...
 */
guint
bobgui_box_layout_get_spacing (BobguiBoxLayout *box_layout)
{
  g_return_val_if_fail (BOBGUI_IS_BOX_LAYOUT (box_layout), 0);

  return box_layout->spacing;
}

/**
 * bobgui_box_layout_set_baseline_position:
 * @box_layout: a `BobguiBoxLayout`
 * @position: a `BobguiBaselinePosition`
 *
 ...
 */
void
bobgui_box_layout_set_baseline_position (BobguiBoxLayout        *box_layout,
...
/**
 * bobgui_box_layout_get_baseline_position:
 * @box_layout: a `BobguiBoxLayout`
 *
 * Gets the value set by bobgui_box_layout_set_baseline_position().
 *
 * Returns: the baseline position
 */
BobguiBaselinePosition
bobgui_box_layout_get_baseline_position (BobguiBoxLayout *box_layout)
{
  g_return_val_if_fail (BOBGUI_IS_BOX_LAYOUT (box_layout), BOBGUI_BASELINE_POSITION_CENTER);

  return box_layout->baseline_position;
}

/**
 * bobgui_box_layout_set_baseline_child:
 * @box_layout: a `BobguiBoxLayout`
 ...
 */
void
bobgui_box_layout_set_baseline_child (BobguiBoxLayout *box_layout,
                                   int           child)
{
  g_return_if_fail (BOBGUI_IS_BOX_LAYOUT (box_layout));
...
/**
 * bobgui_box_layout_get_baseline_child:
 * @box_layout: a `BobguiBoxLayout`
 *
 * Gets the value set by bobgui_box_layout_set_baseline_child().
 *
 ...
 */
int
bobgui_box_layout_get_baseline_child (BobguiBoxLayout *box_layout)
{
  g_return_val_if_fail (BOBGUI_IS_BOX_LAYOUT (box_layout), -1);
