...
void
svg_snapshot_push_fill (BobguiSnapshot *snapshot,
                        GskPath     *path,
                        GskFillRule  rule)
{
  ...
  switch (svg_path_classify (path, &rect))
    {
    case PATH_RECT:
      bobgui_snapshot_push_clip (snapshot, &rect.bounds);
      break;
    case PATH_ROUNDED_RECT:
    case PATH_CIRCLE:
      bobgui_snapshot_push_rounded_clip (snapshot, &rect);
      break;
    case PATH_GENERAL:
      bobgui_snapshot_push_fill (snapshot, path, rule);
      break;
    case PATH_EMPTY:
      bobgui_snapshot_push_clip (snapshot, &GRAPHENE_RECT_INIT (0, 0, 0, 0));
      break;
    default:
