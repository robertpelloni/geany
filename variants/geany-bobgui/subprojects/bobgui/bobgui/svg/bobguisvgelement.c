...
svg_element_free (SvgElement *element)
{
  ...
  _bobgui_bitmask_free (element->attrs);
                 ...
                 SvgElementType  type)
{
  ...
  element->attrs = _bobgui_bitmask_new ();
  ...
  element->css_node = bobgui_css_node_new ();
  bobgui_css_node_set_name (element->css_node, g_quark_from_static_string (svg_element_type_get_name (type)));
  if (parent)
    bobgui_css_node_set_parent (element->css_node, parent->css_node);
                                    ...
                                    BobguiSvg     *svg)
{
  ...
  for (unsigned int i = 0; i < svg_string_list_get_length (required_extensions); i++)
    {
      if ((svg->features & BOBGUI_SVG_EXTENSIONS) == 0)
        return TRUE;

      if (strcmp (svg_string_list_get (required_extensions, i), "http://www.bobgui.org/grappa") != 0)
        return TRUE;
    }

  lang = bobgui_get_default_language ();
      ...
      for (unsigned int i = 0; i < element->shapes->len; i++)
        {
          ...
          if (svg_element_get_current_bounds (sh, viewport, svg, &b2))
            {
              if (_bobgui_bitmask_get (sh->attrs, SVG_PROPERTY_TRANSFORM))
                {
...
svg_element_add_color_stop (SvgElement   *element,
                            SvgColorStop *stop)
{
  g_assert (bobgui_css_node_get_parent (svg_color_stop_get_css_node (stop)) == element->css_node);
...
svg_element_add_filter (SvgElement *element,
                        SvgFilter  *filter)
{
  g_assert (bobgui_css_node_get_parent (svg_filter_get_css_node (filter)) == element->css_node);
...
svg_element_add_child (SvgElement *element,
                       SvgElement *child)
{
  g_assert (bobgui_css_node_get_parent (svg_element_get_css_node (child)) == element->css_node);
                            ...
                            SvgValue    *value)
{
  ...
  element->attrs = _bobgui_bitmask_set (element->attrs, attr, value != NULL);
}
...
svg_element_property_is_set (SvgElement  *element,
                             SvgProperty  attr)
{
  return _bobgui_bitmask_get (element->attrs, attr);
}
                    ...
                    const char *id)
{
  ...
  bobgui_css_node_set_id (element->css_node, g_quark_from_string (id));
}
...
svg_element_set_style (SvgElement           *element,
                       const char           *style,
                       const BobguiSvgLocation *location)
{
                          ...
                          GStrv       classes)
{
  ...
  bobgui_css_node_set_classes (element->css_node, (const char **) element->classes);
}
                       ...
                       SvgElement *parent)
{
  ...
  copy->parent = parent;
  copy->attrs = _bobgui_bitmask_copy (element->attrs);
  copy->id = NULL;
  copy->style = g_strdup (element->style);
  copy->classes = g_strdupv (element->classes);
  copy->css_node = bobgui_css_node_new ();
  bobgui_css_node_set_parent (copy->css_node, parent->css_node);
  bobgui_css_node_set_name (copy->css_node, g_quark_from_static_string (svg_element_type_get_name (copy->type)));
  bobgui_css_node_set_classes (copy->css_node, (const char **) copy->classes);
