...
static unsigned int
parse_clip_path_arg (BobguiCssParser *parser,
                     unsigned int  n,
                     gpointer      data)
{
  ...
  if (n == 0)
    {
      if (bobgui_css_parser_try_ident (parser, "nonzero"))
        {
          args->fill_rule = GSK_FILL_RULE_WINDING;
          return 1;
        }
      else if (bobgui_css_parser_try_ident (parser, "evenodd"))
        {
  ...
  args->string = bobgui_css_parser_consume_string (parser);
  if (!args->string)
    return 0;
...
SvgValue *
svg_clip_parse (BobguiCssParser *parser)
{
  SvgValue *value = NULL;
  ...
  if (bobgui_css_parser_has_function (parser, "path"))
    {
      ...
      if (bobgui_css_parser_consume_function (parser, 1, 2, parse_clip_path_arg, &args))
        {
          ...
          if (!svg_path_data_parse_full (args.string, &data))
            {
              ...
              error = g_error_new_literal (BOBGUI_CSS_PARSER_ERROR,
  ...
  if (value == NULL)
    bobgui_css_parser_error_syntax (parser, "Expected a path");
