...
svg_color_stop_new (SvgElement *parent)
{
  ...
  stop->css_node = bobgui_css_node_new ();
  bobgui_css_node_set_name (stop->css_node, g_quark_from_static_string ("stop"));
  bobgui_css_node_set_parent (stop->css_node, parent->css_node);
                       ...
                       const char   *id)
{
  g_free (stop->id);
  stop->id = g_strdup (id);
  bobgui_css_node_set_id (stop->css_node, g_quark_from_string (id));
}
                             ...
                             GStrv         classes)
{
  ...
  bobgui_css_node_set_classes (stop->css_node, (const char **) stop->classes);
}
