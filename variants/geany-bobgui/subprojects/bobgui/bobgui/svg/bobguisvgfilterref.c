...
SvgValue *
svg_filter_ref_parse (BobguiCssParser *parser)
{
  for (unsigned int i = 0; i < G_N_ELEMENTS (filter_ref_values); i++)
    {
      if (filter_ref_values[i].ref &&
          bobgui_css_parser_try_ident (parser, filter_ref_values[i].ref))
  ...
  if (bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_IDENT))
    {
      ...
      value = svg_filter_ref_new_ref (bobgui_css_token_get_string (token));
      bobgui_css_parser_skip (parser);

      return value;
    }

  bobgui_css_parser_error_syntax (parser, "Expected a filter primitive ref value");
  return NULL;
}
