...
unsigned int
css_parser_parse_number (BobguiCssParser *parser,
                         unsigned int  n,
                         gpointer      data)
{
...
unsigned int
css_parser_parse_number_length (BobguiCssParser *parser,
                                unsigned int  n,
                                gpointer      data)
{
  ParserNumber *val = data;
  ...
  if (bobgui_css_token_is (token, BOBGUI_CSS_TOKEN_SIGNED_INTEGER) ||
    ...
    {
      val[n].value = ((const BobguiCssNumberToken *) token)->number;
      val[n].unit = SVG_UNIT_NUMBER;
      bobgui_css_parser_consume_token (parser);
      return 1;
    }

  if (bobgui_css_token_is (token, BOBGUI_CSS_TOKEN_SIGNED_INTEGER_DIMENSION) ||
      ...
      for (unsigned int i = 0; i < SVG_UNIT_TURN; i++)
        {
          if (svg_unit_dimension (i) == SVG_DIMENSION_LENGTH &&
              strcmp (svg_unit_name (i), ((const BobguiCssDimensionToken *) token)->dimension) == 0)
            {
              val[n].value = ((const BobguiCssDimensionToken *) token)->value;
              val[n].unit = i;
              bobgui_css_parser_consume_token (parser);
              return TRUE;
            }
...
unsigned int
css_parser_parse_number_angle (BobguiCssParser *parser,
                               unsigned int  n,
                               gpointer      data)
{
  ParserNumber *val = data;
...
unsigned int
css_parser_parse_number_percentage (BobguiCssParser *parser,
                                    unsigned int  n,
                                    gpointer      data)
{
  ParserNumber *val = data;
...
static gboolean
parse_transform_function (BobguiCssParser *self,
                          ...
                          double       *values)
{
  ...
  g_strlcpy (func, bobgui_css_token_get_string (token), sizeof (func));
  ...
  while (TRUE)
    {
      ...
      if (arg >= max_args)
        {
          bobgui_css_parser_error_syntax (self, "Expected ')' at end of %s()", func);
          break;
        }
      ...
      if (bobgui_css_token_is (token, BOBGUI_CSS_TOKEN_EOF))
        {
          if (arg < min_args)
            {
              bobgui_css_parser_error_syntax (self, "%s() requires at least %u arguments", func, min_args);
              break;
            }
          else
            {
      ...
      else if (!bobgui_css_parser_has_number (self))
        {
          bobgui_css_parser_error_syntax (self, "Unexpected data at end of %s()", func);
          break;
        }
    }
...
SvgValue *
svg_transform_parse_css (BobguiCssParser *parser)
{
  SvgTransform *tf;
  GArray *array;
  ...
  array = g_array_new (FALSE, FALSE, sizeof (PrimitiveTransform));
    ...
    while (TRUE)
    {
      ...
      if (bobgui_css_parser_has_function (parser, "rotate"))
        {
      ...
      else if (bobgui_css_parser_has_function (parser, "matrix"))
        {
          double values[6];
