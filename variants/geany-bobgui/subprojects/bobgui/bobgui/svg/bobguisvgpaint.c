...
static const char *symbolic_fallbacks[] = {
  [BOBGUI_SYMBOLIC_COLOR_FOREGROUND] = "rgb(0,0,0)",
  [BOBGUI_SYMBOLIC_COLOR_ERROR] = "rgb(204,0,0)",
  [BOBGUI_SYMBOLIC_COLOR_WARNING] = "rgb(245,121,0)",
  [BOBGUI_SYMBOLIC_COLOR_SUCCESS] = "rgb(51,209,122)",
  [BOBGUI_SYMBOLIC_COLOR_ACCENT] = "rgb(0,34,255)",
};
...
SvgValue *
svg_paint_new_symbolic (BobguiSymbolicColor symbolic)
{
  static SvgPaint sym[] = {
    { { &SVG_PAINT_CLASS, 0 }, .kind = PAINT_SYMBOLIC, .symbolic = BOBGUI_SYMBOLIC_COLOR_FOREGROUND },
    { { &SVG_PAINT_CLASS, 0 }, .kind = PAINT_SYMBOLIC, .symbolic = BOBGUI_SYMBOLIC_COLOR_ERROR },
    { { &SVG_PAINT_CLASS, 0 }, .kind = PAINT_SYMBOLIC, .symbolic = BOBGUI_SYMBOLIC_COLOR_WARNING },
    { { &SVG_PAINT_CLASS, 0 }, .kind = PAINT_SYMBOLIC, .symbolic = BOBGUI_SYMBOLIC_COLOR_SUCCESS },
    { { &SVG_PAINT_CLASS, 0 }, .kind = PAINT_SYMBOLIC, .symbolic = BOBGUI_SYMBOLIC_COLOR_ACCENT },
  };
...
SvgValue *
svg_paint_parse (BobguiCssParser *parser)
{
  GdkRGBA color;
  ...
  for (BobguiSymbolicColor sym = BOBGUI_SYMBOLIC_COLOR_FOREGROUND;
       sym <= BOBGUI_SYMBOLIC_COLOR_ACCENT;
       sym++)
    {
      if (bobgui_css_parser_try_ident (parser, symbolic_system_color (sym)))
        return svg_paint_new_symbolic (sym);
    }

  if (bobgui_css_parser_try_ident (parser, "context-fill"))
    return svg_paint_new_simple (PAINT_CONTEXT_FILL);

  if (bobgui_css_parser_try_ident (parser, "context-stroke"))
      ...
      if (url)
        {
          ...
          else if (bobgui_css_parser_try_ident (parser, "currentColor"))
            {
...
svg_paint_parse_gpa (const char *string)
{
  ...
  else if (bobgui_css_parser_try_ident (parser, "context-fill"))
    value = svg_paint_new_simple (PAINT_CONTEXT_FILL);
  else if (bobgui_css_parser_try_ident (parser, "context-stroke"))
                   ...
                   SvgComputeContext *context)
{
  ...
  if ((context->svg->features & BOBGUI_SVG_EXTENSIONS) != 0)
    {
      if (svg_paint_is_symbolic (value, &symbolic))
        {
          if (symbolic < context->n_colors)
            return svg_paint_new_rgba (&context->colors[symbolic]);
          else if (BOBGUI_SYMBOLIC_COLOR_FOREGROUND < context->n_colors)
            return svg_paint_new_rgba (&context->colors[BOBGUI_SYMBOLIC_COLOR_FOREGROUND]);
          else
            return svg_paint_new_black ();
        }
    }
