...
#include "bobgui/bobguicssselectorprivate.h"
#include <glib/gstdio.h>
#include "bobguisvgenumtypes.h"
#include "bobguisvgutilsprivate.h"
#include "bobguisvgstringutilsprivate.h"
#include "bobguisvgvalueprivate.h"
#include "bobguisvgkeywordprivate.h"
#include "bobguisvgnumberprivate.h"
#include "bobguisvgnumbersprivate.h"
#include "bobguisvgstringprivate.h"
#include "bobguisvgstringlistprivate.h"
#include "bobguisvgenumprivate.h"
#include "bobguisvgfilterrefprivate.h"
#include "bobguisvgtransformprivate.h"
#include "bobguisvgcolorprivate.h"
#include "bobguisvgpaintprivate.h"
#include "bobguisvgfilterfunctionsprivate.h"
#include "bobguisvgdasharrayprivate.h"
#include "bobguisvgpathprivate.h"
#include "bobguisvgpathdataprivate.h"
#include "bobguisvgclipprivate.h"
#include "bobguisvgmaskprivate.h"
#include "bobguisvgviewboxprivate.h"
#include "bobguisvgcontentfitprivate.h"
#include "bobguisvgorientprivate.h"
#include "bobguisvglanguageprivate.h"
#include "bobguisvghrefprivate.h"
#include "bobguisvgtextdecorationprivate.h"
#include "bobguisvgpathutilsprivate.h"
#include "bobguisvgcolorutilsprivate.h"
#include "bobguisvgpangoutilsprivate.h"
#include "bobguisvglocationprivate.h"
#include "bobguisvgerrorprivate.h"
#include "bobguisvgelementtypeprivate.h"
#include "bobguisvgpropertyprivate.h"
#include "bobguisvgfiltertypeprivate.h"
#include "bobguisvgfilterprivate.h"
#include "bobguisvgcolorstopprivate.h"
#include "bobguisvgelementinternal.h"
#include "bobguisvganimationprivate.h"
 ...
 *
 * `BobguiSvg` objects are created by parsing a subset of SVG,
 * including SVG animations.
 *
 * `BobguiSvg` fills or strokes paths with symbolic or fixed colors.
 ...
 *
 * To play an SVG animation, use [method@Bobgui.Svg.set_frame_clock]
 * to connect the paintable to a frame clock, and call
 * [method@Bobgui.Svg.play] after loading the SVG. The animation can
 * be paused using [method@Bobgui.Svg.pause].
 *
 * To set the current state, use [method@Bobgui.Svg.set_state].
 *
 *
 * ## Error handling
 *
 * Loading an SVG into `BobguiSvg` will always produce a (possibly empty)
 * paintable. BOBGUI will drop things that it can't handle and try to make
 * sense of the rest.
 *
 * To track errors during parsing or rendering, connect to the
 * [signal@Bobgui.Svg::error] signal.
 *
 * For parsing errors in the `BOBGUI_SVG_ERROR` domain, the functions
 * [func@Bobgui.SvgError.get_start], [func@Bobgui.SvgError.get_end],
 * [func@Bobgui.SvgError.get_element] and [func@Bobgui.SvgError.get_attribute]
 ...
 *
 * BobguiSvg also allows to refer to symbolic colors like system colors
...
static void
bobgui_svg_emit_error (BobguiSvg       *svg,
                    const GError *error)
{
...
G_GNUC_PRINTF (6, 7)
static void
bobgui_svg_skipped_element (BobguiSvg               *self,
                         const char           *parent_element,
                         const BobguiSvgLocation *start,
                         const BobguiSvgLocation *end,
                         BobguiSvgError           code,
                         const char           *format,
                         ...)
{
  ...
  error = g_error_new_valist (BOBGUI_SVG_ERROR, code, format, args);
  va_end (args);

  bobgui_svg_error_set_element (error, parent_element);
  bobgui_svg_error_set_location (error, start, end);

  bobgui_svg_emit_error (self, error);
  g_clear_error (&error);
}

G_GNUC_PRINTF (5, 6)
static void
bobgui_svg_invalid_attribute (BobguiSvg               *self,
...
{
  GError *error;
  BobguiSvgLocation start, end;

  if (format)
    {
      va_list args;
      va_start (args, format);
      error = g_error_new_valist (BOBGUI_SVG_ERROR,
    ...
    {
      error = g_error_new (BOBGUI_SVG_ERROR,
                           BOBGUI_SVG_ERROR_INVALID_ATTRIBUTE,
                           "Invalid attribute: %s", attr_name);
    }

  bobgui_svg_error_set_element (error, g_markup_parse_context_get_element (context));
  bobgui_svg_error_set_attribute (error, attr_name);
      ...
      for (unsigned int i = 0; attr_names[i]; i++)
        {
          if (strcmp (attr_names[i], attr_name) == 0)
            {
              bobgui_svg_location_init_attr_range (&start, &end, context, i);
              break;
            }
        }
    }
...
static void
bobgui_svg_markup_error (BobguiSvg              *self,
                      GMarkupParseContext *context,
                      const GError        *markup_error)
{
  ...
  error = g_error_new_literal (BOBGUI_SVG_ERROR,
...
G_GNUC_PRINTF (4, 5)
static void
bobgui_svg_missing_attribute (BobguiSvg              *self,
                           GMarkupParseContext *context,
                           const char          *attr_name,
                           const char          *format,
                           ...)
...
G_GNUC_PRINTF (2, 3)
static void
bobgui_svg_invalid_reference (BobguiSvg     *self,
                           const char *format,
                           ...)
{
  va_list args;
...
static void
bobgui_svg_check_unhandled_attributes (BobguiSvg               *self,
                                    GMarkupParseContext  *context,
                                    const char          **attr_names,
                                    uint64_t              handled)
{
  ...
  for (unsigned int i = 0; i < n; i++)
    {
      if ((handled & BIT (i)) == 0)
        bobgui_svg_invalid_attribute (self, context, attr_names, attr_names[i],
...
G_GNUC_PRINTF (2, 3)
static void
bobgui_svg_update_error (BobguiSvg     *self,
                      const char *format,
                      ...)
{
  va_list args;
...
G_GNUC_PRINTF (2, 3)
static void
bobgui_svg_rendering_error (BobguiSvg     *self,
                         const char *format,
                         ...)
{
  va_list args;
...
static gboolean
find_named_state (BobguiSvg       *svg,
                  const char   *name,
                  unsigned int *state)
{
  ...
  if (g_str_has_prefix (string, "data:"))
    {
      ...
      bytes = bobgui_css_data_url_parse (string, NULL, error);
    ...
    {
      g_set_error (error, BOBGUI_SVG_ERROR, BOBGUI_SVG_ERROR_FEATURE_DISABLED,
  ...
  if (!texture)
    {
      ...
      texture = load_texture (string,
                              (svg->features & BOBGUI_SVG_EXTERNAL_RESOURCES) != 0,
                              &cache_this,
                              error);
...
ensure_fontmap (BobguiSvg *svg)
{
  ...
  if ((svg->features & BOBGUI_SVG_SYSTEM_RESOURCES) == 0)
    {
#ifdef HAVE_PANGOFT
...
static gboolean
add_font_from_file (BobguiSvg      *svg,
                    const char  *path,
                    GError     **error)
{
  ensure_fontmap (svg);
...
static gboolean
add_font_from_bytes (BobguiSvg      *svg,
                     GBytes      *bytes,
                     GError     **error)
{
  GFile *file;
...
static gboolean
add_font_from_url (BobguiSvg               *svg,
                   GMarkupParseContext  *context,
                   const char          **attr_names,
                   const char           *attr_name,
                   const char           *url)
  ...
  if (!scheme || g_ascii_strcasecmp (scheme, "data") != 0)
    {
      ...
      bobgui_svg_invalid_attribute (svg, context, attr_names, attr_name,
...
static gboolean
parse_states_css (BobguiCssParser *parser,
                  BobguiSvg       *svg,
                  uint64_t     *states)
{
  bobgui_css_parser_skip_whitespace (parser);

  if (bobgui_css_parser_try_ident (parser, "all"))
    {
      *states = ALL_STATES;
      return TRUE;
    }
  else if (bobgui_css_parser_try_ident (parser, "none"))
    {
      *states = NO_STATES;
      return TRUE;
    }
  ...
  while (!bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_EOF))
    {
      ...
      if (bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_IDENT))
        {
          char *id = bobgui_css_parser_consume_ident (parser);
          unsigned int u;
      ...
      else if (bobgui_css_parser_has_integer (parser))
        {
          ...
          bobgui_css_parser_consume_integer (parser, &i);
          if (i < 0 || i > 63)
            {
              *states = NO_STATES;
              return FALSE;
      ...
      else if (bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_COMMA))
        return TRUE;
      else
        return FALSE;
    }
...
static gboolean
parse_states (BobguiSvg     *svg,
              const char *text,
              uint64_t   *states)
{
  BobguiCssParser *parser = parser_new_for_string (text);
  gboolean ret;
...
static unsigned int
parse_states_arg (BobguiCssParser *parser,
                  unsigned int  n,
                  gpointer      data)
{
...
static gboolean
time_spec_parse (BobguiCssParser  *parser,
                 BobguiSvg        *svg,
                 TimeSpec      *spec,
                 GError       **error)
{
  ...
  bobgui_css_parser_skip_whitespace (parser);
  if (bobgui_css_parser_try_ident (parser, "indefinite"))
    {
  ...
  else if (bobgui_css_parser_has_function (parser, "StateChange"))
    {
      ParseStatesArg arg = {
        .svg = svg,
      ...
      if (bobgui_css_parser_consume_function (parser, 2, 2, parse_states_arg, &arg))
        {
  ...
  else if (bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_IDENT))
    {
      char *id = bobgui_css_parser_consume_ident (parser);
      ...
      if (bobgui_css_parser_try_token (parser, BOBGUI_CSS_TOKEN_COLON) &&
          strcmp (id, "gpa") == 0)
        {
          ...
          if (bobgui_css_parser_has_function (parser, "states"))
            {
              ParseStatesArg arg = {
                .svg = svg,
                .states = { NO_STATES, NO_STATES },
              ...
              if (bobgui_css_parser_consume_function (parser, 1, 2, parse_states_arg, &arg))
                {
                  spec->type = TIME_SPEC_TYPE_STATES;
                  ...
                  if (!arg.set[1])
                    {
                      ...
                      if (!bobgui_css_parser_try_delim (parser, '.'))
                        return FALSE;
                      if (bobgui_css_parser_try_ident (parser, "begin"))
              ...
              if (*error == NULL)
                g_set_error (error,
                             BOBGUI_SVG_ERROR, BOBGUI_SVG_ERROR_INVALID_SYNTAX,
                              ...
                              int64_t      *out_frame_end)
{
  ...
  if (simple_duration == INDEFINITE || simple_duration == 0)
    {
      if (svg)
        bobgui_svg_update_error (svg, "Not enough data to advance animation %s", a->id);
      *out_rep = 0;
      *out_frame = 0;
      *out_frame_t = 0;
                           ...
                           int64_t       current_time)
{
  if (a->status == ANIMATION_STATUS_INACTIVE)
    {
      a->run_mode = BOBGUI_SVG_RUN_MODE_DISCRETE;
  ...
  else if (a->status == ANIMATION_STATUS_RUNNING)
    {
      ...
      if (a->calc_mode == CALC_MODE_DISCRETE)
        ...
        {
          a->run_mode = BOBGUI_SVG_RUN_MODE_CONTINUOUS;
          a->next_invalidate = a->current.end;
        }
    }
    ...
    {
      a->run_mode = BOBGUI_SVG_RUN_MODE_STOPPED;
      ...
      if (strstr (g_getenv ("SVG_DEBUG") ?:"", "state"))
        {
          ...
          switch (a->run_mode)
            {
            case BOBGUI_SVG_RUN_MODE_CONTINUOUS:
              g_string_append_printf (s, "--> %s\n", format_time (a->next_invalidate));
              break;
            case BOBGUI_SVG_RUN_MODE_DISCRETE:
              g_string_append_printf (s, "> > %s\n", format_time (a->next_invalidate));
              break;
            case BOBGUI_SVG_RUN_MODE_STOPPED:
              g_string_append (s, "\n");
              break;
            default:
              g_assert_not_reached ();
...
static void frame_clock_connect    (BobguiSvg *self);
static void frame_clock_disconnect (BobguiSvg *self);

static void schedule_next_update (BobguiSvg *self);
...
advance_later (gpointer data)
{
  ...
  bobgui_svg_advance (self, MAX (self->current_time, gdk_frame_clock_get_frame_time (self->clock)));
}

static void
schedule_next_update (BobguiSvg *self)
{
  ...
  if (self->run_mode == BOBGUI_SVG_RUN_MODE_CONTINUOUS)
    {
                    ...
                    BobguiSvg        *self)
{
  ...
  bobgui_svg_advance (self, time);
}

static void
frame_clock_connect (BobguiSvg *self)
{
...
static void
frame_clock_disconnect (BobguiSvg *self)
{
  if (self->clock && self->clock_update_id != 0)
    {
  ...
  if (a->attr != SVG_PROPERTY_TRANSFORM || a->type != ANIMATION_TYPE_MOTION)
    {
      ...
      if (ival == NULL)
        {
          bobgui_svg_update_error (context->svg,
...
static void
shape_apply_state (BobguiSvg       *self,
                   SvgElement   *shape,
                   unsigned int  state)
{
  if (svg_element_type_is_path (svg_element_get_type (shape)))
    {
      ...
      if ((self->features & BOBGUI_SVG_ANIMATIONS) == 0)
        {
...
strokewidth_parse (const char  *value,
                   SvgValue   **values)
{
  BobguiCssParser *parser = parser_new_for_string (value);
  unsigned int i;
...
static gboolean
time_spec_parse_one (BobguiCssParser *parser,
                     gpointer      user_data)
{
                            ...
                            GMarkupParseContext  *context)
{
  ...
  if (href_attr)
    {
      if (href_attr[0] != '#')
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, href_attr_name,
                                   "Missing '#' in 'href'");
      else
  ...
  if (begin_attr)
    {
      BobguiCssParser *parser = parser_new_for_string (begin_attr);
      Specs specs = { 0, };
      ...
      if (parser_parse_list (parser, time_spec_parse_one, &specs))
        ...
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "begin", NULL);
        }
      ...
      if (specs.error)
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "begin", "%s", specs.error->message);
  ...
  if (end_attr)
    {
      BobguiCssParser *parser = parser_new_for_string (end_attr);
      Specs specs = { 0, };
      ...
      if (parser_parse_list (parser, time_spec_parse_one, &specs))
        ...
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "end", NULL);
        }
      ...
      if (specs.error)
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "end", "%s", specs.error->message);
          g_error_free (specs.error);
        }
    }
  else
  ...
  if (dur_attr)
    {
      ...
      if (!parse_duration (dur_attr, TRUE, &a->simple_duration))
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "dur", NULL);
  ...
  if (repeat_count_attr)
    {
      ...
      if (!parse_number_or_named (repeat_count_attr, 0, DBL_MAX, "indefinite", REPEAT_FOREVER, &a->repeat_count))
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "repeatCount", NULL);
  ...
  if (repeat_dur_attr)
    {
      ...
      if (!parse_duration (repeat_dur_attr, TRUE, &a->repeat_duration))
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "repeatDur", NULL);
  ...
  if (fill_attr)
    {
      ...
      if (!parse_enum (fill_attr,
                       ...
                       &value))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "fill", NULL);
      else
  ...
  if (restart_attr)
    {
      ...
      if (!parse_enum (restart_attr,
                       ...
                       &value))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "restart", NULL);
      else
  ...
  if (a->type == ANIMATION_TYPE_MOTION)
    {
      if (attr_name_attr)
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "attributeName",
    ...
    {
      bobgui_svg_missing_attribute (data->svg, context, "attributeName", NULL);
      return FALSE;
    }
  /* FIXME: if href is set, current_shape might be the wrong shape */
...
static gboolean
parse_spline (BobguiCssParser *parser,
              gpointer      user_data)
{
                             ...
                             GMarkupParseContext  *context)
{
      ...
      if (type_attr)
        {
          ...
          if (!parse_enum (type_attr,
                           (const char *[]) { "translate", "scale", "rotate",
                            ...
                            &value))
            {
              bobgui_svg_invalid_attribute (data->svg, context, attr_names, "type", NULL);
              return FALSE;
            }
          else
        ...
        {
          bobgui_svg_missing_attribute (data->svg, context, "type", NULL);
          return FALSE;
        }
    }
  else if (type_attr)
  ...
  if (calc_mode_attr)
    {
      ...
      if (!parse_enum (calc_mode_attr,
                       ...
                       &value))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "calcMode", NULL);
      else
  ...
  if (additive_attr)
    {
      ...
      if (!parse_enum (additive_attr,
                       ...
                       &value))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "additive", NULL);
      else
  ...
  if (accumulate_attr)
    {
      ...
      if (!parse_enum (accumulate_attr,
                       ...
                       &value))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "accumulate", NULL);
      else
  ...
  if (color_interpolation_attr)
    {
      ...
      if (!v)
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "color-interpolation", "%s", error->message);
          g_error_free (error);
        }
      else
        {
  ...
  if (values_attr)
    {
      ...
      if (!values || values->len < 2)
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "values", NULL);
  ...
  else if (from_attr && to_attr)
    {
      ...
      if (!values || values->len != 1)
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "from", NULL);
          g_clear_pointer (&values, g_ptr_array_unref);
          return FALSE;
        }
      ...
      if (!tovals || tovals->len != 1)
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "to", NULL);
          g_clear_pointer (&values, g_ptr_array_unref);
  ...
  else if (from_attr && by_attr)
    {
      ...
      if (!byvals || byvals->len != 1)
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "by", NULL);
          g_clear_pointer (&values, g_ptr_array_unref);
  ...
  else if (by_attr)
    {
      ...
      if (svg_value_is_number (by))
        ...
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "by",
          ...
          for (unsigned int i = 0; i < values->len; i++)
            {
              ...
              if (svg_transform_get_length (tf) != 1 || type != TRANSFORM_TRANSLATE)
                {
                  bobgui_svg_invalid_attribute (data->svg, context, attr_names, NULL,
  ...
  if (a->calc_mode == CALC_MODE_PACED)
  ...
  else if (key_times_attr)
    {
      ...
      if (!times)
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "keyTimes", NULL);
          g_clear_pointer (&values, g_ptr_array_unref);
  ...
  if (times != NULL)
    {
      ...
      if (times->len == 0)
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "keyTimes",
                                     "No values");
          g_clear_pointer (&values, g_ptr_array_unref);
          g_clear_pointer (&times, g_array_unref);
          g_clear_pointer (&points, g_array_unref);
  ...
  if (a->calc_mode != CALC_MODE_PACED)
    {
      ...
      if (splines_attr)
        {
          BobguiCssParser *parser = parser_new_for_string (splines_attr);
          unsigned int n;
          ...
          if (!parser_parse_list (parser, parse_spline, params))
            {
              bobgui_css_parser_unref (parser);
              bobgui_svg_invalid_attribute (data->svg, context, attr_names, "keySplines", NULL);
              g_clear_pointer (&values, g_ptr_array_unref);
              g_clear_pointer (&times, g_array_unref);
          ...
          if (n != times->len - 1)
            {
              bobgui_svg_invalid_attribute (data->svg, context, attr_names, "keySplines",
                              ...
                              GMarkupParseContext  *context)
{
      ...
      if (strcmp (path_attr, "none") == 0)
      ...
      else
        {
          ...
          if (svg_path_data_parse_full (path_attr, &path_data))
            ...
            bobgui_svg_invalid_attribute (data->svg, context, attr_names, "path", "Path data is invalid");
        }
  ...
  if (rotate_attr)
    {
      BobguiCssParser *parser = parser_new_for_string (rotate_attr);
      SvgValue *value;
      ...
      if (value)
        ...
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "rotate", NULL);
        }

      bobgui_css_parser_skip_whitespace (parser);
      if (!bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_EOF))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "rotate",
      ...
      if (key_points_attr)
        {
          ...
          if (!points)
            {
              bobgui_svg_invalid_attribute (data->svg, context, attr_names, "keyPoints", NULL);
              g_array_unref (points);
              return FALSE;
            }
          ...
          if (points->len != a->n_frames)
            {
              bobgui_svg_invalid_attribute (data->svg, context, attr_names, "keyPoints",
                                         "Wrong number of values");
              g_array_unref (points);
              return FALSE;
            }
  ...
  if (a->calc_mode == CALC_MODE_PACED)
    {
      ...
      if (a->motion.path && !gsk_path_is_empty (a->motion.path))
        ...
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "calcMode",
          ...
          if (svg_property_applies_to (attr, svg_element_get_type (shape)))
            {
              if (svg_element_property_is_set (shape, attr) &&
              ...
              else
                {
                  ...
                  if (error)
                    {
                      bobgui_svg_invalid_attribute (data->svg, context, attr_names, attr_names[i], "%s", error->message);
                      g_error_free (error);
                    }
                  else if (!value)
                    {
                      bobgui_svg_invalid_attribute (data->svg, context, attr_names, attr_names[i], NULL);
                    }
            ...
            {
              bobgui_svg_invalid_attribute (data->svg, context, attr_names, attr_names[i], "Unknown attribute: %s", attr_names[i]);
            }
...
static void
parse_svg_gpa_attrs (BobguiSvg               *svg,
                     const char           *element_name,
                     const char          **attr_names,
                     const char          **attr_values,
                     uint64_t             *handled,
  ...
  if (state_names_attr)
    {
      ...
      if (strv == NULL)
        {
          bobgui_svg_invalid_attribute (svg, context, attr_names, "gpa:state-names", NULL);
        }
      else
        {
          if (!bobgui_svg_set_state_names (svg, (const char **) strv))
            bobgui_svg_invalid_attribute (svg, context, attr_names, "gpa:state-names", NULL);
          g_strfreev (strv);
        }
    }
      ...
      if (parse_number (state_attr, 0, 63, &v))
        {
          ...
          bobgui_svg_set_state (svg, (unsigned int) v);
        }
        ...
        bobgui_svg_invalid_attribute (svg, context, attr_names, "gpa:state", NULL);
    }
  ...
  if (version_attr)
    {
      ...
      if ((end && *end != '\0') || version != 1)
        bobgui_svg_invalid_attribute (svg, context, attr_names, "gpa:version",
                                   "Must be 1");
      else
                       ...
                       GMarkupParseContext  *context)
{
  ...
  if (stroke_attr)
    {
      ...
      if (value)
        ...
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:stroke", NULL);
        }
    }
  ...
  if (fill_attr)
    {
      ...
      if (value)
        ...
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:fill", NULL);
        }
    }
  ...
  if (strokewidth_attr)
    {
      ...
      if (strokewidth_parse (strokewidth_attr, values) &&
        ...
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:stroke-width", NULL);
        }
  ...
  if (states_attr)
    {
      if (!parse_states (data->svg, states_attr, &states))
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:states", NULL);
          states = ALL_STATES;
        }
      else
        {
  ...
  if (origin_attr)
    {
      if (!parse_number (origin_attr, 0, 1, &origin))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:origin", NULL);
    }
  ...
  if (transition_type_attr)
    {
      if (!parse_enum (transition_type_attr,
                       (const char *[]) { "none", "animate", "morph", "fade" }, 4,
                        &transition_type))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:transition-type", NULL);
    }
  ...
  if (transition_duration_attr)
    {
      if (!parse_duration (transition_duration_attr, FALSE, &transition_duration))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:transition-duration", NULL);
    }
  ...
  if (transition_delay_attr)
    {
      if (!parse_duration (transition_delay_attr, FALSE, &transition_delay))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:transition-delay", NULL);
    }
  ...
  if (transition_easing_attr)
    {
      ...
      if (!parse_enum (transition_easing_attr,
                       (const char *[]) { "linear", "ease-in-out", "ease-in",
                                          "ease-out", "ease" }, 5,
                        &transition_easing))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:transition-easing", NULL);
    }
  ...
  if (animation_type_attr)
    {
      if (!parse_enum (animation_type_attr,
                       (const char *[]) { "none", "automatic", }, 2,
                        &has_animation))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:animation-type", NULL);
    }
  ...
  if (has_animation && animation_direction_attr)
    {
      if (!parse_enum (animation_direction_attr,
                       (const char *[]) { "none", "normal", "alternate", "reverse",
                        ...
                        &animation_direction))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:animation-direction", NULL);
    }
  ...
  if (animation_duration_attr)
    {
      if (!parse_duration (animation_duration_attr, FALSE, &animation_duration))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:animation-duration", NULL);
    }
  ...
  if (animation_repeat_attr)
    {
      if (!parse_number_or_named (animation_repeat_attr, 0, DBL_MAX, "indefinite", REPEAT_FOREVER, &animation_repeat))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:animation-repeat", NULL);
    }
  ...
  if (animation_segment_attr)
    {
      if (!parse_number (animation_segment_attr, 0, 1, &animation_segment))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:animation-segment", NULL);
    }
  ...
  if (animation_easing_attr)
    {
      if (!parse_enum (animation_easing_attr,
                       (const char *[]) { "linear", "ease-in-out", "ease-in",
                                          "ease-out", "ease" }, 5,
                        &animation_easing))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:animation-easing", NULL);
    }
  ...
  if (attach_pos_attr)
    {
      if (!parse_number (attach_pos_attr, 0, 1, &attach_pos))
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "gpa:attach-pos", NULL);
    }
  ...
  if ((data->svg->features & BOBGUI_SVG_ANIMATIONS) == 0)
    return;
  ...
  if (transition_type != GPA_TRANSITION_NONE ||
      animation_direction != GPA_ANIMATION_NONE)
    {
      ...
      if (svg_element_property_is_set (shape, SVG_PROPERTY_FILTER) &&
        ...
        bobgui_svg_invalid_attribute (data->svg, context, attr_names, "filter",
              ...
              ...)
{
  ...
  bobgui_svg_location_init_tag_start (&data->skip.start, context);
...
start_collect_text (ParserData          *data,
                    GMarkupParseContext *context)
{
  bobgui_svg_location_init (&data->text.start, context);
                  ...
                  GError              **gmarkup_error)
{
  ...
  BobguiSvgLocation location;

  if (data->skip.to)
    return;

  bobgui_svg_location_init_tag_start (&location, context);
  ...
  if (data->num_loaded_elements++ > LOADING_LIMIT)
    {
      ...
      data->skip.code = BOBGUI_SVG_ERROR_LIMITS_EXCEEDED;
  ...
  if (svg_element_type_lookup (element_name, &shape_type))
    {
      ...
      if (data->current_shape &&
        ...
        {
          skip_element (data, context, BOBGUI_SVG_ERROR_INVALID_ELEMENT, "Parent element can't contain shapes");
          return;
        }
      ...
      if (shape_type != SVG_ELEMENT_USE &&
        ...
        {
          skip_element (data, context, BOBGUI_SVG_ERROR_INVALID_ELEMENT, "<clipPath> can only contain shapes, not %s", element_name);
          return;
        }
      ...
      if (data->current_shape == NULL && svg_element_get_type (shape) == SVG_ELEMENT_SVG)
        {
          ...
          if (data->svg->features & BOBGUI_SVG_EXTENSIONS)
      ...
      bobgui_svg_check_unhandled_attributes (data->svg, context, attr_names, handled);
  ...
  if (strcmp (element_name, "stop") == 0)
    {
      ...
      if (data->current_shape == NULL ||
        ...
        {
          skip_element (data, context, BOBGUI_SVG_ERROR_INVALID_ELEMENT, "<stop> only allowed in <linearGradient> or <radialGradient>");
          return;
        }
      ...
      if (filter_type == SVG_FILTER_MERGE_NODE)
        {
          if (!check_ancestors (context, "feMerge", "filter", NULL))
            {
              skip_element (data, context, BOBGUI_SVG_ERROR_INVALID_ELEMENT, "<%s> only allowed in <feMerge>", element_name);
              return;
            }
               ...
               filter_type == SVG_FILTER_FUNC_A)
        {
          if (!check_ancestors (context, "feComponentTransfer", "filter", NULL))
            {
              skip_element (data, context, BOBGUI_SVG_ERROR_INVALID_ELEMENT, "<%s> only allowed in <feComponentTransfer>", element_name);
              return;
            }
        }
      else
        {
          if (!check_ancestors (context, "filter", NULL))
            {
              skip_element (data, context, BOBGUI_SVG_ERROR_INVALID_ELEMENT, "<%s> only allowed in <filter>", element_name);
              return;
            }
        }
  ...
  if (strcmp (element_name, "rdf:RDF") == 0 ||
      ...
      strcmp (element_name, "rdf:li") == 0)
    {
      if (!has_ancestor (context, "metadata"))
        skip_element (data, context, BOBGUI_SVG_ERROR_IGNORED_ELEMENT, "Ignoring RDF elements outside <metadata>: <%s>", element_name);
      ...
      if (strcmp (element_name, "rdf:li") == 0)
        {
          ...
          if (check_ancestors (context, "rdf:Bag", "dc:subject", "cc:Work", "rdf:RDF", "metadata", NULL))
            ...
            skip_element (data, context, BOBGUI_SVG_ERROR_IGNORED_ELEMENT, "Ignoring RDF element in wrong context: <%s>", element_name);
        }
  ...
  if (strcmp (element_name, "font-face-uri") == 0)
    {
      if (check_ancestors (context, "font-face-src", "font-face", NULL))
        ...
        skip_element (data, context, BOBGUI_SVG_ERROR_IGNORED_ELEMENT, "Ignoring font element in the wrong context: <%s>", element_name);
  ...
  if (strcmp (element_name, "textPath") == 0 ||
    ...
    {
      skip_element (data, context, BOBGUI_SVG_ERROR_NOT_IMPLEMENTED, "<%s> is not supported", element_name);
      return;
    }
  ...
  if (strcmp (element_name, "title") == 0 ||
    ...
    {
      skip_element (data, context, BOBGUI_SVG_ERROR_IGNORED_ELEMENT, "Ignoring metadata and non-standard elements: <%s>", element_name);
      return;
    }
  ...
  if (strcmp (element_name, "set") == 0)
    {
      ...
      if ((data->svg->features & BOBGUI_SVG_ANIMATIONS) == 0)
        {
          skip_element (data, context, BOBGUI_SVG_ERROR_FEATURE_DISABLED, "SvgAnimations are disabled");
          return;
        }

      if (data->current_animation)
        {
          skip_element (data, context, BOBGUI_SVG_ERROR_INVALID_ELEMENT, "Nested animation elements are not allowed: <set>");
          return;
        }
      ...
      if (!parse_base_animation_attrs (a,
                                       ...
                                       context))
        {
          ...
          skip_element (data, context, BOBGUI_SVG_ERROR_INVALID_ELEMENT, "Skipping <%s> - bad attributes", element_name);
          return;
        }
      ...
      if (!to_attr)
        {
          bobgui_svg_missing_attribute (data->svg, context, "to", NULL);
          svg_animation_drop_and_free (a);
          skip_element (data, context, BOBGUI_SVG_ERROR_INVALID_ELEMENT, "Dropping <set> without 'to'");
          return;
        }
      ...
      if (error)
        {
          bobgui_svg_invalid_attribute (data->svg, context, attr_names, "to", "%s", error->message);
          g_error_free (error);
        }
      else if (!value)
        {
  ...
  if (strcmp (element_name, "animate") == 0 ||
      ...
      strcmp (element_name, "animateMotion") == 0)
    {
      ...
      if ((data->svg->features & BOBGUI_SVG_ANIMATIONS) == 0)
        {
          skip_element (data, context, BOBGUI_SVG_ERROR_IGNORED_ELEMENT, "SvgAnimations are disabled");
          return;
        }

      if (data->current_animation)
        {
          skip_element (data, context, BOBGUI_SVG_ERROR_INVALID_ELEMENT, "Nested animation elements are not allowed: <%s>", element_name);
          return;
        }
  ...
  if (strcmp (element_name, "mpath") == 0)
    {
      ...
      if (data->current_animation == NULL ||
        ...
        {
          skip_element (data, context, BOBGUI_SVG_ERROR_INVALID_ELEMENT, "<mpath> only allowed in <animateMotion>");
          return;
        }
      ...
      if (!data->current_animation->motion.path_ref)
        bobgui_svg_missing_attribute (data->svg, context, "href", NULL);

      return;
    }

  /* If we get here, its all over */
  skip_element (data, context, BOBGUI_SVG_ERROR_IGNORED_ELEMENT, "Unknown element: <%s>", element_name);
}
      ...
      if (data->skip.to == g_markup_parse_context_get_element_stack (context))
        {
          ...
          bobgui_svg_skipped_element (data->svg,
                                   parent,
                                   &data->skip.start,
                                   &end,
                                   data->skip.code,
          ...
          gpointer             user_data)
{
  ...
  bobgui_svg_markup_error (data->svg, context, error);
}
                  ...
                  ParserData *data)
{
  if (svg_clip_get_kind (value) == CLIP_URL &&
      svg_clip_get_shape (value) == NULL)
    {
      ...
      if (!target)
        bobgui_svg_invalid_reference (data->svg,
  ...
  if (svg_mask_get_kind (value) == MASK_URL && svg_mask_get_shape (value) == NULL)
    {
      ...
      if (!target)
        bobgui_svg_invalid_reference (data->svg, "No shape with ID %s (resolving mask)", id);
      else if (target->type != SVG_ELEMENT_MASK)
        bobgui_svg_invalid_reference (data->svg, "SvgElement with ID %s not a <mask> (resolving mask)", id);
      else
        {
  ...
  if (svg_element_get_type (shape) == SVG_ELEMENT_IMAGE || svg_element_get_type (shape) == SVG_ELEMENT_FILTER)
    {
      ...
      if (svg_element_get_type (shape) == SVG_ELEMENT_IMAGE)
        {
          if (g_error_matches (error, BOBGUI_SVG_ERROR, BOBGUI_SVG_ERROR_FEATURE_DISABLED))
            bobgui_svg_emit_error (data->svg, error);
          else
            bobgui_svg_invalid_reference (data->svg, "Failed to load %s (resolving <image>): %s", ref, error->message);
          g_error_free (error);
  ...
  if (svg_href_get_kind (value) != HREF_NONE && svg_href_get_shape (value) == NULL)
    {
      ...
      if (!target)
        {
          bobgui_svg_invalid_reference (data->svg, "No shape with ID %s", ref);
        }
      ...
      if (!target)
        {
          ...
          if ((data->svg->features & BOBGUI_SVG_EXTENSIONS) != 0 &&
              g_str_has_prefix (ref, "gpa:") &&
              parse_symbolic_color (ref + strlen ("gpa:"), &symbolic))
            return; /* Handled later */

          bobgui_svg_invalid_reference (data->svg, "No shape with ID %s (resolving fill or stroke)", ref);
        }
      else if (target->type != SVG_ELEMENT_LINEAR_GRADIENT &&
               target->type != SVG_ELEMENT_RADIAL_GRADIENT &&
               target->type != SVG_ELEMENT_PATTERN)
        {
          bobgui_svg_invalid_reference (data->svg, "SvgElement with ID %s not a paint server (resolving fill or stroke)", ref);
        }
      else
        {
          ...
          if (spec->type == TIME_SPEC_TYPE_SYNC && spec->sync.base == NULL)
            {
              ...
              if (!spec->sync.base)
                bobgui_svg_invalid_reference (data->svg, "No animation with ID %s", spec->sync.ref);
              else
  ...
  while (n_waiting > 0)
    {
      ...
      if (n_waiting == g_hash_table_size (waiting))
        {
          bobgui_svg_update_error (svg, "Cyclic dependency detected");
          has_cycle = TRUE;
        }
...
/* Most of this is adapted from bobguicssprovider.c */
...
static void
svg_css_scanner_parser_error (BobguiCssParser         *parser,
                              const BobguiCssLocation *css_start,
                              const BobguiCssLocation *css_end,
                              const GError         *css_error,
                              gpointer              user_data)
{
  ParserData *data = user_data;
  if (css_error->domain == BOBGUI_CSS_PARSER_ERROR)
    {
      GError *error;
      BobguiSvgLocation start, end;
      ...
      if (data->current_shape)
        {
          bobgui_svg_error_set_element (error, svg_element_type_get_name (svg_element_get_type (data->current_shape)));
          bobgui_svg_error_set_attribute (error, "style");
        }
                     ...
                     GBytes        *bytes)
{
  ...
  scanner->parser = bobgui_css_parser_new_for_bytes (bytes, file, svg_css_scanner_parser_error, data, NULL);
...
parse_import (SvgCssScanner *scanner)
{
  ...
  if (bobgui_css_parser_has_token (scanner->parser, BOBGUI_CSS_TOKEN_STRING))
    {
      ...
      if (url)
        {
          if (bobgui_css_parser_get_file (scanner->parser))
  ...
  else if ((scanner->data->svg->features & BOBGUI_SVG_EXTERNAL_RESOURCES) == 0)
    {
...
clear_selectors (SvgCssSelectors *selectors)
{
  for (int i = 0; i < svg_css_selectors_get_size (selectors); i++)
    _bobgui_css_selector_free (svg_css_selectors_get (selectors, i));
}
                     ...
                     SvgCssSelectors *selectors)
{
  do
    {
      BobguiCssSelector *selector = _bobgui_css_selector_parse (scanner->parser);
  ...
  if (lookup_property (name, &attr, &marker))
    {
      ...
      if (!bobgui_css_parser_has_token (scanner->parser, BOBGUI_CSS_TOKEN_EOF))
        {
          bobgui_css_parser_error_syntax (scanner->parser, "Junk at end of value for %s", name);
          svg_value_unref (value);
          goto out;
        }
  ...
  for (unsigned int i = 0; i < data->rulesets->len; i++)
    {
      ...
      _bobgui_css_selector_print (r->selector, s);
      g_string_append (s, " {\n");
  ...
  for (unsigned int j = 0; j < r->n_styles; j++)
    {
      ...
      if (set && _bobgui_bitmask_get (set, p->attr))
        continue;
      ...
      if (set)
        set = _bobgui_bitmask_set (set, p->attr, TRUE);
    }
}
...
{
  BobguiCssNode *node;
  BobguiCssNode *freeme = NULL;
  BobguiBitmask *set;
  const char *style = NULL;
  BobguiSvgLocation loc;
  ...
  if (idx > 0)
    ...
    {
      freeme = node = bobgui_css_node_new ();
      style = NULL;
      loc.lines = 0;
      loc.line_chars = 0;
      loc.bytes = 0;
  ...
  set = _bobgui_bitmask_new ();

  for (unsigned int i = 0; i < data->rulesets->len; i++)
    {
      SvgCssRuleset *r = &g_array_index (data->rulesets, SvgCssRuleset, i);
      if (bobgui_css_selector_matches (r->selector, node))
        apply_ruleset_to_shape (r, shape, idx, set, data);
    }

  _bobgui_bitmask_free (set);
                       ...
                       ParserData *data)
{
   ...
   */
  if (data->svg->gpa_version == 0 &&
      ((data->svg->features & BOBGUI_SVG_TRADITIONAL_SYMBOLIC) != 0))
    {
      ...
      if (!classes)
        value = svg_paint_new_symbolic (BOBGUI_SYMBOLIC_COLOR_FOREGROUND);
        ...
        value = svg_paint_new_symbolic (BOBGUI_SYMBOLIC_COLOR_SUCCESS);
      else if (g_strv_has (classes, "warning") ||
               g_strv_has (classes, "warning-fill"))
        value = svg_paint_new_symbolic (BOBGUI_SYMBOLIC_COLOR_WARNING);
      else if (g_strv_has (classes, "error") ||
               g_strv_has (classes, "error-fill"))
        value = svg_paint_new_symbolic (BOBGUI_SYMBOLIC_COLOR_ERROR);
      else
        value = svg_paint_new_symbolic (BOBGUI_SYMBOLIC_COLOR_FOREGROUND);
  ...
  if (svg_element_property_is_set (shape, SVG_PROPERTY_STROKE))
    {
      ...
      if (svg_paint_get_kind (paint) != PAINT_NONE)
        data->svg->used |= BOBGUI_SVG_USES_STROKES;
    }
}
...
static void
bobgui_svg_init_from_bytes (BobguiSvg *self,
                         GBytes *bytes)
{
  ParserData data;
  GMarkupParseContext *context;
  ...
  if ((self->features & BOBGUI_SVG_SYSTEM_RESOURCES) == 0)
    {
  ...
  for (unsigned int i = 0; i < data.pending_animations->len; i++)
    {
      ...
      if (!shape)
        {
          bobgui_svg_invalid_reference (self,
  ...
  if (self->gpa_version > 0 &&
      (self->features & BOBGUI_SVG_ANIMATIONS) == 0)
    apply_state (self, self->state);
}

static void
bobgui_svg_init_from_resource (BobguiSvg     *self,
                            const char *path)
{
  GBytes *bytes;
  ...
  if (bytes)
    {
      bobgui_svg_init_from_bytes (self, bytes);
      g_bytes_unref (bytes);
    }
}
                       ...
                       BobguiSvgSerializeFlags  flags)
{
  ...
  for (SvgProperty attr = FIRST_SHAPE_PROPERTY; attr <= LAST_SHAPE_PROPERTY; attr++)
    {
      if ((flags & BOBGUI_SVG_SERIALIZE_NO_COMPAT) == 0 &&
          svg->gpa_version > 0 &&
          ...
          attr == SVG_PROPERTY_VISIBILITY)
        {
          ...
          if (flags & BOBGUI_SVG_SERIALIZE_AT_CURRENT_TIME)
            state = svg->state;
          else
      ...
      if (svg_element_property_is_set (shape, attr) ||
          (flags & BOBGUI_SVG_SERIALIZE_AT_CURRENT_TIME))
        {
                     ...
                     BobguiSvgSerializeFlags  flags)
{
  ...
  if ((flags & BOBGUI_SVG_SERIALIZE_EXPAND_GPA_ATTRS) == 0)
    {
...
serialize_animation_status (GString              *s,
                            ...
                            BobguiSvgSerializeFlags  flags)
{
  if (flags & BOBGUI_SVG_SERIALIZE_INCLUDE_STATE)
    {
...
serialize_animation (GString              *s,
                     ...
                     BobguiSvgSerializeFlags  flags)
{
  if (flags & BOBGUI_SVG_SERIALIZE_EXCLUDE_ANIMATION)
    return;
    ...
    case FILTER_REF_STROKE_PAINT:
      {
        ...
        bobgui_snapshot_push_collect (context->snapshot);
        fill_shape (shape, path, paint, opacity, context);
        node = bobgui_snapshot_pop_collect (context->snapshot);
        ...
        case SVG_FILTER_BLUR:
          {
            ...
            if (svg_numbers_get_length (num) == 2 &&
              ...
              bobgui_svg_rendering_error (context->svg,
            ...
            switch (edge_mode)
             ...
             if (std_dev < 0)
               {
                 bobgui_svg_rendering_error (context->svg, "stdDeviation < 0");
            ...
            if (svg_href_get_kind (href) == HREF_NONE)
            ...
            else if (svg_href_get_shape (href) != NULL)
              {
                ...
                if (node)
                  ...
                  {
                    bobgui_svg_rendering_error (context->svg, "No content for <feImage>");
            ...
            PaintContext *context)
{
  ...
  if (strstr (g_getenv ("SVG_DEBUG") ?:"", "nodes"))
    {
      ...
      if (svg_element_get_id (shape))
        bobgui_snapshot_push_debug (context->snapshot, "Group for <%s id='%s'> at line %" G_GSIZE_FORMAT, svg_element_type_get_name (svg_element_get_type (shape)), svg_element_get_id (shape), loc.lines);
      else
        bobgui_snapshot_push_debug (context->snapshot, "Group for <%s> at line %" G_GSIZE_FORMAT, svg_element_type_get_name (svg_element_get_type (shape)), loc.lines);
    }
#endif
  ...
  if (svg_element_get_type (shape) == SVG_ELEMENT_SVG || svg_element_get_type (shape) == SVG_ELEMENT_SYMBOL)
    {
      ...
      if (svg_enum_get (overflow) == OVERFLOW_HIDDEN)
        bobgui_snapshot_push_clip (context->snapshot, &GRAPHENE_RECT_INIT (x, y, width, height));
      ...
      transform = gsk_transform_scale (transform, sx, sy);
      bobgui_snapshot_save (context->snapshot);
      bobgui_snapshot_transform (context->snapshot, transform);
...
#ifdef DEBUG
          if (strstr (g_getenv ("SVG_DEBUG") ?:"", "nodes"))
            bobgui_snapshot_push_debug (context->snapshot, "copy for %s", reason);
#endif
          bobgui_snapshot_push_copy (context->snapshot);
        }
...
#ifdef DEBUG
          if (strstr (g_getenv ("SVG_DEBUG") ?:"", "nodes"))
            bobgui_snapshot_push_debug (context->snapshot, "isolate for %s", reason);
#endif
          bobgui_snapshot_push_isolation (context->snapshot, GSK_ISOLATION_BACKGROUND);
        }
      ...
      if (svg_enum_get (blend) != GSK_BLEND_MODE_DEFAULT)
        {
          ...
          bobgui_snapshot_push_blend (context->snapshot, svg_enum_get (blend));
          bobgui_snapshot_append_paste (context->snapshot, &bounds, 0);
          bobgui_snapshot_pop (context->snapshot);
        }
    }
...
#ifdef DEBUG
          if (strstr (g_getenv ("SVG_DEBUG") ?:"", "nodes"))
            bobgui_snapshot_push_debug (context->snapshot, "fill or clip for clip");
#endif
      ...
      else
        {
          ...
          if (svg_transform_is_none (ctf) &&
          ...
          else
            {
#ifdef DEBUG
              if (strstr (g_getenv ("SVG_DEBUG") ?:"", "nodes"))
                bobgui_snapshot_push_debug (context->snapshot, "mask for clip");
#endif
              bobgui_snapshot_push_mask (context->snapshot, GSK_MASK_MODE_ALPHA);
              ...
              if (svg_enum_get (svg_element_get_current_value (clip_shape, SVG_PROPERTY_CONTENT_UNITS)) == COORD_UNITS_OBJECT_BOUNDING_BOX)
                {
                  ...
                  bobgui_snapshot_restore (context->snapshot);
                }
              ...
              bobgui_snapshot_pop (context->snapshot); /* mask */
            }
        }
      ...
      context->op != CLIPPING)
    {
...
#ifdef DEBUG
      if (strstr (g_getenv ("SVG_DEBUG") ?:"", "nodes"))
        bobgui_snapshot_push_debug (context->snapshot, "mask for masking");
#endif
      bobgui_snapshot_push_mask (context->snapshot, svg_enum_get (svg_element_get_current_value (mask_shape, SVG_PROPERTY_MASK_TYPE)));
      ...
      if (svg_element_property_is_set (mask_shape, SVG_PROPERTY_X) ||
          ...
          svg_element_property_is_set (mask_shape, SVG_PROPERTY_HEIGHT))
        {
          ...
          if (has_clip)
            bobgui_snapshot_push_clip (context->snapshot, &mask_clip);
        }
  ...
  if (context->op != CLIPPING && svg_element_get_type (shape) != SVG_ELEMENT_MASK)
    {
      if (svg_number_get (opacity, 1) != 1)
        bobgui_snapshot_push_opacity (context->snapshot, svg_number_get (opacity, 1));
      ...
      if (!svg_filter_functions_is_none (filter))
        {
          ...
          bobgui_snapshot_append_node (context->snapshot, node2);
      ...
      if (svg_href_get_shape (href))
        {
          ...
          bobgui_svg_invalid_reference (context->svg,
                       ...
                       PaintContext          *context)
{
  ...
  bobgui_snapshot_add_linear_gradient (context->snapshot, paint_bounds, &start, &end, g);
                       ...
                       PaintContext          *context)
{
  ...
  bobgui_snapshot_transform (context->snapshot, gradient_transform);
  ...
  if (!isnormal (gradient_bounds.size.width) ||
      !isnormal (gradient_bounds.size.height))
    {
      ...
      bobgui_snapshot_append_node (context->snapshot, node);
      gsk_render_node_unref (node);
    }
  else
    {
      bobgui_snapshot_add_radial_gradient (context->snapshot,
                                        &gradient_bounds,
               ...
               PaintContext          *context)
{
  ...
  if (!isfinite (pattern_bounds.size.width) || pattern_bounds.size.width == 0 ||
    ...
    {
      bobgui_snapshot_push_repeat (context->snapshot, &pattern_bounds, &child_bounds);
              ...
              PaintContext          *context)
{
  ...
  if (server == NULL ||
    ...
    {
      bobgui_snapshot_add_color (context->snapshot,
      ...
      if (color_stops->len == 1)
        {
          ...
          bobgui_snapshot_add_color (context->snapshot, &c, paint_bounds);
          gdk_color_finish (&c);
          return;
        }
                     ...
                     PaintContext *context)
{
  ...
  stroke = svg_element_create_basic_stroke (shape, context->viewport, context->svg->features & BOBGUI_SVG_EXTENSIONS, context->weight);
            ...
            PaintContext *context)
{
  ...
  switch (svg_paint_get_kind (paint))
    ...
    case PAINT_COLOR:
      {
        ...
        bobgui_snapshot_add_color (context->snapshot, &color, &bounds);
        bobgui_snapshot_pop (context->snapshot);
        gdk_color_finish (&color);
      }
      break;
    ...
    case PAINT_SERVER_WITH_CURRENT_COLOR:
      {
        if (opacity < 1)
          bobgui_snapshot_push_opacity (context->snapshot, opacity);
              ...
              PaintContext *context)
{
  ...
  switch (svg_paint_get_kind (paint))
    ...
    case PAINT_SERVER_WITH_CURRENT_COLOR:
      ...
      child = bobgui_snapshot_pop_collect (context->snapshot);
              ...
              VertexKind          kind)
{
  ...
  if (svg_enum_get (overflow) == OVERFLOW_HIDDEN)
    bobgui_snapshot_push_clip (context->snapshot, &GRAPHENE_RECT_INIT (0, 0, view_box.size.width, view_box.size.height));
        ...
        case TEXT_NODE_CHARACTERS:
          {
            ...
            bobgui_snapshot_append_border (context->snapshot, &brd, (const float[4]){ 3.f, 3.f, 3.f, 3.f }, (const GdkRGBA[4]){ red, red, red, red });
#endif
            ...
            if (svg_paint_get_kind (paint) == PAINT_COLOR)
              {
                ...
                gdk_color_init_copy (&color, svg_paint_get_color (paint));
                color.alpha *= opacity;
                bobgui_snapshot_translate (context->snapshot, &GRAPHENE_POINT_INIT (node->characters.x, node->characters.y));
                bobgui_snapshot_rotate (context->snapshot, node->characters.r);
                bobgui_snapshot_add_layout (context->snapshot, node->characters.layout, &color);
              }
            else if (paint_is_server (svg_paint_get_kind (paint)))
              {
                ...
                bobgui_snapshot_translate (context->snapshot, &GRAPHENE_POINT_INIT (node->characters.x, node->characters.y));
                bobgui_snapshot_rotate (context->snapshot, node->characters.r);
                bobgui_snapshot_append_layout (context->snapshot, node->characters.layout, &GDK_RGBA_BLACK);
                bobgui_snapshot_pop (context->snapshot);
                paint_server (paint, bounds, bounds, context);
                bobgui_snapshot_pop (context->snapshot);
            ...
            if (svg_paint_get_kind (paint) == PAINT_COLOR)
              {
                ...
                bobgui_snapshot_append_stroke (context->snapshot, path, stroke, &GDK_RGBA_BLACK);
                bobgui_snapshot_pop (context->snapshot);
                bobgui_snapshot_add_color (context->snapshot, &color, bounds);
                bobgui_snapshot_pop (context->snapshot);
                gsk_path_unref (path);
              ...
              PaintContext *context)
{
  ...
  bobgui_snapshot_append_texture (context->snapshot, texture, &vb);
      ...
      if (svg_href_get_shape (svg_element_get_current_value (shape, SVG_PROPERTY_HREF)) != NULL)
        {
          ...
          if (shape_is_ancestor (use_shape, shape))
            {
              bobgui_svg_rendering_error (context->svg, "not following invalid <use> href");
              return;
            }
      ...
      if (gsk_path_get_bounds (path, &bounds))
        {
          ...
          bobgui_snapshot_push_fill (context->snapshot, path, clip_rule);
          bobgui_snapshot_add_color (context->snapshot, &color, &bounds);
          bobgui_snapshot_pop (context->snapshot);
        }
    }
              ...
              PaintContext *context)
{
  ...
  if (context->instance_count++ > DRAWING_LIMIT)
    {
      bobgui_svg_rendering_error (context->svg, "excessive instance count, aborting");
      return;
    }
                ...
                double         weight)
{
  ...
  if ((self->used & BOBGUI_SVG_USES_STROKES) != 0 &&
      self->weight < 1 &&
  ...
  if ((self->features & BOBGUI_SVG_EXTENSIONS) != 0)
    {
      if ((self->used & BOBGUI_SVG_USES_SYMBOLIC_FOREGROUND) != 0 &&
           self->node_for.n_colors > BOBGUI_SYMBOLIC_COLOR_FOREGROUND &&
           n_colors > BOBGUI_SYMBOLIC_COLOR_FOREGROUND &&
           !gdk_rgba_equal (&self->node_for.colors[BOBGUI_SYMBOLIC_COLOR_FOREGROUND],
      ...
      if ((self->used & BOBGUI_SVG_USES_SYMBOLIC_ERROR) != 0 &&
          self->node_for.n_colors > BOBGUI_SYMBOLIC_COLOR_ERROR &&
          n_colors > BOBGUI_SYMBOLIC_COLOR_ERROR &&
          !gdk_rgba_equal (&self->node_for.colors[BOBGUI_SYMBOLIC_COLOR_ERROR],
      ...
      if ((self->used & BOBGUI_SVG_USES_SYMBOLIC_WARNING) != 0 &&
          self->node_for.n_colors > BOBGUI_SYMBOLIC_COLOR_WARNING &&
          n_colors > BOBGUI_SYMBOLIC_COLOR_WARNING &&
          !gdk_rgba_equal (&self->node_for.colors[BOBGUI_SYMBOLIC_COLOR_WARNING],
      ...
      if ((self->used & BOBGUI_SVG_USES_SYMBOLIC_SUCCESS) != 0 &&
          self->node_for.n_colors > BOBGUI_SYMBOLIC_COLOR_SUCCESS &&
          n_colors > BOBGUI_SYMBOLIC_COLOR_SUCCESS &&
          !gdk_rgba_equal (&self->node_for.colors[BOBGUI_SYMBOLIC_COLOR_SUCCESS],
      ...
      if ((self->used & BOBGUI_SVG_USES_SYMBOLIC_ACCENT) != 0 &&
          self->node_for.n_colors > BOBGUI_SYMBOLIC_COLOR_ACCENT &&
          n_colors > BOBGUI_SYMBOLIC_COLOR_ACCENT &&
          !gdk_rgba_equal (&self->node_for.colors[BOBGUI_SYMBOLIC_COLOR_ACCENT],
                          &colors[BOBGUI_SYMBOLIC_COLOR_ACCENT]))
        {
 ...
 */
static void
bobgui_svg_snapshot_with_weight (BobguiSymbolicPaintable  *paintable,
                              BobguiSnapshot           *snapshot,
                              double                 width,
                              double                 height,
                              const GdkRGBA         *colors,
                              ...
                              double                 weight)
{
  BobguiSvg *self = BOBGUI_SVG (paintable);
  ...
  if (self->load_time == INDEFINITE)
    {
      ...
      bobgui_svg_set_load_time (self, current_time);
    }
  ...
  if (!can_reuse_node (self, width, height, colors, n_colors, weight))
    {
       ...
       */
      if (self->gpa_version == 0 &&
          (self->features & BOBGUI_SVG_TRADITIONAL_SYMBOLIC) != 0 &&
          colors[BOBGUI_SYMBOLIC_COLOR_FOREGROUND].alpha < 1)
        {
          used_opacity = colors[BOBGUI_SYMBOLIC_COLOR_FOREGROUND].alpha;
      ...
      if (self->gpa_version == 0 &&
          ...
          used_opacity < 1)
        {
          bobgui_snapshot_push_opacity (snapshot, used_opacity);
        }
      ...
      if (self->overflow == BOBGUI_OVERFLOW_HIDDEN)
      ...
      self->node = bobgui_snapshot_pop_collect (snapshot);
...
static void
bobgui_svg_snapshot_symbolic (BobguiSymbolicPaintable *paintable,
                           BobguiSnapshot          *snapshot,
                           double                width,
                           double                height,
                           const GdkRGBA        *colors,
                           size_t                 n_colors)
{
  bobgui_svg_snapshot_with_weight (paintable,
                                snapshot,
                                width, height,
                                colors, n_colors,
                                400);
...
static void
bobgui_svg_init_symbolic_paintable_interface (BobguiSymbolicPaintableInterface *iface)
{
  iface->snapshot_symbolic = bobgui_svg_snapshot_symbolic;
  iface->snapshot_with_weight = bobgui_svg_snapshot_with_weight;
}
...
static void
bobgui_svg_snapshot (GdkPaintable *paintable,
                  BobguiSnapshot  *snapshot,
                  double        width,
                  double        height)
{
  bobgui_svg_snapshot_symbolic (BOBGUI_SYMBOLIC_PAINTABLE (paintable),
                             snapshot,
                             width, height,
                             NULL, 0);
}
...
static int
bobgui_svg_get_intrinsic_width (GdkPaintable *paintable)
{
  BobguiSvg *self = BOBGUI_SVG (paintable);
...
static int
bobgui_svg_get_intrinsic_height (GdkPaintable *paintable)
{
  BobguiSvg *self = BOBGUI_SVG (paintable);
...
static double
bobgui_svg_get_intrinsic_aspect_ratio (GdkPaintable *paintable)
{
  BobguiSvg *self = BOBGUI_SVG (paintable);
  SvgValue *vb;
  graphene_rect_t view_box;
...
static void
bobgui_svg_init_paintable_interface (GdkPaintableInterface *iface)
{
  iface->snapshot = bobgui_svg_snapshot;
  iface->get_intrinsic_width = bobgui_svg_get_intrinsic_width;
  iface->get_intrinsic_height = bobgui_svg_get_intrinsic_height;
  iface->get_intrinsic_aspect_ratio = bobgui_svg_get_intrinsic_aspect_ratio;
}

/* }}} */
/* {{{ GObject boilerplate */

struct _BobguiSvgClass
{
...
G_DEFINE_TYPE_WITH_CODE (BobguiSvg, bobgui_svg, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (GDK_TYPE_PAINTABLE,
                                                bobgui_svg_init_paintable_interface)
                         G_IMPLEMENT_INTERFACE (BOBGUI_TYPE_SYMBOLIC_PAINTABLE,
                                                bobgui_svg_init_symbolic_paintable_interface))

static void
bobgui_svg_init (BobguiSvg *self)
{
  self->weight = -1;
  self->overflow = BOBGUI_OVERFLOW_HIDDEN;
  ...
  self->playing = FALSE;
  self->run_mode = BOBGUI_SVG_RUN_MODE_STOPPED;

  self->features = BOBGUI_SVG_DEFAULT_FEATURES;
...
static void
bobgui_svg_dispose (GObject *object)
{
  BobguiSvg *self = BOBGUI_SVG (object);
  ...
  G_OBJECT_CLASS (bobgui_svg_parent_class)->dispose (object);
}

static void
bobgui_svg_get_property (GObject      *object,
...
static void
bobgui_svg_set_property (GObject      *object,
                      unsigned int  property_id,
                      const GValue *value,
                      GParamSpec   *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_RESOURCE:
      bobgui_svg_load_from_resource (self, g_value_get_string (value));
      break;

    case PROP_FEATURES:
      bobgui_svg_set_features (self, g_value_get_flags (value));
      break;

    case PROP_PLAYING:
      bobgui_svg_set_playing (self, g_value_get_boolean (value));
      break;

    case PROP_STATE:
      bobgui_svg_set_state (self, g_value_get_uint (value));
      break;

    case PROP_WEIGHT:
      bobgui_svg_set_weight (self, g_value_get_double (value));
      break;

    case PROP_OVERFLOW:
      bobgui_svg_set_overflow (self, g_value_get_enum (value));
      break;
...
static void
bobgui_svg_class_init (BobguiSvgClass *class)
{
  ...
  object_class->dispose = bobgui_svg_dispose;
  object_class->get_property = bobgui_svg_get_property;
  object_class->set_property = bobgui_svg_set_property;
  ...
  properties[PROP_FEATURES] =
    g_param_spec_flags ("features", NULL, NULL,
                        BOBGUI_TYPE_SVG_FEATURES, BOBGUI_SVG_DEFAULT_FEATURES,
                        G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY);
   ...
   *
   * To set this property, use the [method@Bobgui.Svg.play] and
   * [method@Bobgui.Svg.pause] functions.
   *
   * Since: 4.22
   */
  ...
  properties[PROP_OVERFLOW] =
    g_param_spec_enum ("overflow", NULL, NULL,
                       BOBGUI_TYPE_OVERFLOW, BOBGUI_OVERFLOW_HIDDEN,
                       G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY);
   ...
   *
   * The expected error values are in the [error@Bobgui.SvgError] enumeration,
   * context information about the location of parsing errors can
   * be obtained with the various `bobgui_svg_error` functions.
   *
...
static void
animation_state_dump (BobguiSvg *self)
{
...
BobguiSvg *
bobgui_svg_copy (BobguiSvg *orig)
{
  GBytes *bytes = NULL;
  BobguiSvg *svg;

  bytes = bobgui_svg_serialize_full (orig, NULL, 0, BOBGUI_SVG_SERIALIZE_DEFAULT);
  svg = bobgui_svg_new_from_bytes (bytes);
  g_bytes_unref (bytes);

  bobgui_svg_set_weight (svg, bobgui_svg_get_weight (orig));
  bobgui_svg_set_state (svg, bobgui_svg_get_state (orig));

  return svg;
}

gboolean
bobgui_svg_equal (BobguiSvg *svg1,
               BobguiSvg *svg2)
{
  if (svg1 == svg2)
    return TRUE;
...
static void
collect_next_update (BobguiSvg *self)
{
  BobguiSvgRunMode run_mode = BOBGUI_SVG_RUN_MODE_STOPPED;
  ...
  if (strstr (g_getenv ("SVG_DEBUG") ?: "", "continuous"))
    run_mode = BOBGUI_SVG_RUN_MODE_CONTINUOUS;
#endif
...
static void
invalidate_for_next_update (BobguiSvg *self)
{
  if (self->next_update <= self->current_time ||
      self->run_mode == BOBGUI_SVG_RUN_MODE_CONTINUOUS)
    {
...
static void
update_animation_state (BobguiSvg *self)
{
  shape_update_animation_state (self->content, self->current_time);
}

/*< private>
 * bobgui_svg_set_load_time:
 ...
 */
void
bobgui_svg_set_load_time (BobguiSvg  *self,
                       int64_t  load_time)
{
  g_return_if_fail (BOBGUI_IS_SVG (self));
 ...
 * clock, via [method@Bobgui.Svg.play].
 *
 ...
 */
void
bobgui_svg_advance (BobguiSvg  *self,
                 int64_t  current_time)
{
  g_return_if_fail (BOBGUI_IS_SVG (self));
  g_return_if_fail (self->load_time < INDEFINITE);
 ...
 * timeout. See [method@Bobgui.Svg.get_next_update] for
 * determining how long the timeout should be.
 *
 * Note that this function is only useful when *not*
 * running the animations automatically via [method@Bobgui.Svg.play].
 *
 * Since: 4.22
 */
BobguiSvgRunMode
bobgui_svg_get_run_mode (BobguiSvg *self)
{
  return self->run_mode;
}

/*< private >
 * bobgui_svg_get_next_update:
 * @self: an SVG paintable
 *
 ...
 */
int64_t
bobgui_svg_get_next_update (BobguiSvg *self)
{
...
/* copied from bobguisymbolicpaintable.c */
            ...
            unsigned int   n_colors)
{
  GdkRGBA default_colors[5] = {
    [BOBGUI_SYMBOLIC_COLOR_FOREGROUND] = { 0.745, 0.745, 0.745, 1.0 },
    [BOBGUI_SYMBOLIC_COLOR_ERROR] = { 0.797, 0, 0, 1.0 },
    [BOBGUI_SYMBOLIC_COLOR_WARNING] = { 0.957, 0.473, 0.242, 1.0 },
    [BOBGUI_SYMBOLIC_COLOR_SUCCESS] = { 0.305, 0.602, 0.023, 1.0 },
    [BOBGUI_SYMBOLIC_COLOR_ACCENT] = { 0.208, 0.518, 0.894, 1.0 },
  };
 ...
 * [method@Bobgui.Svg.advance] to reproduce content
 ...
 */
GBytes *
bobgui_svg_serialize_full (BobguiSvg               *self,
                        const GdkRGBA        *colors,
                        size_t                n_colors,
                        BobguiSvgSerializeFlags  flags)
{
  ...
  if (self->gpa_version > 0 || (flags & BOBGUI_SVG_SERIALIZE_INCLUDE_STATE))
    {
      string_indent (s, ATTR_INDENT);
      g_string_append (s, "xmlns:gpa='https://www.bobgui.org/grappa'");
      string_indent (s, ATTR_INDENT);
 ...
 */
void
bobgui_svg_set_playing (BobguiSvg   *self,
                     gboolean  playing)
{
  int64_t current_time;
...
/*< private >
 * bobgui_svg_clear_content:
 * @self: an SVG paintable
 *
 * Resets the paintable to the initial, empty state.
 */
void
bobgui_svg_clear_content (BobguiSvg *self)
{
  g_clear_pointer (&self->timeline, timeline_free);
  g_clear_pointer (&self->content, svg_element_free);
  g_clear_pointer (&self->images, g_hash_table_unref);
...
GskRenderNode *
bobgui_svg_apply_filter (BobguiSvg                *svg,
                      const char            *filter_id,
                      const graphene_rect_t *bounds,
                      GskRenderNode         *source)
{
  ...
  paint_context.viewport_stack = NULL;
  paint_context.snapshot = bobgui_snapshot_new ();
  ...
  node = bobgui_snapshot_free_to_node (paint_context.snapshot);
  g_assert (node == NULL);
 ...
 */
gboolean
bobgui_svg_set_state_names (BobguiSvg      *svg,
                         const char **names)
{
...
bobgui_svg_new (void)
{
  return g_object_new (BOBGUI_TYPE_SVG, NULL);
}

/**
 * bobgui_svg_new_from_bytes:
 * @bytes: the data
 *
 ...
 */
BobguiSvg *
bobgui_svg_new_from_bytes (GBytes *bytes)
{
  BobguiSvg *self = g_object_new (BOBGUI_TYPE_SVG, NULL);
 ...
 */
BobguiSvg *
bobgui_svg_new_from_resource (const char *path)
{
  BobguiSvg *self = g_object_new (BOBGUI_TYPE_SVG, NULL);

  bobgui_svg_init_from_resource (self, path);
 ...
 * connect to the [signal@Bobgui.Svg::error] signal.
 *
 ...
 */
void
bobgui_svg_load_from_bytes (BobguiSvg *self,
                         GBytes *bytes)
{
  g_return_if_fail (BOBGUI_IS_SVG (self));

  bobgui_svg_set_playing (self, FALSE);
  bobgui_svg_clear_content (self);
 ...
 */
void
bobgui_svg_load_from_resource (BobguiSvg     *self,
                            const char *path)
{
  g_return_if_fail (BOBGUI_IS_SVG (self));
 ...
 */
GBytes *
bobgui_svg_serialize (BobguiSvg *self)
{
  return bobgui_svg_serialize_full (self, NULL, 0, BOBGUI_SVG_SERIALIZE_DEFAULT);
}

/**
 * bobgui_svg_write_to_file:
 * @self: an SVG paintable
 ...
 */
gboolean
bobgui_svg_write_to_file (BobguiSvg      *self,
                       const char  *filename,
                       GError     **error)
{
  GBytes *bytes;
  ...
  bytes = bobgui_svg_serialize (self);
 ...
 */
void
bobgui_svg_set_weight (BobguiSvg *self,
                    double  weight)
{
  g_return_if_fail (BOBGUI_IS_SVG (self));
 ...
 */
double
bobgui_svg_get_weight (BobguiSvg *self)
{
  g_return_val_if_fail (BOBGUI_IS_SVG (self), -1);

  return self->weight;
}

/**
 * bobgui_svg_set_state:
 * @self: an SVG paintable
 ...
 */
void
bobgui_svg_set_state (BobguiSvg       *self,
                   unsigned int  state)
{
  unsigned int previous_state;
 ...
 */
unsigned int
bobgui_svg_get_state (BobguiSvg *self)
{
  g_return_val_if_fail (BOBGUI_IS_SVG (self), -1);
 ...
 * `BobguiSvg` is cleared or reloaded, so if you
 ...
 */
const char **
bobgui_svg_get_state_names (BobguiSvg       *self,
                         unsigned int *length)
{
  g_return_val_if_fail (BOBGUI_IS_SVG (self), NULL);
 ...
 */
void
bobgui_svg_set_features (BobguiSvg         *self,
 ...
 */
BobguiSvgFeatures
bobgui_svg_get_features (BobguiSvg *self)
{
  g_return_val_if_fail (BOBGUI_IS_SVG (self), BOBGUI_SVG_DEFAULT_FEATURES);

  return self->features;
}

/**
 * bobgui_svg_set_overflow:
 * @self: an SVG paintable
 ...
 * changed when using a `BobguiSvg` as a paintable.
 *
 * Since: 4.24
 */
void
bobgui_svg_set_overflow (BobguiSvg      *self,
 ...
 */
BobguiOverflow
bobgui_svg_get_overflow (BobguiSvg *self)
{
  g_return_val_if_fail (BOBGUI_IS_SVG (self), BOBGUI_OVERFLOW_HIDDEN);
...
/**
 * bobgui_svg_set_frame_clock:
 * @self: an SVG paintable
 * @clock: the frame clock
 *
 * Sets a frame clock.
 *
 * Without a frame clock, BobguiSvg will not advance animations.
 *
 * Since: 4.22
 */
void
bobgui_svg_set_frame_clock (BobguiSvg        *self,
                         GdkFrameClock *clock)
{
  g_return_if_fail (BOBGUI_IS_SVG (self));
  ...
  if (self->clock && self->playing)
    {
      if (self->load_time == INDEFINITE)
        bobgui_svg_set_load_time (self, gdk_frame_clock_get_frame_time (clock));
      else
        bobgui_svg_advance (self, gdk_frame_clock_get_frame_time (clock));
    }
 ...
 */
void
bobgui_svg_play (BobguiSvg *self)
{
  g_return_if_fail (BOBGUI_IS_SVG (self));

  bobgui_svg_set_playing (self, TRUE);
}
 ...
 */
void
bobgui_svg_pause (BobguiSvg *self)
{
  g_return_if_fail (BOBGUI_IS_SVG (self));
