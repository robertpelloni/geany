...
gboolean
svg_number_parse2 (BobguiCssParser        *parser,
                   double               min,
                   double               max,
                   ...
                   SvgUnit             *u)
{
      ...
      if (svg_unit_dimension (i) == SVG_DIMENSION_LENGTH)
        {
          if (flags & SVG_PARSE_LENGTH)
            ...
            {
              bobgui_css_parser_error_value (parser, "Lengths are not allowed here");
              return FALSE;
            }
        }
      else if (svg_unit_dimension (i) == SVG_DIMENSION_TIME)
        {
          if (flags & SVG_PARSE_TIME)
            ...
            {
              bobgui_css_parser_error_value (parser, "Times are not allowed here");
              return FALSE;
            }
        }
      else if (svg_unit_dimension (i) == SVG_DIMENSION_ANGLE)
        {
          if (flags & SVG_PARSE_ANGLE)
            ...
            {
              bobgui_css_parser_error_value (parser, "Angles are not allowed here");
              return FALSE;
            }
        }
      else
  ...
  if (number < min || number > max)
    {
      bobgui_css_parser_error_value (parser, "Out of range");
      return FALSE;
    }
...
SvgValue *
svg_number_parse (BobguiCssParser        *parser,
                  double               min,
                  double               max,
                  SvgNumberParseFlags  flags)
{
