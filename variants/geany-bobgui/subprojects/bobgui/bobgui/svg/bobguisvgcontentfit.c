...
SvgValue *
svg_content_fit_parse (BobguiCssParser *parser)
{
  Align align_x;
  Align align_y;
  MeetOrSlice meet;
  ...
  if (bobgui_css_parser_try_ident (parser, "xMinYMin"))
    {
      align_x = ALIGN_MIN;
      align_y = ALIGN_MIN;
    }
  else if (bobgui_css_parser_try_ident (parser, "xMinYMid"))
    {
      align_x = ALIGN_MIN;
      align_y = ALIGN_MID;
    }
  else if (bobgui_css_parser_try_ident (parser, "xMinYMax"))
    {
      align_x = ALIGN_MIN;
      align_y = ALIGN_MAX;
    }
  else if (bobgui_css_parser_try_ident (parser, "xMidYMin"))
    {
      align_x = ALIGN_MID;
      align_y = ALIGN_MIN;
    }
  else if (bobgui_css_parser_try_ident (parser, "xMidYMid"))
    {
      align_x = ALIGN_MID;
      align_y = ALIGN_MID;
    }
  else if (bobgui_css_parser_try_ident (parser, "xMidYMax"))
    {
      align_x = ALIGN_MID;
      align_y = ALIGN_MAX;
    }
  else if (bobgui_css_parser_try_ident (parser, "xMaxYMin"))
    {
      align_x = ALIGN_MAX;
      align_y = ALIGN_MIN;
    }
  else if (bobgui_css_parser_try_ident (parser, "xMaxYMid"))
    {
      align_x = ALIGN_MAX;
      align_y = ALIGN_MID;
    }
  else if (bobgui_css_parser_try_ident (parser, "xMaxYMax"))
    {
      align_x = ALIGN_MAX;
      align_y = ALIGN_MAX;
    }
  ...
  if (!bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_EOF))
    {
      if (bobgui_css_parser_try_ident (parser, "meet"))
        meet = MEET;
      else if (bobgui_css_parser_try_ident (parser, "slice"))
        meet = SLICE;
      else
        return NULL;
    }
