...
static SvgColor static_color_values[] = {
  { { &SVG_COLOR_CLASS, 0 }, .kind = COLOR_SYMBOLIC, .symbolic = BOBGUI_SYMBOLIC_COLOR_FOREGROUND, },
  { { &SVG_COLOR_CLASS, 0 }, .kind = COLOR_SYMBOLIC, .symbolic = BOBGUI_SYMBOLIC_COLOR_ERROR, },
  { { &SVG_COLOR_CLASS, 0 }, .kind = COLOR_SYMBOLIC, .symbolic = BOBGUI_SYMBOLIC_COLOR_WARNING, },
  { { &SVG_COLOR_CLASS, 0 }, .kind = COLOR_SYMBOLIC, .symbolic = BOBGUI_SYMBOLIC_COLOR_SUCCESS, },
  { { &SVG_COLOR_CLASS, 0 }, .kind = COLOR_SYMBOLIC, .symbolic = BOBGUI_SYMBOLIC_COLOR_ACCENT, },
...
SvgValue *
svg_color_new_symbolic (BobguiSymbolicColor symbolic)
{
...
static const char *symbolic_system_color_names[] = {
    [BOBGUI_SYMBOLIC_COLOR_FOREGROUND] = "SymbolicForeground",
    [BOBGUI_SYMBOLIC_COLOR_ERROR] = "SymbolicError",
    [BOBGUI_SYMBOLIC_COLOR_WARNING] = "SymbolicWarning",
    [BOBGUI_SYMBOLIC_COLOR_SUCCESS] = "SymbolicSuccess",
    [BOBGUI_SYMBOLIC_COLOR_ACCENT] = "SymbolicAccent",
  };

const char *
symbolic_system_color (BobguiSymbolicColor sym)
{ 
  return symbolic_system_color_names[sym];
};

SvgValue *
svg_color_parse (BobguiCssParser *parser)
{
  GdkRGBA rgba;
  ...
  for (unsigned int i = 0; i < G_N_ELEMENTS (symbolic_system_color_names); i++)
    if (bobgui_css_parser_try_ident (parser, symbolic_system_color_names[i]))
      ...
      if ((context->svg->features & BOBGUI_SVG_EXTENSIONS) != 0)
        {
          if (color->symbolic < context->n_colors)
            ...
            return svg_color_new_rgba (&context->colors[BOBGUI_SYMBOLIC_COLOR_FOREGROUND]);
          else
            return svg_color_new_black ();
        }
      else
