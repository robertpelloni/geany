                    ...
                    BobguiCssParser  *parser)
{
  for (unsigned int i = 0; i < n_values; i++)
    {
      if (values[i].name &&
          bobgui_css_parser_try_ident (parser, values[i].name))
...
SvgValue * \
svg_ ## class_name ## _parse (BobguiCssParser *parser) \
{ \
  SvgValue *value; \
\
  value = svg_enum_try_parse (class_name ## _values, \
                              ...
                              parser); \
  if (value == NULL) \
    bobgui_css_parser_error_syntax (parser, "Unknown #EnumType value"); \
\
  return value; \
}
...
SvgValue *
svg_visibility_parse (BobguiCssParser *parser)
{
  if (bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_IDENT))
    {
      ...
      if (strcmp (bobgui_css_token_get_string (token), "hidden") == 0 ||
          strcmp (bobgui_css_token_get_string (token), "collapse") == 0)
...
SvgValue *
svg_display_parse (BobguiCssParser *parser)
{
  if (bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_IDENT))
    {
      const BobguiCssToken *token = bobgui_css_parser_get_token (parser);
      ...
      if (strcmp (bobgui_css_token_get_string (token), "none") == 0)
...
SvgValue *
svg_paint_order_parse (BobguiCssParser *parser)
{
  if (bobgui_css_parser_try_ident (parser, "normal"))
    return svg_paint_order_new (PAINT_ORDER_FILL_STROKE_MARKERS);

  if (bobgui_css_parser_try_ident (parser, "fill"))
    {
      bobgui_css_parser_skip_whitespace (parser);
      if (bobgui_css_parser_try_ident (parser, "stroke"))
        {
          bobgui_css_parser_skip_whitespace (parser);
          bobgui_css_parser_try_ident (parser, "markers");
          return svg_paint_order_new (PAINT_ORDER_FILL_STROKE_MARKERS);
        }
      else if (bobgui_css_parser_try_ident (parser, "markers"))
        {
          bobgui_css_parser_skip_whitespace (parser);
          bobgui_css_parser_try_ident (parser, "stroke");
  ...
  else if (bobgui_css_parser_try_ident (parser, "stroke"))
    {
      bobgui_css_parser_skip_whitespace (parser);
      if (bobgui_css_parser_try_ident (parser, "fill"))
        {
        ...
        {
          bobgui_css_parser_skip_whitespace (parser);
          bobgui_css_parser_try_ident (parser, "fill");
  ...
  bobgui_css_parser_error_syntax (parser, "Expected a paint order value");
  return NULL;
}
