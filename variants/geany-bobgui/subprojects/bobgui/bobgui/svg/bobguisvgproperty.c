...
#include "bobguisvgenumsprivate.h"
...
static SvgValue *
parse_points (BobguiCssParser *parser)
{
  if (bobgui_css_parser_try_ident (parser, "none"))
  ...
  else
    {
      ...
      if (p != NULL && svg_numbers_get_length (p) % 2 == 1)
        {
          bobgui_css_parser_error_syntax (parser, "Odd number of coordinates");
...
static SvgValue *
parse_language (BobguiCssParser *parser)
{
  if (bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_IDENT))
    {
      ...
      lang = pango_language_from_string (bobgui_css_token_get_string (token));
...
static SvgValue *
parse_custom_ident (BobguiCssParser *parser)
{
  if (bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_IDENT))
    {
      const BobguiCssToken *token = bobgui_css_parser_get_token (parser);
      const char *string = bobgui_css_token_get_string (token);
      SvgValue *value;

      if (strcmp (string, "initial") == 0 ||
          strcmp (string, "inherited") == 0)
        {
          bobgui_css_parser_error_value (parser, "Can't use %s here", string);
          return NULL;
        }
...
static SvgValue *
parse_language_list (BobguiCssParser *parser)
{
  GPtrArray *langs;
  SvgValue *value;
  ...
  if (strcmp (string, "none") == 0)
  ...
  else
    {
      ...
      if (!svg_path_data_parse_full (string, &data))
        g_set_error (error,
                     BOBGUI_CSS_PARSER_ERROR, BOBGUI_CSS_PARSER_ERROR_UNKNOWN_VALUE,
...
static SvgValue *
parse_font_size (BobguiCssParser *parser)
{
  SvgValue *v;
...
static SvgValue *
parse_letter_spacing (BobguiCssParser *parser)
{
...
static SvgValue *
parse_offset (BobguiCssParser *parser)
{
  return svg_number_parse (parser, -DBL_MAX, DBL_MAX, SVG_PARSE_NUMBER|SVG_PARSE_PERCENTAGE);
}

static SvgValue *
parse_ref_x (BobguiCssParser *parser)
{
  if (bobgui_css_parser_try_ident (parser, "left"))
    return svg_percentage_new (0);
  else if (bobgui_css_parser_try_ident (parser, "center"))
...
static SvgValue *
parse_ref_y (BobguiCssParser *parser)
{
  if (bobgui_css_parser_try_ident (parser, "top"))
    return svg_percentage_new (0);
  else if (bobgui_css_parser_try_ident (parser, "center"))
    return svg_percentage_new (50);
...
static SvgValue *
parse_length_percentage_or_auto (BobguiCssParser *parser)
{
...
static SvgValue *
parse_number_optional_number (BobguiCssParser *parser)
{
  SvgValue *values;
...
static SvgValue *
parse_transform_origin (BobguiCssParser *parser)
{
  double d[2];
  SvgUnit u[2];
...
static SvgValue *
parse_font_family (BobguiCssParser *parser)
{
  ...
  while (1)
    {
      ...
      if (bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_IDENT))
        {
          ...
          g_string_append (string, bobgui_css_token_get_string (token));
          bobgui_css_parser_skip (parser);
      ...
      else if (bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_STRING))
        {
          token = bobgui_css_parser_get_token (parser);
          g_strv_builder_add (builder, bobgui_css_token_get_string (token));
          bobgui_css_parser_skip (parser);
        }
...
static SvgValue *
marker_ref_parse (BobguiCssParser *parser)
{
  if (bobgui_css_parser_try_ident (parser, "none"))
    {
  ...
  bobgui_css_parser_error_syntax (parser, "Expected a marker reference");
  return NULL;
}
...
typedef struct
{
  ...
  SvgValue * (* parse_value)        (BobguiCssParser    *parser);
...
static void
parse_value_error (BobguiCssParser         *parser,
                    ...
                    GError      **error)
{
  ...
  parser = bobgui_css_parser_new_for_bytes (bytes, NULL, parse_value_error, error, NULL);
  g_bytes_unref (bytes);

  bobgui_css_parser_skip_whitespace (parser);
  if (bobgui_css_parser_try_ident (parser, "inherit"))
    value = svg_inherit_new ();
  else if (bobgui_css_parser_try_ident (parser, "initial"))
  ...
  bobgui_css_parser_skip_whitespace (parser);
  if (value && !bobgui_css_parser_has_token (parser, BOBGUI_CSS_TOKEN_EOF))
    {
      g_clear_pointer (&value, svg_value_unref);
      g_clear_error (error);
      g_set_error (error, BOBGUI_SVG_ERROR, BOBGUI_SVG_ERROR_INVALID_SYNTAX, "Junk at end of value");
    }
  ...
  if (value && !shape_attr_value_is_valid (attr, value))
    {
      g_clear_pointer (&value, svg_value_unref);
      g_set_error (error,
                   BOBGUI_SVG_ERROR, BOBGUI_SVG_ERROR_INVALID_ATTRIBUTE,
                   "Value is not valid");
    }
