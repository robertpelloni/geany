                ...
                size_t   n_colors)
{
  ...
  bobgui_symbolic_paintable_snapshot_symbolic (BOBGUI_SYMBOLIC_PAINTABLE (svg),
                                            snapshot,
                                            width, height,
                                            colors, n_colors);
...
static void
svg_load_error (BobguiSvg       *svg,
                const GError *error,
                gpointer      user_data)
{
  char **unsupported = user_data;

  if (g_error_matches (error, BOBGUI_SVG_ERROR, BOBGUI_SVG_ERROR_NOT_IMPLEMENTED))
    {
...
static GdkTexture *
load_symbolic_svg (BobguiSvg      *svg,
                   int          width,
                   int          height,
                   const char  *fg_color,
                   ...
                   GError     **error)
{
  ...
  gdk_rgba_parse (&colors[BOBGUI_SYMBOLIC_COLOR_FOREGROUND], fg_color);
  gdk_rgba_parse (&colors[BOBGUI_SYMBOLIC_COLOR_SUCCESS], success_color);
  gdk_rgba_parse (&colors[BOBGUI_SYMBOLIC_COLOR_WARNING], warning_color);
  gdk_rgba_parse (&colors[BOBGUI_SYMBOLIC_COLOR_ERROR], error_color);
      ...
      if (unsupported)
        {
          ...
          if (paintable)
            {
              BOBGUI_DEBUG (ICONFALLBACK, "Falling back to a texture for %s: %s", path, unsupported);
              g_clear_object (&svg);
              g_free (unsupported);
            }
          else
            {
              BOBGUI_DEBUG (ICONFALLBACK, "Expect misrendering; %s uses unsupported features: %s", path, unsupported);
              g_free (unsupported);
