...
struct _BobguiSearchEngineTracker3
{
...
struct _BobguiSearchEngineTracker3Class
{
...
typedef struct
{
  ...
  BobguiSearchEngineTracker3 *engine;
  gboolean got_results;
} CursorData;

static void bobgui_search_engine_tracker3_initable_iface_init (GInitableIface *iface);

G_DEFINE_TYPE_WITH_CODE (BobguiSearchEngineTracker3,
                         bobgui_search_engine_tracker3,
                         BOBGUI_TYPE_SEARCH_ENGINE,
                         G_IMPLEMENT_INTERFACE (G_TYPE_INITABLE,
                                                bobgui_search_engine_tracker3_initable_iface_init))
...
finalize (GObject *object)
{
  ...
  g_debug ("Finalizing BobguiSearchEngineTracker3");

  engine = BOBGUI_SEARCH_ENGINE_TRACKER3 (object);
  ...
  G_OBJECT_CLASS (bobgui_search_engine_tracker3_parent_class)->finalize (object);
}

static void
free_hit (gpointer data)
{
  BobguiSearchHit *hit = data;
...
handle_cursor_idle_cb (gpointer user_data)
{
  ...
  BobguiSearchEngineTracker3 *engine = data->engine;
  ...
  _bobgui_search_engine_hits_added (BOBGUI_SEARCH_ENGINE (engine), hits);
...
cursor_data_free (gpointer user_data)
{
  ...
  _bobgui_search_engine_finished (BOBGUI_SEARCH_ENGINE (data->engine),
                               data->got_results);
                ...
                gpointer                user_data)
{
  ...
  engine = BOBGUI_SEARCH_ENGINE_TRACKER3 (user_data);
  ...
  if (!cursor)
    {
      _bobgui_search_engine_error (BOBGUI_SEARCH_ENGINE (engine), error->message);
      g_error_free (error);
...
static void
bobgui_search_engine_tracker3_start (BobguiSearchEngine *engine)
{
  BobguiSearchEngineTracker3 *tracker;
  TrackerSparqlStatement *statement;
  const char *search_text;
  char *match;
  GFile *location;

  tracker = BOBGUI_SEARCH_ENGINE_TRACKER3 (engine);
  ...
  if (tracker->query == NULL)
    {
      g_debug ("Attempt to start a new search with no BobguiQuery, doing nothing");
      return;
    }

  search_text = bobgui_query_get_text (tracker->query);
  location = bobgui_query_get_location (tracker->query);
...
static void
bobgui_search_engine_tracker3_stop (BobguiSearchEngine *engine)
{
  BobguiSearchEngineTracker3 *tracker;
...
static void
bobgui_search_engine_tracker3_set_query (BobguiSearchEngine *engine,
                                      BobguiQuery        *query)
{
  BobguiSearchEngineTracker3 *tracker;
...
static void
bobgui_search_engine_tracker3_class_init (BobguiSearchEngineTracker3Class *class)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (class);
  BobguiSearchEngineClass *engine_class = BOBGUI_SEARCH_ENGINE_CLASS (class);

  gobject_class->finalize = finalize;

  engine_class->set_query = bobgui_search_engine_tracker3_set_query;
  engine_class->start = bobgui_search_engine_tracker3_start;
  engine_class->stop = bobgui_search_engine_tracker3_stop;
}

static void
bobgui_search_engine_tracker3_init (BobguiSearchEngineTracker3 *engine)
{
...
static gboolean
bobgui_search_engine_tracker3_initable_init (GInitable     *initable,
                                          GCancellable  *cancellable,
                                          GError       **error)
{
  BobguiSearchEngineTracker3 *engine;

  engine = BOBGUI_SEARCH_ENGINE_TRACKER3 (initable);
...
static void
bobgui_search_engine_tracker3_initable_iface_init (GInitableIface *iface)
{
  iface->init = bobgui_search_engine_tracker3_initable_init;
}

BobguiSearchEngine *
bobgui_search_engine_tracker3_new (void)
{
  BobguiSearchEngineTracker3 *engine;
  GError *error = NULL;

  g_debug ("Creating BobguiSearchEngineTracker3...");

  engine = g_initable_new (BOBGUI_TYPE_SEARCH_ENGINE_TRACKER3,
                           NULL, &error, NULL);
  if (!engine)
    {
