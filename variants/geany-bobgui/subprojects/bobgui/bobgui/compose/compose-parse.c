 ...
 * character data, and definitions for the builtin compose table of BOBGUI.
 * Run it like this:
 *
 *   cpp -DXCOMM='#' Compose.pre | sed -e 's/^ *#/#/' > Compose
 *   compose-parse Compose sequences-little-endian sequences-big-endian chars bobguicomposedata.h
 *
 * The BOBGUI build expects the output files to be in the source tree, in
 * the bobgui/compose directory.
 */
int
main (int argc, char *argv[])
{
  ...
  table = bobgui_compose_table_parse (argv[1], NULL);
  if (!table)
  ...
  str = g_string_new ("");
  g_string_append (str,
                   "#ifndef __BOBGUI_COMPOSE_DATA__\n"
                   "#define __BOBGUI_COMPOSE_DATA__\n"
                   "\n");
