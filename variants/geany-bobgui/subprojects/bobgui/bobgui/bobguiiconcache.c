...
BobguiIconCache *
bobgui_icon_cache_ref (BobguiIconCache *cache)
{
  cache->ref_count++;
  return cache;
}
...
void
bobgui_icon_cache_unref (BobguiIconCache *cache)
{
  cache->ref_count --;

  if (cache->ref_count == 0)
    {
      BOBGUI_DEBUG (ICONTHEME, "unmapping icon cache");
...
BobguiIconCache *
bobgui_icon_cache_new_for_path (const char *path)
{
  BobguiIconCache *cache = NULL;
  GMappedFile *map;
  ...
  BOBGUI_DEBUG (ICONTHEME, "look for icon cache in %s", path);
  ...
  if (st.st_mtime < path_st.st_mtime)
    {
      BOBGUI_DEBUG (ICONTHEME, "icon cache outdated");
      goto done;
    }
  ...
  if (BOBGUI_DEBUG_CHECK (ICONTHEME))
    {
      CacheInfo info;
  ...
  BOBGUI_DEBUG (ICONTHEME, "found icon cache for %s", path);

  cache = g_new0 (BobguiIconCache, 1);
  cache->ref_count = 1;
  cache->map = map;
...
BobguiIconCache *
bobgui_icon_cache_new (const char *data)
{
...
static int
get_directory_index (BobguiIconCache *cache,
...
GHashTable *
bobgui_icon_cache_list_icons_in_directory (BobguiIconCache *cache,
                                        const char   *directory,
                                        BobguiStringSet *set)
{
  int directory_index;
              ...
              if (g_str_equal (name + len - strlen (".symbolic"), ".symbolic") && (flags & ICON_CACHE_FLAG_PNG_SUFFIX) != 0)
                {
                  ...
                  interned_name = bobgui_string_set_add (set, string->str);
                  flags |= ICON_CACHE_FLAG_SYMBOLIC_PNG_SUFFIX;
                  flags &= ~ICON_CACHE_FLAG_PNG_SUFFIX;
                }
              else
                interned_name = bobgui_string_set_add (set, name);
