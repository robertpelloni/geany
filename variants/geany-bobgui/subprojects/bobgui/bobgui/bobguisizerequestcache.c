...
void
_bobgui_size_request_cache_init (SizeRequestCache *cache)
{
...
free_sizes_x (SizeRequestX **sizes)
{
  ...
  for (i = 0; i < BOBGUI_SIZE_REQUEST_CACHED_SIZES && sizes[i] != NULL; i++)
    g_free (sizes[i]);
...
void
_bobgui_size_request_cache_free (SizeRequestCache *cache)
{
  if (cache->requests_x)
...
void
_bobgui_size_request_cache_clear (SizeRequestCache *cache)
{
  _bobgui_size_request_cache_free (cache);
  _bobgui_size_request_cache_init (cache);
}

void
_bobgui_size_request_cache_commit (SizeRequestCache *cache,
  ...
  if (orientation == BOBGUI_ORIENTATION_HORIZONTAL)
    {
      ...
      if (cache->flags[BOBGUI_ORIENTATION_HORIZONTAL].cached_size_valid &&
      ...
      for (i = 0; i < cache->flags[BOBGUI_ORIENTATION_VERTICAL].n_cached_requests; i++)
        {
      ...
      for (i = 0; i < n_sizes; i++)
	{
      ...
      if (n_sizes < BOBGUI_SIZE_REQUEST_CACHED_SIZES)
	{
	...
	{
	  if (++cache->flags[orientation].last_cached_request == BOBGUI_SIZE_REQUEST_CACHED_SIZES)
	    cache->flags[orientation].last_cached_request = 0;
	}

      if (cache->requests_x == NULL)
	cache->requests_x = g_new0 (SizeRequestX *, BOBGUI_SIZE_REQUEST_CACHED_SIZES);
  ...
  else
    {
...
#ifdef G_ENABLE_DEBUG
      if (cache->flags[BOBGUI_ORIENTATION_VERTICAL].cached_size_valid &&
      ...
      for (i = 0; i < cache->flags[BOBGUI_ORIENTATION_HORIZONTAL].n_cached_requests; i++)
        {
	...
	cache->requests_y = g_new0 (SizeRequestY *, BOBGUI_SIZE_REQUEST_CACHED_SIZES);
 ...
 * the Clutter toolkit but has evolved for other BOBGUI requirements.
 */
gboolean
_bobgui_size_request_cache_lookup (const SizeRequestCache *cache,
                                BobguiOrientation          orientation,
                                int                     for_size,
                                int                    *minimum,
                                int                    *natural,
  ...
  if (orientation == BOBGUI_ORIENTATION_HORIZONTAL)
      ...
      else
	{
	  /* Search for an already cached size */
          for (i = 0, p = cache->flags[BOBGUI_ORIENTATION_HORIZONTAL].n_cached_requests; i < p; i++)
            {
	...
	{
	  /* Search for an already cached size */
          for (i = 0, p = cache->flags[BOBGUI_ORIENTATION_VERTICAL].n_cached_requests; i < p; i++)
            {
