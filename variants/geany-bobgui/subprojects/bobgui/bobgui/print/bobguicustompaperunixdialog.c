...
#define LEGACY_CUSTOM_PAPER_FILENAME ".bobgui-custom-papers"
#define CUSTOM_PAPER_FILENAME "custom-papers"


typedef struct
{
  BobguiUnit    display_unit;
  BobguiWidget *spin_button;
} UnitWidget;

struct _BobguiCustomPaperUnixDialog
{
  BobguiDialog parent_instance;

  BobguiWidget *listview;
  BobguiWidget *values_box;
  BobguiWidget *printer_combo;
  BobguiWidget *width_widget;
  BobguiWidget *height_widget;
  BobguiWidget *top_widget;
  BobguiWidget *bottom_widget;
  BobguiWidget *left_widget;
  BobguiWidget *right_widget;
...
typedef struct _BobguiCustomPaperUnixDialogClass BobguiCustomPaperUnixDialogClass;

struct _BobguiCustomPaperUnixDialogClass
{
...
G_DEFINE_TYPE (BobguiCustomPaperUnixDialog, bobgui_custom_paper_unix_dialog, BOBGUI_TYPE_DIALOG)


static void bobgui_custom_paper_unix_dialog_constructed (GObject *object);
static void bobgui_custom_paper_unix_dialog_finalize  (GObject                *object);
static void populate_dialog                        (BobguiCustomPaperUnixDialog *dialog);



BobguiUnit
_bobgui_print_get_default_user_units (void)
{
  ...
  if (imperial && imperial[0] == 2 )
    return BOBGUI_UNIT_INCH;  /* imperial */
  if (imperial && imperial[0] == 1 )
    return BOBGUI_UNIT_MM;  /* metric */
#endif

  if (strcmp (e, "default:inch")==0)
    return BOBGUI_UNIT_INCH;
  else if (strcmp (e, "default:mm"))
    g_warning ("Whoever translated default:mm did so wrongly.");
  return BOBGUI_UNIT_MM;
}
...
GList *
_bobgui_load_custom_papers (void)
{
  GKeyFile *keyfile;
  char *filename;
  char **groups;
  ...
  for (i = 0; i < n_groups; ++i)
    {
      ...
      page_setup = bobgui_page_setup_new_from_key_file (keyfile, groups[i], NULL);
      if (!page_setup)
        continue;
...
void
bobgui_print_load_custom_papers (GListStore *store)
{
  GList *papers, *p;
  BobguiPageSetup *page_setup;
  ...
  papers = _bobgui_load_custom_papers ();
...
static void
bobgui_print_save_custom_papers (GListStore *store)
{
  GKeyFile *keyfile;
  ...
  for (i = 0; i < g_list_model_get_n_items (G_LIST_MODEL (store)); i++)
    {
      ...
      bobgui_page_setup_to_key_file (page_setup, keyfile, group);
      g_object_unref (page_setup);
    }
...
static void
bobgui_custom_paper_unix_dialog_class_init (BobguiCustomPaperUnixDialogClass *class)
{
  G_OBJECT_CLASS (class)->constructed = bobgui_custom_paper_unix_dialog_constructed;
  G_OBJECT_CLASS (class)->finalize = bobgui_custom_paper_unix_dialog_finalize;
}

static void
custom_paper_dialog_response_cb (BobguiDialog *dialog,
                                 int        response,
                                 gpointer   user_data)
{
  BobguiCustomPaperUnixDialog *self = BOBGUI_CUSTOM_PAPER_UNIX_DIALOG (dialog);

  bobgui_print_save_custom_papers (self->custom_paper_list);
}
...
static void
bobgui_custom_paper_unix_dialog_init (BobguiCustomPaperUnixDialog *dialog)
{
...
G_GNUC_END_IGNORE_DEPRECATIONS
  ...
  printer = bobgui_printer_new (_("Margins from Printer…"), NULL, FALSE);
...
static void
bobgui_custom_paper_unix_dialog_constructed (GObject *object)
{
  gboolean use_header;

  G_OBJECT_CLASS (bobgui_custom_paper_unix_dialog_parent_class)->constructed (object);

G_GNUC_BEGIN_IGNORE_DEPRECATIONS
  g_object_get (object, "use-header-bar", &use_header, NULL);
  if (!use_header)
    {
      bobgui_dialog_add_buttons (BOBGUI_DIALOG (object),
                              _("_Close"), BOBGUI_RESPONSE_CLOSE,
                              NULL);
      bobgui_dialog_set_default_response (BOBGUI_DIALOG (object), BOBGUI_RESPONSE_CLOSE);
    }
G_GNUC_END_IGNORE_DEPRECATIONS
}

static void
bobgui_custom_paper_unix_dialog_finalize (GObject *object)
{
  BobguiCustomPaperUnixDialog *dialog = BOBGUI_CUSTOM_PAPER_UNIX_DIALOG (object);
  GList *node;
  ...
  G_OBJECT_CLASS (bobgui_custom_paper_unix_dialog_parent_class)->finalize (object);
}

/**
 * bobgui_custom_paper_unix_dialog_new:
 * @title: (nullable): the title of the dialog
 * @parent: (nullable): transient parent of the dialog
 *
 * Creates a new custom paper dialog.
 *
 * Returns: the new `BobguiCustomPaperUnixDialog`
 */
BobguiWidget *
_bobgui_custom_paper_unix_dialog_new (BobguiWindow   *parent,
                                   const char *title)
{
  BobguiWidget *result;
  ...
  result = g_object_new (BOBGUI_TYPE_CUSTOM_PAPER_UNIX_DIALOG,
                         "title", title,
...
static void
load_print_backends (BobguiCustomPaperUnixDialog *dialog)
{
  GListModel *full_list;
  GListStore *printer_list_list;
  GList *node;
  ...
  for (node = dialog->print_backends; node != NULL; node = node->next)
    {
      BobguiPrintBackend *backend = BOBGUI_PRINT_BACKEND (node->data);
      g_list_store_append (printer_list_list, bobgui_print_backend_get_printers (backend));
    }
}

static void unit_widget_changed (BobguiCustomPaperUnixDialog *dialog);

static BobguiWidget *
new_unit_widget (BobguiCustomPaperUnixDialog *dialog,
                 BobguiUnit                   unit,
                 BobguiWidget                *mnemonic_label)
{
  BobguiWidget *hbox, *button, *label;
  UnitWidget *data;
  ...
  button = bobgui_spin_button_new_with_range (0.0, 9999.0, 1);
  bobgui_widget_set_valign (button, BOBGUI_ALIGN_BASELINE_FILL);
  if (unit == BOBGUI_UNIT_INCH)
    bobgui_spin_button_set_digits (BOBGUI_SPIN_BUTTON (button), 2);
  else
    bobgui_spin_button_set_digits (BOBGUI_SPIN_BUTTON (button), 1);
  ...
  if (unit == BOBGUI_UNIT_INCH)
    label = bobgui_label_new (_("inch"));
  else
    label = bobgui_label_new (_("mm"));
  bobgui_widget_set_valign (label, BOBGUI_ALIGN_BASELINE_FILL);

  bobgui_box_append (BOBGUI_BOX (hbox), label);
  bobgui_label_set_mnemonic_widget (BOBGUI_LABEL (mnemonic_label), button);
...
static double
unit_widget_get (BobguiWidget *unit_widget)
{
  UnitWidget *data = g_object_get_data (G_OBJECT (unit_widget), "unit-data");
  return _bobgui_print_convert_to_mm (bobgui_spin_button_get_value (BOBGUI_SPIN_BUTTON (data->spin_button)),
                                   data->display_unit);
}

static void
unit_widget_set (BobguiWidget *unit_widget,
                 double     value)
{
  UnitWidget *data;

  data = g_object_get_data (G_OBJECT (unit_widget), "unit-data");
  bobgui_spin_button_set_value (BOBGUI_SPIN_BUTTON (data->spin_button),
                             _bobgui_print_convert_from_mm (value, data->display_unit));
}

static void
update_combo_sensitivity_from_printers (BobguiCustomPaperUnixDialog *dialog)
{
  ...
  bobgui_widget_set_sensitive (dialog->printer_combo, sensitive);
}

static void
update_custom_widgets_from_list (BobguiCustomPaperUnixDialog *dialog)
{
  GListModel *model;
  BobguiPageSetup *page_setup;

  model = G_LIST_MODEL (bobgui_list_view_get_model (BOBGUI_LIST_VIEW (dialog->listview)));
  page_setup = bobgui_single_selection_get_selected_item (BOBGUI_SINGLE_SELECTION (model));

  dialog->non_user_change = TRUE;
  if (page_setup != NULL)
    {
      unit_widget_set (dialog->width_widget,
                       bobgui_page_setup_get_paper_width (page_setup, BOBGUI_UNIT_MM));
      unit_widget_set (dialog->height_widget,
                       bobgui_page_setup_get_paper_height (page_setup, BOBGUI_UNIT_MM));
      unit_widget_set (dialog->top_widget,
                       bobgui_page_setup_get_top_margin (page_setup, BOBGUI_UNIT_MM));
      unit_widget_set (dialog->bottom_widget,
                       bobgui_page_setup_get_bottom_margin (page_setup, BOBGUI_UNIT_MM));
      unit_widget_set (dialog->left_widget,
                       bobgui_page_setup_get_left_margin (page_setup, BOBGUI_UNIT_MM));
      unit_widget_set (dialog->right_widget,
                       bobgui_page_setup_get_right_margin (page_setup, BOBGUI_UNIT_MM));

      bobgui_widget_set_sensitive (dialog->values_box, TRUE);
    }
  else
    {
      bobgui_widget_set_sensitive (dialog->values_box, FALSE);
    }
...
selected_custom_paper_changed (GObject                  *list,
                               GParamSpec               *pspec,
                               BobguiCustomPaperUnixDialog *dialog)
{
  update_custom_widgets_from_list (dialog);
}

static void
unit_widget_changed (BobguiCustomPaperUnixDialog *dialog)
{
  ...
  if (page_setup != NULL)
    {
      ...
      bobgui_paper_size_set_size (paper_size, w, h, BOBGUI_UNIT_MM);
      ...
      bobgui_page_setup_set_top_margin (page_setup, top, BOBGUI_UNIT_MM);
      bobgui_page_setup_set_bottom_margin (page_setup, bottom, BOBGUI_UNIT_MM);
      bobgui_page_setup_set_left_margin (page_setup, left, BOBGUI_UNIT_MM);
      bobgui_page_setup_set_right_margin (page_setup, right, BOBGUI_UNIT_MM);
    }
}

static gboolean
custom_paper_name_used (BobguiCustomPaperUnixDialog *dialog,
                        const char               *name)
{
  BobguiPageSetup *page_setup;
  BobguiPaperSize *paper_size;
  ...
  for (i = 0; i < g_list_model_get_n_items (G_LIST_MODEL (dialog->custom_paper_list)); i++)
    {
      ...
      paper_size = bobgui_page_setup_get_paper_size (page_setup);
      if (strcmp (name, bobgui_paper_size_get_name (paper_size)) == 0)
        {
          g_object_unref (page_setup);
          return TRUE;
        }
...
static void
add_custom_paper (BobguiCustomPaperUnixDialog *dialog)
{
  BobguiPageSetup *page_setup;
  BobguiPaperSize *paper_size;
  char *name;
  ...
  paper_size = bobgui_paper_size_new_custom (name, name,
                                          bobgui_page_setup_get_paper_width (page_setup, BOBGUI_UNIT_MM),
                                          bobgui_page_setup_get_paper_height (page_setup, BOBGUI_UNIT_MM),
                                          BOBGUI_UNIT_MM);
...
static void
remove_custom_paper (BobguiCustomPaperUnixDialog *dialog)
{
  GListModel *model;
  guint selected;

  model = G_LIST_MODEL (bobgui_list_view_get_model (BOBGUI_LIST_VIEW (dialog->listview)));
  selected = bobgui_single_selection_get_selected (BOBGUI_SINGLE_SELECTION (model));
  if (selected != BOBGUI_INVALID_LIST_POSITION)
    g_list_store_remove (dialog->custom_paper_list, selected);
}

static void
set_margins_from_printer (BobguiCustomPaperUnixDialog *dialog,
                          BobguiPrinter               *printer)
{
  double top, bottom, left, right;

  top = bottom = left = right = 0;
  if (!bobgui_printer_get_hard_margins (printer, &top, &bottom, &left, &right))
    return;

  dialog->non_user_change = TRUE;
  unit_widget_set (dialog->top_widget, _bobgui_print_convert_to_mm (top, BOBGUI_UNIT_POINTS));
  unit_widget_set (dialog->bottom_widget, _bobgui_print_convert_to_mm (bottom, BOBGUI_UNIT_POINTS));
  unit_widget_set (dialog->left_widget, _bobgui_print_convert_to_mm (left, BOBGUI_UNIT_POINTS));
  unit_widget_set (dialog->right_widget, _bobgui_print_convert_to_mm (right, BOBGUI_UNIT_POINTS));
  dialog->non_user_change = FALSE;
...
static void
get_margins_finished_callback (BobguiPrinter               *printer,
                               gboolean                  success,
                               BobguiCustomPaperUnixDialog *dialog)
{
  g_signal_handler_disconnect (dialog->request_details_printer,
  ...
  bobgui_drop_down_set_selected (BOBGUI_DROP_DOWN (dialog->printer_combo), 0);
}

static void
margins_from_printer_changed (BobguiCustomPaperUnixDialog *dialog)
{
  guint selected;
  BobguiPrinter *printer;
  ...
  if (selected != 0)
    {
      ...
      if (printer)
        {
          if (bobgui_printer_has_details (printer))
            {
              set_margins_from_printer (dialog, printer);
              bobgui_drop_down_set_selected (BOBGUI_DROP_DOWN (dialog->printer_combo), 0);
            }
...
wrap_in_frame (const char *label,
               BobguiWidget   *child)
{
  BobguiWidget *frame, *label_widget;
  char *bold_text;
  ...
  frame = bobgui_box_new (BOBGUI_ORIENTATION_VERTICAL, 6);
  bobgui_box_append (BOBGUI_BOX (frame), label_widget);
  ...
  bobgui_box_append (BOBGUI_BOX (frame), child);
...
setup_item (BobguiSignalListItemFactory *factory,
            BobguiListItem              *item)
{
  bobgui_list_item_set_child (item, bobgui_editable_label_new (""));
}
               ...
               BobguiListItem *item)
{
  ...
  new_text = bobgui_editable_get_text (BOBGUI_EDITABLE (object));

  paper_size = bobgui_paper_size_new_custom (new_text, new_text,
                                          bobgui_page_setup_get_paper_width (page_setup, BOBGUI_UNIT_MM),
                                          bobgui_page_setup_get_paper_height (page_setup, BOBGUI_UNIT_MM), BOBGUI_UNIT_MM);
  bobgui_page_setup_set_paper_size (page_setup, paper_size);
  bobgui_paper_size_free (paper_size);
}

static void
state_changed (BobguiWidget     *item,
               BobguiStateFlags  old_state,
               BobguiWidget     *label)
{
  gboolean selected;

  selected = (bobgui_widget_get_state_flags (item) & BOBGUI_STATE_FLAG_SELECTED) != 0;
  bobgui_editable_set_editable (BOBGUI_EDITABLE (label), selected);
}
           ...
           BobguiListItem              *item)
{
  ...
  paper_size = bobgui_page_setup_get_paper_size (page_setup);
  bobgui_editable_set_text (BOBGUI_EDITABLE (label),
                         bobgui_paper_size_get_display_name (paper_size));
  g_signal_connect (label, "notify::text",
                    G_CALLBACK (label_changed), item);
  g_signal_connect (bobgui_widget_get_parent (label), "state-flags-changed",
             ...
             BobguiListItem              *item)
{
  ...
  g_signal_handlers_disconnect_by_func (bobgui_widget_get_parent (label), state_changed, label);
}
                   ...
                   BobguiListItem              *item)
{
  ...
  bobgui_label_set_label (BOBGUI_LABEL (label), bobgui_printer_get_name (printer));
}

static void
populate_dialog (BobguiCustomPaperUnixDialog *dialog)
{
  BobguiDialog *cpu_dialog = BOBGUI_DIALOG (dialog);
  BobguiWidget *content_area;
  BobguiWidget *grid, *label, *widget, *frame, *combo;
  BobguiWidget *hbox, *vbox, *listview, *scrolled, *toolbar, *button;
  BobguiUnit user_units;
  BobguiSelectionModel *model;
  BobguiListItemFactory *factory;

G_GNUC_BEGIN_IGNORE_DEPRECATIONS
  content_area = bobgui_dialog_get_content_area (cpu_dialog);
G_GNUC_END_IGNORE_DEPRECATIONS

  bobgui_box_set_spacing (BOBGUI_BOX (content_area), 2); /* 2 * 5 + 2 = 12 */

  hbox = bobgui_box_new (BOBGUI_ORIENTATION_HORIZONTAL, 18);
  ...
  bobgui_scrolled_window_set_policy (BOBGUI_SCROLLED_WINDOW (scrolled),
                                  BOBGUI_POLICY_NEVER, BOBGUI_POLICY_AUTOMATIC);
  bobgui_scrolled_window_set_has_frame (BOBGUI_SCROLLED_WINDOW (scrolled), TRUE);
  bobgui_box_append (BOBGUI_BOX (vbox), scrolled);

  model = BOBGUI_SELECTION_MODEL (bobgui_single_selection_new (g_object_ref (G_LIST_MODEL (dialog->custom_paper_list))));
  ...
  bobgui_widget_set_size_request (listview, 140, -1);

  dialog->listview = listview;

  bobgui_scrolled_window_set_child (BOBGUI_SCROLLED_WINDOW (scrolled), listview);

  toolbar = bobgui_box_new (BOBGUI_ORIENTATION_HORIZONTAL, 0);

  bobgui_widget_add_css_class (toolbar, "linked");

  bobgui_box_append (BOBGUI_BOX (vbox), toolbar);

  button = bobgui_button_new_from_icon_name ("list-add-symbolic");
  g_signal_connect_swapped (button, "clicked", G_CALLBACK (add_custom_paper), dialog);

  bobgui_box_append (BOBGUI_BOX (toolbar), button);

  button = bobgui_button_new_from_icon_name ("list-remove-symbolic");
  g_signal_connect_swapped (button, "clicked", G_CALLBACK (remove_custom_paper), dialog);

  bobgui_box_append (BOBGUI_BOX (toolbar), button);

  user_units = _bobgui_print_get_default_user_units ();

  vbox = bobgui_box_new (BOBGUI_ORIENTATION_VERTICAL, 18);
  ...
  label = bobgui_label_new_with_mnemonic (_("_Width:"));
  bobgui_widget_set_halign (label, BOBGUI_ALIGN_START);
  bobgui_widget_set_valign (label, BOBGUI_ALIGN_BASELINE_FILL);
  ...
  bobgui_grid_attach (BOBGUI_GRID (grid), widget, 1, 0, 1, 1);

  label = bobgui_label_new_with_mnemonic (_("_Height:"));
  bobgui_widget_set_halign (label, BOBGUI_ALIGN_START);
  bobgui_widget_set_valign (label, BOBGUI_ALIGN_BASELINE_FILL);
  ...
  bobgui_grid_attach (BOBGUI_GRID (grid), widget, 1, 1, 1, 1);
  ...
  label = bobgui_label_new_with_mnemonic (_("_Top:"));
  bobgui_widget_set_halign (label, BOBGUI_ALIGN_START);
  bobgui_widget_set_valign (label, BOBGUI_ALIGN_BASELINE_FILL);
  bobgui_grid_attach (BOBGUI_GRID (grid), label, 0, 0, 1, 1);
  ...
  label = bobgui_label_new_with_mnemonic (_("_Bottom:"));
  bobgui_widget_set_halign (label, BOBGUI_ALIGN_START);
  bobgui_widget_set_valign (label, BOBGUI_ALIGN_BASELINE_FILL);
  bobgui_grid_attach (BOBGUI_GRID (grid), label, 0, 1, 1, 1);
  ...
  label = bobgui_label_new_with_mnemonic (_("_Left:"));
  bobgui_widget_set_halign (label, BOBGUI_ALIGN_START);
  bobgui_widget_set_valign (label, BOBGUI_ALIGN_BASELINE_FILL);
  ...
  bobgui_grid_attach (BOBGUI_GRID (grid), widget, 1, 2, 1, 1);

  label = bobgui_label_new_with_mnemonic (_("_Right:"));
  bobgui_widget_set_halign (label, BOBGUI_ALIGN_START);
  bobgui_widget_set_valign (label, BOBGUI_ALIGN_BASELINE_FILL);
  ...
  bobgui_grid_attach (BOBGUI_GRID (grid), widget, 1, 3, 1, 1);

  hbox = bobgui_box_new (BOBGUI_ORIENTATION_HORIZONTAL, 0);
  bobgui_grid_attach (BOBGUI_GRID (grid), hbox, 0, 4, 2, 1);

  combo = bobgui_drop_down_new (g_object_ref (dialog->printer_list), NULL);
  ...
  bobgui_drop_down_set_factory (BOBGUI_DROP_DOWN (combo), factory);
