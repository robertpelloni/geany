/* bobguicupssecretsutils.h: Helper to use a secrets service for printer passwords
                   ...
                   const char  **additional_labels)
{
  ...
  if (printer_uri == NULL)
    {
      BOBGUI_DEBUG (PRINTING, "create_attributes called with invalid parameters.");
      return NULL;
    }
               ...
               gpointer      user_data)
{
  ...
  if (attributes == NULL)
    {
      BOBGUI_DEBUG (PRINTING, "Failed to lookup attributes.");
  ...
  if (pw_field == -1)
    {
      /* should not happen... */
      BOBGUI_DEBUG (PRINTING, "No password required?.");
      g_variant_unref (output);
      goto fail;
    }
  else
    {
      ...
      if (secret == NULL || g_variant_n_children (secret) != 4)
        {
          BOBGUI_DEBUG (PRINTING, "Get secret response invalid.");
          if (secret != NULL)
      ...
      if (ba_passwd == NULL)
        {
          BOBGUI_DEBUG (PRINTING, "Invalid / no secret found.");
      ...
      if (auth_info[i] == NULL)
        {
          ...
          BOBGUI_DEBUG (PRINTING, "Failed to lookup required attribute: %s.",
...
fail:
  /* Error out */
  BOBGUI_DEBUG (PRINTING, "Failed to lookup secret.");
                ...
                gpointer      user_data)
{
  ...
  if (item != NULL && strlen (item) > 1)
    {
      BOBGUI_DEBUG (PRINTING, "Successfully stored auth info.");
      g_task_return_pointer (task, NULL, NULL);
      g_object_unref (task);
      return;
    }
...
do_store_auth_info (GTask *task)
{
  ...
  if (attributes == NULL)
    {
      BOBGUI_DEBUG (PRINTING, "Failed to create attributes.");
      g_task_return_pointer (task, NULL, NULL);
      g_object_unref (task);
      return;
    }
  ...
  if (password == NULL)
    {
      BOBGUI_DEBUG (PRINTING, "No secret to store.");
      g_task_return_pointer (task, NULL, NULL);
      g_object_unref (task);
      return;
    }
                     ...
                     gpointer         user_data)
{
  ...
  if (dismissed == NULL)
    {
      BOBGUI_DEBUG (PRINTING, "Invalid prompt signal.");
      g_task_return_pointer (task, NULL, NULL);
      g_object_unref (task);
      return;
    }
  ...
  if (is_dismissed)
    {
      BOBGUI_DEBUG (PRINTING, "Collection unlock dismissed.");
      g_task_return_pointer (task, NULL, NULL);
      g_object_unref (task);
      return;
    }
                      ...
                      gpointer      user_data)
{
  ...
  if (subresult == NULL)
    {
      BOBGUI_DEBUG (PRINTING, "Invalid ReadAlias response.");
      g_task_return_pointer (task, NULL, NULL);
      g_object_unref (task);
      return;
    }
               ...
               gpointer      user_data)
{
  ...
  if (locked == NULL)
    {
      BOBGUI_DEBUG (PRINTING, "Failed to look up \"Locked\" property on item.");
      g_task_return_pointer (task, NULL, NULL);
      g_object_unref (task);
      return;
    }
                 ...
                 gpointer      user_data)
{
  ...
  for (i = 0; i < array_cnt; i++)
    {
      ...
      if (item_paths == NULL)
        {
          BOBGUI_DEBUG (PRINTING, "SearchItems returned invalid result.");
          continue;
        }
  ...
  if (!found_item)
    {
      BOBGUI_DEBUG (PRINTING, "No match found in secrets service.");
      g_task_return_pointer (task, NULL, NULL);
      g_object_unref (task);
      return;
    }
                 ...
                 gpointer      user_data)
{
  ...
  if (session_variant == NULL)
    {
      BOBGUI_DEBUG (PRINTING, "Invalid session path response.");
      g_variant_unref (output);
      g_task_return_pointer (task, NULL, NULL);
      g_object_unref (task);
      return;
  ...
  if (task_data->session_path == NULL)
    {
      BOBGUI_DEBUG (PRINTING, "Invalid session path string value.");
...
/**
 * bobgui_cups_secrets_service_watch:
 ...
 */
guint
bobgui_cups_secrets_service_watch (GBusNameAppearedCallback appeared,
...
/**
 * bobgui_cups_secrets_service_query_task:
 ...
 */
void
bobgui_cups_secrets_service_query_task (gpointer              source_object,
               ...
               gpointer      user_data)
{
  ...
  if (error != NULL)
    {
      BOBGUI_DEBUG (PRINTING, "Failed to store auth info: %s", error->message);
      g_error_free (error);
    }

  g_object_unref (task);
  BOBGUI_DEBUG (PRINTING, "bobgui_cups_secrets_service_store finished.");
}

/**
 * bobgui_cups_secrets_service_store:
 ...
 */
void
bobgui_cups_secrets_service_store (char        **auth_info,
                                ...
                                const char   *printer_uri)
{
  ...
  if (auth_info == NULL || auth_info_labels == NULL || printer_uri == NULL)
    {
      BOBGUI_DEBUG (PRINTING, "Invalid call to bobgui_cups_secrets_service_store.");
      return;
    }
