/* bobguigridlayout.c: Layout manager for grid-like widgets
 ...
 * using the [class@Bobgui.GridLayoutChild] associated to each child widget.
 *
 * The behaviour of `BobguiGridLayout` when several children occupy the same
 * grid cell is undefined.
 *
 * `BobguiGridLayout` can be used like a `BobguiBoxLayout` if all children are
 * attached to the same row or column; however, if you only ever need a
 * single row or column, you should consider using `BobguiBoxLayout`.
 */

/**
 * BobguiGridLayoutChild:
 *
 * `BobguiLayoutChild` subclass for children in a `BobguiGridLayout`.
 */
#include "config.h"
...
#define CHILD_COLUMN(child)     ((child)->attach[BOBGUI_ORIENTATION_HORIZONTAL].pos)
#define CHILD_COL_SPAN(child)   ((child)->attach[BOBGUI_ORIENTATION_HORIZONTAL].span)
#define CHILD_ROW(child)        ((child)->attach[BOBGUI_ORIENTATION_VERTICAL].pos)
#define CHILD_ROW_SPAN(child)   ((child)->attach[BOBGUI_ORIENTATION_VERTICAL].span)
...
G_DEFINE_TYPE (BobguiGridLayoutChild, bobgui_grid_layout_child, BOBGUI_TYPE_LAYOUT_CHILD)

static void
bobgui_grid_layout_child_set_property (GObject      *gobject,
                                    guint         prop_id,
                                    const GValue *value,
                                    GParamSpec   *pspec)
{
  BobguiGridLayoutChild *self = BOBGUI_GRID_LAYOUT_CHILD (gobject);

  switch (prop_id)
    {
    case PROP_CHILD_COLUMN:
      bobgui_grid_layout_child_set_column (self, g_value_get_int (value));
      break;

    case PROP_CHILD_ROW :
      bobgui_grid_layout_child_set_row (self, g_value_get_int (value));
      break;

    case PROP_CHILD_COLUMN_SPAN:
      bobgui_grid_layout_child_set_column_span (self, g_value_get_int (value));
      break;

    case PROP_CHILD_ROW_SPAN:
      bobgui_grid_layout_child_set_row_span (self, g_value_get_int (value));
      break;
...
static void
bobgui_grid_layout_child_get_property (GObject    *gobject,
                                    guint       prop_id,
                                    GValue     *value,
                                    GParamSpec *pspec)
{
...
static void
bobgui_grid_layout_child_class_init (BobguiGridLayoutChildClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->set_property = bobgui_grid_layout_child_set_property;
  gobject_class->get_property = bobgui_grid_layout_child_get_property;

  /**
   * BobguiGridLayoutChild:column:
   *
  ...
  /**
   * BobguiGridLayoutChild:column-span:
   *
  ...
  /**
   * BobguiGridLayoutChild:row-span:
   *
...
static void
bobgui_grid_layout_child_init (BobguiGridLayoutChild *self)
{
...
/**
 * bobgui_grid_layout_child_set_row:
 * @child: a `BobguiGridLayoutChild`
 * @row: the row for @child
 *
 * Sets the row to place @child in.
 */
void
bobgui_grid_layout_child_set_row (BobguiGridLayoutChild *child,
                               int                 row)
{
  g_return_if_fail (BOBGUI_IS_GRID_LAYOUT_CHILD (child));
  ...
  bobgui_layout_manager_layout_changed (bobgui_layout_child_get_layout_manager (BOBGUI_LAYOUT_CHILD (child)));

  g_object_notify_by_pspec (G_OBJECT (child), child_props[PROP_CHILD_ROW]);
}

/**
 * bobgui_grid_layout_child_get_row:
 * @child: a `BobguiGridLayoutChild`
 *
 ...
 */
int
bobgui_grid_layout_child_get_row (BobguiGridLayoutChild *child)
{
  g_return_val_if_fail (BOBGUI_IS_GRID_LAYOUT_CHILD (child), 0);

  return CHILD_ROW (child);
}

/**
 * bobgui_grid_layout_child_set_column:
 * @child: a `BobguiGridLayoutChild`
 ...
 */
void
bobgui_grid_layout_child_set_column (BobguiGridLayoutChild *child,
                                  int                 column)
{
  g_return_if_fail (BOBGUI_IS_GRID_LAYOUT_CHILD (child));
...
/**
 * bobgui_grid_layout_child_get_column:
 * @child: a `BobguiGridLayoutChild`
 *
 ...
 */
int
bobgui_grid_layout_child_get_column (BobguiGridLayoutChild *child)
{
  g_return_val_if_fail (BOBGUI_IS_GRID_LAYOUT_CHILD (child), 0);
...
/**
 * bobgui_grid_layout_child_set_column_span:
 * @child: a `BobguiGridLayoutChild`
 ...
 */
void
bobgui_grid_layout_child_set_column_span (BobguiGridLayoutChild *child,
                                       int                 span)
{
  g_return_if_fail (BOBGUI_IS_GRID_LAYOUT_CHILD (child));
...
/**
 * bobgui_grid_layout_child_get_column_span:
 * @child: a `BobguiGridLayoutChild`
 *
 ...
 */
int
bobgui_grid_layout_child_get_column_span (BobguiGridLayoutChild *child)
{
  g_return_val_if_fail (BOBGUI_IS_GRID_LAYOUT_CHILD (child), 1);

  return CHILD_COL_SPAN (child);
}

/**
 * bobgui_grid_layout_child_set_row_span:
 * @child: a `BobguiGridLayoutChild`
 * @span: the span of @child
 *
 ...
 */
void
bobgui_grid_layout_child_set_row_span (BobguiGridLayoutChild *child,
                                    int                 span)
{
  g_return_if_fail (BOBGUI_IS_GRID_LAYOUT_CHILD (child));
...
/**
 * bobgui_grid_layout_child_get_row_span:
 * @child: a `BobguiGridLayoutChild`
 *
 ...
 */
int
bobgui_grid_layout_child_get_row_span (BobguiGridLayoutChild *child)
{
  g_return_val_if_fail (BOBGUI_IS_GRID_LAYOUT_CHILD (child), 1);
...
#define ROWS(layout)    (&(layout)->linedata[BOBGUI_ORIENTATION_HORIZONTAL])
#define COLUMNS(layout) (&(layout)->linedata[BOBGUI_ORIENTATION_VERTICAL])
...
typedef struct {
  BobguiGridLayout *layout;
  BobguiWidget *widget;

  GridLines lines[2];
} GridRequest;

struct _BobguiGridLayout
{
...
G_DEFINE_TYPE (BobguiGridLayout, bobgui_grid_layout, BOBGUI_TYPE_LAYOUT_MANAGER)

static inline BobguiGridLayoutChild *
get_grid_child (BobguiGridLayout *self,
                BobguiWidget     *child)
{
  BobguiLayoutManager *manager = BOBGUI_LAYOUT_MANAGER (self);

  return (BobguiGridLayoutChild *) bobgui_layout_manager_get_layout_child (manager, child);
}

static int
get_spacing (BobguiGridLayout  *self,
             BobguiWidget      *widget,
             BobguiOrientation  orientation)
{
...
grid_request_count_lines (GridRequest *request)
{
  ...
  for (child = _bobgui_widget_get_first_child (request->widget);
       child != NULL;
       child = _bobgui_widget_get_next_sibling (child))
    {
      BobguiGridLayoutChild *grid_child = get_grid_child (request->layout, child);
                   ...
                   BobguiOrientation  orientation)
{
  ...
  for (child = _bobgui_widget_get_first_child (request->widget);
       ...
       child = _bobgui_widget_get_next_sibling (child))
    {
      ...
      if (attach->span == 1 && bobgui_widget_compute_expand (child, orientation))
      ...
      if (line->minimum_above != -1)
        {
          ...
          baseline_pos = bobgui_grid_layout_get_row_baseline_position (request->layout, i + lines->min);
...
grid_request_homogeneous (GridRequest    *request,
                          BobguiOrientation  orientation)
{
  BobguiGridLayout *self = request->layout;
                             ...
                             int            *expand_lines)
{
  ...
  for (child = _bobgui_widget_get_first_child (request->widget);
       ...
       child = _bobgui_widget_get_next_sibling (child))
    {
      ...
      line = &lines->lines[attach->pos - lines->min];
      line->empty = FALSE;
      if (bobgui_widget_compute_expand (child, orientation))
        line->expand = TRUE;
    }
  ...
  for (child = _bobgui_widget_get_first_child (request->widget);
       ...
       child = _bobgui_widget_get_next_sibling (child))
    {
      ...
      if (!has_expand && bobgui_widget_compute_expand (child, orientation))
        {
          for (i = 0; i < attach->span; i++)
            {
              if (attach->pos + i >= max || attach->pos + 1 < min)
                                 ...
                                 int        max)
{
  ...
  sizes = g_newa (BobguiRequestedSize, nonempty);
  ...
  size = bobgui_distribute_natural_allocation (MAX (0, size), nonempty, sizes);
                       ...
                       int             total_size)
{
  ...
  baseline = bobgui_widget_get_baseline (request->widget);

  if (orientation == BOBGUI_ORIENTATION_VERTICAL && baseline != -1 &&
                       ...
                       BobguiOrientation  orientation)
{
  ...
  allocated_baseline = bobgui_widget_get_baseline (request->widget);
...
static void
bobgui_grid_layout_get_size (BobguiGridLayout  *self,
                          BobguiWidget      *widget,
                          BobguiOrientation  orientation,
                          int            *minimum,
                          int            *natural,
...
static void
bobgui_grid_layout_get_size_for_size (BobguiGridLayout  *self,
                                   BobguiWidget      *widget,
                                   BobguiOrientation  orientation,
                                   int             size,
                                   int            *minimum,
...
static void
bobgui_grid_layout_measure (BobguiLayoutManager *manager,
                         BobguiWidget        *widget,
                         BobguiOrientation    orientation,
                         int               for_size,
                         int              *minimum,
                         ...
                         int              *natural_baseline)
{
  BobguiGridLayout *self = BOBGUI_GRID_LAYOUT (manager);

  if ((orientation == BOBGUI_ORIENTATION_HORIZONTAL &&
       bobgui_widget_get_request_mode (widget) == BOBGUI_SIZE_REQUEST_WIDTH_FOR_HEIGHT) ||
      (orientation == BOBGUI_ORIENTATION_VERTICAL &&
       bobgui_widget_get_request_mode (widget) == BOBGUI_SIZE_REQUEST_HEIGHT_FOR_WIDTH))
    bobgui_grid_layout_get_size_for_size (self, widget, orientation, for_size,
                                       minimum, natural,
                                       minimum_baseline, natural_baseline);
  else
    bobgui_grid_layout_get_size (self, widget, orientation,
                              minimum, natural,
                              minimum_baseline, natural_baseline);
}
                ...
                int                *baseline)
{
  ...
  *position = lines->lines[attach->pos - lines->min].position;
  if (attach->span == 1 &&
      (bobgui_widget_get_valign (child) == BOBGUI_ALIGN_BASELINE_CENTER ||
       bobgui_widget_get_valign (child) == BOBGUI_ALIGN_BASELINE_FILL))
                                ...
                                int          grid_height)
{
  ...
  for (child = _bobgui_widget_get_first_child (request->widget);
       ...
       child = _bobgui_widget_get_next_sibling (child))
    {
      ...
      allocate_child (request, BOBGUI_ORIENTATION_HORIZONTAL, child, grid_child, &x, &width, &ignore);
      allocate_child (request, BOBGUI_ORIENTATION_VERTICAL, child, grid_child, &y, &height, &baseline);
      ...
      if (_bobgui_widget_get_direction (request->widget) == BOBGUI_TEXT_DIR_RTL)
...
#define GET_SIZE(width, height, orientation) (orientation == BOBGUI_ORIENTATION_HORIZONTAL ? width : height)

static void
bobgui_grid_layout_allocate (BobguiLayoutManager *manager,
                          BobguiWidget        *widget,
                          int               width,
                          int               height,
                          int              baseline)
...
static void
bobgui_grid_layout_set_property (GObject      *gobject,
                              guint         prop_id,
                              const GValue *value,
                              GParamSpec   *pspec)
{
  BobguiGridLayout *self = BOBGUI_GRID_LAYOUT (gobject);

  switch (prop_id)
    {
    case PROP_ROW_SPACING:
      bobgui_grid_layout_set_row_spacing (self, g_value_get_int (value));
      break;

    case PROP_COLUMN_SPACING:
      bobgui_grid_layout_set_column_spacing (self, g_value_get_int (value));
      break;

    case PROP_ROW_HOMOGENEOUS:
      bobgui_grid_layout_set_row_homogeneous (self, g_value_get_boolean (value));
      break;

    case PROP_COLUMN_HOMOGENEOUS:
      bobgui_grid_layout_set_column_homogeneous (self, g_value_get_boolean (value));
      break;

    case PROP_BASELINE_ROW:
      bobgui_grid_layout_set_baseline_row (self, g_value_get_int (value));
      break;
...
static void
bobgui_grid_layout_get_property (GObject    *gobject,
                              guint       prop_id,
                              GValue     *value,
                              GParamSpec *pspec)
{
...
static void
bobgui_grid_layout_finalize (GObject *gobject)
{
  BobguiGridLayout *self = BOBGUI_GRID_LAYOUT (gobject);

  g_clear_pointer (&self->row_properties, g_array_unref);

  G_OBJECT_CLASS (bobgui_grid_layout_parent_class)->finalize (gobject);
}

static void
bobgui_grid_layout_class_init (BobguiGridLayoutClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  BobguiLayoutManagerClass *layout_class = BOBGUI_LAYOUT_MANAGER_CLASS (klass);

  layout_class->layout_child_type = BOBGUI_TYPE_GRID_LAYOUT_CHILD;
  layout_class->measure = bobgui_grid_layout_measure;
  layout_class->allocate = bobgui_grid_layout_allocate;

  gobject_class->set_property = bobgui_grid_layout_set_property;
  gobject_class->get_property = bobgui_grid_layout_get_property;
  gobject_class->finalize = bobgui_grid_layout_finalize;

  /**
   * BobguiGridLayout:row-spacing:
   *
  ...
  /**
   * BobguiGridLayout:column-spacing:
   *
  ...
  /**
   * BobguiGridLayout:row-homogeneous:
   *
  ...
  /**
   * BobguiGridLayout:column-homogeneous:
   *
  ...
  /**
   * BobguiGridLayout:baseline-row:
   *
   * The row to align to the baseline, when `BobguiWidget:valign` is set
   * to %BOBGUI_ALIGN_BASELINE.
   */
...
static void
bobgui_grid_layout_init (BobguiGridLayout *self)
{
}

/**
 * bobgui_grid_layout_new:
 *
 * Creates a new `BobguiGridLayout`.
 *
 * Returns: the newly created `BobguiGridLayout`
 */
BobguiLayoutManager *
bobgui_grid_layout_new (void)
{
  return g_object_new (BOBGUI_TYPE_GRID_LAYOUT, NULL);
}

/**
 * bobgui_grid_layout_set_row_homogeneous:
 * @grid: a `BobguiGridLayout`
 * @homogeneous: %TRUE to make rows homogeneous
 *
 * Sets whether all rows of @grid should have the same height.
 */
void
bobgui_grid_layout_set_row_homogeneous (BobguiGridLayout *grid,
                                     gboolean       homogeneous)
{
  g_return_if_fail (BOBGUI_IS_GRID_LAYOUT (grid));
  ...
  bobgui_layout_manager_layout_changed (BOBGUI_LAYOUT_MANAGER (grid));
  g_object_notify_by_pspec (G_OBJECT (grid), layout_props[PROP_ROW_HOMOGENEOUS]);
}

/**
 * bobgui_grid_layout_get_row_homogeneous:
 * @grid: a `BobguiGridLayout`
 *
 ...
 */
gboolean
bobgui_grid_layout_get_row_homogeneous (BobguiGridLayout *grid)
{
  g_return_val_if_fail (BOBGUI_IS_GRID_LAYOUT (grid), FALSE);

  return COLUMNS (grid)->homogeneous;
}

/**
 * bobgui_grid_layout_set_row_spacing:
 * @grid: a `BobguiGridLayout`
 ...
 */
void
bobgui_grid_layout_set_row_spacing (BobguiGridLayout *grid,
                                 guint          spacing)
{
  g_return_if_fail (BOBGUI_IS_GRID_LAYOUT (grid));
...
/**
 * bobgui_grid_layout_get_row_spacing:
 * @grid: a `BobguiGridLayout`
 *
 * Retrieves the spacing set with bobgui_grid_layout_set_row_spacing().
 *
 * Returns: the spacing between consecutive rows
 */
guint
bobgui_grid_layout_get_row_spacing (BobguiGridLayout *grid)
{
  g_return_val_if_fail (BOBGUI_IS_GRID_LAYOUT (grid), 0);

  return COLUMNS (grid)->spacing;
}

/**
 * bobgui_grid_layout_set_column_homogeneous:
 * @grid: a `BobguiGridLayout`
 ...
 */
void
bobgui_grid_layout_set_column_homogeneous (BobguiGridLayout *grid,
                                        gboolean       homogeneous)
{
  g_return_if_fail (BOBGUI_IS_GRID_LAYOUT (grid));
...
/**
 * bobgui_grid_layout_get_column_homogeneous:
 * @grid: a `BobguiGridLayout`
 *
 ...
 */
gboolean
bobgui_grid_layout_get_column_homogeneous (BobguiGridLayout *grid)
{
  g_return_val_if_fail (BOBGUI_IS_GRID_LAYOUT (grid), FALSE);
...
/**
 * bobgui_grid_layout_set_column_spacing:
 * @grid: a `BobguiGridLayout`
 ...
 */
void
bobgui_grid_layout_set_column_spacing (BobguiGridLayout *grid,
                                    guint          spacing)
{
  g_return_if_fail (BOBGUI_IS_GRID_LAYOUT (grid));
  g_return_if_fail (spacing <= G_MAXINT16);
...
/**
 * bobgui_grid_layout_get_column_spacing:
 * @grid: a `BobguiGridLayout`
 *
 * Retrieves the spacing set with bobgui_grid_layout_set_column_spacing().
 *
 * Returns: the spacing between consecutive columns
 */
guint
bobgui_grid_layout_get_column_spacing (BobguiGridLayout *grid)
{
  g_return_val_if_fail (BOBGUI_IS_GRID_LAYOUT (grid), 0);
...
find_row_properties (BobguiGridLayout *self,
		     int            row)
{
  int i;
...
static GridRowProperties *
get_row_properties_or_create (BobguiGridLayout *self,
			      int            row)
{
...
get_row_properties_or_default (BobguiGridLayout *self,
			       int            row)
{
  GridRowProperties *props;
...
/**
 * bobgui_grid_layout_set_row_baseline_position:
 * @grid: a `BobguiGridLayout`
 * @row: a row index
 ...
 */
void
bobgui_grid_layout_set_row_baseline_position (BobguiGridLayout       *grid,
                                           int                  row,
...
/**
 * bobgui_grid_layout_get_row_baseline_position:
 * @grid: a `BobguiGridLayout`
 * @row: a row index
 *
 ...
 * [method@Bobgui.GridLayout.set_row_baseline_position],
 * the default value of %BOBGUI_BASELINE_POSITION_CENTER
 * is returned.
 *
 * Returns: the baseline position of @row
 */
BobguiBaselinePosition
bobgui_grid_layout_get_row_baseline_position (BobguiGridLayout *grid,
                                           int            row)
{
  const GridRowProperties *props;

  g_return_val_if_fail (BOBGUI_IS_GRID_LAYOUT (grid), BOBGUI_BASELINE_POSITION_CENTER);
...
/**
 * bobgui_grid_layout_set_baseline_row:
 * @grid: a `BobguiGridLayout`
 * @row: the row index
 *
 ...
 */
void
bobgui_grid_layout_set_baseline_row (BobguiGridLayout *grid,
                                  int            row)
{
  g_return_if_fail (BOBGUI_IS_GRID_LAYOUT (grid));
...
/**
 * bobgui_grid_layout_get_baseline_row:
 * @grid: a `BobguiGridLayout`
 *
 * Retrieves the row set with bobgui_grid_layout_set_baseline_row().
 *
 * Returns: the global baseline row
 */
int
bobgui_grid_layout_get_baseline_row (BobguiGridLayout *grid)
{
  g_return_val_if_fail (BOBGUI_IS_GRID_LAYOUT (grid), BOBGUI_BASELINE_POSITION_CENTER);
