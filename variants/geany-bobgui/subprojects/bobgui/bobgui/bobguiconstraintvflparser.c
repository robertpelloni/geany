/* bobguiconstraintvflparser.c: VFL constraint definition parser
 *
...
struct _BobguiConstraintVflParser
{
  char *buffer;
  gsize buffer_len;
...
/* NOTE: These two symbols are defined in bobguiconstraintlayout.h, but we
...
{
  BobguiConstraintVflParser *res = g_new0 (BobguiConstraintVflParser, 1);
...
static int
get_default_spacing (BobguiConstraintVflParser *parser)
{
                ...
                GError **error)
{
  ...
  if (*cur == '=')
  ...
  else if (*cur == '>')
    {
      ...
      if (*cur == '=')
        {
          *relation = BOBGUI_CONSTRAINT_RELATION_GE;
          *endptr = (char *) cur + 1;
          return TRUE;
        }
  ...
  else if (*cur == '<')
    {
      ...
      if (*cur == '=')
        {
          *relation = BOBGUI_CONSTRAINT_RELATION_LE;
          *endptr = (char *) cur + 1;
          return TRUE;
        }
...
static gboolean 
has_metric (BobguiConstraintVflParser *parser,
            const char *name)
{
...
static gboolean
has_view (BobguiConstraintVflParser *parser,
          const char *name)
{
...
static gboolean 
parse_predicate (BobguiConstraintVflParser *parser,
                 const char *cursor,
                 VflPredicate *predicate,
                 char **endptr,
                 GError **error)
{
  ...
  if (*end == '=' || *end == '>' || *end == '<')
    ...
    predicate->relation = BOBGUI_CONSTRAINT_RELATION_EQ;
  ...
  if (*end == '@')
    {
      ...
      if (g_ascii_isdigit (*end))
        ...
        {
          priority = BOBGUI_CONSTRAINT_STRENGTH_WEAK;
          end += 4;
        }
      else if (strncmp (end, "medium", 6) == 0)
        {
          priority = BOBGUI_CONSTRAINT_STRENGTH_MEDIUM;
          end += 6;
        }
      else if (strncmp (end, "strong", 6) == 0)
        {
          priority = BOBGUI_CONSTRAINT_STRENGTH_STRONG;
          end += 6;
        }
      else if (strncmp (end, "required", 8) == 0)
        {
          priority = BOBGUI_CONSTRAINT_STRENGTH_REQUIRED;
          end += 8;
        }
      else
        {
    ...
    predicate->priority = BOBGUI_CONSTRAINT_STRENGTH_REQUIRED;
...
static gboolean
parse_view (BobguiConstraintVflParser *parser,
            const char *cursor,
            VflView *view,
            char **endptr,
            GError **error)
  ...
  while (*end != '\0')
    {
      ...
      g_debug ("*** Found predicate: %s.%s %s %g %s (%g)\n",
               ...
               cur_predicate.attr,
               cur_predicate.relation == BOBGUI_CONSTRAINT_RELATION_EQ ? "==" :
               cur_predicate.relation == BOBGUI_CONSTRAINT_RELATION_LE ? "<=" :
               cur_predicate.relation == BOBGUI_CONSTRAINT_RELATION_GE ? ">=" :
               "unknown relation",
               cur_predicate.constant,
               cur_predicate.priority == BOBGUI_CONSTRAINT_STRENGTH_WEAK ? "weak" :
               cur_predicate.priority == BOBGUI_CONSTRAINT_STRENGTH_MEDIUM ? "medium" :
               cur_predicate.priority == BOBGUI_CONSTRAINT_STRENGTH_STRONG ? "strong" :
               cur_predicate.priority == BOBGUI_CONSTRAINT_STRENGTH_REQUIRED ? "required" :
               "explicit strength",
...
static void
bobgui_constraint_vfl_parser_clear (BobguiConstraintVflParser *parser)
{
...
bobgui_constraint_vfl_parser_free (BobguiConstraintVflParser *parser)
{
  ...
  bobgui_constraint_vfl_parser_clear (parser);
                                           ...
                                           int *n_constraints)
{
  ...
  constraints = g_array_new (FALSE, FALSE, sizeof (BobguiConstraintVfl));

  iter = parser->views;
  while (iter != NULL)
    {
      BobguiConstraintVfl c;
      ...
      if ((iter->spacing.flags & SPACING_SET) != 0)
        {
          ...
          if ((iter->spacing.flags & SPACING_PREDICATE) != 0)
          ...
          else if ((iter->spacing.flags & SPACING_DEFAULT) != 0)
            {
              ...
              c.relation = BOBGUI_CONSTRAINT_RELATION_EQ;
              c.strength = BOBGUI_CONSTRAINT_STRENGTH_REQUIRED;
            }
          else
            {
  ...
  for (int i = 0; i < constraints->len; i++)
    {
      const BobguiConstraintVfl *c = &g_array_index (constraints, BobguiConstraintVfl, i);
  ...
  return (BobguiConstraintVfl *) g_array_free (constraints, FALSE);
}
