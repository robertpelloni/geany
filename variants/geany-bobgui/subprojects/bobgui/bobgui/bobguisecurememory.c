...
/* bobguisecurememory.c - API for allocating memory that is non-pageable
...
#define DO_LOCK() \
	BOBGUI_SECURE_GLOBALS.lock ();

#define DO_UNLOCK() \
	BOBGUI_SECURE_GLOBALS.unlock ();
...
#define BOBGUI_SECURE_POOL_VER_STR             "1.0"

static int show_warning = 1;
static int bobgui_secure_warnings = 1;
static GMutex memory_mutex;

static void
bobgui_memory_lock (void)
{
  g_mutex_lock (&memory_mutex);
}

static void
bobgui_memory_unlock (void)
{
  g_mutex_unlock (&memory_mutex);
}

static BobguiSecureGlob BOBGUI_SECURE_GLOBALS = {
  .lock = bobgui_memory_lock,
  .unlock = bobgui_memory_unlock,
  .fallback_alloc = g_realloc,
  .fallback_free = g_free,
  .pool_data = NULL,
  .pool_version = BOBGUI_SECURE_POOL_VER_STR,
};
...
{
        if (BOBGUI_SECURE_GLOBALS.pool_version == NULL ||
	    strcmp (BOBGUI_SECURE_GLOBALS.pool_version, BOBGUI_SECURE_POOL_VER_STR) != 0) {
                return 0;
        }
...
pool_alloc (void)
{
		...
		if (show_warning && bobgui_secure_warnings)
			fprintf (stderr, "the secure memory pool version does not match the code '%s' != '%s'\n",
			         BOBGUI_SECURE_GLOBALS.pool_version ? BOBGUI_SECURE_GLOBALS.pool_version : "(null)",
			         BOBGUI_SECURE_POOL_VER_STR);
		show_warning = 0;
		return NULL;
	}
	...
	for (pool = BOBGUI_SECURE_GLOBALS.pool_data; pool != NULL; pool = pool->next) {
	...
	if (pool == NULL) {
		...
		pool = pages;
		pool->next = BOBGUI_SECURE_GLOBALS.pool_data;
		BOBGUI_SECURE_GLOBALS.pool_data = pool;
		pool->length = len;
		pool->used = 0;
		pool->unused = NULL;
...
pool_free (void* item)
{
	...
	for (at = (Pool **)&BOBGUI_SECURE_GLOBALS.pool_data, pool = *at;
...
pool_valid (void* item)
{
	...
	/* Find which block this one belongs to */
	for (pool = BOBGUI_SECURE_GLOBALS.pool_data; pool; pool = pool->next) {
		beg = (char*)pool->items;
		end = (char*)pool + pool->length - sizeof (Item);
		if (ptr >= beg && ptr <= end)
                   ...
                   const char *during_tag)
{
	...
	*sz = (*sz + pgsize -1) & ~(pgsize - 1);
	...
	if (mlock (pages, *sz) < 0) {
		if (show_warning && bobgui_secure_warnings && errno != EPERM) {
	...
	DEBUG_ALLOC ("bobgui-secure-memory: new block ", *sz);
...
sec_release_pages (void *pages, size_t sz)
{
	...
	if (munlock (pages, sz) < 0 && bobgui_secure_warnings)
		fprintf (stderr, "couldn't unlock private memory: %s\n", strerror (errno));

	if (munmap (pages, sz) < 0 && bobgui_secure_warnings)
		fprintf (stderr, "couldn't unmap private anonymous memory: %s\n", strerror (errno));

	DEBUG_ALLOC ("bobgui-secure-memory: freed block ", sz);
...
void*
bobgui_secure_alloc_full (const char *tag,
                       size_t length,
                       int flags)
{
	Block *block;
	...
	if (!memory && (flags & BOBGUI_SECURE_USE_FALLBACK) && BOBGUI_SECURE_GLOBALS.fallback_alloc != NULL) {
		memory = BOBGUI_SECURE_GLOBALS.fallback_alloc (NULL, length);
...
void*
bobgui_secure_realloc_full (const char *tag,
                         void *memory,
                         size_t length,
                         int flags)
{
	...
	if (memory == NULL)
		return bobgui_secure_alloc_full (tag, length, flags);
	if (!length) {
		bobgui_secure_free_full (memory, flags);
		return NULL;
	}
	...
	if (!block) {
		if ((flags & BOBGUI_SECURE_USE_FALLBACK) && BOBGUI_SECURE_GLOBALS.fallback_alloc) {
			/*
			 * In this case we can't zero the returned memory,
			 * because we don't know what the block size was.
			 */
			return BOBGUI_SECURE_GLOBALS.fallback_alloc (memory, length);
		} else {
			if (bobgui_secure_warnings)
	...
	if (donew) {
		alloc = bobgui_secure_alloc_full (tag, length, flags);
		if (alloc) {
...
void
bobgui_secure_free (void *memory)
{
	bobgui_secure_free_full (memory, BOBGUI_SECURE_USE_FALLBACK);
}

void
bobgui_secure_free_full (void *memory, int flags)
{
	Block *block = NULL;
	...
	if (block != NULL || !(flags & BOBGUI_SECURE_USE_FALLBACK))
		VALGRIND_FREELIKE_BLOCK (memory, sizeof (word_t));
#endif
	...
	if (!block) {
		if ((flags & BOBGUI_SECURE_USE_FALLBACK) && BOBGUI_SECURE_GLOBALS.fallback_free) {
			BOBGUI_SECURE_GLOBALS.fallback_free (memory);
		} else {
			if (bobgui_secure_warnings)
				fprintf (stderr, "memory does not belong to secure memory pool: 0x%08" G_GUINTPTR_FORMAT "x\n",
				         (guintptr) memory);
...
int
bobgui_secure_check (const void *memory)
{
	Block *block = NULL;
...
static bobgui_secure_rec *
records_for_ring (Cell *cell_ring,
                  bobgui_secure_rec *records,
                  unsigned int *count,
                  unsigned int *total)
{
	bobgui_secure_rec *new_rec;
	...
	do {
		if (*count >= allocated) {
			new_rec = realloc (records, sizeof (bobgui_secure_rec) * (allocated + 32));
			if (new_rec == NULL) {
				*count = 0;
				free (records);
				return NULL;
...
bobgui_secure_rec *
bobgui_secure_records (unsigned int *count)
{
	bobgui_secure_rec *records = NULL;
	Block *block = NULL;
	unsigned int total;
...
char*
bobgui_secure_strdup_full (const char *tag,
                        const char *str,
                        int options)
{
	size_t len;
	...
	len = strlen (str) + 1;
	res = (char *)bobgui_secure_alloc_full (tag, len, options);
	strcpy (res, str);
	return res;
}

char *
bobgui_secure_strndup_full (const char *tag,
                         const char *str,
                         size_t length,
                         int options)
{
...
void
bobgui_secure_clear (void *p, size_t length)
{
	volatile char *vp;
...
void
bobgui_secure_strclear (char *str)
{
	if (!str)
		return;
	bobgui_secure_clear ((unsigned char*)str, strlen (str));
}

void
bobgui_secure_strfree (char *str)
{
	/*
	...
	bobgui_secure_strclear (str);
	bobgui_secure_free_full (str, BOBGUI_SECURE_USE_FALLBACK);
}
