 ...
 *
 * A `BobguiSectionModel` groups successive items into so-called sections. List widgets
 * like `BobguiListView` and `BobguiGridView` then allow displaying section headers for
 * these sections by installing a header factory.
 *
 * Many BOBGUI list models support sections inherently, or they pass through the sections
 * of a model they are wrapping.
 *
 * When the section groupings of a model change, the model will emit the
 * [signal@Bobgui.SectionModel::sections-changed] signal by calling the
 * [method@Bobgui.SectionModel.sections_changed] function. All sections in the given range
...
G_DEFINE_INTERFACE (BobguiSectionModel, bobgui_section_model, G_TYPE_LIST_MODEL)
...
static void
bobgui_section_model_default_get_section (BobguiSectionModel *self,
                                       guint            position,
                                       guint           *out_start,
                                       guint           *out_end)
{
...
static void
bobgui_section_model_default_init (BobguiSectionModelInterface *iface)
{
  iface->get_section = bobgui_section_model_default_get_section;

  /**
   * BobguiSectionModel::sections-changed:
   * @model: a `BobguiSectionModel`
   ...
   * [signal@Bobgui.SectionModel::sections-changed] signal for all the items
   * it covers.
   *
   * Since: 4.12
   */
...
/**
 * bobgui_section_model_get_section:
 * @self: a `BobguiSectionModel`
 ...
 */
void
bobgui_section_model_get_section (BobguiSectionModel *self,
                               guint            position,
                               guint           *out_start,
                               guint           *out_end)
{
  BobguiSectionModelInterface *iface;

  g_return_if_fail (BOBGUI_IS_SECTION_MODEL (self));
  g_return_if_fail (out_start != NULL);
  g_return_if_fail (out_end != NULL);

  iface = BOBGUI_SECTION_MODEL_GET_IFACE (self);
  iface->get_section (self, position, out_start, out_end);

  g_warn_if_fail (*out_start < *out_end);
}

/* A version of bobgui_section_model_get_section() that handles NULL
 * (treats it as the empty list) and any GListModel (treats it as
 * a single section).
 **/
void
bobgui_list_model_get_section (GListModel *self,
                            guint       position,
                            guint      *out_start,
                            guint      *out_end)
{
  ...
  if (!BOBGUI_IS_SECTION_MODEL (self))
    {
  ...
  bobgui_section_model_get_section (BOBGUI_SECTION_MODEL (self), position, out_start, out_end);
}

/**
 * bobgui_section_model_sections_changed:
 * @self: a `BobguiSectionModel`
 * @position: the first changed item
 * @n_items: the number of changed items
 *
 * This function emits the [signal@Bobgui.SectionModel::sections-changed]
 ...
 */
void
bobgui_section_model_sections_changed (BobguiSectionModel *self,
                                    guint            position,
                                    guint            n_items)
{
  g_return_if_fail (BOBGUI_IS_SECTION_MODEL (self));
