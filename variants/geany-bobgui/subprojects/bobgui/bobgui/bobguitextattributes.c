/* bobguitextattributes.c - text attributes
 *
...
/**
 * bobgui_text_attributes_new:
 * 
 * Creates a `BobguiTextAttributes`, which describes
 * a set of properties on some text.
 * 
 * Returns: a new `BobguiTextAttributes`,
 *   free with bobgui_text_attributes_unref().
 */
BobguiTextAttributes*
bobgui_text_attributes_new (void)
{
  BobguiTextAttributes *values;

  values = g_new0 (BobguiTextAttributes, 1);

  /* 0 is a valid value for most of the struct */
  values->refcount = 1;

  values->language = bobgui_get_default_language ();
...
/**
 * bobgui_text_attributes_copy:
 * @src: a `BobguiTextAttributes` to be copied
 *
 * Copies @src and returns a new `BobguiTextAttributes`.
 *
 * Returns: a copy of @src,
 *   free with bobgui_text_attributes_unref()
 */
BobguiTextAttributes*
bobgui_text_attributes_copy (BobguiTextAttributes *src)
{
  BobguiTextAttributes *dest;

  dest = bobgui_text_attributes_new ();
  bobgui_text_attributes_copy_values (src, dest);

  return dest;
}

/**
 * bobgui_text_attributes_copy_values:
 * @src: a `BobguiTextAttributes`
 * @dest: another `BobguiTextAttributes`
 *
 * Copies the values from @src to @dest so that @dest has
 * the same values as @src. Frees existing values in @dest.
 */
void
bobgui_text_attributes_copy_values (BobguiTextAttributes *src,
                                 BobguiTextAttributes *dest)
{
  guint orig_refcount;
...
/**
 * bobgui_text_attributes_ref:
 * @values: a `BobguiTextAttributes`
 *
 * Increments the reference count on @values.
 *
 * Returns: the `BobguiTextAttributes` that were passed in
 **/
BobguiTextAttributes *
bobgui_text_attributes_ref (BobguiTextAttributes *values)
{
 ...
 **/
void
bobgui_text_attributes_unref (BobguiTextAttributes *values)
{
...
void
_bobgui_text_attributes_fill_from_tags (BobguiTextAttributes *dest,
                                     GPtrArray         *tags)
{
  ...
  for (guint n = 0; n < tags->len; n++)
    {
      BobguiTextTag *tag = g_ptr_array_index (tags, n);
      BobguiTextAttributes *vals = tag->priv->values;

      g_assert (tag->priv->table != NULL);
      if (n > 0)
        g_assert (((BobguiTextTag*)g_ptr_array_index (tags, n))->priv->priority > ((BobguiTextTag *)g_ptr_array_index (tags, n - 1))->priv->priority);
      ...
      if (vals->font)
	{
      ...
      if (vals->direction != BOBGUI_TEXT_DIR_NONE)
...
gboolean
_bobgui_text_tag_affects_size (BobguiTextTag *tag)
{
...
gboolean
_bobgui_text_tag_affects_nonsize_appearance (BobguiTextTag *tag)
{
  BobguiTextTagPrivate *priv = tag->priv;
