...
static BobguiXdpOpenURI *openuri;
  ...
  if (g_once_init_enter (&openuri_inited))
    {
      ...
      if (connection != NULL)
        {
          openuri = bobgui_xdp_open_uri_proxy_new_sync (connection, 0,
                                                     PORTAL_BUS_NAME,
                                                     PORTAL_OBJECT_PATH,
                                                     NULL, &error);
          if (openuri == NULL)
          ...
          else if (bobgui_xdp_open_uri_get_version (openuri) < 3)
            {
              g_warning ("Not a supported version of the OpenURI portal: %u", bobgui_xdp_open_uri_get_version (openuri));
...
gboolean
bobgui_openuri_portal_is_available (void)
{
...
open_uri_data_free (OpenUriData *data)
{
  ...
  if (data->parent && data->parent_handle)
    bobgui_window_unexport_handle (data->parent, data->parent_handle);
                   ...
                   gpointer         user_data)
{
  ...
  switch (response)
    ...
    case XDG_DESKTOP_PORTAL_CANCELLED:
      g_task_return_new_error (task, BOBGUI_DIALOG_ERROR, BOBGUI_DIALOG_ERROR_DISMISSED, "The portal dialog was dismissed by the user");
      break;
    case XDG_DESKTOP_PORTAL_FAILED:
    default:
      g_task_return_new_error (task, BOBGUI_DIALOG_ERROR, BOBGUI_DIALOG_ERROR_FAILED, "The application launch failed");
      break;
    }
...
open_call_done (GObject      *source,
                ...
                gpointer      user_data)
{
  BobguiXdpOpenURI *portal = BOBGUI_XDP_OPEN_URI (source);
  GTask *task = user_data;
  ...
  switch (data->call)
    {
    case OPEN_FILE:
      res = bobgui_xdp_open_uri_call_open_file_finish (portal, &path, NULL, result, &error);
      break;
    case OPEN_FOLDER:
      res = bobgui_xdp_open_uri_call_open_directory_finish (portal, &path, NULL, result, &error);
      break;
    case OPEN_URI:
      res = bobgui_xdp_open_uri_call_open_uri_finish (portal, &path, result, &error);
      break;
    default:
          ...
          GTask        *task)
{
  ...
  g_task_return_new_error (task,
                           BOBGUI_DIALOG_ERROR, BOBGUI_DIALOG_ERROR_CANCELLED,
          ...
          GAsyncReadyCallback  callback)
{
  ...
  if (!open_folder)
    {
      if (data->flags & BOBGUI_OPENURI_FLAGS_ASK)
        g_variant_builder_add (&opt_builder, "{sv}", "ask", g_variant_new_boolean (TRUE));
      if (data->flags & BOBGUI_OPENURI_FLAGS_WRITABLE)
  ...
  if (file && g_file_is_native (file))
      ...
      if (open_folder)
        {
          ...
          bobgui_xdp_open_uri_call_open_directory (openuri,
        ...
        {
          data->call = OPEN_FILE;
          bobgui_xdp_open_uri_call_open_file (openuri,
                                        parent_window ? parent_window : "",
                                        g_variant_new ("h", fd_id),
                                        opts,
                                        fd_list,
  ...
  else
    {
      ...
      data->call = OPEN_URI;
      bobgui_xdp_open_uri_call_open_uri (openuri,
                                   parent_window ? parent_window : "",
                                   uri ? uri : data->uri,
                                   opts,
                                   NULL,
                        ...
                        gpointer    user_data)
{
  ...
  if (window)
    display = bobgui_widget_get_display (BOBGUI_WIDGET (window));
  else
...
void
bobgui_openuri_portal_open_async (GFile               *file,
                               gboolean             open_folder,
                               BobguiOpenuriFlags      flags,
                               BobguiWindow           *parent,
                               GCancellable        *cancellable,
                               ...
                               gpointer             user_data)
{
  ...
  g_task_set_source_tag (data->task, bobgui_openuri_portal_open_async);
...
gboolean
bobgui_openuri_portal_open_finish (GAsyncResult  *result,
                                GError       **error)
{
  g_return_val_if_fail (g_task_get_source_tag (G_TASK (result)) == bobgui_openuri_portal_open_async, FALSE);

  return g_task_propagate_boolean (G_TASK (result), error);
}

void
bobgui_openuri_portal_open_uri_async (const char          *uri,
                                   BobguiWindow           *parent,
                                   GCancellable        *cancellable,
                                   GAsyncReadyCallback  callback,
                                   gpointer             user_data)
{
  ...
  data->task = g_task_new (parent, cancellable, callback, user_data);
  g_task_set_check_cancellable (data->task, FALSE);
  g_task_set_source_tag (data->task, bobgui_openuri_portal_open_uri_async);
...
gboolean
bobgui_openuri_portal_open_uri_finish (GAsyncResult  *result,
                                    GError       **error)
{
  g_return_val_if_fail (g_task_get_source_tag (G_TASK (result)) == bobgui_openuri_portal_open_uri_async, FALSE);

  return g_task_propagate_boolean (G_TASK (result), error);
}

gboolean
bobgui_openuri_portal_can_open (const char *uri)
{
  const char *scheme;
  GVariantBuilder opt_builder;
  gboolean supported;
  ...
  if (bobgui_xdp_open_uri_get_version (openuri) < 5)
    return TRUE;

  g_variant_builder_init (&opt_builder, G_VARIANT_TYPE_VARDICT);
  if (!bobgui_xdp_open_uri_call_scheme_supported_sync (openuri,
                                                    scheme,
