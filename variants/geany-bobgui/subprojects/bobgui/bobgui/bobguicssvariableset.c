...
{
  if (pointer != NULL)
    bobgui_css_variable_value_unref (pointer);
}

BobguiCssVariableSet *
bobgui_css_variable_set_new (void)
{
  BobguiCssVariableSet *self = g_new0 (BobguiCssVariableSet, 1);
...
BobguiCssVariableSet *
bobgui_css_variable_set_ref (BobguiCssVariableSet *self)
{
  self->ref_count++;
...
void
bobgui_css_variable_set_unref (BobguiCssVariableSet *self)
{
  self->ref_count--;
  if (self->ref_count > 0)
    return;
  ...
  g_clear_pointer (&self->parent, bobgui_css_variable_set_unref);
  g_free (self);
}

BobguiCssVariableSet *
bobgui_css_variable_set_copy (BobguiCssVariableSet *self)
{
  BobguiCssVariableSet *ret = bobgui_css_variable_set_new ();
  GHashTableIter iter;
  gpointer id;
  ...
  while (g_hash_table_iter_next (&iter, &id, (gpointer) &value))
    bobgui_css_variable_set_add (ret, GPOINTER_TO_INT (id), value);

  bobgui_css_variable_set_set_parent (ret, self->parent);

  return ret;
}

void
bobgui_css_variable_set_set_parent (BobguiCssVariableSet *self,
                                 BobguiCssVariableSet *parent)
{
  if (self->parent)
    bobgui_css_variable_set_unref (self->parent);

  self->parent = parent;

  if (self->parent)
    bobgui_css_variable_set_ref (self->parent);
}

void
bobgui_css_variable_set_add (BobguiCssVariableSet   *self,
                          int                  id,
                          BobguiCssVariableValue *value)
{
  BobguiCssCustomPropertyPool *pool = bobgui_css_custom_property_pool_get ();
  ...
  g_hash_table_insert (self->variables,
                       GINT_TO_POINTER (bobgui_css_custom_property_pool_ref (pool, id)),
                       value ? bobgui_css_variable_value_ref (value) : NULL);
}

static gboolean check_variable (BobguiCssVariableSet   *self,
...
static gboolean
check_references (BobguiCssVariableSet   *self,
                  GHashTable          *unvisited_variables,
                  GArray              *stack,
                  BobguiCssVariableValue *value)
{
  ...
  for (i = 0; i < value->n_references; i++)
    {
      ...
      int ref_id = bobgui_css_custom_property_pool_lookup (pool, ref->name);

      if (g_hash_table_contains (unvisited_variables, GINT_TO_POINTER (ref_id)))
        {
          BobguiCssVariableValue *ref_value;
...
static gboolean
check_variable (BobguiCssVariableSet   *self,
                GHashTable          *unvisited_variables,
                GArray              *stack,
                int                  id,
                BobguiCssVariableValue *value)
...
void
bobgui_css_variable_set_resolve_cycles (BobguiCssVariableSet *self)
{
...
bobgui_css_variable_set_lookup (BobguiCssVariableSet  *self,
                             int                 id,
                             BobguiCssVariableSet **source)
{
  BobguiCssVariableValue *ret = g_hash_table_lookup (self->variables, GINT_TO_POINTER (id));
  ...
  if (self->parent)
    return bobgui_css_variable_set_lookup (self->parent, id, source);
...
static void
list_ids_recursive (BobguiCssVariableSet *self,
                    GHashTable        *table)
{
  GHashTableIter iter;
  gpointer id;
...
GArray *
bobgui_css_variable_set_list_ids (BobguiCssVariableSet  *self)
{
  BobguiCssCustomPropertyPool *pool = bobgui_css_custom_property_pool_get ();
...
gboolean
bobgui_css_variable_set_equal (BobguiCssVariableSet *set1,
                            BobguiCssVariableSet *set2)
{
  GHashTableIter iter;
  gpointer id;
  BobguiCssVariableValue *value1;
  ...
  while (g_hash_table_iter_next (&iter, &id, (gpointer) &value1))
    {
      BobguiCssVariableValue *value2 = g_hash_table_lookup (set2->variables, id);

      if (value2 == NULL)
         return FALSE;

      if (!bobgui_css_variable_value_equal (value1, value2))
         return FALSE;
    }
