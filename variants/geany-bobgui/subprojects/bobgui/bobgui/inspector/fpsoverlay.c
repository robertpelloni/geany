...
typedef struct _BobguiFpsInfo BobguiFpsInfo;

struct _BobguiFpsInfo
{
  PangoFont *font;
...
struct _BobguiFpsOverlay
{
  BobguiInspectorOverlay parent_instance;

  GHashTable *infos; /* BobguiWidget => BobguiFpsInfo */
};

struct _BobguiFpsOverlayClass
{
  BobguiInspectorOverlayClass parent_class;
};

G_DEFINE_TYPE (BobguiFpsOverlay, bobgui_fps_overlay, BOBGUI_TYPE_INSPECTOR_OVERLAY)

static void
bobgui_fps_info_free (gpointer data)
{
  BobguiFpsInfo *info = data;
...
static BobguiFpsInfo *
bobgui_fps_info_new (BobguiWidget *widget)
{
  PangoLayout *layout;
  ...
  info = g_new0 (BobguiFpsInfo, 1);

  layout = bobgui_widget_create_pango_layout (widget, "000000.00 fps");
...
static double
bobgui_fps_overlay_get_fps (BobguiWidget *widget)
{
...
static gboolean
bobgui_fps_overlay_force_redraw (BobguiWidget     *widget,
                              GdkFrameClock *clock,
                              gpointer       unused)
{
...
static void
bobgui_fps_overlay_snapshot (BobguiInspectorOverlay *overlay,
                          BobguiSnapshot         *snapshot,
                          GskRenderNode       *node,
                          BobguiWidget           *widget)
{
  BobguiFpsOverlay *self = BOBGUI_FPS_OVERLAY (overlay);
  BobguiFpsInfo *info;
  gint64 now;
  double fps;
  ...
  if (info == NULL)
    {
      info = bobgui_fps_info_new (widget);
  ...
  if (BOBGUI_IS_WINDOW (widget))
    {
      BobguiWidget *child = bobgui_window_get_child (BOBGUI_WINDOW (widget));
      if (!child ||
          !bobgui_widget_compute_bounds (child, widget, &bounds))
        has_bounds = bobgui_widget_compute_bounds (widget, widget, &bounds);
      else
        has_bounds = bobgui_widget_compute_bounds (child, widget, &bounds);
    }
   else
    {
      has_bounds = bobgui_widget_compute_bounds (widget, widget, &bounds);
  ...
  if (has_bounds)
    bobgui_snapshot_translate (snapshot, &GRAPHENE_POINT_INIT (bounds.origin.x + bounds.size.width - info->width, bounds.origin.y));

  if (overlay_opacity < 1.0)
    bobgui_snapshot_push_opacity (snapshot, overlay_opacity);

  fps = bobgui_fps_overlay_get_fps (widget);
  if (fps != 0.0)
    {
      ...
      bobgui_snapshot_append_node (snapshot, fps_node);
  ...
  bobgui_widget_add_tick_callback (widget, bobgui_fps_overlay_force_redraw, NULL, NULL);
}

static void
bobgui_fps_overlay_queue_draw (BobguiInspectorOverlay *overlay)
{
  BobguiFpsOverlay *self = BOBGUI_FPS_OVERLAY (overlay);
  GHashTableIter iter;
  gpointer widget;
  ...
  while (g_hash_table_iter_next (&iter, &widget, NULL))
    {
      GdkSurface *surface = bobgui_native_get_surface (bobgui_widget_get_native (widget));
      if (surface)
...
static void
bobgui_fps_overlay_dispose (GObject *object)
{
  BobguiFpsOverlay *self = BOBGUI_FPS_OVERLAY (object);

  g_hash_table_unref (self->infos);

  G_OBJECT_CLASS (bobgui_fps_overlay_parent_class)->dispose (object);
}

static void
bobgui_fps_overlay_class_init (BobguiFpsOverlayClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  BobguiInspectorOverlayClass *overlay_class = BOBGUI_INSPECTOR_OVERLAY_CLASS (klass);

  overlay_class->snapshot = bobgui_fps_overlay_snapshot;
  overlay_class->queue_draw = bobgui_fps_overlay_queue_draw;

  gobject_class->dispose = bobgui_fps_overlay_dispose;
}

static void
bobgui_fps_overlay_init (BobguiFpsOverlay *self)
{
  self->infos = g_hash_table_new_full (g_direct_hash, g_direct_equal, NULL, bobgui_fps_info_free);
}

BobguiInspectorOverlay *
bobgui_fps_overlay_new (void)
{
  BobguiFpsOverlay *self;

  self = g_object_new (BOBGUI_TYPE_FPS_OVERLAY, NULL);
