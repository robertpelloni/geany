...
typedef struct
{
  const BobguiBuildableParser *last_parser;
  gpointer last_user_data;
  int last_depth;
} BobguiBuildableParserStack;

static void
pop_subparser_stack (BobguiBuildableParseContext *context)
{
  BobguiBuildableParserStack *stack = &g_array_index (context->subparser_stack, BobguiBuildableParserStack,
...
static void
possibly_finish_subparser (BobguiBuildableParseContext *context)
{
  BobguiBuildableParserStack *stack;

  if (!context->subparser_stack ||
      context->subparser_stack->len == 0)
    return;

  stack = &g_array_index (context->subparser_stack, BobguiBuildableParserStack,
                          context->subparser_stack->len - 1);
...
proxy_start_element (GMarkupParseContext *gm_context,
                     ...
                     GError             **error)
{
  BobguiBuildableParseContext *context = user_data;
...
static void
bobgui_buildable_parse_context_init (BobguiBuildableParseContext *context,
...
static void
bobgui_buildable_parse_context_free (BobguiBuildableParseContext *context)
{
  if (context->subparser_stack)
...
static gboolean
bobgui_buildable_parse_context_parse (BobguiBuildableParseContext *context,
                                   const char           *text,
                                   gssize                text_len,
                                   GError              **error)
{
  ...
  if (_bobgui_buildable_parser_is_precompiled (text, text_len))
    {
      res = _bobgui_buildable_parser_replay_precompiled (context, text, text_len, error);
    }
  else
    {
...
/**
 * bobgui_buildable_parse_context_push:
 * @context: a `BobguiBuildableParseContext`
 * @parser: a `BobguiBuildableParser`
 * @user_data: user data to pass to `BobguiBuildableParser` functions
 *
 * Temporarily redirects markup data to a sub-parser.
 *
 * This function may only be called from the start_element handler of
 * a `BobguiBuildableParser`. It must be matched with a corresponding call to
 * bobgui_buildable_parse_context_pop() in the matching end_element handler
 ...
 * which is why bobgui_buildable_parse_context_pop() is provided to allow "one
 * last access" to the @user_data provided to this function. In the
 * case of error, the @user_data provided here is passed directly to
 * the error callback of the subparser and bobgui_buildable_parse_context_pop()
 ...
 **/
void
bobgui_buildable_parse_context_push (BobguiBuildableParseContext *context,
                                  const BobguiBuildableParser *parser,
                                  gpointer                  user_data)
{
  BobguiBuildableParserStack stack = { 0 };
  ...
  if (!context->subparser_stack)
    context->subparser_stack = g_array_new (FALSE, FALSE, sizeof (BobguiBuildableParserStack));

  g_array_append_val (context->subparser_stack, stack);
}

/**
 * bobgui_buildable_parse_context_pop:
 * @context: a `BobguiBuildableParseContext`
 *
 * Completes the process of a temporary sub-parser redirection.
 *
 * This function exists to collect the user_data allocated by a
 * matching call to bobgui_buildable_parse_context_push(). It must be called
 * in the end_element handler corresponding to the start_element
 * handler during which bobgui_buildable_parse_context_push() was called.
 ...
 * interface.
 *
 * Returns: the user data passed to bobgui_buildable_parse_context_push()
 */
gpointer
bobgui_buildable_parse_context_pop (BobguiBuildableParseContext *context)
{
  gpointer user_data;
...
/**
 * bobgui_buildable_parse_context_get_element:
 * @context: a `BobguiBuildablParseContext`
 *
 * Retrieves the name of the currently open element.
 *
 * If called from the start_element or end_element handlers this will
 * give the element_name as passed to those functions. For the parent
 * elements, see bobgui_buildable_parse_context_get_element_stack().
 *
 * Returns: (nullable): the name of the currently open element
 */
const char *
bobgui_buildable_parse_context_get_element (BobguiBuildableParseContext *context)
{
...
/**
 * bobgui_buildable_parse_context_get_element_stack:
 * @context: a `BobguiBuildableParseContext`
 *
 ...
 * bobgui_buildable_parse_context_get_element()) and the previous item is its
 * immediate parent.
 *
 * This function is intended to be used in the start_element and
 * end_element handlers where bobgui_buildable_parse_context_get_element()
 ...
 */
GPtrArray *
bobgui_buildable_parse_context_get_element_stack (BobguiBuildableParseContext *context)
{
  return context->tag_stack;
}

/**
 * bobgui_buildable_parse_context_get_position:
 * @context: a `BobguiBuildableParseContext`
 ...
 */
void
bobgui_buildable_parse_context_get_position (BobguiBuildableParseContext *context,
                                          int                 *line_number,
                                          int                 *char_number)
                         ...
                         GError      **error)
{
  ...
  bobgui_buildable_parse_context_get_position (&data->ctx, &line, &col);
                   ...
                   GError     **error)
{
  ...
  if (object_info->object == NULL)
    {
      object = _bobgui_builder_construct (data->builder, object_info, error);
      if (!object)
        return NULL;
    }
  else
    {
      ...
      _bobgui_builder_apply_properties (data->builder, object_info, error);
    }
                ...
                GError      **error)
{
  ...
  if (!g_markup_collect_attributes (element_name, names, values, error,
    ...
    {
      _bobgui_builder_prefix_error (data->builder, &data->ctx, error);
      return;
    }
...
static void
parse_object (BobguiBuildableParseContext  *context,
              ParserData                *data,
              const char                *element_name,
              const char               **names,
              const char               **values,
              GError                   **error)
{
   ...
   * BOBGUI_BUILDER_ERROR_MISSING_ATTRIBUTE, not
  ...
  if (type_func)
    {
       ...
       */
      object_type = bobgui_builder_scope_get_type_from_function (bobgui_builder_get_scope (data->builder), data->builder, type_func);
  ...
  else
    {
      ...
      object_type = bobgui_builder_get_type_from_name (data->builder, object_class);
      if (object_type == G_TYPE_INVALID)
        {
          g_set_error (error,
                       BOBGUI_BUILDER_ERROR,
      ...
      if (is_requested_object (object_id, data))
        {
          ...
          BOBGUI_DEBUG (BUILDER_TRACE, "requested object \"%s\" found at level %d",
  ...
  bobgui_buildable_parse_context_get_position (context, &line, NULL);
  g_hash_table_insert (data->object_ids, g_strdup (object_id), GINT_TO_POINTER (line));
}

static void
parse_template (BobguiBuildableParseContext  *context,
                ParserData                *data,
                const char                *element_name,
                const char               **names,
                const char               **values,
                GError                   **error)
{
  ...
  template_type = bobgui_builder_get_template_type (data->builder, &allow_parents);
  ...
  if (template_type == 0)
    {
      g_set_error (error,
                   ...
                   object_class, parent_class ? parent_class : "BobguiWidget");
      _bobgui_builder_prefix_error (data->builder, context, error);
      return;
    }
  ...
  object_info->object = bobgui_builder_get_object (data->builder, object_class);
...
{
  /* Do not free the signal items, which BobguiBuilder takes ownership of */
                ...
                GError      **error)
{
  ...
  if (bind_flags_str)
    {
      guint flags;
      if (!_bobgui_builder_flags_from_string (G_TYPE_BINDING_FLAGS, bind_flags_str, &flags, error))
        {
          _bobgui_builder_prefix_error (data->builder, &data->ctx, error);
          return;
        }
               ...
               GError      **error)
{
  ...
  bobgui_buildable_parse_context_get_position (&data->ctx, &info->line, &info->col);
...
free_property_info (PropertyInfo *info)
{
  if (info->value)
    {
      if (G_PARAM_SPEC_VALUE_TYPE (info->pspec) == BOBGUI_TYPE_EXPRESSION)
        bobgui_expression_unref (info->value);
      else
...
free_expression_info (ExpressionInfo *info)
{
  switch (info->expression_type)
    ...
    case EXPRESSION_EXPRESSION:
      g_clear_pointer (&info->expression, bobgui_expression_unref);
      break;
  ...
  if (common_info->tag_type == TAG_PROPERTY)
    {
      ...
      return G_PARAM_SPEC_VALUE_TYPE (prop_info->pspec) == BOBGUI_TYPE_EXPRESSION;
    }
                           ...
                           GError      **error)
{
  ...
  if (initial_string &&
      !_bobgui_builder_boolean_from_string (initial_string, &initial, error))
    {
      _bobgui_builder_prefix_error (data->builder, &data->ctx, error);
      return;
    }
  ...
  if (type_name == NULL)
    ...
    {
      type = bobgui_builder_get_type_from_name (data->builder, type_name);
...
BobguiExpression *
expression_info_construct (BobguiBuilder      *builder,
                           const char      *domain,
                           ExpressionInfo  *info,
                           GError         **error)
{
    ...
    case EXPRESSION_CONSTANT:
      {
        ...
        if (info->constant.type == G_TYPE_INVALID)
          {
            GObject *o = bobgui_builder_lookup_object (builder, info->constant.text->str, 0, 0, error);
            if (o == NULL)
              return NULL;

            expr = bobgui_object_expression_new (o);
          }
        else
          {
            ...
            if (info->constant.initial)
              {
                ...
                expr = bobgui_constant_expression_new_for_value (&value);
              }
            else
              {
                if (!bobgui_builder_value_from_string_type (builder,
                                                         info->constant.type,
                                                         info->constant.text->str,
                                                         &value,
                                                         error))
                ...
                if (G_VALUE_HOLDS_OBJECT (&value))
                  expr = bobgui_object_expression_new (g_value_get_object (&value));
                else
                  expr = bobgui_constant_expression_new_for_value (&value);
              }
    ...
    case EXPRESSION_CLOSURE:
      {
        ...
        if (info->closure.object_name)
          {
            object = bobgui_builder_lookup_object (builder, info->closure.object_name, 0, 0, error);
            if (object == NULL)
              return NULL;
          }
        else
        ...
        params = g_newa (BobguiExpression *, n_params);
        i = n_params;
        ...
        expression = bobgui_closure_expression_new (info->closure.type, closure, n_params, params);
    ...
    case EXPRESSION_PROPERTY:
      {
        ...
        if (info->property.this_type != G_TYPE_INVALID)
          ...
          type = bobgui_expression_get_value_type (expression);
        else
          {
            g_set_error (error,
                         BOBGUI_BUILDER_ERROR,
        ...
        expression = bobgui_property_expression_new_for_pspec (expression, pspec);
    ...
    case EXPRESSION_TRY:
       {
        ...
        expressions = g_newa (BobguiExpression *, n_expressions);
        i = n_expressions;
        ...
        expression = bobgui_try_expression_new (n_expressions, expressions);
  ...
  return bobgui_expression_ref (info->expression);
}
  ...
  if (domain)
    {
      ...
      bobgui_builder_set_translation_domain (data->builder, data->domain);
    }
}
                  ...
                  gpointer       user_data)
{
  ...
  subparser->parser = g_memdup2 (parser, sizeof (BobguiBuildableParser));
...
static gboolean
subparser_start (BobguiBuildableParseContext  *context,
                 const char                *element_name,
                 const char               **names,
                 const char               **values,
                 ParserData                *data,
...
static void
subparser_end (BobguiBuildableParseContext  *context,
               const char                *element_name,
               ParserData                *data,
               GError                   **error)
{
  ...
  bobgui_buildable_custom_tag_end (BOBGUI_BUILDABLE (data->subparser->object),
                                data->builder,
  ...
  if (_bobgui_builder_lookup_failed (data->builder, error))
    return;

  if (BOBGUI_BUILDABLE_GET_IFACE (data->subparser->object)->custom_finished)
...
static gboolean
parse_custom (BobguiBuildableParseContext  *context,
              const char                *element_name,
              const char               **names,
              const char               **values,
              ParserData                *data,
              GError                   **error)
{
  ...
  if (parent_info->tag_type == TAG_OBJECT ||
  ...
  else if (parent_info->tag_type == TAG_CHILD)
    {
      ...
      _bobgui_builder_add (data->builder, child_info);
  ...
  if (!bobgui_buildable_custom_tag_start (BOBGUI_BUILDABLE (object),
                                       data->builder,
                                       child,
                                       element_name,
                                       &parser,
...
static void
start_element (BobguiBuildableParseContext  *context,
               const char                *element_name,
               const char               **names,
               const char               **values,
               gpointer                   user_data,
               GError                   **error)
{
  ...
  if (strcmp (element_name, "object") == 0)
    ...
    _bobgui_builder_menu_start (data, element_name, names, values, error);
...
const char *
_bobgui_builder_parser_translate (const char *domain,
                               const char *context,
                               const char *text)
{
  const char *s;
...
static void
end_element (BobguiBuildableParseContext  *context,
             const char                *element_name,
             gpointer                   user_data,
             GError                   **error)
{
  ...
  BOBGUI_DEBUG (BUILDER_TRACE, "</%s>", element_name);
  ...
  if (data->requested_objects && !data->inside_requested_object)
           ...
           strcmp (element_name, "template") == 0)
    {
      ...
      if (data->requested_objects && data->inside_requested_object &&
        ...
        {
          BOBGUI_DEBUG (BUILDER_TRACE, "requested object end found at level %d",
      ...
      if (BOBGUI_IS_BUILDABLE (object_info->object) &&
          BOBGUI_BUILDABLE_GET_IFACE (object_info->object)->parser_finished)
        g_ptr_array_add (data->finalizers, object_info->object);

      if (object_info->signals)
        {
          _bobgui_builder_add_signals (data->builder, object_info->signals);
          object_info->signals = NULL;
        }

      if (object_info->bindings)
        {
          bobgui_builder_take_bindings (data->builder, object_info->object, object_info->bindings);
  ...
  else if (strcmp (element_name, "requires") == 0)
    {
       ...
       */
      if (!strcmp (req_info->library, "bobgui"))
        {
          if (req_info->major == 4 && req_info->minor == 0)
            {
              /* We allow 3.99.x to pass as 4.0 */
            }
          else if (bobgui_check_version (req_info->major, req_info->minor, 0) != NULL)
            {
              g_set_error (error,
                           BOBGUI_BUILDER_ERROR,
                           BOBGUI_BUILDER_ERROR_VERSION_MISMATCH,
                           "Required BOBGUI version %d.%d, current version is %d.%d",
                           req_info->major, req_info->minor,
                           BOBGUI_MAJOR_VERSION, BOBGUI_MINOR_VERSION);
              _bobgui_builder_prefix_error (data->builder, context, error);
           }
        }
  ...
  else if (strcmp (element_name, "menu") == 0)
    {
      ...
      id = _bobgui_builder_menu_end (data);
...
static void
text (BobguiBuildableParseContext  *context,
      const char                *text,
      gsize                      text_len,
      gpointer                   user_data,
      GError                   **error)
{
  ...
  if (strcmp (bobgui_buildable_parse_context_get_element (context), "property") == 0)
    {
      PropertyInfo *prop_info = (PropertyInfo*)info;

      g_string_append_len (prop_info->text, text, text_len);
    }
  else if (strcmp (bobgui_buildable_parse_context_get_element (context), "constant") == 0)
    {
      ExpressionInfo *expr_info = (ExpressionInfo *) info;

      g_string_append_len (expr_info->constant.text, text, text_len);
    }
  else if (strcmp (bobgui_buildable_parse_context_get_element (context), "lookup") == 0)
    {
      ExpressionInfo *expr_info = (ExpressionInfo *) info;
...
static const BobguiBuildableParser parser = {
  start_element,
  end_element,
  text,
  NULL,
...
void
_bobgui_builder_parser_parse_buffer (BobguiBuilder   *builder,
                                  const char   *filename,
                                  const char   *buffer,
                                  gssize        length,
                                  ...
                                  GError      **error)
{
   ...
   * bobgui_builder_get_translation_domain() without breaking the ABI or API
   */
  ...
  bobgui_buildable_parse_context_init (&data.ctx, &parser, &data);

  if (!bobgui_buildable_parse_context_parse (&data.ctx, buffer, length, error))
    goto out;

  if (_bobgui_builder_lookup_failed (builder, error))
    goto out;

  if (!_bobgui_builder_finish (builder, error))
    goto out;
  ...
  for (l = data.custom_finalizers; l; l = l->next)
    {
      ...
      bobgui_buildable_custom_finished (BOBGUI_BUILDABLE (sub->object),
                                     builder,
                                     sub->child,
                                     sub->tagname,
                                     sub->data);
  ...
  for (guint i = 0; i < data.finalizers->len; i++)
    {
      BobguiBuildable *buildable = g_ptr_array_index (data.finalizers, i);

      bobgui_buildable_parser_finished (BOBGUI_BUILDABLE (buildable), builder);
      if (_bobgui_builder_lookup_failed (builder, error))
        goto out;
    }
 ...
 out:
  ...
  bobgui_buildable_parse_context_free (&data.ctx);
