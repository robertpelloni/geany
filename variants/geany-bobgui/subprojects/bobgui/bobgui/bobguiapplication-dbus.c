...
G_DEFINE_TYPE (BobguiApplicationImplDBus, bobgui_application_impl_dbus, BOBGUI_TYPE_APPLICATION_IMPL)
...
static GDBusProxy*
bobgui_application_get_proxy_if_service_present (GDBusConnection *connection,
...
screensaver_signal_portal (GDBusConnection *connection,
                           ...
                           gpointer          data)
{
  BobguiApplicationImplDBus *dbus = (BobguiApplicationImplDBus *)data;
  gboolean active;
  GVariant *state;
  ...
  bobgui_application_set_screensaver_active (dbus->impl.application, active);
...
static char *
get_state_file (BobguiApplicationImpl *impl,
...
static void
debug_override_restore_reason (BobguiRestoreReason *reason)
{
  const char *str = g_getenv ("BOBGUI_OVERRIDE_RESTORE_REASON");
  if (G_LIKELY (!str))
    return;
  else if (strcmp (str, "pristine") == 0)
    *reason = BOBGUI_RESTORE_REASON_PRISTINE;
  else if (strcmp (str, "launch") == 0)
    *reason = BOBGUI_RESTORE_REASON_LAUNCH;
  else if (strcmp (str, "recover") == 0)
    *reason = BOBGUI_RESTORE_REASON_RECOVER;
  else if (strcmp (str, "restore") == 0)
    *reason = BOBGUI_RESTORE_REASON_RESTORE;
  else
    g_warning ("Unknown BOBGUI_OVERRIDE_RESTORE_REASON value: %s", str);
}

static gboolean
request_restore (BobguiApplicationImplDBus *dbus)
{
  BobguiApplicationImpl *impl = (BobguiApplicationImpl*) dbus;
  const char *app_id;
  ...
  if (!reply)
    {
      if (g_error_matches (error, G_DBUS_ERROR, G_DBUS_ERROR_UNKNOWN_METHOD))
        BOBGUI_DEBUG (SESSION, "gnome-session does not support session saving");
      else
  ...
  BOBGUI_DEBUG (SESSION, "Registered restore, reason %s instance-id %s",
             g_enum_get_value (g_type_class_get (BOBGUI_TYPE_RESTORE_REASON), dbus->reason)->value_nick,
             dbus->instance_id);
  ...
  while (g_variant_iter_loop (discard_iter, "s", &discard))
    {
      ...
      BOBGUI_DEBUG (SESSION, "Cleaning up instance id: %s", discard);
...
static void
bobgui_application_impl_dbus_startup (BobguiApplicationImpl *impl,
                                   gboolean            support_save)
{
  ...
  if (!support_save || !request_restore (dbus))
    {
      dbus->reason = BOBGUI_RESTORE_REASON_LAUNCH;
  ...
  dbus->inhibit_proxy = bobgui_application_get_proxy_if_service_present (dbus->session,
  ...
  dbus->session_path = bobgui_get_portal_session_path (dbus->session, &token);
...
static void
bobgui_application_impl_dbus_shutdown (BobguiApplicationImpl *impl)
{
  BobguiApplicationImplDBus *dbus = (BobguiApplicationImplDBus *) impl;
...
GQuark bobgui_application_impl_dbus_export_id_quark (void);

G_DEFINE_QUARK (BobguiApplicationImplDBus export id, bobgui_application_impl_dbus_export_id)

GQuark bobgui_application_impl_dbus_window_state_quark (void);
G_DEFINE_QUARK (BobguiApplicationImplDBus window state, bobgui_application_impl_dbus_window_state)

static void
bobgui_application_impl_dbus_window_added (BobguiApplicationImpl *impl,
                                        BobguiWindow          *window,
                                        GVariant           *state)
{
  BobguiApplicationImplDBus *dbus = (BobguiApplicationImplDBus *) impl;
  ...
  if (!dbus->session || !BOBGUI_IS_APPLICATION_WINDOW (window))
    return;

  /* Export the action group of this window, based on its id */
  actions = bobgui_application_window_get_action_group (BOBGUI_APPLICATION_WINDOW (window));

  path = bobgui_application_impl_dbus_get_window_path (dbus, window);
  id = g_dbus_connection_export_action_group (dbus->session, path, actions, NULL);
  g_free (path);

  g_object_set_qdata (G_OBJECT (window), bobgui_application_impl_dbus_export_id_quark (), GUINT_TO_POINTER (id));

  if (state)
    {
      g_object_set_qdata_full (G_OBJECT (window), bobgui_application_impl_dbus_window_state_quark (), g_variant_ref (state), (GDestroyNotify) g_variant_unref);
    }
}

GVariant *
bobgui_application_impl_dbus_get_window_state (BobguiApplicationImplDBus *impl,
                                            BobguiWindow              *window)
{
  return (GVariant *) g_object_get_qdata (G_OBJECT (window), bobgui_application_impl_dbus_window_state_quark ());
}

static void
bobgui_application_impl_dbus_window_removed (BobguiApplicationImpl *impl,
                                          BobguiWindow          *window)
{
  BobguiApplicationImplDBus *dbus = (BobguiApplicationImplDBus *) impl;
  guint id;

  id = GPOINTER_TO_UINT (g_object_get_qdata (G_OBJECT (window), bobgui_application_impl_dbus_export_id_quark ()));
  if (id)
    {
      g_dbus_connection_unexport_action_group (dbus->session, id);
      g_object_set_qdata (G_OBJECT (window), bobgui_application_impl_dbus_export_id_quark (), NULL);
    }
}

static void
bobgui_application_impl_dbus_window_forget (BobguiApplicationImpl *impl,
                                         BobguiWindow          *window)
{
  g_object_set_qdata (G_OBJECT (window), bobgui_application_impl_dbus_window_state_quark (), NULL);
}

static void
bobgui_application_impl_dbus_active_window_changed (BobguiApplicationImpl *impl,
                                                 BobguiWindow          *window)
{
}

static void
bobgui_application_impl_dbus_publish_menu (BobguiApplicationImplDBus  *dbus,
                                        const char              *type,
                                        GMenuModel              *model,
                                        guint                   *id,
                                        char                   **path)
...
static void
bobgui_application_impl_dbus_set_app_menu (BobguiApplicationImpl *impl,
                                        GMenuModel         *app_menu)
{
  BobguiApplicationImplDBus *dbus = (BobguiApplicationImplDBus *) impl;

  bobgui_application_impl_dbus_publish_menu (dbus, "appmenu", app_menu, &dbus->app_menu_id, &dbus->app_menu_path);
}

static void
bobgui_application_impl_dbus_set_menubar (BobguiApplicationImpl *impl,
                                       GMenuModel         *menubar)
{
  BobguiApplicationImplDBus *dbus = (BobguiApplicationImplDBus *) impl;

  bobgui_application_impl_dbus_publish_menu (dbus, "menubar", menubar, &dbus->menubar_id, &dbus->menubar_path);
}
...
static guint
bobgui_application_impl_dbus_inhibit (BobguiApplicationImpl         *impl,
                                   BobguiWindow                  *window,
...
static void
bobgui_application_impl_dbus_uninhibit (BobguiApplicationImpl *impl,
                                     guint               cookie)
{
  BobguiApplicationImplDBus *dbus = (BobguiApplicationImplDBus *) impl;
...
static BobguiRestoreReason
bobgui_application_impl_dbus_get_restore_reason (BobguiApplicationImpl *impl)
{
  BobguiApplicationImplDBus *dbus = (BobguiApplicationImplDBus *) impl;
...
static void
bobgui_application_impl_dbus_clear_restore_reason (BobguiApplicationImpl *impl)
{
  BobguiApplicationImplDBus *dbus = (BobguiApplicationImplDBus *) impl;
  dbus->reason = BOBGUI_RESTORE_REASON_LAUNCH;
}
...
static void
bobgui_application_impl_dbus_store_state (BobguiApplicationImpl *impl,
                                       GVariant           *state)
{
  char *file;
  ...
  if (file)
    {
      BOBGUI_DEBUG (SESSION, "Store state, file %s", file);
...
static void
bobgui_application_impl_dbus_forget_state (BobguiApplicationImpl *impl)
{
  char *file;
...
static GVariant *
bobgui_application_impl_dbus_retrieve_state (BobguiApplicationImpl *impl)
{
  char *file;
  char *contents;
  gsize length;
  ...
  if (g_file_get_contents (file, &contents, &length, NULL))
    {
      BOBGUI_DEBUG (SESSION, "Retrieve state, file %s", file);
...
static void
bobgui_application_impl_dbus_init (BobguiApplicationImplDBus *dbus)
{
}

static void
bobgui_application_impl_dbus_finalize (GObject *object)
{
  BobguiApplicationImplDBus *dbus = (BobguiApplicationImplDBus *) object;
  ...
  G_OBJECT_CLASS (bobgui_application_impl_dbus_parent_class)->finalize (object);
}

static void
bobgui_application_impl_dbus_class_init (BobguiApplicationImplDBusClass *class)
{
  BobguiApplicationImplClass *impl_class = BOBGUI_APPLICATION_IMPL_CLASS (class);
  GObjectClass *gobject_class = G_OBJECT_CLASS (class);

  impl_class->startup = bobgui_application_impl_dbus_startup;
  impl_class->shutdown = bobgui_application_impl_dbus_shutdown;
  impl_class->window_added = bobgui_application_impl_dbus_window_added;
  impl_class->window_removed = bobgui_application_impl_dbus_window_removed;
  impl_class->window_forget = bobgui_application_impl_dbus_window_forget;
  impl_class->active_window_changed = bobgui_application_impl_dbus_active_window_changed;
  impl_class->set_app_menu = bobgui_application_impl_dbus_set_app_menu;
  impl_class->set_menubar = bobgui_application_impl_dbus_set_menubar;
  impl_class->inhibit = bobgui_application_impl_dbus_inhibit;
  impl_class->uninhibit = bobgui_application_impl_dbus_uninhibit;
  impl_class->get_restore_reason = bobgui_application_impl_dbus_get_restore_reason;
  impl_class->clear_restore_reason = bobgui_application_impl_dbus_clear_restore_reason;
  impl_class->store_state = bobgui_application_impl_dbus_store_state;
  impl_class->forget_state = bobgui_application_impl_dbus_forget_state;
  impl_class->retrieve_state = bobgui_application_impl_dbus_retrieve_state;

  gobject_class->finalize = bobgui_application_impl_dbus_finalize;
}

char *
bobgui_application_impl_dbus_get_window_path (BobguiApplicationImplDBus *dbus,
                                           BobguiWindow *window)
{
  if (dbus->session && BOBGUI_IS_APPLICATION_WINDOW (window))
    return g_strdup_printf ("%s/window/%d",
                            dbus->object_path,
                            bobgui_application_window_get_id (BOBGUI_APPLICATION_WINDOW (window)));
  else
    return NULL;
}
