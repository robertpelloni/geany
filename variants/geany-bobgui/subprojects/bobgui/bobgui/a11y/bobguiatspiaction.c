/* bobguiatspiaction.c: ATSPI Action implementation
 *
...
struct _Action
{
  ...
  gboolean (* is_enabled) (BobguiAtSpiContext *context);
  gboolean (* activate) (BobguiAtSpiContext *context);
};

static char *
get_keybinding (BobguiAtSpiContext *self)
{
  const char *mnemonic = "";
  const char *shortcut = "";
  ...
  if (bobgui_at_context_has_accessible_relation (BOBGUI_AT_CONTEXT (self), BOBGUI_ACCESSIBLE_RELATION_LABELLED_BY))
    {
      BobguiAccessibleValue *value = bobgui_at_context_get_accessible_relation (BOBGUI_AT_CONTEXT (self), BOBGUI_ACCESSIBLE_RELATION_LABELLED_BY);
      GList *l = bobgui_reference_list_accessible_value_get (value);
      if (l)
        {
  ...
  if (bobgui_at_context_has_accessible_property (BOBGUI_AT_CONTEXT (self), BOBGUI_ACCESSIBLE_PROPERTY_KEY_SHORTCUTS))
    {
      BobguiAccessibleValue *value = bobgui_at_context_get_accessible_property (BOBGUI_AT_CONTEXT (self), BOBGUI_ACCESSIBLE_PROPERTY_KEY_SHORTCUTS);
      shortcut = bobgui_string_accessible_value_get (value);
    }
...
static void
action_handle_method (BobguiAtSpiContext        *self,
                      ...
                      int                     n_actions)
{
  if (g_strcmp0 (method_name, "GetName") == 0)
  ...
  else if (g_strcmp0 (method_name, "DoAction") == 0)
    {
      ...
      if (!bobgui_widget_is_sensitive (widget) || !bobgui_widget_get_visible (widget))
        {
...
static GVariant *
action_handle_get_property (BobguiAtSpiContext  *self,
...
static gboolean
color_swatch_select (BobguiAtSpiContext *self)
{
  BobguiAccessible *accessible = bobgui_at_context_get_accessible (BOBGUI_AT_CONTEXT (self));
  bobgui_color_swatch_select (BOBGUI_COLOR_SWATCH (accessible));
  return TRUE;
}

static gboolean
color_swatch_activate (BobguiAtSpiContext *self)
{
  BobguiAccessible *accessible = bobgui_at_context_get_accessible (BOBGUI_AT_CONTEXT (self));
  bobgui_color_swatch_activate (BOBGUI_COLOR_SWATCH (accessible));
  return TRUE;
}

static gboolean
color_swatch_customize (BobguiAtSpiContext *self)
{
  BobguiAccessible *accessible = bobgui_at_context_get_accessible (BOBGUI_AT_CONTEXT (self));
  bobgui_color_swatch_customize (BOBGUI_COLOR_SWATCH (accessible));
  return TRUE;
}

static gboolean
color_swatch_is_enabled (BobguiAtSpiContext *self)
{
  BobguiAccessible *accessible = bobgui_at_context_get_accessible (BOBGUI_AT_CONTEXT (self));
  return bobgui_color_swatch_get_selectable (BOBGUI_COLOR_SWATCH (accessible));
}
...
static gboolean is_primary_icon_enabled (BobguiAtSpiContext *self);
static gboolean is_secondary_icon_enabled (BobguiAtSpiContext *self);
static gboolean activate_primary_icon (BobguiAtSpiContext *self);
static gboolean activate_secondary_icon (BobguiAtSpiContext *self);
...
static gboolean
is_primary_icon_enabled (BobguiAtSpiContext *self)
{
  BobguiAccessible *accessible = bobgui_at_context_get_accessible (BOBGUI_AT_CONTEXT (self));
  BobguiEntry *entry = BOBGUI_ENTRY (accessible);

  return bobgui_entry_get_icon_storage_type (entry, BOBGUI_ENTRY_ICON_PRIMARY) != BOBGUI_IMAGE_EMPTY;
}

static gboolean
activate_primary_icon (BobguiAtSpiContext *self)
{
  BobguiAccessible *accessible = bobgui_at_context_get_accessible (BOBGUI_AT_CONTEXT (self));
  BobguiEntry *entry = BOBGUI_ENTRY (accessible);

  return bobgui_entry_activate_icon (entry, BOBGUI_ENTRY_ICON_PRIMARY);
}

static gboolean
is_secondary_icon_enabled (BobguiAtSpiContext *self)
{
  BobguiAccessible *accessible = bobgui_at_context_get_accessible (BOBGUI_AT_CONTEXT (self));
  BobguiEntry *entry = BOBGUI_ENTRY (accessible);

  return bobgui_entry_get_icon_storage_type (entry, BOBGUI_ENTRY_ICON_SECONDARY) != BOBGUI_IMAGE_EMPTY;
}

static gboolean
activate_secondary_icon (BobguiAtSpiContext *self)
{
  BobguiAccessible *accessible = bobgui_at_context_get_accessible (BOBGUI_AT_CONTEXT (self));
  BobguiEntry *entry = BOBGUI_ENTRY (accessible);

  return bobgui_entry_activate_icon (entry, BOBGUI_ENTRY_ICON_SECONDARY);
}
...
static gboolean is_peek_enabled (BobguiAtSpiContext *self);
static gboolean activate_peek (BobguiAtSpiContext *self);
...
static gboolean
is_peek_enabled (BobguiAtSpiContext *self)
{
  BobguiAccessible *accessible = bobgui_at_context_get_accessible (BOBGUI_AT_CONTEXT (self));
  BobguiPasswordEntry *entry = BOBGUI_PASSWORD_ENTRY (accessible);

  if (!bobgui_password_entry_get_show_peek_icon (entry))
    return FALSE;
...
static gboolean
activate_peek (BobguiAtSpiContext *self)
{
  BobguiAccessible *accessible = bobgui_at_context_get_accessible (BOBGUI_AT_CONTEXT (self));
  BobguiPasswordEntry *entry = BOBGUI_PASSWORD_ENTRY (accessible);

  bobgui_password_entry_toggle_peek (entry);
...
static gboolean is_clear_enabled (BobguiAtSpiContext *self);
static gboolean activate_clear (BobguiAtSpiContext *self);
...
static gboolean
is_clear_enabled (BobguiAtSpiContext *self)
{
  BobguiAccessible *accessible = bobgui_at_context_get_accessible (BOBGUI_AT_CONTEXT (self));
  BobguiSearchEntry *entry = BOBGUI_SEARCH_ENTRY (accessible);

  const char *str = bobgui_editable_get_text (BOBGUI_EDITABLE (entry));
...
static gboolean
activate_clear (BobguiAtSpiContext *self)
{
  BobguiAccessible *accessible = bobgui_at_context_get_accessible (BOBGUI_AT_CONTEXT (self));

  bobgui_editable_set_text (BOBGUI_EDITABLE (accessible), "");
...
static gboolean
is_valid_action (BobguiActionMuxer *muxer,
                 const char     *action_name)
{
  ...
  if (!bobgui_action_muxer_query_action (muxer, action_name,
                                      &enabled,
                                      &param_type, NULL,
                                      NULL, NULL))
    return FALSE;
...
static void
add_muxer_actions (BobguiActionMuxer   *muxer,
                   char            **actions,
                   int               n_actions,
...
static const char *
get_action_at_index (BobguiActionMuxer  *muxer,
                     char           **actions,
                     int              n_actions,
                     int              pos)
{
...
static int
get_valid_actions (BobguiActionMuxer  *muxer,
                   char           **actions,
                   int              n_actions)
{
                      ...
                      gpointer               user_data)
{
  ...
  BobguiActionMuxer *parent_muxer = parent ? _bobgui_widget_get_action_muxer (parent, FALSE) : NULL;
  ...
  if (muxer != parent_muxer)
    actions = bobgui_action_muxer_list_actions (muxer, TRUE);
  ...
  if (g_strcmp0 (method_name, "GetName") == 0 ||
      ...
      g_strcmp0 (method_name, "GetDescription") == 0)
    {
      ...
      if (action != NULL && bobgui_widget_is_sensitive (widget))
  ...
  else if (g_strcmp0 (method_name, "DoAction") == 0)
    {
      ...
      if (action != NULL && bobgui_widget_is_sensitive (widget))
        {
          gboolean res = bobgui_widget_activate_action_variant (widget, action, NULL);
  ...
  else if (g_strcmp0 (method_name, "GetActions") == 0)
    {
      ...
      if (n_actions >= 0 && bobgui_widget_is_sensitive (widget))
...
bobgui_atspi_get_action_vtable (BobguiAccessible *accessible)
{
  if (BOBGUI_IS_BUTTON (accessible) ||
      BOBGUI_IS_MODEL_BUTTON (accessible))
    return &button_action_vtable;
  else if (BOBGUI_IS_ENTRY (accessible))
    return &entry_action_vtable;
  else if (BOBGUI_IS_EXPANDER (accessible))
    return &expander_action_vtable;
  else if (BOBGUI_IS_PASSWORD_ENTRY (accessible))
    return &password_entry_action_vtable;
  else if (BOBGUI_IS_SEARCH_ENTRY (accessible))
    return &search_entry_action_vtable;
  else if (BOBGUI_IS_SWITCH (accessible))
    return &switch_action_vtable;
  else if (BOBGUI_IS_COLOR_SWATCH (accessible))
    return &color_swatch_action_vtable;
  else if (BOBGUI_IS_WIDGET (accessible))
