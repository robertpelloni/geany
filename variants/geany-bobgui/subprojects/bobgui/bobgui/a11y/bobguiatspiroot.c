/* bobguiatspiroot.c: AT-SPI root object
 *
...
G_DEFINE_TYPE (BobguiAtSpiRoot, bobgui_at_spi_root, G_TYPE_OBJECT)

static void
bobgui_at_spi_root_finalize (GObject *gobject)
{
  BobguiAtSpiRoot *self = BOBGUI_AT_SPI_ROOT (gobject);
  ...
  G_OBJECT_CLASS (bobgui_at_spi_root_parent_class)->finalize (gobject);
}

static void
bobgui_at_spi_root_dispose (GObject *gobject)
{
  BobguiAtSpiRoot *self = BOBGUI_AT_SPI_ROOT (gobject);
  ...
  G_OBJECT_CLASS (bobgui_at_spi_root_parent_class)->dispose (gobject);
}

static void
bobgui_at_spi_root_set_property (GObject      *gobject,
                              guint         prop_id,
                              const GValue *value,
                              GParamSpec   *pspec)
{
...
static void
bobgui_at_spi_root_get_property (GObject    *gobject,
                              guint       prop_id,
                              GValue     *value,
                              GParamSpec *pspec)
{
                           ...
                           gpointer               user_data)
{
  if (g_strcmp0 (method_name, "GetLocale") == 0)
    ...
    {
      BobguiAtSpiRoot *root = user_data;
...
handle_application_get_property (GDBusConnection       *connection,
                                 ...
                                 gpointer               user_data)
{
  BobguiAtSpiRoot *self = user_data;
  GVariant *res = NULL;
                          ...
                          gpointer               user_data)
{
  ...
  if (g_strcmp0 (method_name, "GetRole") == 0)
  ...
  else if (g_strcmp0 (method_name, "GetChildAtIndex") == 0)
    {
      ...
      BobguiATContext *context = bobgui_accessible_get_at_context (BOBGUI_ACCESSIBLE (window));

      const char *name = g_dbus_connection_get_unique_name (self->connection);
      const char *path = bobgui_at_spi_context_get_context_path (BOBGUI_AT_SPI_CONTEXT (context));
  ...
  else if (g_strcmp0 (method_name, "GetChildren") == 0)
    {
      ...
      for (guint i = 0; i < n_toplevels; i++)
        {
          BobguiWidget *window = g_list_model_get_item (self->toplevels, i);
...
void
bobgui_at_spi_root_child_changed (BobguiAtSpiRoot             *self,
                               ...
                               BobguiAccessible            *child)
{
  ...
  for (i = 0, n = g_list_model_get_n_items (self->toplevels); i < n; i++)
    {
      BobguiAccessible *item = g_list_model_get_item (self->toplevels, i);
      ...
      if (!bobgui_accessible_should_present (item))
        continue;
  ...
  if (child == NULL)
    {
      window_ref = bobgui_at_spi_null_ref ();
    }
  else
    {
      BobguiATContext *context = bobgui_accessible_get_at_context (child);

      window_ref = bobgui_at_spi_context_to_ref (BOBGUI_AT_SPI_CONTEXT (context));
  ...
  switch (change)
    {
    case BOBGUI_ACCESSIBLE_CHILD_CHANGE_ADDED:
      state = BOBGUI_ACCESSIBLE_CHILD_STATE_ADDED;
      break;
    case BOBGUI_ACCESSIBLE_CHILD_CHANGE_REMOVED:
      state = BOBGUI_ACCESSIBLE_CHILD_STATE_REMOVED;
      break;
    default:
                              ...
                              gpointer user_data)
{
  ...
  if (g_strcmp0 (object_path, ATSPI_REGISTRY_PATH) == 0 &&
      ...
      g_strcmp0 (signal_name, "EventListenerRegistered") == 0)
    {
      ...
      if (count == NULL)
        {
          BOBGUI_DEBUG (A11Y, "Registering event listener (%s, %s) on the a11y bus",
                     sender,
        ...
        {
          BOBGUI_DEBUG (A11Y, "Reference count for event listener %s reached saturation", sender);
        }
      else
        {
          BOBGUI_DEBUG (A11Y, "Incrementing refcount for event listener %s", sender);
          *count += 1;
        }
    }
}
  ...
  if (g_strcmp0 (object_path, ATSPI_REGISTRY_PATH) == 0 &&
      ...
      g_strcmp0 (signal_name, "EventListenerDeregistered") == 0)
    {
      ...
      if (G_UNLIKELY (count == NULL))
        ...
        {
          BOBGUI_DEBUG (A11Y, "Decreasing refcount for listener %s", sender);
          *count -= 1;
        }
      else
        {
          BOBGUI_DEBUG (A11Y, "Deregistering event listener %s on the a11y bus", sender);
...
check_flatpak_portal_version (unsigned int minimum_version)
{
  ...
  BOBGUI_DEBUG (A11Y, "Flatpak portal version: %u (required: %u)", flatpak_portal_version, minimum_version);
...
typedef struct {
  ...
  BobguiAtSpiRootRegisterFunc register_func;
} RegistrationData;
                       ...
                       gpointer      user_data)
{
  ...
  BobguiAtSpiRoot *self = data->root;
  ...
  if (reply != NULL)
    {
      ...
      BOBGUI_DEBUG (A11Y, "Connected to the a11y registry at (%s, %s)",
                       self->desktop_name,
                       self->desktop_path);
    }

  /* Register the cache object */
  self->cache = bobgui_at_spi_cache_new (self->connection, ATSPI_CACHE_PATH, self);

  /* Drain the list of queued BobguiAtSpiContexts, and add them to the cache */
      ...
      for (GList *l = self->queued_contexts; l != NULL; l = l->next)
        {
          ...
          bobgui_at_spi_cache_add_context (self->cache, l->data);
        }

      g_clear_pointer (&self->queued_contexts, g_list_free);
    }

  self->toplevels = bobgui_window_get_toplevels ();
  ...
  if (gdk_running_in_sandbox () &&
    ...
    {
      BOBGUI_DEBUG (A11Y, "Sandboxed does not allow event listener registration");
...
root_register (gpointer user_data)
{
  ...
  BOBGUI_DEBUG (A11Y, "Registering (%s, %s) on the a11y bus",
                   unique_name,
                   self->root_path);
...
/*< private >
 * bobgui_at_spi_root_queue_register:
 * @self: a `BobguiAtSpiRoot`
 ...
 */
void
bobgui_at_spi_root_queue_register (BobguiAtSpiRoot             *self,
                                BobguiAtSpiContext          *context,
                                BobguiAtSpiRootRegisterFunc  func)
{
  ...
  if (self->cache != NULL)
    {
      ...
      bobgui_at_spi_cache_add_context (self->cache, context);
      return;
    }
  else
    {
  ...
  gdk_source_set_static_name_by_id (self->register_id, "[bobgui] ATSPI root registration");
}

void
bobgui_at_spi_root_unregister (BobguiAtSpiRoot    *self,
                            BobguiAtSpiContext *context)
{
  if (self->queued_contexts != NULL)
  ...
  if (self->cache != NULL)
    bobgui_at_spi_cache_remove_context (self->cache, context);
}

static void
bobgui_at_spi_root_constructed (GObject *gobject)
{
  BobguiAtSpiRoot *self = BOBGUI_AT_SPI_ROOT (gobject);
  ...
  if (self->base_path == NULL)
    {
      ...
      self->base_path = g_strconcat ("/org/bobgui/application/",
                                     base_name,
                                     "/a11y",
                                     NULL);
...
out:
  G_OBJECT_CLASS (bobgui_at_spi_root_parent_class)->constructed (gobject);
}

static void
bobgui_at_spi_root_class_init (BobguiAtSpiRootClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->constructed = bobgui_at_spi_root_constructed;
  gobject_class->set_property = bobgui_at_spi_root_set_property;
  gobject_class->get_property = bobgui_at_spi_root_get_property;
  gobject_class->dispose = bobgui_at_spi_root_dispose;
  gobject_class->finalize = bobgui_at_spi_root_finalize;
...
static void
bobgui_at_spi_root_init (BobguiAtSpiRoot *self)
{
}

BobguiAtSpiRoot *
bobgui_at_spi_root_new (const char *bus_address)
{
  g_return_val_if_fail (bus_address != NULL, NULL);

  return g_object_new (BOBGUI_TYPE_AT_SPI_ROOT,
                       "bus-address", bus_address,
                       NULL);
}

GDBusConnection *
bobgui_at_spi_root_get_connection (BobguiAtSpiRoot *self)
{
  g_return_val_if_fail (BOBGUI_IS_AT_SPI_ROOT (self), NULL);

  return self->connection;
}

BobguiAtSpiCache *
bobgui_at_spi_root_get_cache (BobguiAtSpiRoot *self)
{
  g_return_val_if_fail (BOBGUI_IS_AT_SPI_ROOT (self), NULL);
...
/*< private >
 * bobgui_at_spi_root_to_ref:
 * @self: a `BobguiAtSpiRoot`
 *
 * Returns an ATSPI object reference for the `BobguiAtSpiRoot` node.
 *
 * Returns: (transfer floating): a `GVariant` with the root reference
 */
GVariant *
bobgui_at_spi_root_to_ref (BobguiAtSpiRoot *self)
{
  g_return_val_if_fail (BOBGUI_IS_AT_SPI_ROOT (self), NULL);
...
const char *
bobgui_at_spi_root_get_base_path (BobguiAtSpiRoot *self)
{
  g_return_val_if_fail (BOBGUI_IS_AT_SPI_ROOT (self), NULL);
...
gboolean
bobgui_at_spi_root_has_event_listeners (BobguiAtSpiRoot *self)
{
  g_return_val_if_fail (BOBGUI_IS_AT_SPI_ROOT (self), FALSE);
