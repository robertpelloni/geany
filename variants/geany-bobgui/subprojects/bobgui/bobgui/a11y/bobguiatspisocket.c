/* bobguiatspisocket.c: AT-SPI socket object
 *
 ...
 * BobguiAtApiSocket:
 *
 * `BobguiAtApiSocket` is an AT-SPI specific `BobguiAccessible` interface for
...
static void bobgui_accessible_interface_init (BobguiAccessibleInterface *iface);

G_DEFINE_FINAL_TYPE_WITH_CODE (BobguiAtSpiSocket, bobgui_at_spi_socket, G_TYPE_OBJECT,
                               G_IMPLEMENT_INTERFACE (G_TYPE_INITABLE,
                                                      g_initable_interface_init)
                               G_IMPLEMENT_INTERFACE (BOBGUI_TYPE_ACCESSIBLE,
                                                      bobgui_accessible_interface_init))
...
static void
set_accessible_role (BobguiAtSpiSocket    *self,
                     BobguiAccessibleRole  role)
{
  g_return_if_fail (!bobgui_accessible_role_is_abstract (role));

  if (self->at_context == NULL || !bobgui_at_context_is_realized (self->at_context))
    {
      self->accessible_role = role;

      if (self->at_context != NULL)
        bobgui_at_context_set_accessible_role (self->at_context, role);

      g_object_notify (G_OBJECT (self), "accessible-role");
    }
  else
    {
      char *role_str = g_enum_to_string (BOBGUI_TYPE_ACCESSIBLE_ROLE, self->accessible_role);
...
static gboolean
bobgui_at_spi_socket_initable_init (GInitable     *initable,
                                 GCancellable  *cancellable,
                                 GError       **error)
{
  BobguiAtSpiSocket *self = (BobguiAtSpiSocket *) initable;

  g_assert (BOBGUI_IS_AT_SPI_SOCKET (initable));
  ...
  self->at_context = bobgui_at_context_create (self->accessible_role,
                                            BOBGUI_ACCESSIBLE (self),
                                            gdk_display_get_default ());

  /* Sockets are strictly specific to AT-SPI */
  if (self->at_context != NULL && !BOBGUI_IS_AT_SPI_CONTEXT (self->at_context))
    {
...
{
  iface->init = bobgui_at_spi_socket_initable_init;
}

static BobguiATContext *
bobgui_at_spi_socket_get_at_context (BobguiAccessible *accessible)
{
  BobguiAtSpiSocket *self = (BobguiAtSpiSocket *) accessible;

  g_assert (BOBGUI_IS_AT_SPI_SOCKET (accessible));
...
static gboolean
bobgui_at_spi_socket_get_platform_state (BobguiAccessible              *self,
...
bobgui_at_spi_socket_get_first_accessible_child (BobguiAccessible *accessible)
{
  return NULL;
}

static BobguiAccessible *
bobgui_at_spi_socket_get_next_accessible_sibling (BobguiAccessible *accessible)
{
  return NULL;
}

static gboolean
bobgui_at_spi_socket_get_bounds (BobguiAccessible *accessible,
                              int           *x,
                              int           *y,
                              int           *width,
                              int           *height)
{
  ...
  accessible_parent = bobgui_accessible_get_accessible_parent (accessible);
  if (accessible_parent == NULL)
    return FALSE;

  res = bobgui_accessible_get_bounds (accessible_parent, x, y, width, height);
...
static void
bobgui_accessible_interface_init (BobguiAccessibleInterface *iface)
{
  iface->get_at_context = bobgui_at_spi_socket_get_at_context;
  iface->get_platform_state = bobgui_at_spi_socket_get_platform_state;
  iface->get_first_accessible_child = bobgui_at_spi_socket_get_first_accessible_child;
  iface->get_next_accessible_sibling = bobgui_at_spi_socket_get_next_accessible_sibling;
  iface->get_bounds = bobgui_at_spi_socket_get_bounds;
}

static void
bobgui_at_spi_socket_dispose (GObject *object)
{
  BobguiAtSpiSocket *self = (BobguiAtSpiSocket *)object;
  ...
  G_OBJECT_CLASS (bobgui_at_spi_socket_parent_class)->dispose (object);
}

static void
bobgui_at_spi_socket_get_property (GObject    *object,
                                guint       prop_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
  BobguiAtSpiSocket *self = BOBGUI_AT_SPI_SOCKET (object);
...
static void
bobgui_at_spi_socket_set_property (GObject      *object,
                                guint         prop_id,
                                const GValue *value,
                                GParamSpec   *pspec)
{
...
static void
bobgui_at_spi_socket_class_init (BobguiAtSpiSocketClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->dispose = bobgui_at_spi_socket_dispose;
  object_class->get_property = bobgui_at_spi_socket_get_property;
  object_class->set_property = bobgui_at_spi_socket_set_property;

  /**
   * BobguiAtSpiSocket:bus-name:
   *
...
static void
bobgui_at_spi_socket_init (BobguiAtSpiSocket *self)
{
  self->accessible_role = BOBGUI_ACCESSIBLE_ROLE_GENERIC;
}

GVariant *
bobgui_at_spi_socket_to_ref (BobguiAtSpiSocket *self)
{
  return g_variant_new ("(so)", self->bus_name, self->object_path);
}

static void
update_embedded_state (BobguiAtSpiSocket *self,
                       gboolean        embedded)
{
  ...
  bobgui_accessible_update_state (BOBGUI_ACCESSIBLE (self),
                               BOBGUI_ACCESSIBLE_STATE_HIDDEN, !embedded,
                               -1);
...
socket_embedded_cb (GObject      *source_object,
                    ...
                    gpointer      user_data)
{
  BobguiAtSpiSocket *self = (BobguiAtSpiSocket *)user_data;
  GVariant *res = NULL;
  GError *error = NULL;
...
void
bobgui_at_spi_socket_embed (BobguiAtSpiSocket  *self,
                         GDBusConnection *connection)
{
  ...
  context_path = bobgui_at_spi_context_get_context_path (BOBGUI_AT_SPI_CONTEXT (self->at_context));
 ...
 */
BobguiAccessible *
bobgui_at_spi_socket_new (const char  *bus_name,
                       const char  *object_path,
                       GError     **error)
{
  return g_initable_new (BOBGUI_TYPE_AT_SPI_SOCKET,
                         NULL,
                         error,
                         "bus-name", bus_name,
...
/**
 * bobgui_at_spi_socket_get_bus_name:
 * @self: a #BobguiAtSpiSocket
 *
 ...
 */
const char *
bobgui_at_spi_socket_get_bus_name (BobguiAtSpiSocket *self)
{
  g_return_val_if_fail (BOBGUI_IS_AT_SPI_SOCKET (self), NULL);

  return self->bus_name;
}

/**
 * bobgui_at_spi_socket_get_object_path:
 * @self: a #BobguiAtSpiSocket
 *
 ...
 */
const char *
bobgui_at_spi_socket_get_object_path (BobguiAtSpiSocket *self)
{
  g_return_val_if_fail (BOBGUI_IS_AT_SPI_SOCKET (self), NULL);
