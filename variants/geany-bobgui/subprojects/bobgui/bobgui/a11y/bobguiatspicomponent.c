/* bobguiatspicomponent.c: AT-SPI Component implementation
 *
...
#include "bobguiatspisocket.h"
#include "bobguipopover.h"

#include "a11y/atspi/atspi-component.h"

static BobguiAccessible *
find_first_accessible_non_socket (BobguiAccessible *accessible)
{
  ...
  while (parent != NULL)
    {
      ...
      if (!BOBGUI_IS_AT_SPI_SOCKET (parent))
        return parent;
...
static BobguiAccessible *
accessible_at_point (BobguiAccessible *parent,
                     int            x,
                     int            y,
                     bool           children_only)
{
  BobguiAccessible *result = NULL;
  int px, py, width, height;

  if (!bobgui_accessible_get_bounds (parent, &px, &py, &width, &height))
    return NULL;

  if (!children_only && x >= px && x <= px + width && y >= py && y <= py + height)
    result = parent;

  for (BobguiAccessible *child = bobgui_accessible_get_first_accessible_child (parent);
       child != NULL;
       child = bobgui_accessible_get_next_accessible_sibling (child))
    {
      BobguiAccessible *found = accessible_at_point (child, x - px, y - py, FALSE);
  ...
  if (g_strcmp0 (method_name, "Contains") == 0)
    {
      ...
      bobgui_at_spi_translate_coordinates_to_accessible (accessible, coordtype, x, y, &x, &y);

      if (bobgui_accessible_get_bounds (accessible, &bounds_x, &bounds_y, &width, &height))
  ...
  else if (g_strcmp0 (method_name, "GetAccessibleAtPoint") == 0)
    {
      ...
      if (!child)
        {
          g_dbus_method_invocation_return_value (invocation, g_variant_new ("(@(so))", bobgui_at_spi_null_ref ()));
        }
      else
        {
          BobguiATContext *context = bobgui_accessible_get_at_context (child);
          BobguiAtSpiContext *ctx = BOBGUI_AT_SPI_CONTEXT (context);

          /* Realize the ATContext in order to get its ref */
          bobgui_at_context_realize (context);

          g_dbus_method_invocation_return_value (invocation, g_variant_new ("(@(so))", bobgui_at_spi_context_to_ref (ctx)));
  ...
  else if (g_strcmp0 (method_name, "GetExtents") == 0)
    {
      ...
      bobgui_accessible_get_bounds (accessible, &x, &y, &width, &height);

      g_variant_get (parameters, "(u)", &coordtype);

      bobgui_at_spi_translate_coordinates_from_accessible (accessible, coordtype, 0, 0, &x, &y);
  ...
  else if (g_strcmp0 (method_name, "GetLayer") == 0)
    {
      ...
      if (self->accessible_role == BOBGUI_ACCESSIBLE_ROLE_WINDOW)
        layer = ATSPI_COMPONENT_LAYER_WINDOW;
      else if (BOBGUI_IS_POPOVER (accessible))
  ...
  else if (g_strcmp0 (method_name, "GetAlpha") == 0)
    {
      ...
      if (BOBGUI_IS_WIDGET (accessible))
        opacity = bobgui_widget_get_opacity (BOBGUI_WIDGET (accessible));
...
bobgui_atspi_get_component_vtable (BobguiAccessible *accessible)
{
