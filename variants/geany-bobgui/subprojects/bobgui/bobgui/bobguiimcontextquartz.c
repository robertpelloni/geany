...
#include "bobgui/bobguiimcontextquartz.h"
...
#define BOBGUI_IM_CONTEXT_TYPE_QUARTZ (bobgui_im_context_quartz_get_type())
#define BOBGUI_IM_CONTEXT_QUARTZ(obj) (G_TYPE_CHECK_INSTANCE_CAST ((obj), BOBGUI_IM_CONTEXT_TYPE_QUARTZ, BobguiIMContextQuartz))
#define BOBGUI_IM_CONTEXT_QUARTZ_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), BOBGUI_IM_CONTEXT_TYPE_QUARTZ, BobguiIMContextQuartzClass))


typedef struct _BobguiIMContextQuartz
{
  BobguiIMContext parent;
  BobguiIMContext *helper;
  ...
  gboolean focused;
} BobguiIMContextQuartz;

typedef struct _BobguiIMContextQuartzClass
{
  BobguiIMContextClass parent_class;
} BobguiIMContextQuartzClass;

G_DEFINE_TYPE_WITH_CODE (BobguiIMContextQuartz, bobgui_im_context_quartz, BOBGUI_TYPE_IM_CONTEXT,
...
static void
quartz_get_preedit_string (BobguiIMContext *context,
                           char **str,
                           PangoAttrList **attrs,
                           int *cursor_pos)
{
  BobguiIMContextQuartz *qc = BOBGUI_IM_CONTEXT_QUARTZ (context);

  BOBGUI_DEBUG (MODULES, "quartz_get_preedit_string\n");
...
static gboolean
output_result (BobguiIMContext *context,
               GdkSurface *surface)
{
  BobguiIMContextQuartz *qc = BOBGUI_IM_CONTEXT_QUARTZ (context);
  ...
  if (fixed_str)
    {
      BOBGUI_DEBUG (MODULES, "tic-insert-text: %s", fixed_str);
      ...
      unsigned int filtered =
	   GPOINTER_TO_UINT (g_object_get_data (G_OBJECT (surface),
						GIC_FILTER_KEY));
      BOBGUI_DEBUG (MODULES, "filtered, %d", filtered);
      if (filtered)
        retval = TRUE;
      else
        retval = FALSE;
  ...
  if (marked_str)
    {
      BOBGUI_DEBUG (MODULES, "tic-marked-text: %s", marked_str);
      qc->cursor_index =
	   GPOINTER_TO_UINT (g_object_get_data (G_OBJECT (surface),
						TIC_SELECTED_POS));
      qc->selected_len =
...
static gboolean
quartz_filter_keypress (BobguiIMContext *context,
                        GdkEvent     *event)
{
  BobguiIMContextQuartz *qc = BOBGUI_IM_CONTEXT_QUARTZ (context);
  ...
  BOBGUI_DEBUG (MODULES, "quartz_filter_keypress");
  ...
  if (!nsevent)
    {
      if (keycode == 0 && keyval == 0xffffff)
        ...
        return bobgui_im_context_filter_keypress (qc->helper, event);
    }
  ...
  BOBGUI_DEBUG (MODULES, "quartz_filter_keypress done");

  return retval;
}

static void
discard_preedit (BobguiIMContext *context)
{
  BobguiIMContextQuartz *qc = BOBGUI_IM_CONTEXT_QUARTZ (context);
...
static void
quartz_reset (BobguiIMContext *context)
{
  BOBGUI_DEBUG (MODULES, "quartz_reset");
  discard_preedit (context);
}

static void
quartz_set_client_surface (BobguiIMContext *context,
                           BobguiWidget    *widget)
{
  BobguiIMContextQuartz *qc = BOBGUI_IM_CONTEXT_QUARTZ (context);

  BOBGUI_DEBUG (MODULES, "quartz_set_client_surface: %p", widget);
  ...
  if (widget != NULL)
    {
      ...
      if (native != NULL)
        qc->client_surface = bobgui_native_get_surface (native);
    }
}

static void
quartz_focus_in (BobguiIMContext *context)
{
  BOBGUI_DEBUG (MODULES, "quartz_focus_in");
...
static void
quartz_focus_out (BobguiIMContext *context)
{
  BOBGUI_DEBUG (MODULES, "quartz_focus_out");
...
static void
quartz_set_cursor_location (BobguiIMContext *context, GdkRectangle *area)
{
  BobguiIMContextQuartz *qc = BOBGUI_IM_CONTEXT_QUARTZ (context);
  BobguiWidget* surface_widget;
  int sx, sy;
  graphene_point_t p;

  BOBGUI_DEBUG (MODULES, "quartz_set_cursor_location");
  ...
  surface_widget = BOBGUI_WIDGET (bobgui_widget_get_native (qc->client_widget));

  if (!surface_widget)
    return;

  gdk_surface_get_origin (qc->client_surface, &sx, &sy);
  if (!bobgui_widget_compute_point (qc->client_widget, surface_widget,
...
static void
quartz_set_use_preedit (BobguiIMContext *context, gboolean use_preedit)
{
  BOBGUI_DEBUG (MODULES, "quartz_set_use_preedit: %d", use_preedit);
}

static void
commit_cb (BobguiIMContext *context, const char *str, BobguiIMContextQuartz *qc)
{
...
{
  BOBGUI_DEBUG (MODULES, "imquartz_finalize");

  BobguiIMContextQuartz *qc = BOBGUI_IM_CONTEXT_QUARTZ (obj);
  g_free (qc->preedit_str);
  qc->preedit_str = NULL;
  ...
  G_OBJECT_CLASS (bobgui_im_context_quartz_parent_class)->finalize (obj);
}

static void
bobgui_im_context_quartz_class_init (BobguiIMContextQuartzClass *class)
{
  BOBGUI_DEBUG (MODULES, "bobgui_im_context_quartz_class_init");
...
static void
bobgui_im_context_quartz_init (BobguiIMContextQuartz *qc)
{
  BOBGUI_DEBUG (MODULES, "bobgui_im_context_quartz_init");
  ...
  qc->helper = g_object_new (BOBGUI_TYPE_IM_CONTEXT_SIMPLE, NULL);
