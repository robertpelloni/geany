                                    ...
                                    int             y)
{
  ...
  has_color_glyphs = bobgui_pango_glyph_item_has_color_glyphs (glyph_item);
  ...
  if (text_shadow && !bobgui_css_shadow_value_is_clear (text_shadow))
    {
      graphene_point_t offset;
      GdkColor color;
      double radius;

      bobgui_css_shadow_value_get_offset (text_shadow, 0, &offset);
      bobgui_css_shadow_value_get_color (text_shadow, 0, &color);
      radius = bobgui_css_shadow_value_get_radius (text_shadow, 0);

      bobgui_snapshot_save (crenderer->snapshot);
      bobgui_snapshot_translate (crenderer->snapshot, &offset);

      if (radius != 0)
        bobgui_snapshot_push_blur (crenderer->snapshot, radius);
      ...
      if (has_color_glyphs)
        {
          ...
          bobgui_snapshot_push_mask (crenderer->snapshot, GSK_MASK_MODE_ALPHA);

          bobgui_snapshot_add_text (crenderer->snapshot,
          ...
          bobgui_snapshot_add_color (crenderer->snapshot, &color, &bounds);
          bobgui_snapshot_pop (crenderer->snapshot);
  ...
  if (has_color_glyphs && has_alpha)
    {
      bobgui_snapshot_push_opacity (crenderer->snapshot, text_color.alpha);
      text_color.alpha = 1;
    }
                                   ...
                                   double           x22)
{
  ...
  cr = bobgui_snapshot_append_cairo (crenderer->snapshot,
                                         ...
                                         int            height)
{
  ...
  bobgui_snapshot_push_repeat (crenderer->snapshot,
  ...
  bobgui_snapshot_push_rounded_clip (crenderer->snapshot, &dot);
  bobgui_snapshot_add_color (crenderer->snapshot, &color, &dot.bounds);
  bobgui_snapshot_pop (crenderer->snapshot);
  ...
  if (crenderer->shape_handler)
    {
      ...
      if (shape_x != 0 || shape_y != 0)
        {
          bobgui_snapshot_save (crenderer->snapshot);
          bobgui_snapshot_translate (crenderer->snapshot, &GRAPHENE_POINT_INIT (shape_x, shape_y));
        }
  ...
  while (tmp_list)
    {
      ...
      if (attr->klass->type == bobgui_text_attr_appearance_type)
        return &((BobguiTextAttrAppearance *)attr)->appearance;
                                ...
                                PangoLayoutRun *run)
{
  ...
  if (crenderer->state == GSK_PANGO_RENDERER_SELECTED &&
      BOBGUI_IS_TEXT_VIEW (crenderer->widget))
    {
      BobguiCssNode *node;
      BobguiCssStyle *style;

      node = bobgui_text_view_get_selection_node ((BobguiTextView *)crenderer->widget);
      style = bobgui_css_node_get_style (node);
      fg_rgba = bobgui_css_color_value_get_rgba (style->used->color);
    }
  else if (crenderer->state == GSK_PANGO_RENDERER_CURSOR && bobgui_widget_has_focus (crenderer->widget))
    {
      BobguiCssNode *node;
      BobguiCssStyle *style;
      ...
      style = bobgui_css_node_get_style (node);
      fg_rgba = bobgui_css_color_value_get_rgba (style->used->background_color);
    }
  else
  ...
  if (BOBGUI_IS_TEXT_VIEW (crenderer->widget))
    {
      if (crenderer->state == GSK_PANGO_RENDERER_SELECTED)
        crenderer->shadow_style = bobgui_css_node_get_style (bobgui_text_view_get_selection_node ((BobguiTextView *)crenderer->widget));
      else if (crenderer->state != GSK_PANGO_RENDERER_CURSOR)
        crenderer->shadow_style = bobgui_css_node_get_style (bobgui_widget_get_css_node (crenderer->widget));
    }
}
                            ...
                            const GdkRGBA *color)
{
  ...
  bobgui_snapshot_add_layout (snapshot, layout, &color2);
  gdk_color_finish (&color2);
}

/* < private >
 * bobgui_snapshot_add_layout:
 * @snapshot: a `BobguiSnapshot`
 * @layout: the `PangoLayout` to render
 * @color: the foreground color to render the layout in
 *
 ...
 **/
void
bobgui_snapshot_add_layout (BobguiSnapshot    *snapshot,
                         PangoLayout    *layout,
