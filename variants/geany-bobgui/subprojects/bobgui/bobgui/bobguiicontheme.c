...
#include "bobguiiconcacheprivate.h"
...
#define GDK_ARRAY_TYPE_NAME BobguiStrvBuilder
#define GDK_ARRAY_NAME bobgui_strv_builder
 ...
 *
 * In many cases, named themes are used indirectly, via [class@Bobgui.Image]
 * rather than directly, but looking up icons directly is also simple. The
 * `BobguiIconTheme` object acts as a database of all the icons in the current
 * theme. You can create new `BobguiIconTheme` objects, but it’s much more
 * efficient to use the standard icon theme of the `BobguiWidget` so that the
 ...
 *
 * icon_theme = bobgui_icon_theme_get_for_display (bobgui_widget_get_display (my_widget));
...
typedef struct _BobguiStringSetChunk BobguiStringSetChunk;
...
struct _BobguiStringSetChunk {
  BobguiStringSetChunk *next;
  char data[STRING_SET_CHUNK_SIZE];
};

struct _BobguiStringSet {
  GHashTable *hash;
  BobguiStringSetChunk *chunks;
  int used_in_chunk;
};

#if 0
static void
dump_string_set (BobguiStringSet *set)
{
  BobguiStringSetChunk *chunk = set->chunks;
...
static void
bobgui_string_set_init (BobguiStringSet *set)
{
...
static void
bobgui_string_set_destroy (BobguiStringSet *set)
{
  BobguiStringSetChunk *next, *chunk;
...
static const char *
bobgui_string_set_lookup (BobguiStringSet *set,
                       const char   *string)
{
...
static void
bobgui_string_set_list (BobguiStringSet *set,
                     GHashTable   *result)
{
  GHashTableIter iter;
  gpointer key;
...
const char *
bobgui_string_set_add (BobguiStringSet *set,
                    const char   *string)
{
  ...
  if (intern == NULL)
    {
      BobguiStringSetChunk *chunk;
      int string_len = strlen (string) + 1;

      if (string_len > STRING_SET_CHUNK_SIZE - set->used_in_chunk)
        {
          gsize chunk_size = sizeof (BobguiStringSetChunk);
...
/* Threading:
 *
 * BobguiIconTheme is partially threadsafe, construction and setup can
 ...
 *
 * All private functions that take a BobguiIconTheme (or one of its
 ...
 * and lru_cache in BobguiIconTheme as well as its reverse pointer
 * BobguiIcon->in_cache. This is sometimes taken with the theme lock held
 ...
 * avoid this all such references are done via the BobguiIconThemeRef
...
typedef struct _BobguiIconThemeClass     BobguiIconThemeClass;


typedef struct _BobguiIconThemeRef BobguiIconThemeRef;

/* Acts as a database of information about an icon theme.
 * Normally, you retrieve the icon theme for a particular
 * display using bobgui_icon_theme_get_for_display() and it
 * will contain information about current icon theme for
 * that display, but you can also create a new `BobguiIconTheme`
 * object and set the icon theme name explicitly using
 * bobgui_icon_theme_set_theme_name().
 */
struct _BobguiIconTheme
{
  GObject parent_instance;
  BobguiIconThemeRef *ref;

  GHashTable *icon_cache;                       /* Protected by icon_cache lock */

  BobguiIconPaintable *lru_cache[LRU_CACHE_SIZE];  /* Protected by icon_cache lock */
  int lru_cache_current;                        /* Protected by icon_cache lock */

  BobguiStringSet icons;
  ...
  GdkDisplay *display;
  BobguiSettings *display_settings;
...
struct _BobguiIconThemeClass
{
  GObjectClass parent_class;

  void (* changed)  (BobguiIconTheme *self);
};
...
typedef struct
{
  char *dir;
  time_t mtime;
  BobguiIconCache *cache;
  gboolean exists;
} IconThemeDirMtime;

static void              bobgui_icon_theme_finalize          (GObject          *object);
static void              bobgui_icon_theme_dispose           (GObject          *object);
...
static BobguiIconPaintable *theme_lookup_icon                (IconTheme        *theme,
                                                           const char       *icon_name,
                                                           int               size,
                                                           int               scale);
static void              theme_subdir_load                (BobguiIconTheme     *self,
                                                           IconTheme        *theme,
                                                           GKeyFile         *theme_file,
                                                           char             *subdir);
static void              do_theme_change                  (BobguiIconTheme     *self);
static void              blow_themes                      (BobguiIconTheme     *self);
static gboolean          rescan_themes                    (BobguiIconTheme     *self);
static inline IconCacheFlag
                         suffix_from_name                 (const char       *name);
static void              bobgui_icon_theme_unset_display     (BobguiIconTheme     *self);
static void              bobgui_icon_theme_set_display       (BobguiIconTheme     *self,
                                                           GdkDisplay       *display);
static void              update_current_theme__mainthread (BobguiIconTheme     *self);
static gboolean          ensure_valid_themes              (BobguiIconTheme     *self);
...
static BobguiIconThemeRef *
bobgui_icon_theme_ref_new (BobguiIconTheme *theme)
{
  BobguiIconThemeRef *ref = g_new0 (BobguiIconThemeRef, 1);
...
static BobguiIconThemeRef *
bobgui_icon_theme_ref_ref (BobguiIconThemeRef *ref)
{
...
static void
bobgui_icon_theme_ref_unref (BobguiIconThemeRef *ref)
{
...
static BobguiIconTheme *
bobgui_icon_theme_ref_aquire (BobguiIconThemeRef *ref)
{
...
static void
bobgui_icon_theme_ref_release (BobguiIconThemeRef *ref)
{
  BobguiIconTheme *theme;
...
static void
bobgui_icon_theme_ref_dispose (BobguiIconThemeRef *ref)
{
  bobgui_icon_theme_ref_aquire (ref);
  ref->theme = NULL;
  bobgui_icon_theme_ref_release (ref);
}

static void
bobgui_icon_theme_lock (BobguiIconTheme *self)
{
  g_mutex_lock (&self->ref->lock);
}

static void
bobgui_icon_theme_unlock (BobguiIconTheme *self)
{
...
/****************** Icon cache ***********************
 *
 * The icon cache, this spans both BobguiIconTheme and BobguiIconPaintable,
...
/* The LRU cache is a short list of BobguiIconPaintables that are kept
...
static gboolean
_icon_cache_should_lru_cache (BobguiIconPaintable *icon)
{
 ...
 * the lock held */
static BobguiIconPaintable *
_icon_cache_add_to_lru_cache (BobguiIconTheme     *theme,
                              BobguiIconPaintable *icon)
{
  BobguiIconPaintable *old_icon = NULL;
...
static BobguiIconPaintable *
icon_cache_lookup (BobguiIconTheme *theme,
                   IconKey      *key)
{
  BobguiIconPaintable *old_icon = NULL;
  BobguiIconPaintable *icon;
 ...
 */
static void
icon_uncached_cb (BobguiIconPaintable *icon)
{
...
void
icon_cache_mark_used_if_cached (BobguiIconPaintable *icon)
{
  BobguiIconPaintable *old_icon = NULL;
...
static void
icon_cache_add (BobguiIconTheme     *theme,
                BobguiIconPaintable *icon)
{
  BobguiIconPaintable *old_icon = NULL;
...
void
icon_cache_remove (BobguiIconPaintable *icon)
{
  G_LOCK (icon_cache);
  if (icon->in_cache)
...
static void
icon_cache_clear (BobguiIconTheme *theme)
{
  int i;
  BobguiIconPaintable *old_icons[LRU_CACHE_SIZE];
...
G_DEFINE_TYPE (BobguiIconTheme, bobgui_icon_theme, G_TYPE_OBJECT)

/**
 * bobgui_icon_theme_new:
 *
 * Creates a new icon theme object.
 *
 * Icon theme objects are used to lookup up an icon by name
 * in a particular icon theme. Usually, you’ll want to use
 * [func@Bobgui.IconTheme.get_for_display] rather than creating
 * a new icon theme object for scratch.
 *
 * Returns: the newly created `BobguiIconTheme` object.
 */
BobguiIconTheme *
bobgui_icon_theme_new (void)
{
  return g_object_new (BOBGUI_TYPE_ICON_THEME, NULL);
}
...
{
  BobguiIconTheme *self = BOBGUI_ICON_THEME (source_object);

  bobgui_icon_theme_lock (self);
  ensure_valid_themes (self);
  bobgui_icon_theme_unlock (self);
  g_task_return_pointer (task, NULL, NULL);
}

static void
bobgui_icon_theme_load_in_thread (BobguiIconTheme *self)
{
  GTask *task;
...
/**
 * bobgui_icon_theme_get_for_display:
 ...
 * [ctor@Bobgui.IconTheme.new] and setting the display yourself; by using
 * this function a single icon theme object will be shared between users.
 *
 * Returns: (transfer none): A unique `BobguiIconTheme` associated with
 ...
 */
BobguiIconTheme *
bobgui_icon_theme_get_for_display (GdkDisplay *display)
{
  BobguiIconTheme *self;

  g_return_val_if_fail (GDK_IS_DISPLAY (display), NULL);

  self = g_object_get_data (G_OBJECT (display), "bobgui-icon-theme");
  if (!self)
    {
      self = bobgui_icon_theme_new ();
      self->is_display_singleton = TRUE;
      g_object_set_data (G_OBJECT (display), I_("bobgui-icon-theme"), self);

      /* Call this after setting the user-data, because it recurses into bobgui_icon_theme_get_for_display via the thememing machinery */
      bobgui_icon_theme_set_display (self, display);

      /* Queue early read of the default themes, we read the icon theme name in set_display(). */
      bobgui_icon_theme_load_in_thread (self);
    }
...
static void
bobgui_icon_theme_get_property (GObject      *object,
                             guint         prop_id,
                             GValue       *value,
                             GParamSpec   *pspec)
{
  BobguiIconTheme *self = BOBGUI_ICON_THEME (object);
  ...
  switch (prop_id)
    ...
    case PROP_ICON_NAMES:
      g_value_take_boxed (value, bobgui_icon_theme_get_icon_names (self));
      break;
    ...
    case PROP_THEME_NAME:
      g_value_take_string (value, bobgui_icon_theme_get_theme_name (self));
      break;
...
static void
bobgui_icon_theme_set_property (GObject      *object,
                             guint         prop_id,
                             const GValue *value,
                             GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_DISPLAY:
      bobgui_icon_theme_set_display (self, g_value_get_object (value));
      break;

    case PROP_SEARCH_PATH:
      bobgui_icon_theme_set_search_path (self, g_value_get_boxed (value));
      break;

    case PROP_RESOURCE_PATH:
      bobgui_icon_theme_set_resource_path (self, g_value_get_boxed (value));
      break;

    case PROP_THEME_NAME:
      bobgui_icon_theme_set_theme_name (self, g_value_get_string (value));
      break;
...
static void
bobgui_icon_theme_class_init (BobguiIconThemeClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->finalize = bobgui_icon_theme_finalize;
  gobject_class->dispose = bobgui_icon_theme_dispose;
  gobject_class->get_property = bobgui_icon_theme_get_property;
  gobject_class->set_property = bobgui_icon_theme_set_property;

  /**
   * BobguiIconTheme::changed:
   * @self: the icon theme
   *
   * Emitted when the icon theme changes.
   *
   * This can happen because current icon theme is switched or
   * because BOBGUI detects that a change has occurred in the
   * contents of the current icon theme.
   */
  signal_changed = g_signal_new (I_("changed"),
                                 G_TYPE_FROM_CLASS (klass),
                                 G_SIGNAL_RUN_LAST,
                                 G_STRUCT_OFFSET (BobguiIconThemeClass, changed),
                                 NULL, NULL,
                                 NULL,
                                 G_TYPE_NONE, 0);

  /**
   * BobguiIconTheme:display:
   *
  ...
  /**
   * BobguiIconTheme:search-path:
   *
   * The search path for this icon theme.
   *
   * When looking for icons, BOBGUI will search for a subdirectory of
  ...
  /**
   * BobguiIconTheme:theme-name:
   *
   * The name of the icon theme that is being used.
   *
   * Unless set to a different value, this will be the value of
   * the `BobguiSettings:bobgui-icon-theme-name` property of the `BobguiSettings`
...
display_closed__mainthread_unlocked (GdkDisplay      *display,
                                     ...
                                     BobguiIconThemeRef *ref)
{
  BobguiIconTheme *self = bobgui_icon_theme_ref_aquire (ref);
  ...
  if (self)
    {
      ...
      if (was_display_singleton)
        {
          g_object_set_data (G_OBJECT (display), I_("bobgui-icon-theme"), NULL);
          self->is_display_singleton = FALSE;
        }

      bobgui_icon_theme_unset_display (self);
...
static void
update_current_theme__mainthread (BobguiIconTheme *self)
{
  ...
  if (!self->custom_theme)
    {
      ...
      if (self->display)
        {
          BobguiSettings *settings = bobgui_settings_get_for_display (self->display);
          g_object_get (settings, "bobgui-icon-theme-name", &theme, NULL);
        }
...
/* Callback when the icon theme BobguiSetting changes
 */
static void
theme_changed__mainthread_unlocked (BobguiSettings     *settings,
                                    GParamSpec      *pspec,
                                    BobguiIconThemeRef *ref)
{
  BobguiIconTheme *self = bobgui_icon_theme_ref_aquire (ref);
...
static void
bobgui_icon_theme_unset_display (BobguiIconTheme *self)
{
  if (self->display)
    {
...
void
bobgui_icon_theme_set_display (BobguiIconTheme *self,
                            GdkDisplay   *display)
{
  g_return_if_fail (BOBGUI_ICON_THEME (self));
  ...
  if (display)
    {
      ...
      self->display_settings = bobgui_settings_get_for_display (display);

      g_signal_connect_data (display, "closed",
                             G_CALLBACK (display_closed__mainthread_unlocked),
                             bobgui_icon_theme_ref_ref (self->ref),
                             (GClosureNotify)bobgui_icon_theme_ref_unref,
                             0);
      g_signal_connect_data (self->display_settings, "notify::bobgui-icon-theme-name",
...
{
  if (dir_mtime->cache)
    bobgui_icon_cache_unref (dir_mtime->cache);

  g_free (dir_mtime->dir);
}

static void
bobgui_icon_theme_init (BobguiIconTheme *self)
{
  const char * const *xdg_data_dirs;
  int i, j;
  int len;

  self->ref = bobgui_icon_theme_ref_new (self);
  ...
  self->resource_path[0] = g_strdup ("/org/bobgui/libbobgui/icons/");
...
{
  BobguiIconThemeRef *ref = (BobguiIconThemeRef *)user_data;
  BobguiIconTheme *self;
  GdkDisplay *display = NULL;

  self = bobgui_icon_theme_ref_aquire (ref);
  if (self)
    {
      ...
      if (display)
        {
          ...
          bobgui_system_setting_changed (display, BOBGUI_SYSTEM_SETTING_ICON_THEME);
...
static void
queue_theme_changed (BobguiIconTheme *self)
{
  if (!self->theme_changed_idle)
    {
      self->theme_changed_idle = g_idle_add_full (BOBGUI_PRIORITY_RESIZE - 2,
                                                  theme_changed_idle__mainthread_unlocked,
                                                  bobgui_icon_theme_ref_ref (self->ref),
                                                  (GDestroyNotify)bobgui_icon_theme_ref_unref);
      gdk_source_set_static_name_by_id (self->theme_changed_idle, "[bobgui] theme_changed_idle");
    }
}

static void
do_theme_change (BobguiIconTheme *self)
{
  ...
  BOBGUI_DISPLAY_DEBUG (self->display, ICONTHEME, "change to icon theme \"%s\"", self->current_theme);
  blow_themes (self);
...
static void
blow_themes (BobguiIconTheme *self)
{
  if (self->themes_valid)
    {
      ...
      bobgui_string_set_destroy (&self->icons);
    }
  self->themes = NULL;
  self->unthemed_icons = NULL;
  self->themes_valid = FALSE;
...
int
bobgui_icon_theme_get_serial (BobguiIconTheme *self)
{
  return self->serial;
}

static void
bobgui_icon_theme_dispose (GObject *object)
{
  BobguiIconTheme *self = BOBGUI_ICON_THEME (object);

  /* We make sure all outstanding BobguiIconThemeRefs to us are NULLed
   ...
   * (but not in finalize).
   */
  bobgui_icon_theme_ref_dispose (self->ref);

  G_OBJECT_CLASS (bobgui_icon_theme_parent_class)->dispose (object);
}

static void
bobgui_icon_theme_finalize (GObject *object)
{
  BobguiIconTheme *self = BOBGUI_ICON_THEME (object);
  ...
  bobgui_icon_theme_ref_unref (self->ref);

  G_OBJECT_CLASS (bobgui_icon_theme_parent_class)->finalize (object);
}

/**
 * bobgui_icon_theme_set_search_path:
 * @self: a `BobguiIconTheme`
 ...
 *
 * When looking for an icon theme, BOBGUI will search for a subdirectory
 ...
 */
void
bobgui_icon_theme_set_search_path (BobguiIconTheme       *self,
                                const char * const *path)
{
  char **search_path;

  g_return_if_fail (BOBGUI_IS_ICON_THEME (self));
...
/**
 * bobgui_icon_theme_get_search_path:
 * @self: a `BobguiIconTheme`
 *
 * Gets the current search path.
 *
 * See [method@Bobgui.IconTheme.set_search_path].
 *
 * Returns: (transfer full) (array zero-terminated=1) (element-type filename) (nullable):
 *   a list of icon theme path directories
 */
char **
bobgui_icon_theme_get_search_path (BobguiIconTheme  *self)
{
  char **paths;

  g_return_val_if_fail (BOBGUI_IS_ICON_THEME (self), NULL);
...
/**
 * bobgui_icon_theme_add_search_path:
 * @self: a `BobguiIconTheme`
 ...
 */
void
bobgui_icon_theme_add_search_path (BobguiIconTheme *self,
                                const char   *path)
{
  char **paths;
  int len;
  ...
  bobgui_icon_theme_set_search_path (self, (const char * const *)paths);

  g_free (paths);
}

/**
 * bobgui_icon_theme_set_resource_path:
 * @self: a `BobguiIconTheme`
 ...
 */
void
bobgui_icon_theme_set_resource_path (BobguiIconTheme       *self,
                                  const char * const *path)
{
  char **search_path;
...
/**
 * bobgui_icon_theme_get_resource_path:
 * @self: a `BobguiIconTheme`
 *
 * Gets the current resource path.
 *
 * See [method@Bobgui.IconTheme.set_resource_path].
 *
 * Returns: (transfer full) (array zero-terminated=1) (element-type utf8) (nullable):
 *   A list of resource paths
 */
char **
bobgui_icon_theme_get_resource_path (BobguiIconTheme  *self)
{
  char **paths;
...
/**
 * bobgui_icon_theme_add_resource_path:
 * @self: a `BobguiIconTheme`
 ...
 */
void
bobgui_icon_theme_add_resource_path (BobguiIconTheme *self,
                                  const char   *path)
{
  char **paths;
  int len;
  ...
  bobgui_icon_theme_set_resource_path (self, (const char * const *)paths);
 ...
 *
 * Sets the name of the icon theme that the `BobguiIconTheme` object uses
 * overriding system configuration.
 *
 * This function cannot be called on the icon theme objects returned
 * from [func@Bobgui.IconTheme.get_for_display].
 */
void
bobgui_icon_theme_set_theme_name (BobguiIconTheme *self,
...
/**
 * bobgui_icon_theme_get_theme_name:
 * @self: a `BobguiIconTheme`
 *
 ...
 */
char *
bobgui_icon_theme_get_theme_name (BobguiIconTheme *self)
{
  char *theme_name;
  ...
  if (self->custom_theme)
  ...
  else
    {
      if (self->display)
        {
          BobguiSettings *settings = bobgui_settings_get_for_display (self->display);
          g_object_get (settings, "bobgui-icon-theme-name", &theme_name, NULL);
        }
      else
        theme_name = NULL;
    }
...
static void
insert_theme (BobguiIconTheme *self,
              const char   *theme_name)
{
  int i;
  GList *l;
  ...
  if (theme_file == NULL)
    {
      if (strcmp (theme_name, FALLBACK_ICON_THEME) == 0)
        {
          const char *resource_path = "/org/bobgui/libbobgui/icons/hicolor.index.theme";
          GBytes *index;
...
static void
add_unthemed_icon (BobguiIconTheme *self,
                   const char   *dir,
                   const char   *file,
                   gboolean      is_resource)
{
...
static void
load_themes (BobguiIconTheme *self)
{
  GDir *gdir;
  int base;
  char *dir;
  ...
  bobgui_string_set_init (&self->icons);
  ...
  for (base = 0; self->search_path[base]; base++)
    {
      ...
      dir_mtime->cache = bobgui_icon_cache_new_for_path (dir);
  ...
  if (BOBGUI_DISPLAY_DEBUG_CHECK (self->display, ICONTHEME))
    {
      GList *l;
      GString *s;
...
static gboolean
ensure_valid_themes (BobguiIconTheme *self)
{
...
static BobguiIconPaintable *
real_choose_icon (BobguiIconTheme      *self,
  ...
  for (l = self->themes; l; l = l->next)
    {
      ...
      for (i = 0; icon_names[i] && icon_name_is_symbolic (icon_names[i], -1); i++)
        {
          icon_name = bobgui_string_set_lookup (&self->icons, icon_names[i]);
          if (icon_name)
            {
      ...
      if (unthemed_icon)
        {
          ...
          if (name)
            {
              icon = bobgui_icon_paintable_new_for_path (name,
                                                      unthemed_icon->is_resource,
                                                      size,
                                                      scale);
              bobgui_icon_paintable_set_icon_name (icon, icon_names[i]);
              goto out;
            }
        }
    }
      ...
      if (hIcon)
        {
          ...
          GdkTexture *texture;
          win32_icon = gdk_win32_icon_to_pixbuf_libbobgui_only (hIcon, NULL, NULL);
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
          texture = gdk_texture_new_for_pixbuf (win32_icon);
G_GNUC_END_IGNORE_DEPRECATIONS
          icon = bobgui_icon_paintable_new_for_texture (texture, size, scale);
  ...
  if (icon == NULL)
    {
      if (BOBGUI_DEBUG_CHECK (ICONFALLBACK))
        {
...
static void
icon_name_list_add_icon (BobguiStrvBuilder *icons,
                         const char     *dir_suffix,
                         char           *icon_name)
{
  if (dir_suffix)
    bobgui_strv_builder_append (icons, g_strconcat (icon_name, dir_suffix, NULL));
  bobgui_strv_builder_append (icons, icon_name);
}

static BobguiIconPaintable *
choose_icon (BobguiIconTheme      *self,
             const char        *icon_names[],
             int                size,
             int                scale,
             ...
             BobguiIconLookupFlags flags)
{
  ...
  if ((flags & BOBGUI_ICON_LOOKUP_FORCE_REGULAR) && has_symbolic)
    {
      bobgui_strv_builder_init (&new_names);
      for (i = 0; icon_names[i]; i++)
        {
          if (icon_name_is_symbolic (icon_names[i], -1))
      ...
      icon = real_choose_icon (self,
                               (const char **) bobgui_strv_builder_get_data (&new_names),
                               size,
                               scale,
                               flags & ~(BOBGUI_ICON_LOOKUP_FORCE_REGULAR | BOBGUI_ICON_LOOKUP_FORCE_SYMBOLIC));

      bobgui_strv_builder_clear (&new_names);
    }
  else if ((flags & BOBGUI_ICON_LOOKUP_FORCE_SYMBOLIC) && has_regular)
    {
      bobgui_strv_builder_init (&new_names);
      for (i = 0; icon_names[i]; i++)
        {
...
load_icon_thread (GTask        *task,
                  ...
                  GCancellable *cancellable)
{
  BobguiIconPaintable *self = BOBGUI_ICON_PAINTABLE (source_object);

  bobgui_icon_paintable_load_in_thread (self);
  g_task_return_pointer (task, NULL, NULL);
}

/**
 * bobgui_icon_theme_lookup_icon:
 * @self: a `BobguiIconTheme`
 ...
 * returning a `BobguiIconPaintable`.
 *
 ...
 * for missing icons you need to use [method@Bobgui.IconTheme.has_icon].
 *
 * Note that you probably want to listen for icon theme changes and
 * update the icon. This is usually done by overriding the
 * BobguiWidgetClass.css-changed() function.
 *
 * Returns: (transfer full): a `BobguiIconPaintable` object
 *   containing the icon.
 */
BobguiIconPaintable *
bobgui_icon_theme_lookup_icon (BobguiIconTheme       *self,
                            const char         *icon_name,
                            ...
                            BobguiIconLookupFlags  flags)
{
  ...
  BOBGUI_DISPLAY_DEBUG (self->display, ICONTHEME, "looking up icon %s for scale %d", icon_name, scale);
  ...
  if (flags & BOBGUI_ICON_LOOKUP_PRELOAD)
    {
...
/**
 * bobgui_icon_theme_error_quark:
 *
 * Registers an error quark for [class@Bobgui.IconTheme] errors.
 *
 * Returns: the error quark
 **/
GQuark
bobgui_icon_theme_error_quark (void)
{
  return g_quark_from_static_string ("bobgui-icon-theme-error-quark");
}

/**
 * bobgui_icon_theme_has_icon:
 * @self: a `BobguiIconTheme`
 ...
 */
gboolean
bobgui_icon_theme_has_icon (BobguiIconTheme *self,
                         const char   *icon_name)
{
  gboolean res = FALSE;

  g_return_val_if_fail (BOBGUI_IS_ICON_THEME (self), FALSE);
  ...
  if (bobgui_string_set_lookup (&self->icons, icon_name) != NULL ||
 ...
 */
gboolean
bobgui_icon_theme_has_gicon (BobguiIconTheme *self,
                          GIcon        *gicon)
{
  ...
  for (int i = 0; names[i]; i++)
    {
      if (bobgui_string_set_lookup (&self->icons, names[i]) != NULL ||
...
/**
 * bobgui_icon_theme_get_icon_sizes:
 * @self: a `BobguiIconTheme`
 * @icon_name: the name of an icon
 *
 ...
 */
int *
bobgui_icon_theme_get_icon_sizes (BobguiIconTheme *self,
                               const char   *icon_name)
{
  GList *l;
  int i;
  ...
  interned_icon_name = bobgui_string_set_lookup (&self->icons, icon_name);
...
/**
 * bobgui_icon_theme_get_icon_names:
 * @self: a `BobguiIconTheme`
 *
 ...
 */
char **
bobgui_icon_theme_get_icon_names (BobguiIconTheme *self)
{
  GHashTable *icons;
  GHashTableIter iter;
  char **names;
  ...
  bobgui_string_set_list (&self->icons, icons);
...
static gboolean
rescan_themes (BobguiIconTheme *self)
{
  int stat_res;
  GStatBuf stat_buf;
  guint i;
  ...
  if (min_dir_size)
    {
      ...
      icon = bobgui_icon_paintable_new_for_path (path, dir->is_resource, size, scale);
      g_free (path);
      g_free (filename);
      bobgui_icon_paintable_set_icon_name (icon, icon_name);
...
static GHashTable *
scan_directory (BobguiIconTheme  *self,
                char          *full_dir,
                BobguiStringSet  *set)
{
  GDir *gdir;
  ...
  BOBGUI_DISPLAY_DEBUG (self->display, ICONTHEME, "scanning directory %s", full_dir);
  ...
  while ((name = g_dir_read_name (gdir)))
    {
      ...
      interned = bobgui_string_set_add (set, name);
...
static GHashTable *
scan_resource_directory (BobguiIconTheme  *self,
                         const char    *full_dir,
                         BobguiStringSet  *set)
{
  GHashTable *icons = NULL;
  ...
  BOBGUI_DISPLAY_DEBUG (self->display, ICONTHEME, "scanning resource directory %s", full_dir);
...
static void
theme_subdir_load (BobguiIconTheme *self,
                   IconTheme    *theme,
                   GKeyFile     *theme_file,
                   char         *subdir)
{
          ...
          if (dir_mtime->cache == NULL)
            {
              ...
              dir_mtime->cache = bobgui_icon_cache_new_for_path (dir_mtime->dir);
            }

          if (dir_mtime->cache != NULL)
            icons = bobgui_icon_cache_list_icons_in_directory (dir_mtime->cache, subdir, &self->icons);
          else
...
/**
 * bobgui_icon_theme_lookup_by_gicon:
 * @self: a `BobguiIconTheme`
 ...
 */
BobguiIconPaintable *
bobgui_icon_theme_lookup_by_gicon (BobguiIconTheme       *self,
                                GIcon              *gicon,
                                int                 size,
                                int                 scale,
                                BobguiTextDirection    direction,
                                BobguiIconLookupFlags  flags)
{
  BobguiIconPaintable *paintable = NULL;
  ...
  if (GDK_IS_TEXTURE (gicon))
    {
      paintable = bobgui_icon_paintable_new_for_texture (GDK_TEXTURE (gicon), size, scale);
    }
  else if (GDK_IS_PIXBUF (gicon))
    {
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
      GdkTexture *texture = gdk_texture_new_for_pixbuf (GDK_PIXBUF (gicon));
      paintable = bobgui_icon_paintable_new_for_texture (texture, size, scale);
      g_object_unref (texture);
G_GNUC_END_IGNORE_DEPRECATIONS
    }
  else if (G_IS_FILE_ICON (gicon))
    {
      ...
      paintable = bobgui_icon_paintable_new_for_file (file, size, scale);
    }
  else if (G_IS_LOADABLE_ICON (gicon))
    {
      paintable = bobgui_icon_paintable_new_for_loadable (G_LOADABLE_ICON (gicon), size, scale);
    }
  else if (G_IS_THEMED_ICON (gicon))
    {
      ...
      paintable = bobgui_icon_theme_lookup_icon (self, names[0], &names[1], size, scale, direction, flags);
    }
  else
    {
      g_debug ("Unhandled GIcon type %s", G_OBJECT_TYPE_NAME (gicon));
      paintable = bobgui_icon_paintable_new_for_path (IMAGE_MISSING_RESOURCE_PATH, TRUE, size, scale);
      bobgui_icon_paintable_set_icon_name (paintable, "image-missing");
    }
...
/**
 * bobgui_icon_theme_get_display:
 * @self: a `BobguiIconTheme`
 *
 * Returns the display that the `BobguiIconTheme` object was
 * created for.
 *
 * Returns: (nullable) (transfer none): the display of @icon_theme
 */
GdkDisplay *
bobgui_icon_theme_get_display (BobguiIconTheme *self)
{
  g_return_val_if_fail (BOBGUI_IS_ICON_THEME (self), NULL);
