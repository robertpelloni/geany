...
} BobguiBuilderMenuState;

static void
bobgui_builder_menu_push_frame (BobguiBuilderMenuState *state,
                             GMenu               *menu,
                             GMenuItem           *item)
{
  struct frame *new;
...
static void
bobgui_builder_menu_pop_frame (BobguiBuilderMenuState *state)
{
...
static void
bobgui_builder_menu_start_element (BobguiBuildableParseContext  *context,
                                ...
                                GError                   **error)
{
  BobguiBuilderMenuState *state = user_data;
...
#define STRING     G_MARKUP_COLLECT_STRING
          ...
          if (COLLECT (G_MARKUP_COLLECT_INVALID, NULL))
            {
              ...
              bobgui_builder_menu_push_frame (state, NULL, item);
            }
          ...
          if (COLLECT (STRING | OPTIONAL, "id", &id))
            {
              ...
              bobgui_builder_menu_push_frame (state, menu, item);

              if (id != NULL)
                _bobgui_builder_add_object (state->parser_data->builder, id, G_OBJECT (menu));
              g_object_unref (menu);
            }
      ...
      if (g_str_equal (element_name, "attribute"))
        {
          ...
          if (COLLECT (STRING,             "name", &name,
                       ...
                       OPTIONAL | STRING,  "type", &typestr))
            {
              ...
              if (translatable &&
                  !bobgui_builder_parse_translatable (translatable, &state->translatable, error))
                {
                  return;
                }
              ...
              bobgui_builder_menu_push_frame (state, NULL, NULL);
            }
      ...
      if (g_str_equal (element_name, "link"))
        {
          ...
          if (COLLECT (STRING,            "name", &name,
                       STRING | OPTIONAL, "id",   &id))
            {
              ...
              bobgui_builder_menu_push_frame (state, menu, NULL);
...

  {
    ...
    element_stack = bobgui_buildable_parse_context_get_element_stack (context);
...
static void
bobgui_builder_menu_end_element (BobguiBuildableParseContext  *context,
                              const char                *element_name,
                              gpointer                   user_data,
                              GError                   **error)
{
  ...
  bobgui_builder_menu_pop_frame (state);
...
static void
bobgui_builder_menu_text (BobguiBuildableParseContext  *context,
                       const char                *text,
                       gsize                      text_len,
                       gpointer                   user_data,
                       GError                   **error)
    ...
    if (!g_ascii_isspace (text[i]))
      {
        if (state->string)
          ...
          g_set_error (error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
                       ...
                       bobgui_buildable_parse_context_get_element (context));
        break;
      }
}

static void
bobgui_builder_menu_error (BobguiBuildableParseContext *context,
                        GError                   *error,
                        gpointer                  user_data)
{
  BobguiBuilderMenuState *state = user_data;
...
static BobguiBuildableParser bobgui_builder_menu_subparser =
{
  bobgui_builder_menu_start_element,
  bobgui_builder_menu_end_element,
  bobgui_builder_menu_text,
  bobgui_builder_menu_error
};

void
_bobgui_builder_menu_start (ParserData   *parser_data,
                         const char   *element_name,
                         const char **attribute_names,
                         const char **attribute_values,
                         GError      **error)
{
  BobguiBuilderMenuState *state;
  const char *id = NULL;
  char *internal_id = NULL;

  state = g_new0 (BobguiBuilderMenuState, 1);
  state->parser_data = parser_data;
  bobgui_buildable_parse_context_push (&parser_data->ctx, &bobgui_builder_menu_subparser, state);
...
char *
_bobgui_builder_menu_end (ParserData *parser_data)
{
  BobguiBuilderMenuState *state;
  char *id;

  state = bobgui_buildable_parse_context_pop (&parser_data->ctx);
  id = state->id;
  bobgui_builder_menu_pop_frame (state);
