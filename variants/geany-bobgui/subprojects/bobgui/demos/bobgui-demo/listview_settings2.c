 ...
 * #Keywords: BobguiListHeaderFactory, BobguiSectionModel
 *
 * This demo shows an alternative settings viewer for GSettings.
 *
 * It demonstrates how to implement support for sections with BobguiListView.
 *
...
static void
item_value_changed (BobguiEntry    *entry,
                    GParamSpec  *pspec,
                    BobguiListItem *item)
{
  SettingsKey *self;
  ...
  self = bobgui_list_item_get_item (item);
...
revert:
  bobgui_widget_error_bell (BOBGUI_WIDGET (entry));
...
BobguiWidget *
do_listview_settings2 (BobguiWidget *do_widget)
{
  if (window == NULL)
    {
      ...
      bobgui_builder_cscope_add_callback (scope, settings_key_get_search_string);
      ...
      bobgui_builder_add_from_resource (builder, "/listview_settings2/listview_settings2.ui", &error);
      ...
      listview = BOBGUI_LIST_VIEW (bobgui_builder_get_object (builder, "listview"));
      filter = BOBGUI_FILTER (bobgui_builder_get_object (builder, "filter"));
      ...
      treemodel = bobgui_tree_list_model_new (model,
                                           ...
                                           NULL,
                                           NULL);
      model = G_LIST_MODEL (bobgui_map_list_model_new (G_LIST_MODEL (treemodel), map_settings_to_keys, NULL, NULL));
      model = G_LIST_MODEL (bobgui_flatten_list_model_new (model));
      model = G_LIST_MODEL (bobgui_filter_list_model_new (model, g_object_ref (filter)));
