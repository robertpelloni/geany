...
{
  BobguiLayoutManagerClass parent_class;
};

G_DEFINE_TYPE (DemoLayout, demo_layout, BOBGUI_TYPE_LAYOUT_MANAGER)

static void
demo_layout_measure (BobguiLayoutManager *layout_manager,
                     BobguiWidget        *widget,
...
static void
demo_layout_allocate (BobguiLayoutManager *layout_manager,
                      BobguiWidget        *widget,
                      int               width,
                      int               height,
                      int               baseline)
{
  ...
  for (child = bobgui_widget_get_first_child (widget), i = 0;
       ...
       child = bobgui_widget_get_next_sibling (child), i++)
    {
      ...
      bobgui_widget_size_allocate (child,
                                &(const BobguiAllocation){ x, y, child_width, child_height},
                                -1);
    }
}

static BobguiSizeRequestMode
demo_layout_get_request_mode (BobguiLayoutManager *layout_manager,
                              BobguiWidget        *widget)
{
