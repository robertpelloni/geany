/* Lists/Clocks
 * #Keywords: BobguiGridView, BobguiListItemFactory, GListModel
 *
 ...
 *
 * Typically, this will be done using BobguiBuilder .ui files with the
...
#define BOBGUI_TYPE_CLOCK (bobgui_clock_get_type ())
G_DECLARE_FINAL_TYPE (BobguiClock, bobgui_clock, BOBGUI, CLOCK, GObject)

/* This is our object. It's just a timezone */
typedef struct _BobguiClock BobguiClock;
struct _BobguiClock
{
 ...
 */
static GDateTime *
bobgui_clock_get_time (BobguiClock *clock)
{
  if (clock->timezone)
 ...
 */
static void
bobgui_clock_snapshot (GdkPaintable *paintable,
                    GdkSnapshot  *snapshot,
                    double        width,
                    double        height)
{
  BobguiClock *self = BOBGUI_CLOCK (paintable);
  GDateTime *time;
...
#define BLACK ((GdkRGBA) { 0, 0, 0, 1 })
   ...
   * the aspect ratio.
   */
  bobgui_snapshot_scale (snapshot, MIN (width, height) / 100.0, MIN (width, height) / 100.0);
   ...
   */
  time = bobgui_clock_get_time (self);
   ...
   */
  bobgui_snapshot_save (snapshot);
  bobgui_snapshot_rotate (snapshot, 30 * g_date_time_get_hour (time) + 0.5 * g_date_time_get_minute (time));
  gsk_rounded_rect_init_from_rect (&outline, &GRAPHENE_RECT_INIT(-2, -23, 4, 25), 2);
  bobgui_snapshot_push_rounded_clip (snapshot, &outline);
  bobgui_snapshot_append_color (snapshot, &BLACK, &outline.bounds);
   ...
   */
  bobgui_snapshot_save (snapshot);
  bobgui_snapshot_rotate (snapshot, 6 * g_date_time_get_minute (time));
  ...
  bobgui_snapshot_save (snapshot);
  bobgui_snapshot_rotate (snapshot, 6 * g_date_time_get_second (time));
...
static int
bobgui_clock_get_intrinsic_width (GdkPaintable *paintable)
{
  return 100;
}

static int
bobgui_clock_get_intrinsic_height (GdkPaintable *paintable)
{
  return 100;
}
 ...
 */
static void
bobgui_clock_paintable_init (GdkPaintableInterface *iface)
{
  iface->snapshot = bobgui_clock_snapshot;
  iface->get_intrinsic_width = bobgui_clock_get_intrinsic_width;
  iface->get_intrinsic_height = bobgui_clock_get_intrinsic_height;
}

/* Finally, we define the type. The important part is adding the
 * paintable interface, so BOBGUI knows that this object can indeed
 * be drawn.
 */
G_DEFINE_TYPE_WITH_CODE (BobguiClock, bobgui_clock, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (GDK_TYPE_PAINTABLE,
                                                bobgui_clock_paintable_init))

static GParamSpec *properties[N_PROPS] = { NULL, };

static void
bobgui_clock_get_property (GObject    *object,
                        guint       property_id,
                        GValue     *value,
                        GParamSpec *pspec)
{
  BobguiClock *self = BOBGUI_CLOCK (object);
  ...
  switch (property_id)
    ...
    case PROP_TIME:
      g_value_take_boxed (value, bobgui_clock_get_time (self));
      break;
...
static void
bobgui_clock_set_property (GObject      *object,
                        guint         property_id,
                        const GValue *value,
                        GParamSpec   *pspec)
{
 ...
 */
static gboolean
bobgui_clock_tick (gpointer unused)
{
  GSList *l;

  for (l = ticking_clocks; l; l = l->next)
    {
      BobguiClock *clock = l->data;
...
static void
bobgui_clock_stop_ticking (BobguiClock *self)
{
...
static void
bobgui_clock_start_ticking (BobguiClock *self)
{
  /* if no clock is ticking yet, start */
  if (ticking_clock_id == 0)
    ticking_clock_id = g_timeout_add_seconds (1, bobgui_clock_tick, NULL);

  ticking_clocks = g_slist_prepend (ticking_clocks, self);
}

static void
bobgui_clock_finalize (GObject *object)
{
  BobguiClock *self = BOBGUI_CLOCK (object);

  bobgui_clock_stop_ticking (self);

  g_free (self->location);
  g_clear_pointer (&self->timezone, g_time_zone_unref);

  G_OBJECT_CLASS (bobgui_clock_parent_class)->finalize (object);
}

static void
bobgui_clock_class_init (BobguiClockClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->get_property = bobgui_clock_get_property;
  gobject_class->set_property = bobgui_clock_set_property;
  gobject_class->finalize = bobgui_clock_finalize;
...
static void
bobgui_clock_init (BobguiClock *self)
{
  bobgui_clock_start_ticking (self);
}

static BobguiClock *
bobgui_clock_new (const char *location,
               GTimeZone  *_tz)
{
  BobguiClock *result;

  result = g_object_new (BOBGUI_TYPE_CLOCK,
                         "location", location,
                         "timezone", _tz,
                         NULL);
...
create_clocks_model (void)
{
  ...
  result = g_list_store_new (BOBGUI_TYPE_CLOCK);

  /* local time */
  clock = bobgui_clock_new ("local", NULL);
  g_list_store_append (result, clock);
  g_object_unref (clock);
  /* UTC time */
  clock = bobgui_clock_new ("UTC", g_time_zone_new_utc ());
  g_list_store_append (result, clock);
  g_object_unref (clock);
  /* A bunch of timezones with BOBGUI hackers */
  clock = bobgui_clock_new ("San Francisco", g_time_zone_new_identifier ("America/Los_Angeles"));
  g_list_store_append (result, clock);
  g_object_unref (clock);
  clock = bobgui_clock_new ("Xalapa", g_time_zone_new_identifier ("America/Mexico_City"));
  g_list_store_append (result, clock);
  g_object_unref (clock);
  clock = bobgui_clock_new ("Boston", g_time_zone_new_identifier ("America/New_York"));
  g_list_store_append (result, clock);
  g_object_unref (clock);
  clock = bobgui_clock_new ("London", g_time_zone_new_identifier ("Europe/London"));
  g_list_store_append (result, clock);
  g_object_unref (clock);
  clock = bobgui_clock_new ("Berlin", g_time_zone_new_identifier ("Europe/Berlin"));
  g_list_store_append (result, clock);
  g_object_unref (clock);
  clock = bobgui_clock_new ("Moscow", g_time_zone_new_identifier ("Europe/Moscow"));
  g_list_store_append (result, clock);
  g_object_unref (clock);
  clock = bobgui_clock_new ("New Delhi", g_time_zone_new_identifier ("Asia/Kolkata"));
  g_list_store_append (result, clock);
  g_object_unref (clock);
  clock = bobgui_clock_new ("Shanghai", g_time_zone_new_identifier ("Asia/Shanghai"));
  g_list_store_append (result, clock);
  g_object_unref (clock);
...
{
  BobguiWidget *box, *picture, *location_label, *time_label;
  BobguiExpression *clock_expression, *expression;
  ...
  clock_expression = bobgui_property_expression_new (BOBGUI_TYPE_LIST_ITEM, expression, "item");

  /* Bind the clock's location to a label.
   * This is easy: We just get the "location" property of the clock.
   */
  expression = bobgui_property_expression_new (BOBGUI_TYPE_CLOCK,
                                            bobgui_expression_ref (clock_expression),
                                            "location");
  /* Now create the label and bind the expression to it. */
  location_label = bobgui_label_new (NULL);
  bobgui_expression_bind (expression, location_label, "label", location_label);
  bobgui_box_append (BOBGUI_BOX (box), location_label);


  /* Here we bind the item itself to a GdkPicture.
   * This is simply done by using the clock expression itself.
   */
  expression = bobgui_expression_ref (clock_expression);
  /* Now create the widget and bind the expression to it. */
  picture = bobgui_picture_new ();
  bobgui_expression_bind (expression, picture, "paintable", picture);
  bobgui_box_append (BOBGUI_BOX (box), picture);
  bobgui_accessible_update_relation (BOBGUI_ACCESSIBLE (picture),
                                  BOBGUI_ACCESSIBLE_RELATION_LABELLED_BY, location_label, NULL,
                                  -1);
  ...
  expression = bobgui_cclosure_expression_new (G_TYPE_STRING,
                                            NULL,
                                            1, (BobguiExpression *[1]) { expression },
                                            G_CALLBACK (convert_time_to_string),
                                            NULL, NULL);
  /* Now create the label and bind the expression to it. */
  time_label = bobgui_label_new (NULL);
  bobgui_expression_bind (expression, time_label, "label", time_label);
  bobgui_box_append (BOBGUI_BOX (box), time_label);

  bobgui_expression_unref (clock_expression);
}

static BobguiWidget *window = NULL;

BobguiWidget *
do_listview_clocks (BobguiWidget *do_widget)
{
  if (window == NULL)
    {
      BobguiWidget *gridview, *sw;
      BobguiListItemFactory *factory;
      BobguiSelectionModel *model;

      /* This is the normal window setup code every demo does */
      window = bobgui_window_new ();
      bobgui_window_set_title (BOBGUI_WINDOW (window), "Clocks");
      ...
      model = BOBGUI_SELECTION_MODEL (bobgui_no_selection_new (create_clocks_model ()));
      gridview = bobgui_grid_view_new (model, factory);
      bobgui_accessible_update_property (BOBGUI_ACCESSIBLE (gridview),
                                      BOBGUI_ACCESSIBLE_PROPERTY_LABEL, "Clocks",
                                      -1);
