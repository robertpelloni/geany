/* Printing/Printing
 *
 * BobguiPrintOperation offers a simple API to support printing
             ...
             gpointer           user_data)
{
  ...
  bobgui_print_operation_set_n_pages (operation, data->num_pages);
}
...
static void
end_print (BobguiPrintOperation *operation,
           BobguiPrintContext   *context,
           gpointer           user_data)
{
  PrintData *data = (PrintData *)user_data;
...
BobguiWidget *
do_printing (BobguiWidget *do_widget)
{
  BobguiPrintOperation *operation;
  BobguiPrintSettings *settings;
  PrintData *data;
  GError *error = NULL;

  operation = bobgui_print_operation_new ();
  ...
  bobgui_print_operation_set_use_full_page (operation, FALSE);
  bobgui_print_operation_set_unit (operation, BOBGUI_UNIT_POINTS);
  bobgui_print_operation_set_embed_page_setup (operation, TRUE);

  settings = bobgui_print_settings_new ();

  bobgui_print_settings_set (settings, BOBGUI_PRINT_SETTINGS_OUTPUT_BASENAME, "bobgui-demo");
  bobgui_print_operation_set_print_settings (operation, settings);

  bobgui_print_operation_run (operation, BOBGUI_PRINT_OPERATION_ACTION_PRINT_DIALOG, BOBGUI_WINDOW (do_widget), &error);
  ...
  if (error)
    {
      ...
      dialog = bobgui_alert_dialog_new ("%s", error->message);
      bobgui_alert_dialog_show (dialog, BOBGUI_WINDOW (do_widget));
