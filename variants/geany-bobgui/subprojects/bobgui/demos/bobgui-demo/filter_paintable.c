...
struct _BobguiFilterPaintableClass
{
...
static void
bobgui_filter_paintable_snapshot (GdkPaintable *paintable,
                               GdkSnapshot  *snapshot,
                               double        width,
                               double        height)
{
  BobguiFilterPaintable *self = BOBGUI_FILTER_PAINTABLE (paintable);
  ...
  if (self->brightness != 1)
    {
      ...
      bobgui_snapshot_push_color_matrix (snapshot, &matrix, graphene_vec4_zero ());
    }
  ...
  if (!gsk_component_transfer_equal (self->red_transfer, identity) ||
    ...
    bobgui_snapshot_push_component_transfer (snapshot,
                                          self->red_transfer,
                                          self->green_transfer,
                                          self->blue_transfer,
                                          self->alpha_transfer);
  ...
  if (self->blur != 0)
    bobgui_snapshot_push_blur (snapshot, self->blur);

  bobgui_snapshot_append_texture (snapshot, self->texture,
...
static int
bobgui_filter_paintable_get_intrinsic_width (GdkPaintable *paintable)
{
  BobguiFilterPaintable *self = BOBGUI_FILTER_PAINTABLE (paintable);
...
static int
bobgui_filter_paintable_get_intrinsic_height (GdkPaintable *paintable)
{
  BobguiFilterPaintable *self = BOBGUI_FILTER_PAINTABLE (paintable);
...
static void
bobgui_filter_paintable_paintable_init (GdkPaintableInterface *iface)
{
  iface->snapshot = bobgui_filter_paintable_snapshot;
  iface->get_intrinsic_width = bobgui_filter_paintable_get_intrinsic_width;
  iface->get_intrinsic_height = bobgui_filter_paintable_get_intrinsic_height;
}

G_DEFINE_TYPE_WITH_CODE (BobguiFilterPaintable, bobgui_filter_paintable, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (GDK_TYPE_PAINTABLE,
                                                bobgui_filter_paintable_paintable_init))

static void
bobgui_filter_paintable_get_property (GObject    *object,
                                   guint       prop_id,
                                   GValue     *value,
                                   GParamSpec *pspec)
{
  BobguiFilterPaintable *self = BOBGUI_FILTER_PAINTABLE (object);
...
static void
bobgui_filter_paintable_set_property (GObject      *object,
                                   guint         prop_id,
                                   const GValue *value,
                                   GParamSpec   *pspec)
{
...
static void
bobgui_filter_paintable_finalize (GObject *object)
{
  BobguiFilterPaintable *self = BOBGUI_FILTER_PAINTABLE (object);
  ...
  G_OBJECT_CLASS (bobgui_filter_paintable_parent_class)->finalize (object);
}

static void
bobgui_filter_paintable_class_init (BobguiFilterPaintableClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = bobgui_filter_paintable_finalize;
  object_class->set_property = bobgui_filter_paintable_set_property;
  object_class->get_property = bobgui_filter_paintable_get_property;
...
static void
bobgui_filter_paintable_init (BobguiFilterPaintable *image)
{
