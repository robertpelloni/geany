...
typedef struct {
  BobguiWidget *the_label;
  BobguiWidget *settings;
  BobguiWidget *description;
  BobguiWidget *font;
  BobguiWidget *script_lang;
  BobguiWidget *feature_list;
  BobguiWidget *variations_grid;
  BobguiWidget *instance_combo;
  BobguiWidget *stack;
  BobguiWidget *entry;
  BobguiWidget *plain_toggle;
  BobguiWidget *waterfall_toggle;
  BobguiWidget *edit_toggle;
  BobguiAdjustment *size_adjustment;
  BobguiAdjustment *letterspacing_adjustment;
  BobguiAdjustment *line_height_adjustment;
  BobguiWidget *foreground;
  BobguiWidget *background;
  BobguiWidget *size_scale;
  BobguiWidget *size_entry;
  BobguiWidget *letterspacing_entry;
  BobguiWidget *line_height_entry;
  GList *feature_items;
  GList *ranges;
...
demo_free (gpointer data)
{
  ...
  bobgui_style_context_remove_provider_for_display (gdk_display_get_default (),
                                                 BOBGUI_STYLE_PROVIDER (demo->provider));
...
{
  if (bobgui_toggle_button_get_active (BOBGUI_TOGGLE_BUTTON (demo->plain_toggle)) ||
      bobgui_toggle_button_get_active (BOBGUI_TOGGLE_BUTTON (demo->waterfall_toggle)))
    {
      bobgui_stack_set_visible_child_name (BOBGUI_STACK (demo->stack), "label");
      update_display ();
    }
}
...
font_features_notify_waterfall (void)
{
  ...
  can_change_size = !bobgui_toggle_button_get_active (BOBGUI_TOGGLE_BUTTON (demo->waterfall_toggle));
  bobgui_widget_set_sensitive (demo->size_scale, can_change_size);
  bobgui_widget_set_sensitive (demo->size_entry, can_change_size);
}
...
update_in_idle (gpointer data)
{
  ...
  str = g_strdup_printf ("%g", bobgui_adjustment_get_value (bd->adjustment));
  bobgui_editable_set_text (BOBGUI_EDITABLE (bd->entry), str);
  g_free (str);
...
static void
basic_value_changed (BobguiAdjustment *adjustment,
                     gpointer       data)
{
  BasicData *bd;
  ...
  bd->entry = BOBGUI_ENTRY (data);

  g_idle_add (update_in_idle, bd);
}

static void
basic_entry_activated (BobguiEntry *entry,
                       gpointer  data)
{
  BobguiAdjustment *adjustment = data;

  double value;
  char *err = NULL;

  value = g_strtod (bobgui_editable_get_text (BOBGUI_EDITABLE (entry)), &err);
  if (err != NULL)
...
swap_colors (void)
{
  ...
  fg = *bobgui_color_dialog_button_get_rgba (BOBGUI_COLOR_DIALOG_BUTTON (demo->foreground));
  bg = *bobgui_color_dialog_button_get_rgba (BOBGUI_COLOR_DIALOG_BUTTON (demo->background));
  bobgui_color_dialog_button_set_rgba (BOBGUI_COLOR_DIALOG_BUTTON (demo->foreground), &bg);
  bobgui_color_dialog_button_set_rgba (BOBGUI_COLOR_DIALOG_BUTTON (demo->background), &fg);
}

static void
font_features_reset_basic (void)
{
  bobgui_adjustment_set_value (demo->size_adjustment, 20);
  bobgui_adjustment_set_value (demo->letterspacing_adjustment, 0);
  bobgui_adjustment_set_value (demo->line_height_adjustment, 1);
  bobgui_color_dialog_button_set_rgba (BOBGUI_COLOR_DIALOG_BUTTON (demo->foreground), &(GdkRGBA){0.,0.,0.,1.});
  bobgui_color_dialog_button_set_rgba (BOBGUI_COLOR_DIALOG_BUTTON (demo->background), &(GdkRGBA){1.,1.,1.,1.});
}
...
update_basic (void)
{
  ...
  desc = bobgui_font_dialog_button_get_font_desc (BOBGUI_FONT_DIALOG_BUTTON (demo->font));

  bobgui_adjustment_set_value (demo->size_adjustment,
...
static void
feat_toggled_cb (BobguiCheckButton *check_button,
                 gpointer        data)
{
...
static void
add_check_group (BobguiWidget   *box,
                 const char  *title,
                 const char **tags)
{
  BobguiWidget *label;
  ...
  bobgui_widget_set_halign (group, BOBGUI_ALIGN_START);
  ...
  for (i = 0; tags[i]; i++)
    {
      ...
      bobgui_box_append (BOBGUI_BOX (group), feat);
  ...
  bobgui_box_append (BOBGUI_BOX (box), group);
}

static void
add_radio_group (BobguiWidget *box,
                 const char  *title,
                 const char **tags)
{
  BobguiWidget *label;
...
update_display (void)
{
...

  {
    ...
    buffer = bobgui_text_view_get_buffer (BOBGUI_TEXT_VIEW (demo->entry));
    bobgui_text_buffer_get_bounds (buffer, &start_iter, &end_iter);
    text = bobgui_text_buffer_get_text (buffer, &start_iter, &end_iter, FALSE);
    text_len = strlen (text);
  }

  do_waterfall = bobgui_toggle_button_get_active (BOBGUI_TOGGLE_BUTTON (demo->waterfall_toggle));

  bobgui_label_set_wrap (BOBGUI_LABEL (demo->the_label), !do_waterfall);
  ...
  else if (bobgui_label_get_selection_bounds (BOBGUI_LABEL (demo->the_label), &ins, &bound))
    {
  ...
  value = bobgui_adjustment_get_value (demo->size_adjustment);
  ...
  if (bobgui_drop_down_get_selected (BOBGUI_DROP_DOWN (demo->script_lang)) != 0)
    {
      ScriptLang *selected;

      selected = bobgui_drop_down_get_selected_item (BOBGUI_DROP_DOWN (demo->script_lang));
  ...
  if (bobgui_adjustment_get_value (demo->letterspacing_adjustment) != 0.)
    {
      attr = pango_attr_letter_spacing_new (bobgui_adjustment_get_value (demo->letterspacing_adjustment));
      attr->start_index = start;
      attr->end_index = end;
      pango_attr_list_insert (attrs, attr);
    }

  if (bobgui_adjustment_get_value (demo->line_height_adjustment) != 1.)
    {
      attr = pango_attr_line_height_new (bobgui_adjustment_get_value (demo->line_height_adjustment));
      attr->start_index = start;
      attr->end_index = end;
      pango_attr_list_insert (attrs, attr);
    }

    {
      ...
      rgba = *bobgui_color_dialog_button_get_rgba (BOBGUI_COLOR_DIALOG_BUTTON (demo->foreground));
      ...
      rgba = *bobgui_color_dialog_button_get_rgba (BOBGUI_COLOR_DIALOG_BUTTON (demo->background));
      bg = gdk_rgba_to_string (&rgba);
      css = g_strdup_printf (".font_features_background { caret-color: %s; background-color: %s; }", fg, bg);
      bobgui_css_provider_load_from_data (demo->provider, css, strlen (css));
      g_free (css);
      g_free (fg);
      g_free (bg);
    }
  ...
  bobgui_label_set_text (BOBGUI_LABEL (demo->description), font_desc);
  bobgui_label_set_text (BOBGUI_LABEL (demo->settings), features);
  ...
  if (do_waterfall)
    {
      ...
      bobgui_label_set_text (BOBGUI_LABEL (demo->the_label), waterfall->str);
      g_string_free (waterfall, TRUE);
    }
  else
    bobgui_label_set_text (BOBGUI_LABEL (demo->the_label), text);

  bobgui_label_set_attributes (BOBGUI_LABEL (demo->the_label), attrs);
...
get_pango_font (void)
{
  ...
  desc = bobgui_font_dialog_button_get_font_desc (BOBGUI_FONT_DIALOG_BUTTON (demo->font));
  context = bobgui_widget_get_pango_context (demo->font);
...
update_script_combo (void)
{
  ...
  language = bobgui_font_dialog_button_get_language (BOBGUI_FONT_DIALOG_BUTTON (demo->font));
  ...
  sortmodel = bobgui_sort_list_model_new (G_LIST_MODEL (store),
                                       BOBGUI_SORTER (bobgui_custom_sorter_new (script_sort, NULL, NULL)));
  bobgui_drop_down_set_model (BOBGUI_DROP_DOWN (demo->script_lang), G_LIST_MODEL (sortmodel));
  ...
  for (guint i = 0; i < g_list_model_get_n_items (G_LIST_MODEL (sortmodel)); i++)
    {
      ...
      if (item->lang_tag == active)
        {
          bobgui_drop_down_set_selected (BOBGUI_DROP_DOWN (demo->script_lang), i);
          break;
        }
    }
      ...
      for (l = demo->feature_items; l; l = l->next)
        {
          ...
          if (strcmp (item->name, "xxxx") == 0)
            bobgui_check_button_set_active (BOBGUI_CHECK_BUTTON (item->feat), TRUE);
        }
  ...
  if (hb_font)
    {
      ...
      feat = bobgui_font_dialog_button_get_font_features (BOBGUI_FONT_DIALOG_BUTTON (demo->font));
      if (feat)
        {
          for (l = demo->feature_items; l; l = l->next)
            {
              ...
              if (p)
                {
                  if (BOBGUI_IS_CHECK_BUTTON (item->feat) && g_object_get_data (G_OBJECT (item->feat), "default"))
                    {
                      bobgui_check_button_set_active (BOBGUI_CHECK_BUTTON (item->feat), p[6] == '1');
                    }
                  else if (BOBGUI_IS_CHECK_BUTTON (item->feat))
                    {
                      set_inconsistent (BOBGUI_CHECK_BUTTON (item->feat), FALSE);
                      bobgui_check_button_set_active (BOBGUI_CHECK_BUTTON (item->feat), p[6] == '1');
                    }
                }
            }
                    ...
                    BobguiEntry      *entry)
{
  ...
  str = g_strdup_printf ("%g", bobgui_adjustment_get_value (adjustment));
...
static void
entry_activated (BobguiEntry *entry,
                 BobguiAdjustment *adjustment)
{
  double value;
  char *err = NULL;
...
static void unset_instance (BobguiAdjustment *adjustment);

static void start_or_stop_axis_animation (BobguiButton *button,
                                          gpointer   data);
...
font_features_reset_variations (void)
{
  ...
  while (g_hash_table_iter_next (&iter, (gpointer *)NULL, (gpointer *)&axis))
    {
      if (axis->tick_cb)
        start_or_stop_axis_animation (BOBGUI_BUTTON (axis->button), axis);
      bobgui_adjustment_set_value (axis->adjustment, axis->default_value);
    }
}
...
static gboolean
animate_axis (BobguiWidget     *widget,
              ...
              gpointer       data)
{
  ...
  lower = bobgui_adjustment_get_lower (axis->adjustment);
  upper = bobgui_adjustment_get_upper (axis->adjustment);

  if (axis->increasing)
    bobgui_adjustment_set_value (axis->adjustment, lower + (upper - lower) * value);
  else
    bobgui_adjustment_set_value (axis->adjustment, upper - (upper - lower) * value);

  return G_SOURCE_CONTINUE;
}

static void
start_or_stop_axis_animation (BobguiButton *button,
                              gpointer   data)
{
  Axis *axis = data;
  ...
  if (axis->tick_cb)
    {
      bobgui_widget_remove_tick_callback (BOBGUI_WIDGET (button), axis->tick_cb);
      axis->tick_cb = 0;
      bobgui_button_set_icon_name (button, "media-playback-start");
    }
  else
    {
      double value, upper, lower;

      bobgui_button_set_icon_name (button, "media-playback-stop");
      axis->tick_cb = bobgui_widget_add_tick_callback (BOBGUI_WIDGET (button), animate_axis, axis, NULL);
      value = bobgui_adjustment_get_value (axis->adjustment);
      lower = bobgui_adjustment_get_lower (axis->adjustment);
      upper = bobgui_adjustment_get_upper (axis->adjustment);
...
add_axis (hb_face_t             *hb_face,
          ...
          int                    i)
{
  BobguiWidget *axis_label;
  BobguiWidget *axis_entry;
  BobguiWidget *axis_scale;
  BobguiAdjustment *adjustment;
  Axis *axis;
  char name[20];
  ...
  axis_label = bobgui_label_new (name);
  bobgui_widget_set_halign (axis_label, BOBGUI_ALIGN_START);
  bobgui_widget_set_valign (axis_label, BOBGUI_ALIGN_BASELINE_FILL);
  bobgui_grid_attach (BOBGUI_GRID (demo->variations_grid), axis_label, 0, i, 1, 1);
  adjustment = bobgui_adjustment_new (value, ax->min_value, ax->max_value,
                                   1.0, 10.0, 0.0);
  axis_scale = bobgui_scale_new (BOBGUI_ORIENTATION_HORIZONTAL, adjustment);
  bobgui_scale_add_mark (BOBGUI_SCALE (axis_scale), ax->default_value, BOBGUI_POS_TOP, NULL);
  bobgui_widget_set_valign (axis_scale, BOBGUI_ALIGN_BASELINE_FILL);
  bobgui_widget_set_hexpand (axis_scale, TRUE);
  bobgui_widget_set_size_request (axis_scale, 100, -1);
  bobgui_grid_attach (BOBGUI_GRID (demo->variations_grid), axis_scale, 1, i, 1, 1);
  axis_entry = bobgui_entry_new ();
  bobgui_widget_set_valign (axis_entry, BOBGUI_ALIGN_BASELINE_FILL);
  bobgui_editable_set_width_chars (BOBGUI_EDITABLE (axis_entry), 4);
  bobgui_editable_set_max_width_chars (BOBGUI_EDITABLE (axis_entry), 4);
  bobgui_widget_set_hexpand (axis_entry, FALSE);
  bobgui_grid_attach (BOBGUI_GRID (demo->variations_grid), axis_entry, 2, i, 1, 1);
  ...
  axis->button = bobgui_button_new_from_icon_name ("media-playback-start");
  bobgui_widget_add_css_class (BOBGUI_WIDGET (axis->button), "circular");
  bobgui_widget_set_valign (BOBGUI_WIDGET (axis->button), BOBGUI_ALIGN_CENTER);
  g_signal_connect (axis->button, "clicked", G_CALLBACK (start_or_stop_axis_animation), axis);
  bobgui_grid_attach (BOBGUI_GRID (demo->variations_grid), axis->button, 3, i, 1, 1);

  adjustment_changed (adjustment, BOBGUI_ENTRY (axis_entry));
              ...
              BobguiStringList *strings)
{
  ...
  bobgui_string_list_append (BOBGUI_STRING_LIST (strings), instance->name);
}

static void
unset_instance (BobguiAdjustment *adjustment)
{
  if (demo->instance_combo)
    bobgui_drop_down_set_selected (BOBGUI_DROP_DOWN (demo->instance_combo), 0);
}

static void
instance_changed (BobguiDropDown *combo)
{
  const char *text;
  Instance *instance;
  Instance ikey;
  ...
  text = bobgui_string_list_get_string (BOBGUI_STRING_LIST (bobgui_drop_down_get_model (combo)),
                                     bobgui_drop_down_get_selected (combo));
  if (text[0] == '\0')
    goto out;
      ...
      if (axis)
        {
          ...
          bobgui_adjustment_set_value (axis->adjustment, value);
...
add_font_plane (int i)
{
  ...
  if (weight_axis && width_axis)
    {
      plane = bobgui_font_plane_new (weight_axis->adjustment,
                                  width_axis->adjustment);

      bobgui_widget_set_size_request (plane, 300, 300);
      bobgui_widget_set_halign (plane, BOBGUI_ALIGN_CENTER);
      bobgui_grid_attach (BOBGUI_GRID (demo->variations_grid), plane, 0, i, 3, 1);
    }
}
...
update_font_variations (void)
{
  ...
  while ((child = bobgui_widget_get_first_child (demo->variations_grid)))
    bobgui_grid_remove (BOBGUI_GRID (demo->variations_grid), child);
  ...
  if (hb_ot_var_get_named_instance_count (hb_face) > 0)
    {
      ...
      label = bobgui_label_new ("Instance");
      bobgui_label_set_xalign (BOBGUI_LABEL (label), 0);
      bobgui_widget_set_halign (label, BOBGUI_ALIGN_START);
      bobgui_widget_set_valign (label, BOBGUI_ALIGN_BASELINE_FILL);
      bobgui_grid_attach (BOBGUI_GRID (demo->variations_grid), label, 0, -1, 1, 1);

      strings = bobgui_string_list_new (NULL);
      combo = bobgui_drop_down_new (G_LIST_MODEL (strings), NULL);

      bobgui_widget_set_halign (combo, BOBGUI_ALIGN_START);
      bobgui_widget_set_valign (combo, BOBGUI_ALIGN_BASELINE_FILL);

      bobgui_string_list_append (strings, "");
      ...
      for (i = 0; i < hb_ot_var_get_named_instance_count (hb_face); i++)
        {
          if (matches_instance (hb_face, i, n_axes, design_coords))
            {
              bobgui_drop_down_set_selected (BOBGUI_DROP_DOWN (combo), i + 1);
              break;
            }
        }

      bobgui_grid_attach (BOBGUI_GRID (demo->variations_grid), combo, 1, -1, 3, 1);
...
font_features_reset_features (void)
{
  ...
  bobgui_label_select_region (BOBGUI_LABEL (demo->the_label), 0, 0);
      ...
      if (BOBGUI_IS_CHECK_BUTTON (item->feat))
        {
          if (strcmp (item->name, "xxxx") == 0)
            ...
            {
              bobgui_check_button_set_active (BOBGUI_CHECK_BUTTON (item->feat), FALSE);
              set_inconsistent (BOBGUI_CHECK_BUTTON (item->feat), TRUE);
            }
        }
    }
...
{
  if (strcmp (bobgui_stack_get_visible_child_name (BOBGUI_STACK (demo->stack)), "entry") != 0)
    {
      BobguiTextBuffer *buffer;
      ...
      g_free (demo->text);
      demo->text = bobgui_text_buffer_get_text (buffer, &start, &end, FALSE);
      bobgui_stack_set_visible_child_name (BOBGUI_STACK (demo->stack), "entry");
      bobgui_widget_grab_focus (demo->entry);
      bobgui_adjustment_set_value (bobgui_scrolled_window_get_vadjustment (BOBGUI_SCROLLED_WINDOW (demo->swin)), 0);
    }
  else
    {
      g_clear_pointer (&demo->text, g_free);
      bobgui_toggle_button_set_active (BOBGUI_TOGGLE_BUTTON (demo->plain_toggle), TRUE);
      update_display ();
    }
}
...
static gboolean
entry_key_press (BobguiEventController *controller,
                 guint               keyval,
                 guint               keycode,
                 GdkModifierType     modifiers,
                 BobguiTextView        *entry)
{
  if (keyval == GDK_KEY_Escape)
    {
      bobgui_text_buffer_set_text (bobgui_text_view_get_buffer (entry), demo->text, -1);
...
{
  demo->sample++;
  bobgui_text_buffer_set_text (bobgui_text_view_get_buffer (BOBGUI_TEXT_VIEW (demo->entry)),
...
BobguiWidget *
do_font_features (BobguiWidget *do_widget)
{
  ...
  if (!window)
    {
      ...
      bobgui_builder_cscope_add_callback (scope, basic_value_changed);
      bobgui_builder_cscope_add_callback (scope, basic_entry_activated);
      bobgui_builder_cscope_add_callback (scope, color_set_cb);
      bobgui_builder_cscope_add_callback (scope, swap_colors);
      bobgui_builder_cscope_add_callback (scope, font_features_reset_basic);
      bobgui_builder_cscope_add_callback (scope, font_features_reset_features);
      bobgui_builder_cscope_add_callback (scope, font_features_reset_variations);
      bobgui_builder_cscope_add_callback (scope, font_features_toggle_plain);
      bobgui_builder_cscope_add_callback (scope, font_features_toggle_edit);
      bobgui_builder_cscope_add_callback (scope, font_features_stop_edit);
      bobgui_builder_cscope_add_callback (scope, font_features_font_changed);
      bobgui_builder_cscope_add_callback (scope, font_features_script_changed);
      bobgui_builder_cscope_add_callback (scope, font_features_notify_waterfall);
      bobgui_builder_cscope_add_callback (scope, set_text_alphabet);
      bobgui_builder_cscope_add_callback (scope, set_text_paragraph);
      bobgui_builder_set_scope (builder, scope);

      demo = g_new0 (FontFeaturesDemo, 1);

      bobgui_builder_add_from_resource (builder, "/font_features/font_features.ui", NULL);

      window = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "window"));
      g_object_set_data_full  (G_OBJECT (window), "demo", demo, demo_free);

      demo->the_label = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "label"));
      demo->settings = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "settings"));
      demo->description = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "description"));
      demo->font = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "font"));
      demo->script_lang = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "script_lang"));
      g_assert (BOBGUI_IS_DROP_DOWN (demo->script_lang));
      expression = bobgui_cclosure_expression_new (G_TYPE_STRING, NULL, 0, NULL, G_CALLBACK (script_lang_get_langname), NULL, NULL);
      bobgui_drop_down_set_expression (BOBGUI_DROP_DOWN (demo->script_lang), expression);
      bobgui_expression_unref (expression);
      demo->feature_list = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "feature_list"));
      demo->stack = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "stack"));
      demo->entry = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "entry"));
      demo->plain_toggle = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "plain_toggle"));
      demo->waterfall_toggle = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "waterfall_toggle"));
      demo->edit_toggle = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "edit_toggle"));
      demo->size_scale = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "size_scale"));
      demo->size_entry = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "size_entry"));
      demo->size_adjustment = BOBGUI_ADJUSTMENT (bobgui_builder_get_object (builder, "size_adjustment"));
      demo->letterspacing_entry = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "letterspacing_entry"));
      demo->letterspacing_adjustment = BOBGUI_ADJUSTMENT (bobgui_builder_get_object (builder, "letterspacing_adjustment"));
      demo->line_height_entry = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "line_height_entry"));
      demo->line_height_adjustment = BOBGUI_ADJUSTMENT (bobgui_builder_get_object (builder, "line_height_adjustment"));
      demo->foreground = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "foreground"));
      demo->background = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "background"));
      demo->swin = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "swin"));

      demo->provider = bobgui_css_provider_new ();
      bobgui_style_context_add_provider_for_display (gdk_display_get_default (),
                                                  BOBGUI_STYLE_PROVIDER (demo->provider), 800);
      ...
      bobgui_widget_add_controller (demo->entry, controller);
      ...
      demo->variations_grid = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "variations_grid"));
