                 ...
                 gpointer   data)
{
  ...
  pixbuf = gdk_pixbuf_new_from_resource_at_scale ("/cursors/images/bobgui-logo.svg",
...
BobguiWidget *
do_cursors (BobguiWidget *do_widget)
{
  if (!window)
    {
      BobguiBuilder *builder;
      BobguiWidget *logo_callback;
      GdkCursor *cursor;
      BobguiCssProvider *provider;

      provider = bobgui_css_provider_new ();
      bobgui_css_provider_load_from_resource (provider, "/cursors/cursors.css");
      bobgui_style_context_add_provider_for_display (gdk_display_get_default (),
                                                  BOBGUI_STYLE_PROVIDER (provider),
                                                  800);
      g_object_unref (provider);

      builder = bobgui_builder_new_from_resource ("/cursors/cursors.ui");
      ...
      logo_callback = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "logo_callback"));
      cursor = gdk_cursor_new_from_callback (cursor_callback, NULL, NULL, NULL);
      bobgui_widget_set_cursor (logo_callback, cursor);
