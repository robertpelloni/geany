...
static BobguiWidget *font_button = NULL;
static BobguiWidget *entry = NULL;
static BobguiWidget *image = NULL;
static BobguiWidget *hinting = NULL;
static BobguiWidget *anti_alias = NULL;
static BobguiWidget *hint_metrics = NULL;
static BobguiWidget *up_button = NULL;
static BobguiWidget *down_button = NULL;
static BobguiWidget *text_radio = NULL;
static BobguiWidget *show_grid = NULL;
static BobguiWidget *show_extents = NULL;
static BobguiWidget *show_pixels = NULL;
static BobguiWidget *show_outlines = NULL;
...
update_image (void)
{
  ...
  if (!context)
    context = bobgui_widget_create_pango_context (image);

  text = bobgui_editable_get_text (BOBGUI_EDITABLE (entry));
  desc = bobgui_font_dialog_button_get_font_desc (BOBGUI_FONT_DIALOG_BUTTON (font_button));

  fopt = cairo_font_options_copy (pango_cairo_context_get_font_options (context));

  switch (bobgui_drop_down_get_selected (BOBGUI_DROP_DOWN (hinting)))
    {
    case 0:
  ...
  if (bobgui_check_button_get_active (BOBGUI_CHECK_BUTTON (hint_metrics)))
    hintmetrics = CAIRO_HINT_METRICS_ON;
  else
    hintmetrics = CAIRO_HINT_METRICS_OFF;
  cairo_font_options_set_hint_metrics (fopt, hintmetrics);

  if (bobgui_check_button_get_active (BOBGUI_CHECK_BUTTON (anti_alias)))
  ...
  if (bobgui_toggle_button_get_active (BOBGUI_TOGGLE_BUTTON (text_radio)))
    {
      ...
      if (bobgui_check_button_get_active (BOBGUI_CHECK_BUTTON (show_grid)))
        {
          int i;
      ...
      if (bobgui_check_button_get_active (BOBGUI_CHECK_BUTTON (show_extents)))
        {
...
G_GNUC_END_IGNORE_DEPRECATIONS
  bobgui_picture_set_paintable (BOBGUI_PICTURE (image), GDK_PAINTABLE (texture));
...
static gboolean
change_alpha (BobguiWidget     *widget,
              GdkFrameClock *clock,
              gpointer       user_data)
{
...
start_alpha_fade (void)
{
  ...
  pixels = bobgui_check_button_get_active (BOBGUI_CHECK_BUTTON (show_pixels));
  outlines = bobgui_check_button_get_active (BOBGUI_CHECK_BUTTON (show_outlines));
  ...
  fading = TRUE;
  bobgui_widget_add_tick_callback (window, change_alpha, NULL, NULL);
}

static void
update_buttons (void)
{
  bobgui_widget_set_sensitive (up_button, scale < 32);
  bobgui_widget_set_sensitive (down_button, scale > 1);
}

static gboolean
scale_up (BobguiWidget *widget,
          GVariant  *args,
          gpointer   user_data)
{
  scale += 1;
...
static gboolean
scale_down (BobguiWidget *widget,
            GVariant  *args,
            gpointer   user_data)
{
  scale -= 1;
...
BobguiWidget *
do_fontrendering (BobguiWidget *do_widget)
{
  if (!window)
    {
      BobguiBuilder *builder;

      builder = bobgui_builder_new_from_resource ("/fontrendering/fontrendering.ui");
      ...
      font_button = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "font_button"));
      up_button = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "up_button"));
      down_button = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "down_button"));
      entry = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "entry"));
      image = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "image"));
      hinting = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "hinting"));
      anti_alias = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "antialias"));
      hint_metrics = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "hint_metrics"));
      text_radio = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "text_radio"));
      show_grid = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "show_grid"));
      show_extents = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "show_extents"));
      show_pixels = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "show_pixels"));
      show_outlines = BOBGUI_WIDGET (bobgui_builder_get_object (builder, "show_outlines"));
