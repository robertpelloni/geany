/* Peg Solitaire
 * #Keywords: BobguiGridView, game, drag-and-drop, dnd
 *
 ...
 *
 * We implement the GdkPaintable interface for them, so we can use BobguiPicture
            ...
            int      dy)
{
   ...
   */
  image = bobgui_grid_get_child_at (grid, x + dx, y + dy);
  if (!BOBGUI_IS_IMAGE (image) ||
      !SOLITAIRE_IS_PEG (bobgui_image_get_paintable (BOBGUI_IMAGE (image))))
    return 0;

  image = bobgui_grid_get_child_at (grid, x + 2*dx, y + 2*dy);
  if (!BOBGUI_IMAGE (image) ||
      SOLITAIRE_IS_PEG (bobgui_image_get_paintable (BOBGUI_IMAGE (image))))
    return 0;
...
check_for_end (BobguiGrid *grid)
{
  ...
  for (x = 0; x < 7; x++)
    {
      for (y = 0; y < 7; y++)
        {
          image = bobgui_grid_get_child_at (grid, x, y);
          if (BOBGUI_IS_IMAGE (image) &&
              SOLITAIRE_IS_PEG (bobgui_image_get_paintable (BOBGUI_IMAGE (image))))
            {
              pegs++;
  ...
  image = bobgui_grid_get_child_at (grid, 3, 3);
  if (pegs == 1 &&
      SOLITAIRE_IS_PEG (bobgui_image_get_paintable (BOBGUI_IMAGE (image))))
    celebrate (TRUE);
  else if (moves == 0)
            ...
            BobguiWidget     *image)
{
   ...
   */
  bobgui_drag_source_set_icon (source, paintable, -2, -2);
   ...
   * on the image.
   */
  bobgui_image_clear (BOBGUI_IMAGE (image));
}
          ...
          BobguiWidget     *image)
{
  ...
  bobgui_image_set_from_paintable (BOBGUI_IMAGE (image), GDK_PAINTABLE (peg));
}
 ...
 */
static gboolean
drop_accept (BobguiDropTarget *target,
             GdkDrop       *drop,
             BobguiWidget     *image)
{
  ...
  if (SOLITAIRE_IS_PEG (bobgui_image_get_paintable (BOBGUI_IMAGE (image))))
    return FALSE;
...
static gboolean
drop_drop (BobguiDropTarget *target,
           const GValue  *value,
           double         x,
           double         y,
           BobguiWidget     *image)
{
  ...
  grid = BOBGUI_GRID (bobgui_widget_get_parent (image));
   ...
   */
  jumped = bobgui_grid_get_child_at (grid,
                                  (image_x + peg->x) / 2,
                                  (image_y + peg->y) / 2);
  /* If the jumped widget does not have a peg in it, this move
   * isn't valid.
   */
  if (!SOLITAIRE_IS_PEG (bobgui_image_get_paintable (BOBGUI_IMAGE (jumped))))
    return FALSE;

  /* Finally, we know it's a legal move. */

  /* Clear the peg of the jumped-over image */
  bobgui_image_clear (BOBGUI_IMAGE (jumped));
...
static void
create_board (BobguiWidget *window)
{
  BobguiWidget *grid;
  BobguiWidget *image;
  int x, y;
  ...
  bobgui_widget_set_valign (grid, BOBGUI_ALIGN_CENTER);
      ...
      for (y = 0; y < 7; y++)
        {
          ...
          bobgui_widget_add_css_class (image, "solitaire-field");
          ...
          bobgui_grid_attach (BOBGUI_GRID (grid), image, x, y, 1, 1);
           ...
           */
          target = bobgui_drop_target_new (SOLITAIRE_TYPE_PEG, GDK_ACTION_MOVE);
           ...
           */
          bobgui_widget_add_controller (image, BOBGUI_EVENT_CONTROLLER (target));
        }
    }
...
static void
restart_game (BobguiButton *button,
              BobguiWidget *window)
{
  create_board (window);
}
...
BobguiWidget *
do_peg_solitaire (BobguiWidget *do_widget)
{
  ...
  if (!window)
    {
      ...
      bobgui_window_set_title (BOBGUI_WINDOW (window), "Peg Solitaire");
