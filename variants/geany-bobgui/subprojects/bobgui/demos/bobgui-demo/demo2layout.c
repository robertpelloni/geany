...
G_DEFINE_TYPE (Demo2Layout, demo2_layout, BOBGUI_TYPE_LAYOUT_MANAGER)

static void
demo2_layout_measure (BobguiLayoutManager *layout_manager,
                      BobguiWidget        *widget,
...
static void
demo2_layout_allocate (BobguiLayoutManager *layout_manager,
                       BobguiWidget        *widget,
                       int               width,
                       int               height,
                       int               baseline)
{
  ...
  bobgui_widget_get_preferred_size (bobgui_widget_get_first_child (widget), &child_req, NULL);
  w = child_req.width;
  h = child_req.height;
  ...
  for (child = bobgui_widget_get_first_child (widget), i = 0;
       ...
       child = bobgui_widget_get_next_sibling (child), i++)
    {
      ...
      bobgui_widget_allocate (child, w, h, -1, transform);
    }
}

static BobguiSizeRequestMode
demo2_layout_get_request_mode (BobguiLayoutManager *layout_manager,
                               BobguiWidget        *widget)
{
