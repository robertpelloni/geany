 ...
 * text. The right pane shows a rotated BobguiLabel widget.
 *
...
static void
rotated_text_draw (BobguiDrawingArea *da,
                   cairo_t        *cr,
                   int             width,
                   int             height,
                   gpointer        data)
...
#define FONT "Serif 18"
  ...
  context = bobgui_widget_create_pango_context (BOBGUI_WIDGET (da));
...
BobguiWidget *
do_rotated_text (BobguiWidget *do_widget)
{
  ...
  if (!window)
    {
      ...
      bobgui_window_set_title (BOBGUI_WINDOW (window), "Rotated Text");
      bobgui_window_set_default_size (BOBGUI_WINDOW (window), 4 * RADIUS, 2 * RADIUS);
      ...
      bobgui_widget_add_css_class (drawing_area, "view");
