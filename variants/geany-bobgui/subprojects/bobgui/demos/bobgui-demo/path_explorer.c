...
G_DEFINE_TYPE (PathExplorer, path_explorer, BOBGUI_TYPE_WIDGET)

static void
motion (BobguiEventControllerMotion *controller,
        double                    x,
        double                    y,
        PathExplorer             *self)
{
...
static void
path_explorer_measure (BobguiWidget      *widget,
...
static void
draw_point (BobguiSnapshot            *snapshot,
...
point_cb (GskPathOperation        op,
          ...
          gpointer                user_data)
{
  BobguiSnapshot *snapshot = user_data;
...
static void
path_explorer_snapshot (BobguiWidget   *widget,
                        BobguiSnapshot *snapshot)
{
  ...
  height = bobgui_widget_get_width (widget);

  bobgui_snapshot_save (snapshot);
  bobgui_snapshot_scale (snapshot, self->zoom, self->zoom);

  if (self->do_fill)
    {
      bobgui_snapshot_push_fill (snapshot, self->segment, self->fill_rule);
  ...
  if (self->do_stroke)
    {
      bobgui_snapshot_push_stroke (snapshot, self->segment, self->stroke);
      bobgui_snapshot_append_color (snapshot,
                                 &self->stroke_color,
                                 &GRAPHENE_RECT_INIT (0, 0, width, height ));
      bobgui_snapshot_pop (snapshot);
  ...
  if (self->distance < 40)
    {
      ...
      bobgui_snapshot_push_fill (snapshot, path, 0);
      bobgui_snapshot_append_color (snapshot,
                                 &(GdkRGBA){ 1, 0, 0, 1},
                                 &GRAPHENE_RECT_INIT (0, 0, width, height ));
      bobgui_snapshot_pop (snapshot);
