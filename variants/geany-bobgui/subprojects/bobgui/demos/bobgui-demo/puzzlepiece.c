...
struct _BobguiPuzzlePieceClass
{
 ...
 */
static void
bobgui_puzzle_piece_snapshot (GdkPaintable *paintable,
                           GdkSnapshot  *snapshot,
                           double        width,
                           double        height)
{
  BobguiPuzzlePiece *self = BOBGUI_PUZZLE_PIECE (paintable);
...
bobgui_puzzle_piece_get_flags (GdkPaintable *paintable)
{
  BobguiPuzzlePiece *self = BOBGUI_PUZZLE_PIECE (paintable);
...
static int
bobgui_puzzle_piece_get_intrinsic_width (GdkPaintable *paintable)
{
  BobguiPuzzlePiece *self = BOBGUI_PUZZLE_PIECE (paintable);
...
static int
bobgui_puzzle_piece_get_intrinsic_height (GdkPaintable *paintable)
{
  BobguiPuzzlePiece *self = BOBGUI_PUZZLE_PIECE (paintable);
...
static double
bobgui_puzzle_piece_get_intrinsic_aspect_ratio (GdkPaintable *paintable)
{
  BobguiPuzzlePiece *self = BOBGUI_PUZZLE_PIECE (paintable);
...
static void
bobgui_puzzle_piece_paintable_init (GdkPaintableInterface *iface)
{
  iface->snapshot = bobgui_puzzle_piece_snapshot;
  iface->get_flags = bobgui_puzzle_piece_get_flags;
  iface->get_intrinsic_width = bobgui_puzzle_piece_get_intrinsic_width;
  iface->get_intrinsic_height = bobgui_puzzle_piece_get_intrinsic_height;
  iface->get_intrinsic_aspect_ratio = bobgui_puzzle_piece_get_intrinsic_aspect_ratio;
}

/* When defining the GType, we need to implement the GdkPaintable interface */
G_DEFINE_TYPE_WITH_CODE (BobguiPuzzlePiece, bobgui_puzzle_piece, G_TYPE_OBJECT,
                         G_IMPLEMENT_INTERFACE (GDK_TYPE_PAINTABLE,
                                                bobgui_puzzle_piece_paintable_init))

/* We need to declare a destructor to release our reference to the
 * puzzle paintable and disconnect our signal handlers.
 */
static void
bobgui_puzzle_piece_dispose (GObject *object)
{
  BobguiPuzzlePiece *self = BOBGUI_PUZZLE_PIECE (object);
  ...
  G_OBJECT_CLASS (bobgui_puzzle_piece_parent_class)->dispose (object);
}

/* Here's the boilerplate for the GObject declaration.
 */
static void
bobgui_puzzle_piece_class_init (BobguiPuzzlePieceClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->dispose = bobgui_puzzle_piece_dispose;
}

static void
bobgui_puzzle_piece_init (BobguiPuzzlePiece *self)
{
}
 ...
 */
GdkPaintable *
bobgui_puzzle_piece_new (GdkPaintable *puzzle,
                      guint         x,
                      guint         y,
                      guint         width,
                      guint         height)
{
  BobguiPuzzlePiece *self;
  ...
  self = g_object_new (BOBGUI_TYPE_PUZZLE_PIECE, NULL);
 ...
 */
GdkPaintable *
bobgui_puzzle_piece_get_puzzle (BobguiPuzzlePiece *self)
{
   ...
   * way easier to debug.
   */
  g_return_val_if_fail (BOBGUI_IS_PUZZLE_PIECE (self), NULL);

  return self->puzzle;
}

guint
bobgui_puzzle_piece_get_x (BobguiPuzzlePiece *self)
{
  g_return_val_if_fail (BOBGUI_IS_PUZZLE_PIECE (self), 0);

  return self->x;
}

guint
bobgui_puzzle_piece_get_y (BobguiPuzzlePiece *self)
{
  g_return_val_if_fail (BOBGUI_IS_PUZZLE_PIECE (self), 0);
