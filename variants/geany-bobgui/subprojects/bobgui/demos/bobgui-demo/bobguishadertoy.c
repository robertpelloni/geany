...
typedef struct {
  ...
  gint64 first_frame;
  guint tick;
} BobguiShadertoyPrivate;

G_DEFINE_TYPE_WITH_PRIVATE (BobguiShadertoy, bobgui_shadertoy, BOBGUI_TYPE_GL_AREA)

static gboolean bobgui_shadertoy_render        (BobguiGLArea     *area,
                                             GdkGLContext  *context);
static void     bobgui_shadertoy_reshape       (BobguiGLArea     *area,
                                             int            width,
                                             int            height);
static void     bobgui_shadertoy_realize       (BobguiWidget     *widget);
static void     bobgui_shadertoy_unrealize     (BobguiWidget     *widget);
static gboolean bobgui_shadertoy_tick          (BobguiWidget     *widget,
                                             GdkFrameClock *frame_clock,
                                             gpointer       user_data);

BobguiWidget *
bobgui_shadertoy_new (void)
{
  return g_object_new (bobgui_shadertoy_get_type (),
                       "allowed-apis", GDK_GL_API_GL | GDK_GL_API_GLES,
                       NULL);
}

static void
drag_begin_cb (BobguiGestureDrag *drag,
               double          x,
               double          y,
               gpointer        user_data)
{
  BobguiShadertoy *shadertoy = BOBGUI_SHADERTOY (user_data);
  BobguiShadertoyPrivate *priv = bobgui_shadertoy_get_instance_private (shadertoy);
  int height = bobgui_widget_get_height (BOBGUI_WIDGET (shadertoy));
  int scale = bobgui_widget_get_scale_factor (BOBGUI_WIDGET (shadertoy));
...
static void
drag_update_cb (BobguiGestureDrag *drag,
                double          dx,
                double          dy,
                gpointer        user_data)
{
  BobguiShadertoy *shadertoy = BOBGUI_SHADERTOY (user_data);
  BobguiShadertoyPrivate *priv = bobgui_shadertoy_get_instance_private (shadertoy);
  int width = bobgui_widget_get_width (BOBGUI_WIDGET (shadertoy));
  int height = bobgui_widget_get_height (BOBGUI_WIDGET (shadertoy));
  int scale = bobgui_widget_get_scale_factor (BOBGUI_WIDGET (shadertoy));
  double x, y;
...
static void
drag_end_cb (BobguiGestureDrag *drag,
             double          dx,
             double          dy,
             gpointer        user_data)
{
...
static void
bobgui_shadertoy_init (BobguiShadertoy *shadertoy)
{
  BobguiShadertoyPrivate *priv = bobgui_shadertoy_get_instance_private (shadertoy);
  BobguiGesture *drag;

  priv->image_shader = g_strdup (default_image_shader);
  priv->tick = bobgui_widget_add_tick_callback (BOBGUI_WIDGET (shadertoy), bobgui_shadertoy_tick, shadertoy, NULL);

  drag = bobgui_gesture_drag_new ();
  bobgui_widget_add_controller (BOBGUI_WIDGET (shadertoy), BOBGUI_EVENT_CONTROLLER (drag));
...
static void
bobgui_shadertoy_finalize (GObject *obj)
{
  BobguiShadertoy *shadertoy = BOBGUI_SHADERTOY (obj);
  BobguiShadertoyPrivate *priv = bobgui_shadertoy_get_instance_private (shadertoy);

  bobgui_widget_remove_tick_callback (BOBGUI_WIDGET (shadertoy), priv->tick);
  g_free (priv->image_shader);

  G_OBJECT_CLASS (bobgui_shadertoy_parent_class)->finalize (obj);
}

static void
bobgui_shadertoy_class_init (BobguiShadertoyClass *klass)
{
  BOBGUI_GL_AREA_CLASS (klass)->render = bobgui_shadertoy_render;
  BOBGUI_GL_AREA_CLASS (klass)->resize = bobgui_shadertoy_reshape;

  BOBGUI_WIDGET_CLASS (klass)->realize = bobgui_shadertoy_realize;
  BOBGUI_WIDGET_CLASS (klass)->unrealize = bobgui_shadertoy_unrealize;

  G_OBJECT_CLASS (klass)->finalize = bobgui_shadertoy_finalize;
}

/* new window size or exposure */
static void
bobgui_shadertoy_reshape (BobguiGLArea *area, int width, int height)
{
  BobguiShadertoyPrivate *priv = bobgui_shadertoy_get_instance_private ((BobguiShadertoy *) area);
...
static gboolean
init_shaders (BobguiShadertoy *shadertoy,
...
static void
bobgui_shadertoy_realize_shader (BobguiShadertoy *shadertoy)
{
  BobguiShadertoyPrivate *priv = bobgui_shadertoy_get_instance_private (shadertoy);
  GdkGLAPI api;
  ...
  api = bobgui_gl_area_get_api (BOBGUI_GL_AREA (shadertoy));
  ...
  if (!init_shaders (shadertoy, vertex_shader, fragment_shader, &error))
    {
      priv->error_set = TRUE;
      bobgui_gl_area_set_error (BOBGUI_GL_AREA (shadertoy), error);
      g_error_free (error);
    }
...
static gboolean
bobgui_shadertoy_render (BobguiGLArea    *area,
                      GdkGLContext *context)
{
  BobguiShadertoy *shadertoy = BOBGUI_SHADERTOY (area);
  BobguiShadertoyPrivate *priv = bobgui_shadertoy_get_instance_private (shadertoy);
  ...
  if (priv->image_shader_dirty)
    bobgui_shadertoy_realize_shader (shadertoy);
...
const char *
bobgui_shadertoy_get_image_shader (BobguiShadertoy *shadertoy)
{
  BobguiShadertoyPrivate *priv = bobgui_shadertoy_get_instance_private (shadertoy);
...
void
bobgui_shadertoy_set_image_shader (BobguiShadertoy *shadertoy,
                                const char   *shader)
{
  BobguiShadertoyPrivate *priv = bobgui_shadertoy_get_instance_private (shadertoy);
  ...
  if (priv->error_set)
    {
      bobgui_gl_area_set_error (BOBGUI_GL_AREA (shadertoy), NULL);
...
static void
bobgui_shadertoy_realize (BobguiWidget *widget)
{
  BobguiGLArea *glarea = BOBGUI_GL_AREA (widget);
  BobguiShadertoy *shadertoy = BOBGUI_SHADERTOY (widget);
  BobguiShadertoyPrivate *priv = bobgui_shadertoy_get_instance_private (shadertoy);
  ...
  BOBGUI_WIDGET_CLASS (bobgui_shadertoy_parent_class)->realize (widget);
...
static void
bobgui_shadertoy_unrealize (BobguiWidget *widget)
{
  BobguiGLArea *glarea = BOBGUI_GL_AREA (widget);
  BobguiShadertoyPrivate *priv = bobgui_shadertoy_get_instance_private ((BobguiShadertoy *) widget);
  ...
  BOBGUI_WIDGET_CLASS (bobgui_shadertoy_parent_class)->unrealize (widget);
}

static gboolean
bobgui_shadertoy_tick (BobguiWidget     *widget,
                    GdkFrameClock *frame_clock,
                    gpointer       user_data)
{
  BobguiShadertoy *shadertoy = BOBGUI_SHADERTOY (widget);
