...
/*
BOBGUI has two clipboards - normal clipboard and primary clipboard
Primary clipboard is only handled
internally by BOBGUI (it's not portable to Windows).
...
For Clipboard:
BOBGUI calls one of the gdk_clipboard_set* () functions (either supplying
...
BOBGUI might also call gdk_clipboard_store_async(), which instructs
...
When something needs to be obtained from clipboard, BOBGUI calls
...
gdk_win32_clipdrop_init (GdkWin32Clipdrop *win32_clipdrop)
{
  ...
  /* These are a bit unusual. It's here to allow BOBGUI applications
  ...
  /* BOBGUI actually has more text formats, but it's unlikely that we'd
   * get anything other than UTF8_STRING these days.
   * BOBGUITEXTBUFFERCONTENTS format can potentially be converted to
