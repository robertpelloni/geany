                         ...
                         const char   *string)
{
  ...
  if (bobgui_builder_value_from_string_type (data->builder, G_TYPE_BOOLEAN, string, &value, NULL))
    {
                        ...
                        PropKind    kind)
{
  struct _Prop {
    ...
    { "BobguiAboutDialog", "program-name", PROP_KIND_OBJECT },
    { "BobguiCalendar", "year", PROP_KIND_OBJECT },
    { "BobguiCalendar", "month", PROP_KIND_OBJECT },
    { "BobguiCalendar", "day", PROP_KIND_OBJECT },
    { "BobguiPlacesSidebar", "show-desktop", PROP_KIND_OBJECT },
    { "BobguiRadioButton", "draw-indicator", PROP_KIND_OBJECT },
    { "BobguiWidget", "hexpand", PROP_KIND_OBJECT },
    { "BobguiWidget", "vexpand", PROP_KIND_OBJECT },
    { "BobguiGridLayoutChild", "row", PROP_KIND_LAYOUT },
    { "BobguiGridLayoutChild", "column", PROP_KIND_LAYOUT },
  };
  gboolean found;
  int k;
...
is_cdata_property (Element *element)
{
  if (g_str_equal (element->element_name, "property") &&
      ...
      has_attribute (element->parent, "class", "BobguiBuilderListItemFactory"))
    return TRUE;
...
static struct {
  ...
  { "BobguiBox", "BobguiBoxLayout" },
  { "BobguiGrid", "BobguiGridLayout" },
  { "BobguiFixed", "BobguiFixedLayout" },
  { "BobguiFileChooserButton", "BobguiBinLayout" },
  { "BobguiFileChooserWidget", "BobguiBinLayout" },
  { "BobguiOverlay", "BobguiOverlayLayout" }
};
                    ...
                    PropKind      kind)
{
  ...
  if (type == G_TYPE_INVALID)
    {
      type = bobgui_builder_get_type_from_name (data->builder, class_name);
      if (type == G_TYPE_INVALID)
        return NULL;
    }
...
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
        /* We're just assuming that the cell layout is using a BobguiCellAreaBox. */
        cell_class = g_type_class_ref (BOBGUI_TYPE_CELL_AREA_BOX);
        pspec = bobgui_cell_area_class_find_cell_property (BOBGUI_CELL_AREA_CLASS (cell_class), canonical_name);
    ...
    case PROP_KIND_LAYOUT:
      {
        ...
        if (layout_manager)
          {
            BobguiLayoutManagerClass *layout_manager_class;

            layout_manager_class = BOBGUI_LAYOUT_MANAGER_CLASS (g_type_class_ref (g_type_from_name (layout_manager)));
                  ...
                  const char   *value_string)
{
  ...
  if (!bobgui_builder_value_from_string (data->builder, pspec, value_string, &value, &error))
    {
    ...
    {
      /* BobguiWidget::visible has a 'smart' default */
      if (pspec->owner_type == BOBGUI_TYPE_WIDGET &&
          g_str_equal (pspec->name, "visible"))
        {
          ...
          if (g_type_is_a (type, BOBGUI_TYPE_ROOT) ||
              g_type_is_a (type, BOBGUI_TYPE_POPOVER))
            default_value = FALSE;
          else
            default_value = TRUE;
      ...
      else if (pspec->owner_type == BOBGUI_TYPE_WINDOW &&
                      ...
                      const char   *string)
{
  ...
  if (bobgui_builder_value_from_string_type (data->builder, type, string, &value, NULL))
    {
      char *strval;
                           ...
                           MyParserData *data)
{
  ...
  struct _Prop {
    ...
    { "BobguiActionBar", "position", PROP_KIND_PACKING },
    { "BobguiButtonBox", "secondary", PROP_KIND_PACKING },
    { "BobguiButtonBox", "non-homogeneous", PROP_KIND_PACKING },
    { "BobguiBox", "position", PROP_KIND_PACKING },
    { "BobguiBox", "pack-type", PROP_KIND_PACKING },
    { "BobguiHeaderBar", "position", PROP_KIND_PACKING },
    { "BobguiPopoverMenu", "position",PROP_KIND_PACKING },
    { "BobguiCheckButton", "draw-indicator", PROP_KIND_OBJECT },
  };
  char *canonical_name;
  gboolean found;
  int i, k;
...
maybe_rename_property (Element *element, MyParserData *data)
{
  ...
  struct _Prop {
    ...
    { "BobguiPopover", "modal", BOBGUI_TYPE_POPOVER, PROP_KIND_OBJECT, "autohide", { NULL, NULL, NULL } },
    { "BobguiWidget", "expand", BOBGUI_TYPE_WIDGET, PROP_KIND_OBJECT, "vexpand", { "hexpand", NULL, NULL } },
    { "BobguiWidget", "margin", BOBGUI_TYPE_WIDGET, PROP_KIND_OBJECT, "margin-bottom", { "margin-start", "margin-end", "margin-top" } },
    { "BobguiWidget", "margin-left", BOBGUI_TYPE_WIDGET, PROP_KIND_OBJECT, "margin-start", { NULL, NULL, NULL } },
    { "BobguiWidget", "margin-right", BOBGUI_TYPE_WIDGET, PROP_KIND_OBJECT, "margin-end", { NULL, NULL, NULL } },
    { "BobguiHeaderBar", "show-close-button", BOBGUI_TYPE_HEADER_BAR, PROP_KIND_OBJECT, "show-title-buttons", { NULL, NULL, NULL } },
    { "BobguiHeaderBar", "custom-title", BOBGUI_TYPE_HEADER_BAR, PROP_KIND_OBJECT, "title-widget", { NULL, NULL, NULL } },
    { "BobguiStack", "homogeneous", BOBGUI_TYPE_STACK, PROP_KIND_OBJECT, "hhomogeneous", { "vhomogeneous", NULL, NULL } },
    { "BobguiImage", "pixbuf", BOBGUI_TYPE_IMAGE, PROP_KIND_OBJECT, "file", { NULL, NULL, NULL } },
    { "BobguiWidget", "can-focus", BOBGUI_TYPE_WIDGET, PROP_KIND_OBJECT, "focusable", { NULL, NULL, NULL } },
  };
  int i, k, l;
  PropKind kind;
...
rewrite_stack_child (Element *child, MyParserData *data)
{
  ...
  new_object->attribute_values[0] = g_strdup ("BobguiStackPage");
...
rewrite_assistant_child (Element *child, MyParserData *data)
{
  ...
  new_object->attribute_names[0] = g_strdup ("class");
  new_object->attribute_values = g_new0 (char *, 2);
  new_object->attribute_values[0] = g_strdup ("BobguiAssistantPage");
  new_object->children = packing->children;
  new_object->parent = child;
  packing->children = NULL;
...
rewrite_notebook_page (Element *child, Element *tab, MyParserData *data)
{
  ...
  new_object->attribute_names[0] = g_strdup ("class");
  new_object->attribute_values = g_new0 (char *, 2);
  new_object->attribute_values[0] = g_strdup ("BobguiNotebookPage");
  new_object->parent = child;
  if (packing)
    {
      new_object->children = packing->children;
                                ...
                                GList *children)
{
  ...
  set_attribute_value (object, "class", "BobguiBox");
             ...
             MyParserData *data)
{
  ...
  Element *center_child = NULL;
  GList *l, *ll;
  BobguiOrientation orientation = BOBGUI_ORIENTATION_HORIZONTAL;

  if (g_str_equal (get_class_name (element), "BobguiVBox"))
    write_box_prop (NULL, element, "orientation", "vertical");

  if (!g_str_equal (get_class_name (element), "BobguiBox"))
    set_attribute_value (element, "class", "BobguiBox");
      ...
      if (g_str_equal (child->element_name, "child"))
        {
          ...
          BobguiPackType pack_type = BOBGUI_PACK_START;
          ...
          else if (pack_type == BOBGUI_PACK_START)
  ...
  if (center_child || end_children)
    {
      set_attribute_value (element, "class", "BobguiCenterBox");
      ...
      for (ll = child->children; ll; ll = ll->next)
        {
          ...
          type = g_type_from_name (class_name);
          if (!g_type_is_a (type, BOBGUI_TYPE_WIDGET))
            continue;
                      ...
                      MyParserData *data)
{
  ...
  if (draw_indicator)
    set_attribute_value (element, "class", "BobguiCheckButton");
  else
    set_attribute_value (element, "class", "BobguiToggleButton");
...
rewrite_separator (Element *element,
                   MyParserData *data)
{
  if (g_str_equal (get_class_name (element), "BobguiVSeparator"))
    write_separator_prop (NULL, element, "orientation", "vertical");

  if (!g_str_equal (get_class_name (element), "BobguiSeparator"))
    set_attribute_value (element, "class", "BobguiSeparator");
}
...
rewrite_requires (Element      *element,
                  MyParserData *data)
{
  if (has_attribute (element, "lib", "bobgui+"))
    {
      set_attribute_value (element, "lib", "bobgui");
      ...
      if (object)
        {
          ...
          if (g_str_equal (class_name, "BobguiToolButton"))
            {
              set_attribute_value (object, "class", "BobguiButton");
            }
          else if (g_str_equal (class_name, "BobguiToggleToolButton") ||
                   g_str_equal (class_name, "BobguiRadioToolButton"))
            {
              set_attribute_value (object, "class", "BobguiToggleButton");
            }
          else if (g_str_equal (class_name, "BobguiSeparatorToolItem"))
            {
              Element *prop;

              set_attribute_value (object, "class", "BobguiSeparator");
              ...
              for (ll = packing->children; ll; ll = ll->next)
                {
                  ...
                  if (has_attribute (elt2, "name", "x"))
                    {
                      if (bobgui_builder_value_from_string_type (data->builder, G_TYPE_INT, elt2->data, &value, NULL))
                        {
                      ...
                      MyParserData *data)
{
  ...
  if (element_is_object_or_template (element) &&
      g_str_equal (get_class_name (element), "BobguiStack"))
    rewrite_stack (element, data);

  if (element_is_object_or_template (element) &&
      g_str_equal (get_class_name (element), "BobguiAssistant"))
    rewrite_assistant (element, data);

  if (element_is_object_or_template (element) &&
      g_str_equal (get_class_name (element), "BobguiNotebook"))
    rewrite_notebook (element, data);

  if (element_is_object_or_template (element) &&
      (g_str_equal (get_class_name (element), "BobguiActionBar") ||
       g_str_equal (get_class_name (element), "BobguiHeaderBar")))
    rewrite_pack_type (element, data);

  if (element_is_object_or_template (element) &&
      g_str_equal (get_class_name (element), "BobguiToolbar"))
    rewrite_toolbar (element, data);

  if (element_is_object_or_template (element) &&
      g_str_equal (get_class_name (element), "BobguiPaned"))
    rewrite_paned (element, data);

  if (element_is_object_or_template (element) &&
      g_str_equal (get_class_name (element), "BobguiDialog"))
    rewrite_dialog (element, data);

  if (element_is_object_or_template (element) &&
      g_str_equal (get_class_name (element), "BobguiOverlay"))
    rewrite_overlay (element, data);

  if (element_is_object_or_template (element) &&
      g_str_equal (get_class_name (element), "BobguiGrid"))
    rewrite_grid_layout (element, data);

  if (element_is_object_or_template (element) &&
      (g_str_equal (get_class_name (element), "BobguiHBox") ||
       g_str_equal (get_class_name (element), "BobguiVBox") ||
       g_str_equal (get_class_name (element), "BobguiBox")))
    rewrite_box (element, data);

  if (element_is_object_or_template (element) &&
      g_str_equal (get_class_name (element), "BobguiFixed"))
    rewrite_fixed (element, data);

  if (element_is_object_or_template (element) &&
      g_str_equal (get_class_name (element), "BobguiRadioButton"))
    rewrite_radio_button (element, data);

  if (element_is_object_or_template (element) &&
      g_str_equal (get_class_name (element), "BobguiScale"))
    rewrite_scale (element, data);

  if (element_is_object_or_template (element) &&
      (g_str_equal (get_class_name (element), "BobguiHSeparator") ||
       g_str_equal (get_class_name (element), "BobguiVSeparator")))
                 ...
                 MyParserData *data)
{
  ...
  if (element_is_object_or_template (element) &&
      (g_str_equal (get_class_name (element), "BobguiAspectFrame") ||
       g_str_equal (get_class_name (element), "BobguiComboBox") ||
       g_str_equal (get_class_name (element), "BobguiComboBoxText") ||
       g_str_equal (get_class_name (element), "BobguiFlowBoxChild") ||
       g_str_equal (get_class_name (element), "BobguiFrame") ||
       g_str_equal (get_class_name (element), "BobguiListBoxRow") ||
       g_str_equal (get_class_name (element), "BobguiOverlay") ||
       g_str_equal (get_class_name (element), "BobguiPopover") ||
       g_str_equal (get_class_name (element), "BobguiPopoverMenu") ||
       g_str_equal (get_class_name (element), "BobguiRevealer") ||
       g_str_equal (get_class_name (element), "BobguiScrolledWindow") ||
       g_str_equal (get_class_name (element), "BobguiSearchBar") ||
       g_str_equal (get_class_name (element), "BobguiViewport") ||
       g_str_equal (get_class_name (element), "BobguiWindow")))
  ...
  if (element_is_object_or_template (element) &&
      g_str_equal (get_class_name (element), "BobguiCenterBox"))
 ...
 * simplifying it.
 *
 * So far, this is just BobguiWidget::visible,
                 ...
                 MyParserData *data)
{
  ...
  if (strcmp (element->element_name, "requires") == 0 &&
      has_attribute (element, "lib", "bobgui+"))
    {
      data->has_bobgui_requires = TRUE;
    }
  ...
  if (element == data->root && !data->has_bobgui_requires)
    {
      Element *requires = add_element (element, "requires");
      set_attribute_value (requires, "lib", "bobgui+");
               ...
               gboolean    convert3to4)
{
  ...
  data.has_bobgui_requires = FALSE;

  if (replace)
    {
      int fd;
      fd = g_file_open_tmp ("bobgui-builder-tool-XXXXXX", &data.output_filename, &error);
      if (fd < 0) {
  ...
  data.builder = bobgui_builder_new ();
             ...
             const char ***argv)
{
  ...
  const GOptionEntry entries[] = {
    ...
    { "3to4", 0, 0, G_OPTION_ARG_NONE, &convert3to4, N_("Convert from BOBGUI 3 to BOBGUI 4"), NULL },
  ...
  g_set_prgname ("bobgui4-builder-tool simplify");
