                 ...
                 gboolean    force)
{
  ...
  GObject *object;
  BobguiWidget *window;
  BobguiWidget *menu_button = NULL;
  BobguiWidget *target = NULL;
  GdkTexture *texture;
  char *save_to;
  GBytes *bytes;
  ...
  else if (BOBGUI_IS_POPOVER (object))
    {
      ...
      bobgui_menu_button_set_popover (BOBGUI_MENU_BUTTON (menu_button), BOBGUI_WIDGET (object));
      bobgui_window_set_child (BOBGUI_WINDOW (window), menu_button);

      target = BOBGUI_WIDGET (object);
    }
  else
    {
  ...
  if (menu_button)
    bobgui_menu_button_popup (BOBGUI_MENU_BUTTON (menu_button));
               ...
               const char ***argv)
{
  ...
  g_set_prgname ("bobgui4-builder-tool render");
