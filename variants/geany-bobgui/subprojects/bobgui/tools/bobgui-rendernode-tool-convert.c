            ...
            const char ***argv)
{
  ...
  GdkRGBA colors[4] = {
    [BOBGUI_SYMBOLIC_COLOR_FOREGROUND] = { 0, 0, 0, 1 },
    [BOBGUI_SYMBOLIC_COLOR_ERROR] =      { 0, 0, 1, 1 },
    [BOBGUI_SYMBOLIC_COLOR_WARNING] =    { 0, 1, 0, 1 },
    [BOBGUI_SYMBOLIC_COLOR_SUCCESS] =    { 1, 0, 0, 1 },
  };
  char *fc = NULL;
  char *sc = NULL;
  char *wc = NULL;
  ...
  g_set_prgname ("bobgui4-rendernode-tool convert");
  ...
  if (recolor)
    {
      colors[BOBGUI_SYMBOLIC_COLOR_FOREGROUND] = fg_default;
      colors[BOBGUI_SYMBOLIC_COLOR_SUCCESS] = success_default;
      colors[BOBGUI_SYMBOLIC_COLOR_WARNING] = warning_default;
      colors[BOBGUI_SYMBOLIC_COLOR_ERROR] = error_default;
      if (fc && !gdk_rgba_parse (&colors[BOBGUI_SYMBOLIC_COLOR_FOREGROUND], fc))
        {
          g_printerr ("Failed to parse as color: %s\n", fc);
          exit (1);
        }
      if (sc && !gdk_rgba_parse (&colors[BOBGUI_SYMBOLIC_COLOR_SUCCESS], sc))
        {
          g_printerr ("Failed to parse as color: %s\n", fc);
          exit (1);
        }
      if (wc && !gdk_rgba_parse (&colors[BOBGUI_SYMBOLIC_COLOR_WARNING], wc))
        {
          g_printerr ("Failed to parse as color: %s\n", fc);
          exit (1);
        }
      if (ec && !gdk_rgba_parse (&colors[BOBGUI_SYMBOLIC_COLOR_ERROR], ec))
        {
          g_printerr ("Failed to parse as color: %s\n", fc);
          exit (1);
        }
