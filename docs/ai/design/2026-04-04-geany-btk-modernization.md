...
3. **First-wave BTK compatibility facade helpers** now live in `src/gtkcompat.h` for tab positioning, widget CSS naming, standard dialog CSS naming, and application CSS-provider attachment/removal so the migration can start from common UI seams instead of raw toolkit calls everywhere.
...
2. **Introduce a compatibility facade** for the most common widget/type/signal helpers — started in this pass for notebook positions, CSS names, standard dialog naming, and CSS providers.
